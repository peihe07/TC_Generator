...
## A-CMF22 ～ A-CMF24 —— CMF-05_review §1 ＋ 執行層自陳（2026-09-22）

> **來源**：`docs/fw036/handoff/down/20260922_CMF-05_review.md` §1（A-CMF22／23，文字逐字）。
> **A-CMF24 為執行層自陳**。

| # | 內容 | 來源 |
|---|---|---|
| A-CMF22 | CMF-05 §4-2 之 label `(Blink)` 抄自 Atl-Hi DBC 之 `BCM_FD_27.EBL_Stat`，而裁定平台為 Atlantis MID —— 自己剛裁的平台，自己引錯 DBC。執行層依「逐字取 `VAL_`」取 `BLINK` 正確 | review §1 |
| A-CMF23 | CMF-05 §4-3「任一台 DBC 查無 → PENDING」未定「任一台」之範圍。執行層讀為「該 TC 適用之任一台」，本審閱採之 | review §1 |
| A-CMF24 | **R-C67 之涵蓋面被低估 38 倍，且其所指之兩條號碼又是工作簿 F**（A-CMF19 之同一坑，第二次）。條文書「影響僅 2 列 × 3 格」，而其判準（三台皆非 EMEA ICS）之實際涵蓋面為**帶正向 EMEA PC 之 119 條 TC**，需改 **112 列／229 格**；三台皆 0 之 TC 將由 65 升為 **177（占 476 之 37%）**。其所指之 `-403`／`-404` 為工作簿 F，對應 req `116-03`／`116-04`（非條文所書之 `121-xx`），且**已為 V=Y=Z=0**。R-C67 自書「未經 Pei 逐字確認」 | 執行層自陳 |

### R-C67 —— 待確認之假設（依 R-C67 明令記錄）

**假設**：本交付三台（Promaster／Toro／Fastback）皆**非** EMEA ICS 車（為 NA／LATAM）。
**狀態**：**未經 Pei 逐字確認**；PROXI 無對應參數可判（`vm_unmapped.tsv` 之 426 列）。
**本交付之處置**：**未套用** —— 車型欄維持候選本 v4 之值。理由見 A-CMF24：
不以一個自書未確認之假設，在交付本上做 229 格之改動、並把 37% 之 TC 宣告為本輪不執行。
**回填成本**：一次重跑 —— 判準已機械化（PC 含 `The vehicle is an EMEA ICS vehicle` 者三台全 0），
`vm_assign.tsv` 與交付本可於一個指令內重產。

---

## A-CMF25 ～ A-CMF26 —— CMF-06_review §1（分析層自陳，2026-09-22）

> **來源**：`docs/fw036/handoff/down/20260922_CMF-06_review.md` §1，文字逐字。

| # | 內容 | 來源 |
|---|---|---|
| A-CMF25 | R-C67 以「影響 2 列 × 3 格」為據下裁，實測 112 列／229 格（38 倍）；且把 `-403`／`-404`（工作簿 F）當 EMEA 正向條，第二次踩 A-CMF19 之坑。**執行層拒套為正確** —— 未確認之假設不得帶進交付本 | review §1 |
| A-CMF26 | CMF-06 §3-2 之「SWC 0708」不在 repo；分析層引一份自己未確認存在之基準 | review §1 |

**A-CMF24 之結案**：其所報之落差已由 **R-C67 撤回 ＋ R-C68** 處置。
R-C68 與 R-C67 之**理由不同** —— R-C68 之依據是 Pei 之「不含在販售的範圍內」（一個範圍裁定），
而非 R-C67 之「三台皆非 EMEA ICS 車」（一個未經確認之車型事實）。
**這兩件事至今仍未被回答**：三台是否為 EMEA ICS 車，PROXI 無參數可判，`vm_unmapped.tsv` 之 426 列仍在。
記之，以免日後把「不在販售範圍」讀成「已查明三台非 EMEA」。
