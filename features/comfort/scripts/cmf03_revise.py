#!/usr/bin/env python3
"""CMF-03 §3 —— Comfort HMI Revise 候選本 v2（0907 本之複本，xlsx_surgical 寫入）。
...
PREV = FEATURE / "sandbox" / "revise_0918" / (
    "FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case "
    "Specification & Result_SWQT_ComfortHMI_20260918.xlsx")
PREV_SHA = "28a0998c8b36a88b182b3fa209492cc5c6fda3999f48e6031d571d66439a266f"
OUT_DIR = FEATURE / "sandbox" / "revise_0921"
OUT = OUT_DIR / (
    "FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case "
    "Specification & Result_SWQT_ComfortHMI_20260921.xlsx")
REPORTS = FEATURE / "docs" / "reports"
RECON_TSV = REPORTS / "cmf03_candidate_cell_recon.tsv"
PREV_TSV = REPORTS / "cmf03_vs_0918_cell_delta.tsv"
...
def main() -> int:
    ...
    if sha256(PREV) != PREV_SHA:
        raise SystemExit("revise_0918 候選本 sha256 不符 ENTRY 037 —— 停")
    tcs = W.load_tcs()
    ...
    print("## §3 候選本 v2 —— invariant 與逐格對帳（自產出檔讀回）\n")
    ...
    # ---- 逐格對帳：相對 revise_0918 候選本（CMF-03 §3）---------------------------
        ...
        for r in range(1, rows + 1):
            for c in range(1, cols + 1):
                ...
                delta.append({"cell": f"{col}{r}", "sheet": name,
                              ...
                              "item": item or "UNATTRIBUTED",
                              "revise_0918": "" if va is None else str(va),
                              "revise_0921": "" if vb is None else str(vb)})
    g("相對 revise_0918 之每一差異格皆歸因到本包一項", not unattr,
      f"未歸因 {len(unattr)} 格 {unattr[:5]}")
    by_item2 = {}
    for x in delta:
        by_item2[x["item"]] = by_item2.get(x["item"], 0) + 1
    print(f"- MEASURED — 相對 revise_0918 之差異格 {len(delta)}：" +
