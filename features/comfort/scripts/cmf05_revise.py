#!/usr/bin/env python3
"""CMF-05 §6 —— Comfort HMI Revise 候選本 v4（0907 本之複本，xlsx_surgical 寫入）。

**車型欄改為逐 TC 填值**（CMF-05 §2）：T＝0、U＝0、W＝0、X＝0 全列固定；
**V／Y／Z 逐條取自 `docs/reports/vm_assign.tsv`**（R-C62 之判定 ＋ R-C48(amend2) 之
Toro 基線 ＋ R-C63 之「三台皆 0 照填」）。
**車型拆題實產 0 條** —— 71 條候選逐條複判後，其差異軸於各該 TC 之**適用車型集合**內
皆同值（理由見上繳包 §3），故無 sibling 新增，TC 總數維持 476。
...
PREV = FEATURE / "sandbox" / "revise_0922" / (
    "FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case "
    "Specification & Result_SWQT_ComfortHMI_20260922.xlsx")
PREV_SHA = "5d3726825ff99cb11b39a4538c340c0fc0ffbd5acab116522381e47ba29817d2"
OUT_DIR = FEATURE / "sandbox" / "revise_0922b"
OUT = OUT_DIR / (
    "FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case "
    "Specification & Result_SWQT_ComfortHMI_20260922_v4.xlsx")
REPORTS = FEATURE / "docs" / "reports"
RECON_TSV = REPORTS / "cmf05_candidate_cell_recon.tsv"
PREV_TSV = REPORTS / "cmf05_vs_v3_cell_delta.tsv"
CHANGE_LOGS = (REPORTS / "cmf02_json_change_log.tsv",
               REPORTS / "cmf03_json_change_log.tsv",
               REPORTS / "cmf04_json_change_log.tsv",
               REPORTS / "cmf05_json_change_log.tsv")
...
# R-C48(amend)／R-G74：Atl-Hi 與無 PROXI 之車型全列 0
VM_FIXED = {"T": 0, "U": 0, "W": 0, "X": 0}
# V／Y／Z 逐條（R-C62／R-C48(amend2)／R-C63）
VM_PER_TC = {"V": "V_Promaster", "Y": "Y_Toro", "Z": "Z_Fastback"}
VM_COLS = tuple(VM_FIXED) + tuple(VM_PER_TC)


def load_vm() -> dict:
    """`vm_assign.tsv` → {tc_id: {"V": 0/1, "Y": …, "Z": …}}。"""
    with VM_ASSIGN.open(encoding="utf-8") as fh:
        rows = list(csv.DictReader(
            (l for l in fh if not l.startswith("#")), delimiter="\t"))
    out = {r["tc_id"]: {c: int(r[k]) for c, k in VM_PER_TC.items()} for r in rows}
    if len(out) != len(rows):
        raise SystemExit("vm_assign.tsv 有重複之 tc_id")
    return out
...
def main() -> int:
    ...
    if sha256(PREV) != PREV_SHA:
        raise SystemExit("revise_0922 候選本 v3 sha256 不符 ENTRY 039 —— 停")
    vm = load_vm()
    tcs = W.load_tcs()
    missing = [t["tc_id"] for t in tcs if t["tc_id"] not in vm]
    if missing:
        raise SystemExit(f"vm_assign.tsv 未涵蓋 {missing}")
    ...
    for i, t in enumerate(plan):
        ...
        for col, v in VM_FIXED.items():          # R-C48(amend)／R-G74
            intended[(r, col)] = v
        for col, v in vm[t["tc_id"]].items():    # R-C62／R-C48(amend2)／R-C63
            intended[(r, col)] = v
    # ---- 尾端新列 -------------------------------------------------------------
    ...
    print("## §6 候選本 v4 —— invariant 與逐格對帳（自產出檔讀回）\n")
        ...
        for r in range(1, rows + 1):
            for c in range(1, cols + 1):
                ...
                delta.append({"cell": f"{col}{r}", "sheet": name,
                              ...
                              "item": item or "UNATTRIBUTED",
                              "v3": "" if va is None else str(va),
                              "v4": "" if vb is None else str(vb)})
    g("相對 revise_0922 (v3) 之每一差異格皆歸因到本包一項", not unattr,
      f"未歸因 {len(unattr)} 格 {unattr[:5]}")
    by_item2 = {}
    for x in delta:
        by_item2[x["item"]] = by_item2.get(x["item"], 0) + 1
    print(f"- MEASURED — 相對 revise_0922 (v3) 之差異格 {len(delta)}：" +
