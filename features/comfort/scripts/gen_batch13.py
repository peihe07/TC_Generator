...
REASONING = {
"16.8": "驗證目標：16.8（ICE7）以十二句定出 MAX DEF 對六項氣候參數之設定、自動關閉與回復，以及五種中斷途徑之落點，十二個 037 leaf 逐句對應（§8.2.1）。關鍵情境條件：EMEA 軸取正向值（16.2）；**第五軸（MAX DEF 有無，出處 3.2）十二條全補**；`-242`（Sync）另補第二軸（16.11）、`-243`（REAR DEFROST）另補第十軸（3.4）、`-247`（AUTO）另補第一軸之 ATC 值（16.3）、`-248`（MAX A/C）另補第四軸（16.13）；依 **R-C34** 第九軸與第十三軸暴露 → 全數補。為什麼這樣切：六項參數設定各自成條（`-237`～`-242`），任一項之失效可獨立定位；四種中斷途徑之落點各異（`-245`～`-248`），亦各自成條；`-244`（風速不中斷）為其反例，與 `-245`（其他變更中斷）成正反兩側（§7）。刻意略過：**其對造 `3.2`（C20）已生成而 `2.8`（C7）之 Defrost 為另一控制** —— 鏡射表記 `16.8 ↔ 3.2` 為 mirrored（七項連動逐字相同），故本節與 `3.2` 為鏡射對而非與 `2.8`；`-237`～`-248` 之判定與 `-018`／`-019` 之 TC 對 TC 比對見上繳 39 §6.3（§8.2.1：不因其為鏡射而互相移植）。",
"16.9": "驗證目標：16.9（ICE8）以兩句定出 Rear Defrost 之 on/off 狀態與 CCM 可用性之灰化，兩個 037 leaf 對應之。關鍵情境條件：EMEA 軸取正向值；**第十軸（REAR DEFROST 有無，出處 3.4）**兩條全補；依 **R-C34** 第九軸與第十三軸暴露 → 補。為什麼這樣切：狀態與灰化之觸發不同（按鍵 vs CCM 狀態），可獨立失效。刻意略過：**ICE8 僅兩句，不含 C8 之「independent of any other climate functions」與鏡面除霜之連動** —— 批次 10 之 `-161`／`-162` 已因此得 `no`；本節不驗該二者，亦不得反向以 C8 補之（§8.2.1）。",
"16.12": "驗證目標：16.12（ICE11）定出 ICS 之五種氣流模式、其單選性與 ON 態之呈現，三個 037 leaf 對應之。關鍵情境條件：EMEA 軸取正向值；依 **R-C34** 第九軸與第十三軸暴露 → 補。**R-C40 之一問已於生成前作答**（59 §1.1）：其 ch2 對造 `2.12.1` 之句子為「**In some non-tri mode equipment types,** airflow modes has 5 states…」，而 ICE11 **無該配置限定語**，故兩者**非逐字相同**，R-C40 之前件不成立；ch2 側 `2.12` 之停下（DR #31）源於 ch2 有**三個**氣流模式集合而其一（4 模式）無正面適用條件，而 **ch16 只陳述一組**，該問題於此不存在 —— 依 R-C40 屬「章節脈絡」而非「條文性質」，故不一致為實然，兩側各具名理由。為什麼這樣切：單選性、呈現與 main category control 之三者可獨立失效。刻意略過：**ICE11 自身之數字不一致** ——「Airflow Modes has **5** states」而其呈現句寫「ON state for the **four** airflow modes」（A-CF13 同型），故 `-251` 只驗「所選之鍵高亮且變大」而**不驗按鈕數**。",
"16.12.1": "驗證目標：16.12.1（ICE11.1）以十句定出 Mode 硬鍵之循環、長按行為、於 Climate main 內外之呈現差異、pop-up 之逾時，以及後排畫面下之作用，十個 037 leaf 逐句對應。關鍵情境條件：EMEA 軸取正向值；依 **R-C34** 第九軸與第十三軸暴露 → 全數補。**R-C40 之一問已於生成前作答**：其 ch2 對造 `2.12.2`（C13.1）之循環為「Face > Face/Feet > Feet > **Feet plus Windshield > then repeat loop / Defrost will not be included**」（**四模式**），而 ICE11.1 為「Face > Face/Feet > Feet > Feet/Windshield > **Windshield**」（**五模式**）—— **兩者非逐字相同**，R-C40 前件不成立，其差異恰落在 DR #31 所治之處（4 模式之適用條件），故 ch2 側之停下不轉移。為什麼這樣切：`-255`／`-257`（Climate main 內外之兩種情形）為同一觸發之兩個分支，其呈現差異可獨立失效。刻意略過：**`-262` 之後排畫面** —— 其可觀察量為**前排**模式（條文明寫 alters the front Mode），故不涉「車輛是否配備後排氣候」之未登記軸；該軸之問題在於**觀察後排**，本條觀察前排（與批次 9 之 `015-04`／`015-05` 之分野）。",
"16.15": "驗證目標：16.15（ICE14）以兩句定出對外後視鏡除霜之 on/off 狀態與其獨立性，兩個 037 leaf 對應之。關鍵情境條件：EMEA 軸取正向值；依 **R-C34** 第九軸與第十三軸暴露 → 補。**DR #40 於本節獨立判定**（59 §1.1：不以 ch2 側之未決為由停下，亦不以其為由生成）：**ICE14 自身無任何配置限定語**，故不補配置式 PC —— 此與 `2.15` 之 `-165`／`-166` 結論相同，**而其依據為本節自身之條文，非承襲 ch2 側之處置**。為什麼這樣切：`-265`（獨立性）之驗證需兩個不同干擾源（AUTO、climate off），因「獨立」之否證只需一個反例。刻意略過：**DR #40 之不對稱於 ch16 側更明顯** —— `16.9`（ICE8）**連引用鏡面除霜都沒有**（C8 尚有「if this feature available」一句），故 ch16 側之語料對該配置**零陳述**；此實測已補入上繳 39 §5。"
}
...
DIST_AXIS = {
    ...
    "16.8": {
        ...
        "delta": "四條中斷條各異 —— `-245` 改溫度／recirc／模式或再按 MAX DEF → "
                 "回前一手動模式且 A/C **開**；`-246` 按 A/C → 回前一手動模式且 "
                 "A/C **關**；`-247` 按 AUTO → 系統進入 **AUTO**；`-248` 按 MAX A/C "
                 "→ 系統進入 **MAX A/C**。另 `-244`（改風速）之 outcome 為 "
