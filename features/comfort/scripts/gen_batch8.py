...
WITHHELD = [
    ("SWE1-HVAC-125-08",
     "「for 12\" Portrait 50% widget, fan speed is also displayed」——"
     "**螢幕尺寸／widget 尺寸不在 profile §3.2 之十五軸內**，依該節「配置軸不在"
     "既有軸內即停」停下。其來源問題即 DR #6（本次交付出哪幾種螢幕配置），未解"),
    ("SWE1-HVAC-126-02",
     "「on 50% widget, features are separated for driver and passenger」——"
     "**widget 尺寸非既有軸**（同 `125-08`）。**本 leaf 為 17.3 三者中唯一仍停下者**："
     "`-01`／`-03` 之「Comfort Features 有無」已由 50 §1 登為第十六軸（功能型）而解封"),
    ("SWE1-HVAC-128-01",
     "「For **dual zone climate with dual airflow modes** equipped vehicles」——"
     "`dual zone` 為第二軸之值（已登記），惟 **「dual airflow modes 有無」未登記**；"
     "三條件之**第一項不成立**：正向值於 2.3.1／14.14／17.5 三節逐字出現，"
     "**其反面（無 dual airflow modes）於全 129 節無任何字面**，故第二個值須由推論補齊"),
    ("SWE1-HVAC-128-02", "同 `128-01`"),
    ("SWE1-HVAC-127-01",
     "「For 8.4/10.1/12 landscaped screens, there will be a toggle button on the "
     "25% widget」—— **整條 clause 以螢幕尺寸為前件**，兩個 leaf 皆停"),
    ("SWE1-HVAC-127-02",
     "同 `127-01`：其 toggle 之存在本身即以 8.4/10.1/12 landscaped 螢幕為條件"),
    ("SWE1-HVAC-129-01",
...
def main() -> None:
    ...
    if leaves + held != 21 or total != 12:
        raise SystemExit(
            f"expected 21 leaves declared / 12 TCs, got {leaves + held} / {total}")
