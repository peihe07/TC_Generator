...
WITHHELD = [
    ...
    ("SWE1-HVAC-040-01", CH9_WHY),
    ("SWE1-HVAC-040-02", CH9_WHY),
    ("SWE1-HVAC-041", CH9_WHY + "。另：本節之 leaf 帶圖（A-CF23 名單內），"
     "其「«Front»／«Rear» 標籤」可由條文逐字取得，**停下之因僅為配置軸**"),
    ("SWE1-HVAC-042-01", CH9_WHY),
    ("SWE1-HVAC-042-02", CH9_WHY),
    ("SWE1-HVAC-042-03", CH9_WHY),
    ("SWE1-HVAC-043", CH9_WHY + "。其一句「The button label will read «Rear»」"
     "所指之按鈕即 `9.4` 之 Rear Climate 按鈕，**同一變體條件**"),
]
...
def main() -> None:
    ...
    if total + held != 46 or total != 38:
        raise SystemExit(f"expected 46 / 38, got {total + held} / {total}")
