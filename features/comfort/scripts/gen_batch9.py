...
REASONING = {
"2.3": "驗證目標：2.3（C2）以九句定出 AUTO 之狀態呈現、其對風速／模式／A-C 之作用、互斥對象、中斷條件與中斷後之落點，以及 MTC 之例外，九個 037 leaf 逐句對應，一葉一 TC（§8.2.1）。關鍵情境條件：第一軸（ATC／MTC）—— `-127`～`-134` 取 ATC 值（C2 末句「AUTO is not shown in MTC configurations」蘊含 AUTO 之存在以 ATC 為前提），`-135` 取 MTC 值，出處皆 2.3；依 **R-C34** 第九軸與第十三軸暴露 → 全數補，EMEA 排除補並逐條答（R-C36-1），第十二軸不補。為什麼這樣切：九者之失效互相獨立，且中斷條件（`-132`）與中斷後落點（`-133`）分屬兩個失效面 —— 前者「有沒有斷」，後者「斷了以後去哪」。刻意略過：**C2 之「the manual mode that most closely matches the auto mode exited」無可判之對照** —— 條文未給 auto 模式與手動模式之對應表，故 `-133` 只驗其確定之另一半（按下特定模式鍵即進該模式），「最接近」之一半不寫入 ER（§8.4.1 禁造值），該缺口於 reasoning 具名而不吸收（§8.4.2）。",
"2.4": "驗證目標：2.4（C3）定出 A/C 之 on/off 狀態與其與 Auto／Defrost／Recirc 之四項連動，四個 037 leaf 對應之，一葉一 TC（§8.2.1）。關鍵情境條件：本節無配置條件，起始 PC 為 test-setup；依 **R-C34** 第九軸與第十三軸暴露 → 全數補，EMEA 排除補並逐條答，第十二軸不補。為什麼這樣切：`-137`（Auto 自動開 A/C）與 `-138`（A/C 中斷 Auto）為相反方向之兩條連動，可各自獨立失效。刻意略過：`-139` 之 ER 以「A/C 按鈕狀態不變」驗「do not show this change」—— 條文所述之隱藏對象為**畫面上之呈現**，非 A/C 之實際開關，故 ER 停在按鈕而不宣稱壓縮機狀態（§6 可觀察性）。**R-C36-1 之逐條答本節四條有三條為 `no`**：ICE3 只述五者之 on/off 而不述任何連動，鏡射表之分界欄已明載。",
"2.5": "驗證目標：2.5（C4）定出 RECIRC 之 on/off 狀態、不可用時之灰化、可自動開啟 A/C，以及車型專屬圖示，四個 037 leaf 對應之，本輪產三條。關鍵情境條件：本節無配置條件，起始 PC 為 test-setup；依 **R-C34** 第九軸與第十三軸暴露 → 全數補，EMEA 排除補並逐條答，第十二軸不補。為什麼這樣切：`-141` 之灰化以 CCM 之可用性狀態為觸發，其失效與 `-140` 之 on/off 無關。刻意略過：**`006-04`（車型專屬 recirc 圖示）停下不產列** —— C4 之「as displayed in **the table**」未指名任何節次，全 129 節亦無該對照表之內容；此與 `16.16` 之座椅 off icon **同型**，併入 `DATA_REQUESTS` #32 之「configuration → icon 對照未定義」類。**16.5（ICE4）逐字重述該句且把表之位置寫成 `Climate Main page table`，仍未給對照** —— 故非 ch16 有而 ch2 無，是兩側皆無。",
"2.10": "驗證目標：2.10（C11）定出 climate off 之畫面、按鈕灰化之例外、category button 與 status bar 之呈現，以及以 temp/fan 控制回復之行為，六個 037 leaf 對應之，一葉一 TC（§8.2.1）。關鍵情境條件：`-144` 另補第五軸（MAX DEF 有無，出處 3.2）與第十軸（REAR DEFROST 有無，出處 3.4）—— 該條之 ER 以該二鍵**不灰化**為觀察對象，其不存在即使該觀察無對象；依 **R-C34** 第九軸與第十三軸暴露 → 全數補，EMEA 排除補並逐條答，第十二軸不補。為什麼這樣切：`-147`（回復至上次等級）與 `-148`（持續按住持續變化）為同一操作之兩個階段，其失效可獨立發生（回復了但停在最低／回復正確但不續動）。刻意略過：C11 之「Climate off affects every climate function with the exception of…」為總述句，037 未另給 leaf，故不另立 TC；其可觀察內容已由 `-144` 之灰化例外涵蓋（§4.5：同一可觀察量不得由兩個 leaf 共用）。",
"2.11": "驗證目標：2.11（C12）定出 SYNC 之狀態呈現與其雙側溫度連動、中斷條件，以及其對前後排之作用，五個 037 leaf 對應之，本輪產三條。關鍵情境條件：第二軸（單區／雙區／四區）—— C12 明文「Sync is not shown for single zone climate configurations」，故起始 PC 即取該軸之非單區值，出處 2.11；依 **R-C34** 第九軸與第十三軸暴露 → 全數補，EMEA 排除補並逐條答，第十二軸不補。為什麼這樣切：`-150`（連動）與 `-151`（中斷）為 SYNC 之兩個相反方向。刻意略過：**`015-04`／`015-05` 停下不產列** —— 兩者之可觀察量皆在**後排**（「alter the Front and Rear passengers」「rear fan speed, mode, or temp」），而「車輛是否配備後排氣候」**不在 profile §3.2 之十六軸內**，其來源即 `2.1` 之 tab 集合問題（`DATA_REQUESTS` #17，未解）。**本節與已生成之 `2.6.1` 重疊之判定**：C5.1 與 C12 皆述 SYNC 之溫度連動與中斷，`pending_sibling.tsv` 已記 `2.6.1 ↔ 2.11` 為 `sibling`，兩側今皆有 TC，其 `duplicate_of` 判定見上繳 35 §8.4。",
"2.13": "驗證目標：2.13（C14）定出 MAX A/C 之畫面採用時機、on/off 狀態呈現，以及其對多項氣候參數之修改與 On/Off 邏輯之委派，三個 037 leaf 對應之，本輪產一條。關鍵情境條件：第四軸（MAX A/C 有無）—— C14 首句「MAX A/C screens/popups are to be used when CCM relays presence of MAX A/C functionality」為明文配置條件，故起始 PC 取該軸之有值，出處 2.13；依 **R-C34** 第九軸與第十三軸暴露 → 補，EMEA 排除補並逐條答，第十二軸不補。為什麼這樣切：`-152` 之可觀察量為按鈕高亮，與被委派之參數修改邏輯無關，故可獨立成條。刻意略過：**`019-02`／`019-03` 停下不產列** —— C14 之「On/Off logic should follow requirements from **VF HVAC document**」為**對外部文件之明文委派**，依 profile §5.3 之判別次序屬 `[BLOCKED-SPEC]`，而該 marker 之白名單增列須經裁定（**R-C26**：豁免不可自取），故停下回報，形態與 `080-02`／`081-02` 相同。**不以 16.13（ICE12）之逐項列舉補之** —— 那是另一套介面之條文，援引即跨介面移植（§8.2.1）。",
}
...
DIST_AXIS = {
    "2.10": {
        ...
        "delta": "`2.10`（C11）自**通則**一側陳述：climate off 時其餘按鈕灰化，"
                 ...
                 "`-144` 之 verification target 為「其餘按鈕變灰」，`-030` 之 target "
...
WITHHELD = [
 ("SWE1-HVAC-004-01", "「Some vehicles with dual zone climate with **dual airflow mode** can have a configuration for dual AUTO modes」—— 其配置條件為「dual airflow modes 有無」，**該軸未登記**（`DATA_REQUESTS` #38：其否定值於全 129 節無任何字面，三條件第一項不成立）"),
 ("SWE1-HVAC-004-02", "同 `004-01`：AUTO 指示器之雙側分列以 dual AUTO 配置為前提"),
 ...
 ("SWE1-HVAC-007-01", "「**Some vehicles have a configuration for** a 3 state toggle recirc button: Auto, Manual, Open」—— RECIRC 控制之型態逐車可變而**該軸未登記**；即 `DATA_REQUESTS` #37 之實測所引三例之一"),
 ("SWE1-HVAC-007-02", "同 `007-01`：三狀態之指示以該配置為前提"),
...
def main() -> None:
    ...
    if leaves + held != 35 or total != 26:
        raise SystemExit(
            f"expected 35 leaves declared / 26 TCs, got {leaves + held} / {total}")
