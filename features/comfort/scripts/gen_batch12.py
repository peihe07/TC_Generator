...
REASONING = {
"16.3": "驗證目標：16.3（ICE2）以九句定出 ICS 介面之 AUTO 狀態呈現、其作用對象、互斥對象、中斷條件與 MTC 例外，九個 037 leaf 逐句對應，一葉一 TC（§8.2.1）。關鍵情境條件：EMEA 軸取**正向值**（16.2）；第一軸 —— `-203`～`-210` 取 ATC 值（ICE2 末句「AUTO is not shown in MTC configurations」蘊含之），`-211` 取 MTC 值；`-207`／`-208` 另補第四軸（MAX A/C，出處 16.13）與第五軸（MAX DEF，出處 3.2）；依 **R-C34** 第九軸與第十三軸暴露 → 全數補，第十二軸不補（ch16 無 tab 條文）。為什麼這樣切：`-207`（互斥）與 `-208`（按下即前往該功能）為同一組關係之兩個方向，可各自獨立失效。刻意略過：**鏡射表反向使用之關鍵結果 —— ICE2 與 C2 在兩處相左，本批驗 ICE2 之版本而不移植 C2**：(一) C2 之互斥對象為四氣流模式與 front defrost，ICE2 為 MAX A/C 與 MAX DEF；(二) C2 云「Auto can change the state of AC, **but do not show this change**」，ICE2 云「**In Auto the A/C button is highlighted**」—— 語意相反。批次 9 之 `-130`／`-131` 已因此得 `no`，此處為其另一側之實作（§8.2.1）。",
...
DIST_AXIS = {
    "16.13": {
        ...
        "delta": "六條之 trigger 各異且其 outcome 亦不全同 —— "
                 "`-231` 改風速 → MAX A/C 解除而**其餘狀態維持**；"
                 "`-232` 改溫度／recirc／模式 → 回前一手動模式"
                 "**除該被改之元素外**；`-233` 按 MAX A/C → 回前一手動模式；"
                 "`-234` 按 A/C → 回前一手動模式且 **A/C 關閉**；"
                 "`-235` 按 AUTO → 系統**進入 AUTO**；"
                 "`-236` 按 MAX DEF → 系統**進入 MAX DEF**。"
                 "**（本串號為撤下 `-224`／`-225` 後之號，見 58 §2；"
