#!/usr/bin/env python3
"""CMF-07 —— Comfort HMI **交付候選本 v2**（0907 本之複本，xlsx_surgical 寫入）。

相對交付本 v1（ENTRY 041）之差異只有兩項：

  R-C68   EMEA ICS 不在販售範圍（Pei 2026-09-22）—— 帶**正向** EMEA PC 之 119 條 TC，
          V／Y／Z 全 0。**列保留、不刪**（R-C46：F／E 不重編，TestRail ID 不丟）。
          依 R-C63 不加 Remarks。實改 112 列／229 格；307 條否定式 EMEA PC 不動
  §2      `Product Document 記錄封面頁`!B5：`Initial Release` → `V1.1`（CMF-06_review §2）

車型欄之值仍一律取 `docs/reports/vm_assign.tsv` —— R-C68 已落於該表（單一真源）。
...
PREV = OUT_DIR_PREV = FEATURE / "sandbox" / "delivery" / (
    "FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case "
    "Specification & Result_SWQT_ComfortHMI_20260922.xlsx")
PREV_SHA = "c2ddf95dcbe1656ef6f16746243fec7e89a0f0dd9f4be27e76d5814df57dbebf"
OUT_DIR = FEATURE / "sandbox" / "delivery"
OUT = OUT_DIR / (
    "FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case "
    "Specification & Result_SWQT_ComfortHMI_20260922_v2.xlsx")
REPORTS = FEATURE / "docs" / "reports"
RECON_TSV = REPORTS / "cmf07_delivery_cell_recon.tsv"
PREV_TSV = REPORTS / "cmf07_vs_v1_cell_delta.tsv"
...
PROD_B5 = "V1.1"                            # CMF-06_review §2：Initial Release → V1.1
SUMMARY = (
...
def main() -> int:
    ...
    if sha256(PREV) != PREV_SHA:
        raise SystemExit("交付候選本 v1 sha256 不符 ENTRY 041 —— 停")
    vm = load_vm()
    tcs = W.load_tcs()
    ...
    intended[(PROD, 5, "B")] = PROD_B5                   # 版本（CMF-06_review §2）
    ...
    print("## CMF-07 交付候選本 v2 —— invariant 與逐格對帳（自產出檔讀回）\n")
    ...
    def attribution(sheet, r, col):
        if sheet == PROD and (r, col) == (5, "B"):
            return "CMF-07 §2 Product Document B5"
        ...
        if r >= NEW_ROW_FIRST:
            return "§3 sibling"
        if col in VM_COLS:
            return "R-C48(amend2)/R-C62/R-C68 車型欄"
        if r == ROW_019_03:
            return "R-C51"
        if col == "N":
            return "R-C49(amend)"
        ...
        for r in range(1, rows + 1):
            for c in range(1, cols + 1):
                ...
                delta.append({"sheet": name, "cell": f"{col}{r}",
                              "item": item or "UNATTRIBUTED",
                              "v1": "" if va is None else str(va),
                              "v2": "" if vb is None else str(vb)})
    g("相對交付候選本 v1 之每一差異格皆歸因到本包一項", not unattr,
      f"未歸因 {len(unattr)} 格 {unattr[:5]}")
    by2 = {}
    for x in delta:
        by2[x["item"]] = by2.get(x["item"], 0) + 1
    print(f"- MEASURED — 相對交付本 v1 之差異格 {len(delta)}：" +
          "、".join(f"{k} {v}" for k, v in sorted(by2.items())))
    with PREV_TSV.open("w", newline="", encoding="utf-8") as fh:
        w = csv.DictWriter(fh, fieldnames=["sheet", "cell", "item", "v1", "v2"],
                           delimiter="\t")
        w.writeheader()
        w.writerows(delta)
