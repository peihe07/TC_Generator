...
"14.3": "驗證目標：14.3（HVACP3.1）定出 HVAC popup 具互動性，兩個 037 leaf 分別對應「可互動」與「可自 popup 調整參數」（§8.2.1）。關鍵情境條件：同 14.1.1。為什麼這樣切：`-169` 驗其可被操作，`-170` 驗操作之結果生效，兩者失效形態不同（按下無反應 vs 有反應而值不變）。刻意略過：條文以 `e.g.` 舉三種 popup（Temp／fan speed／mode），本批以 fan popup 為代表而不逐一列舉 —— 三者之互動性為同一需求，逐一產出即同一可觀察量重複（§4.5）。",
...
"14.19": "驗證目標：14.19（HVACSB6）定出 Climate widget 顯示於當前畫面時八類 popup 之行為，八個 037 leaf 逐項對應（§8.2.1）。關鍵情境條件：條文自身之前件「When the Climate widget is shown on the currently displayed screen」為明文情境條件，標 spec-verbatim（出處 14.19）；`-196`（風速 popup）另取**機型軸**之 R1Low 值 —— 條文明文「show for R1Low, do not show for R1H」；依 **R-C34** 第九軸不補、第十三軸補、EMEA 排除補。為什麼這樣切：八項之行為互異（只在狀態列／顯示／不顯示三種），逐項成條使任一項之失效可定位。刻意略過：**R1H 側之「do not show」未產出 TC** —— 037 之 `104-02` 只寫 `fan Speed Pop-up: show`，未給 R1H 之 leaf；依 R-C33 單位歸 037，不自行增列（§8.2.2），該缺口依 §8.4.2 於此具名。",
"15.1": "驗證目標：15.1（HVACP11.1）之性質**先判再處置**（55 §1.1）：本節**兼具對照表與行為條文兩種性質** —— 其「the HVAC pop ups displayed will follow the chart below」為對照表（該表為圖片，A-CF23），而「all pop ups should display current state of the HVAC systems (not the exact pictures below…)」為可驗之行為條文。**037 已把兩者分開**：其兩個 leaf 皆屬後者（顯示當前狀態／圖為示例），**對照表本身無 leaf**，故本節兩條皆生成，而**對照表之缺口為 R-C16 形態**（037 未產出該 leaf，不產 workbook 列、不入分母），已於上繳 37 §5.2 具名。關鍵情境條件：同 14.1.1。為什麼這樣切：`-201` 驗 popup 顯示當前狀態，`-202` 驗其不依圖片所示，後者以「與圖片不同之值」為輸入，兩者失效形態不同。刻意略過：**不得將表格內容當作行為驗證**（55 §1.1）—— 故無任何一條驗「某功能進入時顯示某 popup」，那正是缺的對照。",
}
...
WITHHELD = [
 ...
 ("SWE1-HVAC-096-01", "`14.12`（HVACP12）之 `knob controls → radial popup` —— **DR #37 之三問未答**：`14.12` 寫「the hard controls」而條文其餘部分顯示同一車上各控制型態不同，該句無法適用（35 §2.1）"),
 ("SWE1-HVAC-096-02", "同 `096-01`：`UP/DOWN toggle → vertical popup`"),
 ("SWE1-HVAC-096-03", "同 `096-01`：`pop-up mirrors physical control action`"),
 ("SWE1-HVAC-098", "`14.14`（HVACP14）「For vehicles with **dual zone climate versions with dual airflow modes** on 8.4/10.1/10.25/12.3 吋 radios」—— **同時卡在兩個未登記之軸**：dual airflow modes（`DATA_REQUESTS` #38，其否定值全語料無字面）與螢幕尺寸（**DR #6**，未解逾十包）"),
...
def main() -> None:
    ...
    if leaves + held != 42 or total != 36:
        raise SystemExit(
            f"expected 42 leaves declared / 36 TCs, got {leaves + held} / {total}")
