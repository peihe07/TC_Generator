# lint036 報告：FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ComfortHMI_20260918.xlsx

- 來源：`features/comfort/sandbox/revise_0918/FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ComfortHMI_20260918.xlsx`（唯讀）
- 資料列數：474
...
| N | 行尾多餘句號 | 0 | 0 | 每行 | 已校準 |
| P | 訊號寫法不合 R-1 v2 | 4 | 4 | 每次命中 | 已校準（SWC 0708：195 —— proc 11／er 184，見上繳 09） |
| Q | 不可見字元（NBSP／全形空格／行尾空白） | 0 | 0 | 每行每欄 | 未校準（R-10(a)，21 包新增） |
| R | Pre-Condition 版面（未編號行／多條件並列） | 1476 | 467 | 每行 | 未校準（R-9(a)，21 包新增） |
...
| 156 | NR1L-ComfortHMI-146 | er | 關係模糊語 'matches' | e that most closely matches the exited AUTO mode ⏎ 3. The rear AUTO button is high |
...
| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 19 | NR1L-ComfortHMI-010 | test_item | 首字小寫 'a' | a pop-up will be shown coming down from that temperature in the status bar to in |
| 20 | NR1L-ComfortHMI-011 | test_item | 首字小寫 'a' | a pop-up will be shown coming down from that temperature in the status bar to in |
| 21 | NR1L-ComfortHMI-012 | test_item | 首字小寫 'for' | for MTC (if the MTC has a Climate screen) it will display a slider bar with the  |
| 109 | NR1L-ComfortHMI-100 | test_item | 首字小寫 'a' | a small pop-up will appear above the Climate main category control (timeout afte |
| 110 | NR1L-ComfortHMI-101 | test_item | 首字小寫 'a' | a small pop-up will appear above the Climate main category control (timeout afte |
| 111 | NR1L-ComfortHMI-102 | test_item | 首字小寫 'a' | a small pop-up will appear above the Climate main category control (timeout afte |
| 342 | NR1L-ComfortHMI-332 | test_item | 首字小寫 'a' | a pop-up will be shown coming down from that temperature in the status bar to in |
| 343 | NR1L-ComfortHMI-333 | test_item | 首字小寫 'for' | for MTC it will display a slider bar with the arrow pointing to the current sett |
| 419 | NR1L-ComfortHMI-409 | test_item | 首字小寫 'the' | the user will be shifted with each press, press and hold of the control will onl |
| 422 | NR1L-ComfortHMI-412 | test_item | 首字小寫 'if' | if the user is not on Climate main when pressing the Mode hard control a small p |
| 423 | NR1L-ComfortHMI-413 | test_item | 首字小寫 'if' | if the user is not on Climate main when pressing the Mode hard control a small p |
| 424 | NR1L-ComfortHMI-414 | test_item | 首字小寫 'a' | a small pop -up will appear above the Climate main category control (timeout aft |
| 425 | NR1L-ComfortHMI-415 | test_item | 首字小寫 'a' | a small pop -up will appear above the Climate main category control (timeout aft |
| 426 | NR1L-ComfortHMI-416 | test_item | 首字小寫 'a' | a small pop -up will appear above the Climate main category control (timeout aft |
...
### P — 訊號寫法不合 R-1 v2（行計 4／列計 4）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 42 | NR1L-ComfortHMI-033 | proc | 車輛屬性之值須以 `[…]` 包覆（037 逐字記法）：'$RECIRC_STAT$ = "not' | 1. PENDING: DR-46 DBC message and raw value for $RECIRC_STAT$ = "not available/B |
| 78 | NR1L-ComfortHMI-069 | proc | 車輛屬性之值須以 `[…]` 包覆（037 逐字記法）：'$RECIRC_STAT$ = "not' | 2. PENDING: DR-46 DBC message and raw value for $RECIRC_STAT$ = "not available/B |
| 80 | NR1L-ComfortHMI-071 | proc | 車輛屬性之值須以 `[…]` 包覆（037 逐字記法）：'$EBL_Stat$ = "not' | 1. PENDING: DR-46 DBC message and raw value for $EBL_Stat$ = "not available/Blin |
| 400 | NR1L-ComfortHMI-390 | proc | 車輛屬性之值須以 `[…]` 包覆（037 逐字記法）：'$EBL_Stat$ = "not' | 1. PENDING: DR-46 DBC message and raw value for $EBL_Stat$ = "not available/Blin |

### R — Pre-Condition 版面（未編號行／多條件並列）（行計 1476／列計 467）
...
| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 42 | NR1L-ComfortHMI-033 | proc | PENDING 佔位（DR-46） | 1. PENDING: DR-46 DBC message and raw value for $RECIRC_STAT$ = "not available/B |
| 78 | NR1L-ComfortHMI-069 | proc | PENDING 佔位（DR-46） | 2. PENDING: DR-46 DBC message and raw value for $RECIRC_STAT$ = "not available/B |
| 80 | NR1L-ComfortHMI-071 | proc | PENDING 佔位（DR-46） | 1. PENDING: DR-46 DBC message and raw value for $EBL_Stat$ = "not available/Blin |
| 400 | NR1L-ComfortHMI-390 | proc | PENDING 佔位（DR-46） | 1. PENDING: DR-46 DBC message and raw value for $EBL_Stat$ = "not available/Blin |
| 476 | NR1L-ComfortHMI-467 | er | PENDING 佔位（DR-47） | 2. PENDING: DR-47 the manual mode the system goes to when AUTO is broken without |
| 477 | NR1L-ComfortHMI-468 | er | PENDING 佔位（DR-47） | 2. PENDING: DR-47 the manual mode the system goes to when AUTO is broken without |

### I-cross — 跨 req_id：觀測窗相同且違例類有交集（R-SU34 v3）（行計 441／列計 441）
...
| 156 | NR1L-ComfortHMI-146 | er | 比較關係 'matches'，而 test_item 上半無數值 | e that most closely matches the exited AUTO mode ⏎ 3. The rear AUTO button is high |
...
### X — 導航路徑無固定入口（§5.8／R-G71）（行計 335／列計 273）
