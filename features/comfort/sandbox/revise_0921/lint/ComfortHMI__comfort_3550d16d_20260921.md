# lint036 報告：FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ComfortHMI_20260907.xlsx

- 來源：`features/comfort/delivered/FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ComfortHMI_20260907.xlsx`（唯讀）
- 資料列數：465
...
| L | test_item 上半過長 (>50 tokens) | 26 | 26 | 每列 | 已校準（R-3 = 50，canon §4.3.1 明文；超限依 R-DIAG21 摘句，不豁免） |
| M | 空欄三態 | 8 | 4 | 每列每欄 | 已校準 |
| N | 行尾多餘句號 | 0 | 0 | 每行 | 已校準 |
| P | 訊號寫法不合 R-1 v2 | 0 | 0 | 每次命中 | 已校準（SWC 0708：195 —— proc 11／er 184，見上繳 09） |
| Q | 不可見字元（NBSP／全形空格／行尾空白） | 0 | 0 | 每行每欄 | 未校準（R-10(a)，21 包新增） |
| R | Pre-Condition 版面（未編號行／多條件並列） | 1450 | 458 | 每行 | 未校準（R-9(a)，21 包新增） |
...
| I-cross | 跨 req_id：觀測窗相同且違例類有交集（R-SU34 v3） | 433 | 433 | 每列每配對（一組命中記二列） | 警示器非判準（R-SU34 v3(c)）—— 命中一律送人裁，不自動判 FAIL |
| W | ER 含比較關係而 test_item 上半無數值（下放包 47 §二 #6） | 3 | 3 | 每次命中 | **待人裁非 FAIL** —— 輸出分二段（下放包 48 §二）：(a) 已裁段只報列數、(b) 新命中段逐列陳述 |
| X | 導航路徑無固定入口（§5.8／R-G71） | 319 | 265 | 每行 | 未校準（§5.8／R-G71，GC-07 新增）—— **WARN 只報不改** |
| Y | PROXI 舊式（R-G70 v4.1：`$Param$ is set to` 為 VF230 同義舊式） | 0 | 0 | 每行 | 未校準（R-G70 v4.1，GC-10 新增）—— **WARN 只報不改**；既有交付本不回修（R-TM13），回修依 R-G72 |

**總計：行計 2243**（列計不加總——同一列可觸發多項檢查）

## 明細

### H — ER 模糊語 (er)（行計 4／列計 4）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 44 | NR1L-ComfortHMI-035 | er | 關係模糊語 'matches' | is displayed and it matches the vehicle's configuration — the model specific ico |
| 156 | NR1L-ComfortHMI-146 | er | 關係模糊語 'matches' | e that most closely matches the exited AUTO mode ⏎ 3. The system is in the pressed |
| 311 | NR1L-ComfortHMI-301 | er | 關係模糊語 'matches' | of the HVAC popups matches the type of the hard controls and mirrors their phys |
| 450 | NR1L-ComfortHMI-440 | er | 關係模糊語 'matches' | eat controls and it matches this vehicle's system configuration |
...
### R — Pre-Condition 版面（未編號行／多條件並列）（行計 1450／列計 458）
...
| 18 | NR1L-ComfortHMI-009 | pre | 多條件並列於同一行 | 3. The vehicle does not have 3 knob HVAC controls with ICS, for which no HVAC sc |
| 18 | NR1L-ComfortHMI-009 | pre | 多條件並列於同一行 | 4. The vehicle is not an EMEA ICS vehicle, whose climate interface is specified  |
...
| 133 | NR1L-ComfortHMI-123 | pre | 多條件並列於同一行 | 3. The vehicle does not have 3 knob HVAC controls with ICS, for which no HVAC sc |
| 133 | NR1L-ComfortHMI-123 | pre | 多條件並列於同一行 | 4. The vehicle is not an EMEA ICS vehicle, whose climate interface is specified  |
...
### I-cross — 跨 req_id：觀測窗相同且違例類有交集（R-SU34 v3）（行計 433／列計 433）
...
| 322 | NR1L-ComfortHMI-312 | expected_result | **窗未完整宣告** —— 訖點無片語可抽，本列不參與 I-cross 比對（R-SU33(b)：ER 須明載窗之起訖） | 起 availability-check → 訖 **未載** |
...
| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 44 | NR1L-ComfortHMI-035 | er | 比較關係 'matches'，而 test_item 上半無數值 | is displayed and it matches the vehicle's configuration — the model specific ico |
| 156 | NR1L-ComfortHMI-146 | er | 比較關係 'matches'，而 test_item 上半無數值 | e that most closely matches the exited AUTO mode ⏎ 3. The system is in the pressed |
| 450 | NR1L-ComfortHMI-440 | er | 比較關係 'matches'，而 test_item 上半無數值 | eat controls and it matches this vehicle's system configuration |

### X — 導航路徑無固定入口（§5.8／R-G71）（行計 319／列計 265）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 15 | NR1L-ComfortHMI-006 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Open the climate screen |
| 15 | NR1L-ComfortHMI-006 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 3. Change the temperature on the climate screen |
...
| 27 | NR1L-ComfortHMI-018 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Select an airflow mode and turn front defrost on from the climate screen |
| 28 | NR1L-ComfortHMI-019 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the state of the "A/C" button on the climate screen with AUTO off |
...
| 58 | NR1L-ComfortHMI-049 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Long-press the temperature up arrow on the climate screen |
| 59 | NR1L-ComfortHMI-050 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the temperature shown on the climate screen |
| 60 | NR1L-ComfortHMI-051 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the temperature shown on the climate screen |
...
| 65 | NR1L-ComfortHMI-056 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
| 65 | NR1L-ComfortHMI-056 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Press the fan up button on the climate screen |
| 66 | NR1L-ComfortHMI-057 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
| 66 | NR1L-ComfortHMI-057 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Touch a fan segment on the climate screen |
| 67 | NR1L-ComfortHMI-058 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
| 67 | NR1L-ComfortHMI-058 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Slide across the fan segments on the climate screen |
| 68 | NR1L-ComfortHMI-059 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
...
| 69 | NR1L-ComfortHMI-060 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
...
| 74 | NR1L-ComfortHMI-065 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed on the climate screen |
...
| 87 | NR1L-ComfortHMI-078 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed, then turn the climate system off using the climate power  |
...
| 92 | NR1L-ComfortHMI-083 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Change the fan speed from the climate screen |
| 92 | NR1L-ComfortHMI-083 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 3. Change the airflow mode from the climate screen |
...
| 110 | NR1L-ComfortHMI-101 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Open a screen other than Climate main and press the Mode hard control once |
| 111 | NR1L-ComfortHMI-102 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Open a screen other than Climate main and press the Mode hard control once |
| 112 | NR1L-ComfortHMI-103 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Press the Mode hard control while on the Climate main screen and read the mai |
...
| 278 | NR1L-ComfortHMI-268 | proc | 導航標的 'tab' 而同 TC 無固定入口 | 1. Note which tab is currently shown on the lower screen |
| 280 | NR1L-ComfortHMI-270 | proc | 導航標的 'tab' 而同 TC 無固定入口 | 1. Note which tab is currently shown in the climate section on the head unit |
...
| 299 | NR1L-ComfortHMI-289 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Open a NAV screen and change the fan speed using the fan speed hard control |
| 299 | NR1L-ComfortHMI-289 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Open a Projection screen and change the fan speed using the fan speed hard co |
| 302 | NR1L-ComfortHMI-292 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Touch the selectable item of the bottom pop-up and read the screen |
...
| 342 | NR1L-ComfortHMI-332 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Set the temperature units to Celsius and open a screen outside the climate ma |
...
| 360 | NR1L-ComfortHMI-350 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Set the temperature units to English and open the climate screen |
| 361 | NR1L-ComfortHMI-351 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Set the temperature units to Metric and open the climate screen |
...
| 369 | NR1L-ComfortHMI-359 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the temperature shown on the climate screen |
| 369 | NR1L-ComfortHMI-359 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Press the temperature up arrow on the climate screen |
| 370 | NR1L-ComfortHMI-360 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the temperature shown on the climate screen |
| 371 | NR1L-ComfortHMI-361 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the temperature shown on the climate screen |
| 372 | NR1L-ComfortHMI-362 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Press and hold the temperature up arrow on the climate screen |
...
| 378 | NR1L-ComfortHMI-368 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
| 378 | NR1L-ComfortHMI-368 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Press the fan up button on the climate screen |
| 379 | NR1L-ComfortHMI-369 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
| 379 | NR1L-ComfortHMI-369 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Touch a fan segment on the climate screen |
| 380 | NR1L-ComfortHMI-370 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
| 380 | NR1L-ComfortHMI-370 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Slide across the fan segments on the climate screen |
| 381 | NR1L-ComfortHMI-371 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
| 382 | NR1L-ComfortHMI-372 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
...
| 383 | NR1L-ComfortHMI-373 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the fan speed shown on the climate screen |
| 384 | NR1L-ComfortHMI-374 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Turn "A/C" off and select an airflow mode other than Windshield on the climat |
...
| 390 | NR1L-ComfortHMI-380 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Select a known airflow mode on the climate screen, then press "MAX DEF" |
...
| 427 | NR1L-ComfortHMI-417 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Note the main category label on the climate screen |
...
| 453 | NR1L-ComfortHMI-443 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Turn one seat control on and another seat control off, then leave the control |
| 453 | NR1L-ComfortHMI-443 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Enter the controls screen |
...
| 458 | NR1L-ComfortHMI-448 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 1. Move the Comfort widget to its second screen and leave the home screen |
| 458 | NR1L-ComfortHMI-448 | proc | 導航標的 'screen' 而同 TC 無固定入口 | 2. Open the home screen |
