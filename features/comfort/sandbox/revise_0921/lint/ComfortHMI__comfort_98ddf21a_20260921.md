# lint036 報告：FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ComfortHMI_20260921.xlsx

- 來源：`features/comfort/sandbox/revise_0921/FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ComfortHMI_20260921.xlsx`（唯讀）
- 資料列數：476
...
| U | PENDING 佔位（四欄全掃，含 ER 側） | 4 | 4 | 每次命中 | 計數用（A-PM16：ER 側原不受任何檢查覆蓋） |
...
| 156 | NR1L-ComfortHMI-146 | er | 關係模糊語 'matches' | e that most closely matches the exited AUTO mode ⏎ 3. The rear "AUTO" button is hi |
...
| 476 | NR1L-ComfortHMI-467 | er | 關係模糊語 'matches' | e that most closely matches the exited AUTO mode |
| 477 | NR1L-ComfortHMI-468 | er | 關係模糊語 'matches' | e that most closely matches the exited AUTO mode |
...
### U — PENDING 佔位（四欄全掃，含 ER 側）（行計 4／列計 4）
...
| 156 | NR1L-ComfortHMI-146 | er | 比較關係 'matches'，而 test_item 上半無數值 | e that most closely matches the exited AUTO mode ⏎ 3. The rear "AUTO" button is hi |
...
| 476 | NR1L-ComfortHMI-467 | er | 比較關係 'matches'，而 test_item 上半無數值 | e that most closely matches the exited AUTO mode |
| 477 | NR1L-ComfortHMI-468 | er | 比較關係 'matches'，而 test_item 上半無數值 | e that most closely matches the exited AUTO mode |
