...
| DR-VT2 | V43 SYSRA DocID `VF655_V43_R3`（247 列）之定性：**非單純誤植 —— 較新版次之部分重匯（75% 逐字重複）且含舊批所無之四節（EPB／APB／Rearview ×2）；VF655／R3 標籤與內容較新（R4 新寫法）之矛盾上問（R-VT22(e)，量測上繳 09）**；另 R3 vs R4；`Melco ID` 全空；拼字／重音清單 | no（惟定性決定未來 037 對齊與 K-10 範圍） | 母體 v2 之批次歸屬 | 已登記，**建議與 DR-VT1 併送（09 包量測為主佐證）** | | |
