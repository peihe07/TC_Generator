...
Feature slug：`vsm_v43`。條號系列 **`R-VT`**（本線單一來源，R-G62 live 取號）。
...
### R-VT23 —— K-11／K-12／K-13 裁：R4 已刪內容之隔離；Layer 2 v2 定表送 Pei；二項補測令

```
R-VT23（分析層裁定 2026-09-02，上繳 10；依據全為 R4 修訂說明逐字與 08／09 兩包實測）
(a) K-11：`-472`／`-473` 比照 superseded 廢（功能由 VF655 新寫法 2 列承接；R4「Replacement of
    …RainSensorLevel_Req」逐字為據）；superseded 檔擴至 5 列。R-VT22(a)(iii) 之覆蓋面不足記分析層
    （規則只掃近似對，未掃「舊名存於其他列」）。
(b) K-12：Blind Spot 6 列（chapter .15，`-577`〜`-582`）隔離為 `deprecated_r4`（規格無對應節＋
    R4 兩處明載刪除）：不刪列只標，不生成 TC，037 重錨終裁。
(c) K-13 逐項：參數本身被刪之組態表列 `-844`（Model_Year）／`-824`／`-825`（LBSS／RBSS）
    → 同 (b) `deprecated_r4`；引用被刪參數之敘述列 `-897`／`-899`／`-907` → 標 `stale_ref_r4` 保留，
    生成時被刪條件子句不入 TC（§8.4.2 以現行規格為範圍），037 重錨終裁。
    淨效果（執行層實測對帳）：有效母體 336 − 2（廢）− 9（deprecated）= 325＋3 列 stale-ref。
(d) Layer 2 v2 定表（分析層，送 Pei 准）：19 組照上繳 10 §二，修二處 ——
    Side and Blind Spot Warnings 因 (b) 去 BSD 6 列改名 **Side Distance Warning**（10）；
    PROXI Configuration 因 (c) 去 3 列为 65。全表見 framework.md v2 表（Pei 准後鎖）。
(e) 補測令（下包）：v2 有效母體之含解得占比重算（詞界式）；`1.14.*` ↔ 規格組態節（para 1287 起）
    token 共現對映（供 PROXI Configuration 組 spec 錨）。
```

### R-VT24 —— Layer 2 v2 鎖定（Pei「准」）

```
R-VT24（Pei 裁定 2026-09-02：「准 准 寫」之首字）
Layer 2 v2 依 R-VT23(d) 鎖定：19 組，組構與命名即裁；各組列數以 11 包 W-2 實測落表，
實測與預期不符者回報不自調。framework v2 表直標「鎖定 (R-VT24)」，不再待准。
...
### R-VT27 —— 上繳 13＋14 合裁：綠色通道 3／3；寫回 dry-run 插隊；K-18〜K-22 裁；Pei 口裁補記

```
R-VT27（分析層裁定 2026-09-02，上繳 13＋14）
(a) SDW 10／LDW 18 覆核通過，零修訂 —— 綠色通道 **3／3 成立**（b2-1／2／3）。
    LDW 之 K-21 處置追認（test_item 逐字保留規格誤寫 Sensibility，ER 依該節實際訊號）；
    -433／-438 之 reasoning 補一句矛盾註（審計層非內容四欄，不計修訂）。
(b) SDW 更正二項採認：test_set 改名未寫回 TSV（報告與資料檔不一致型）已修；
    重音誤判撤回 2 筆（字元類寫窄之靜默少抓，自掴歸因正確）。
    **「資料檔 = 條文」斷言入兩線每包固定自檢**（凡條文規定之欄位值——組名／狀態／旗標——
    上繳須附實測斷言）。
(c) K-18：HMI r315B（checkbox）與規格三選項不符 —— 依 IN §8.6 取規格，併 R-VT26(d) 同型
    入 DR-VT2 佐證；K-19：DR-VT2 拼法佐證以更正後 4 筆為準，另加 LDW 四類（含 Sensibility／
    Intensity 語意矛盾 ×2）；K-22：無下放包作業取號 14 確認。
(d) Pei 口裁補記：「要」＝台帳重生得由 Pei 逐次授權執行層代行（R-VT14(c) 加註；要件：
    重生前零消失斷言，本次 652→697 零消失實測）；「以上皆準」＝K-18／K-19／K-20／
    「資料檔=條文」斷言四項照案；另 R-VT1–R-VT18 之樹外替代量測獲台帳逐條追認（零差異）記實。
(e) 綠色通道生效方式採執行層 §八-2 建議：**先插一次寫回 dry-run**（lint036 首見本線 54 TC），
    紅項歸因交裁後方連跑 b2-4 起（上繳彙報式）。暫代期寫回規則：列序依 leaves_interim_v2
    表序；TC ID `NR1L-VSM43-{n:03d}` 依列位起編（重錨時全重編＋對映表，R-VT18(c) 既定）；
    Q／AB／車型欄留空、S = NA、E 留空（依 R-VL24(b) 交付慣例，跨線同適用）。
    重錨前不得交付不變（R-VT18(c)）。
```
...
| R-VT27 | 2026-09-02 | 台帳已重生（上繳 14 §一）；本檔錨點實測 26 |
