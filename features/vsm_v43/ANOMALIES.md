...
- **裁決（R-VT16(c)，2026-09-02）**：四名**准入排除清單**（偽陽性，標記不刪）；
  PROXI 母體以 **39** 為報表基準。v5 已設排除旗標。**RESOLVED**。
...
## A-VT26 — 五名內部訊號之「解得」係跳過段 1／2 之 DBC 直查（RESOLVED）
...
- **裁決（R-VT16(e)，2026-09-02）**：五列**退回 `未解得(止於段1)`** —— 內部形須有段 1 依據，
  僅段 3 同名不足以稱解得。本線「解得」基線 = **81**（全數 CAN 形、全數有 VAL_）。
  v5 已改判並逐列註明。**RESOLVED**。
...
## A-VT29 — `PROXI.First` 之抽名脈絡已審：跨句誤配之偽陽性（RESOLVED）

- **背景**：05 包 §一-3 令審其抽名脈絡 —— 偽陽性則併 A-VT21 型標記，否則維持 CAN-C 缺口。
- **實測**（自 `word/document.xml` 抽取本逐字定位）：Configuration Parameters 表之每一列尾隨兩句 ——
  `Range value is indicated in the standard PROXI.` 換行 `First Trial Value depends on the project Configuration`。
  上繳 02 之 CAN 形正則 `\b([A-Z][A-Z0-9_]{3,})\.([A-Za-z][A-Za-z0-9_]*)\b`
  於段落串接後跨句配到 **`PROXI` ＋ `.` ＋ `First`**。該句於規格中出現 **30 次以上**（每個 PROXI 參數列各一）。
- **判定**：**抽名偽陽性**，非訊號名，亦非 CAN-C 缺口。
- **處置**：v5 設排除旗標（A-VT21 型，標記不刪）。
  **CAN-C 真缺口自 6 收斂為 5**（名目 6 列，扣除本名）。
- **同型風險**：CAN 形正則在「句號 ＋ 換行 ＋ 大寫字開頭」處都可能跨句誤配。
  本包未重抽名（05 包未令），但此為抽名式之已知缺陷，列入下包建議。

## A-VT30 — E29 寬讀之實效：不改結果，只補錨點（RESOLVED）

- **實測**（R-VT16(b) 寬讀候選集，`Technical Reference` 含 `665` = **247** 列；字面 `VF665` 僅 3 列）：
  以寬讀候選集重跑比對，**新增命中 4 名**，**但四者原本即由「全表比對」命中，結果一列未變**：
...
- **意義（非零效益，但不在結果欄）**：R-P353(ii) 之具名 UI 元件需要**來源錨點**。
  字面讀法（3 列）下，這四名之 HMI 命中來自全表，**無法證明該設定項屬本 VF**；
  寬讀後其 `Technical Reference` = `VF230/665` 逐字載明屬 VF665。
  即**寬讀補的是錨點的可引用性，不是命中數**。
- **例外一名**：`Geolocation_Menu`（亦為 UI+PROXI 雙路徑）**不在寬讀候選集內** ——
  其 HMI 命中列之 `Technical Reference` 不含 `665`。該名之 UI 錨點於 P4 引用時須另尋依據。
