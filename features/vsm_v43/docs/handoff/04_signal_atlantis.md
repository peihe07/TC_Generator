...
對象：執行層。00–03 包續有效；本包只載差異。sha8 報 body_sha8；R-VT11–R-VT15 未入台帳前取樹外 `--out`；台帳不重生（R-VT14(c)，R-VL13 已獲 Pei 追認）。

> ## 補遺（2026-09-02，R-VT15 —— 本補遺優先於下文衝突處）
> Pei 已放件 ATL-Mi DBC：`forms/P363_BH-CAN [07338]_3A_R2.dbc`（分析層驗收：BO_ 99／SG_ 4576／VAL_ 503；原 R-13 28 名逐字命中 26／28）。
> W-5‴ 第 5／6 項改：段 3 對本件實查（latin-1，CRLF），**「解得」合法**（R-VT15(d)）；Atlantis High R1 DBC 降旁證；
> 原 R-13 28 列依本件重判（預期僅餘 2 名未解，R-VT15(b)）；CAN-C 查無者記「未解得（CAN-C DBC 未到件）」。
> §五 E27 作廢（`解得` 為觀測值；每列備註 BO_／SG_ 實名與 VAL_ 有無）；新增 E31：原 R-13 28 列經重判後仍未解者 ≤ 2。
> §七「E27 ≥ 1 即停」作廢。§四 DR 送出事項全數改「Pei 裁先不送」；本線因 DR-VT1 不送繼續止於 P0–P3。
