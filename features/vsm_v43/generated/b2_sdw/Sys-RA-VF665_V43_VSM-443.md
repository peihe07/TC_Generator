...
> IF the user sets "Sdw_Setting.Req" internal signals to "Off" THEN TLM shall set "TELEMATIC_VEHICLE_SETUP.Sdw_Req" B-CAN signal equal to "Off" and sends this signal to IPC
...
## TC 1 — Side distance warning request sent for off selection
...
(Side distance warning request sent for off selection)
```
...
```
1. Select "Setting" = "Off" on the TLM display
2. Read the signal $TELEMATIC_VEHICLE_SETUP.Sdw_Req$ and check that it is 0 (Off)
```

### expected_result

```
1. The menu item "Setting" shows "Off"
2. The signal value $TELEMATIC_VEHICLE_SETUP.Sdw_Req$ = 0 (Off) is received
```

- specification_reference：`Vehicle_Setup_Management_by_VP-LTM_R1L_TBM_VF665_V43_R4_1.11.1.1.5
Sys-RA-VF665_V43_VSM-443`
- design_method：等價劃分 (Equivalence Partitioning, EP)｜priority：P1｜split_flag：False｜distinguishing_axis：Setting option = Off
