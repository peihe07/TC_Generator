...
> IF the user sets "Sdw_Chime_Volume_Setting.Req" internal signals to "Low" THEN TLM shall set "TELEMATIC_VEHICLE_SETUP.SdwChimeVolume_Req" B-CAN signal equal to "Low" and sends this signal to IPC
...
## TC 1 — Side distance warning chime volume request sent for low selection
...
(Side distance warning chime volume request sent for low selection)
```
...
```
1. Select "Chime Volume" = "Low" on the TLM display
2. Read the signal $TELEMATIC_VEHICLE_SETUP.SdwChimeVolume_Req$ and check that it is 0 (Low)
```

### expected_result

```
1. The menu item "Chime Volume" shows "Low"
2. The signal value $TELEMATIC_VEHICLE_SETUP.SdwChimeVolume_Req$ = 0 (Low) is received
```

- specification_reference：`Vehicle_Setup_Management_by_VP-LTM_R1L_TBM_VF665_V43_R4_1.11.1.1.5
Sys-RA-VF665_V43_VSM-448`
- design_method：邊界值分析 (Boundary Value Analysis, BVA)｜priority：P1｜split_flag：False｜distinguishing_axis：Chime Volume option = Low
