...
> IF the user sets "FSCWPlus_Setting.Req" internal signals to "Brake " THEN TLM shall set "TELEMATIC_VEHICLE_SETUP2.FSFCWPlusSetting_Req" B-CAN signal equal to "Brake " and sends this signal to IPC
...
## TC 1 — FCW Setting2 request sent for brake selection
...
(FCW Setting2 request sent for brake selection)
```
...
```
1. Select "Forward Collision Warning Setting2" = "Brake" on the TLM display
2. Read the signal $TELEMATIC_VEHICLE_SETUP2.FSFCWPlusSetting_Req$ and check that it is 2 (Brake)
```

### expected_result

```
1. The menu item "Forward Collision Warning Setting2" shows "Brake"
2. The signal value $TELEMATIC_VEHICLE_SETUP2.FSFCWPlusSetting_Req$ = 2 (Brake) is received
```

- specification_reference：`Vehicle_Setup_Management_by_VP-LTM_R1L_TBM_VF665_V43_R4_1.11.1.1.6
Sys-RA-VF665_V43_VSM-462`
- design_method：等價劃分 (Equivalence Partitioning, EP)｜priority：P1｜split_flag：False｜distinguishing_axis：Setting2 option = Brake
