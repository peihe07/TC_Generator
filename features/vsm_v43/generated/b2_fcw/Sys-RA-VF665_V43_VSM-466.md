...
> IF the user sets "FSCWPlus_Activation_Mode_Setting.Req" internal signals to "Near" THEN TLM shall set "TELEMATIC_VEHICLE_SETUP2.FSFCWPlusActivationMode_Req " B-CAN signal equal to "Near" and sends this signal to IPC
...
## TC 1 — FCW sensitivity request sent for near selection
...
(FCW sensitivity request sent for near selection)
```
...
```
1. Select "Forward Collision Warning Sensitivity" = "Near" on the TLM display
2. Read the signal $TELEMATIC_VEHICLE_SETUP2.FSFCWPlusActivationMode_Req$ and check that it is 0 (Near)
```

### expected_result

```
1. The menu item "Forward Collision Warning Sensitivity" shows "Near"
2. The signal value $TELEMATIC_VEHICLE_SETUP2.FSFCWPlusActivationMode_Req$ = 0 (Near) is received
```

- specification_reference：`Vehicle_Setup_Management_by_VP-LTM_R1L_TBM_VF665_V43_R4_1.11.1.1.6
Sys-RA-VF665_V43_VSM-466`
- design_method：邊界值分析 (Boundary Value Analysis, BVA)｜priority：P1｜split_flag：False｜distinguishing_axis：sensitivity = Near
