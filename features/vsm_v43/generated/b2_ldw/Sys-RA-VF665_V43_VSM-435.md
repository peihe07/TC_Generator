...
> IF the user sets "LDW_Intensity_Setting.Req" internal signals to "Low" THEN TLM shall set "TELEMATIC_VEHICLE_SETUP2.LDW_Intensity_Req" B-CAN signal equal to "Low" and sends this signal to IPC
...
## TC 1 — Lanse Sense Strenght 2 request sent for low selection
...
(Lanse Sense Strenght 2 request sent for low selection)
```
...
The menu item "Lanse Sense Strenght 2" is displayed and reachable
```
...
```
1. Select "Lanse Sense Strenght 2" = "Low" on the TLM display
2. Read the signal $TELEMATIC_VEHICLE_SETUP2.LDW_Intensity_Req$ and check that it is 0 (Low)
```

### expected_result

```
1. The menu item "Lanse Sense Strenght 2" shows "Low"
2. The signal value $TELEMATIC_VEHICLE_SETUP2.LDW_Intensity_Req$ = 0 (Low) is received
```

- specification_reference：`Vehicle_Setup_Management_by_VP-LTM_R1L_TBM_VF665_V43_R4_1.11.1.1.4
Sys-RA-VF665_V43_VSM-435`
- design_method：邊界值分析 (Boundary Value Analysis, BVA)｜priority：P1｜split_flag：False｜distinguishing_axis：Strenght 2 option = Low
