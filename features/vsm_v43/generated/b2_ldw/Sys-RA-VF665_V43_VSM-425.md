...
> IF the user sets "LDW_Sensibility_Setting.Req" internal signals to "Early" THEN TLM shall set "TELEMATIC_VEHICLE_SETUP2.LDW_Sensibility_Req" B-CAN signal equal to "Early" and sends this signal to IPC
...
## TC 1 — Lanse Sense Warning 2 request sent for early selection
...
(Lanse Sense Warning 2 request sent for early selection)
```
...
The menu item "Lanse Sense Warning 2" is displayed and reachable
```
...
```
1. Select "Lanse Sense Warning 2" = "Early" on the TLM display
2. Read the signal $TELEMATIC_VEHICLE_SETUP2.LDW_Sensibility_Req$ and check that it is 0 (Early)
```

### expected_result

```
1. The menu item "Lanse Sense Warning 2" shows "Early"
2. The signal value $TELEMATIC_VEHICLE_SETUP2.LDW_Sensibility_Req$ = 0 (Early) is received
```

- specification_reference：`Vehicle_Setup_Management_by_VP-LTM_R1L_TBM_VF665_V43_R4_1.11.1.1.3
Sys-RA-VF665_V43_VSM-425`
- design_method：邊界值分析 (Boundary Value Analysis, BVA)｜priority：P1｜split_flag：False｜distinguishing_axis：Warning 2 option = Early
