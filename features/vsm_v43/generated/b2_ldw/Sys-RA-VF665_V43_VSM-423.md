...
> WHEN TLM receives "IPC_VEHICLE_SETUP2.LDW_Sensibility" message THEN TLM updates the LDW Sensibility information on its display through "TLM_Vehicle_Setup_Menu.Info" internal signal
...
## TC 1 — Lanse Sense Warning 1 display updated on reception of LDW_Sensibility message
...
(Lanse Sense Warning 1 display updated on reception of LDW_Sensibility message)
```

### pre_conditions

```
The menu item "Lanse Sense Warning 1" is displayed and shows "Early"
```
...
```
1. Send the signal $IPC_VEHICLE_SETUP2.LDW_Sensibility$ = 2 (Late)
2. Read the menu item "Lanse Sense Warning 1" and check that it shows "Late"
```

### expected_result

```
1. The signal $IPC_VEHICLE_SETUP2.LDW_Sensibility$ = 2 (Late) is registered without a bus error
2. The menu item "Lanse Sense Warning 1" shows "Late"
```

- specification_reference：`Vehicle_Setup_Management_by_VP-LTM_R1L_TBM_VF665_V43_R4_1.11.1.1.3
Sys-RA-VF665_V43_VSM-423`
- design_method：功能測試 (Functional based ; no specific technique)｜priority：P1｜split_flag：False｜distinguishing_axis：branch = Leve 3 (two options)
