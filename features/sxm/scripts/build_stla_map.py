#!/usr/bin/env python3
"""Step 1 (SXM) — leaf → STLA id → CFTS024 clause, ReqIF-sourced.

Ported from features/amfm/scripts/build_stla_map.py, then switched to the HYBRID
source ruled in DECISIONS §1. Three sources, each supplying only what it is
authoritative for:

  ReqIF  → clause id, clause text, position in the specification tree.
           Identifiers are ATTRIBUTES (`ReqIF.ForeignID`), so none of the
           docx path's typographic inference applies.
  docx   → the printed section number (heading text) and the scope metadata
           line (`[Artifact Type:…] [ECU:…] [Market:…] [Radio:…]`), NEITHER of
           which the ReqIF carries — every scope attribute is empty on all
           1,604 exported objects, and absorption decisions turn on those tags.
  bracket lookup → nothing. It is retained purely as a VERIFIER, and a
           disagreement with the ReqIF placement raises rather than warns.

What the switch removed: the `7-digit:` paragraph-anchor parse, the
"text runs to the next tagged line or heading" assumption, and the requirement
that heading anchors increase monotonically for a bisect to be defined.

What it did NOT remove: `HEADING_RE` (still the only source of the printed
number, full-width brackets included), and every 037-side fragility — A-SX01's
`(add)` marker and A-AM01's full-width brackets are defects in the Requirement
Title column, which the ReqIF says nothing about.

Three differences from the AMFM copy, each of which would fail silently if
carried over unchanged:

1. **The id is not always the last token.** 11 leaves append a literal `(add)`
   marker after the id (A-SX01 / A-SX03), so the id is SEARCHED for rather than
   matched against end-of-string. The AMFM pattern is effectively anchored by
   its callers and would drop exactly those 11 while reporting a clean 191/202.
2. **The 037's header row is 8 and `Categorization` sits at column AE** — and a
   data row further down carries the words "requirement description" inside a
   prose cell, so the header is located by an exact `Categorization` match
   rather than a substring search.
3. **No external documents.** All 202 leaves resolve inside CFTS024: no
   CFTS011/CFTS004 split, no declared-id override, no duplicate declared ids.
...
Usage:
    python features/sxm/scripts/build_stla_map.py --feature-dir features/sxm
"""
...
from reqif_reader import load_reqif  # noqa: E402
...
# SEARCHED, never anchored to end-of-string: 11 SXM titles carry a trailing
# `(add)` marker after the id (A-SX03).
...
def load_leaves(a03_path: Path) -> list[dict]:
    ...
    wb.close()
    # Located by an EXACT `Categorization` cell: this 037 has a data row whose
    # prose contains "requirement description", which a substring search picks
    # up as the header and then fails on column count.
    hdr = next((i for i, r in enumerate(rows)
                if any(norm(v) == "categorization" for v in r)), None)
    if hdr is None:
        raise BuildError(f"{a03_path.name}: no row with a Categorization header")
...
def load_clauses_reqif(reqif_path: Path, sections: list[dict],
                       docx_meta: dict[int, dict] | None = None) -> dict[int, dict]:
    """{clause id: {metadata, text, section, section_title, anchor}} from ReqIF.

    This replaces `load_paragraph_anchors` as the source of record for the
    primary document. The ReqIF carries the clause id as an ATTRIBUTE
    (`ReqIF.ForeignID`) and the text as another (`ReqIF.Text`), so none of the
    docx path's typographic inference is involved: no `7-digit:` prefix parse,
    no "text runs until the next tagged line or heading" assumption, and no
    requirement that heading anchors increase monotonically.

    What the ReqIF does NOT carry is the printed section number — its outline
    is positional and counts unnumbered description objects, so §1.5.1 Seek Up
    exports as `1.5.16`. The printed number therefore still comes from the
    docx heading text, joined through the heading's own anchor id, which is a
    clause id present in both sources (verified 200/200 for CFTS024).
    """
    clauses = load_reqif(reqif_path)
    printed = {s["anchor"]: s for s in sections}
    by_outline = {c["section"]: cid for cid, c in clauses.items() if c["section"]}

    def enclosing_heading(outline: str) -> int | None:
        """Nearest ancestor in the ReqIF tree that is a printed heading."""
        parts = outline.split(".")
        while len(parts) > 1:
            parts = parts[:-1]
            cid = by_outline.get(".".join(parts))
            if cid is not None and cid in printed:
                return cid
        return None

    out: dict[int, dict] = {}
    for cid, c in clauses.items():
        # Requirement clauses only. The export also carries the 200 heading
        # objects (identified by ReqIF's own `ReqIF.ChapterName`) and 12
        # embedded-object placeholders; counting either as a clause would
        # inflate the unallocated-clause report with things no leaf could ever
        # claim. 1,604 - 200 - 12 = 1,392, exactly the docx clause set.
        if c["attrs"].get("ReqIF.ChapterName"):
            continue
        if not c["text"] or c["text"].strip().lower() == "embedded object":
            continue
        head = enclosing_heading(c["section"]) if c["section"] else None
        sec = printed.get(head) if head else None
        # Scope tags come from the DOCX metadata line, not the ReqIF: this
        # export leaves Artifact Format / ECU / Market / Radio / EE
        # Architecture empty on all 1,604 objects, and those tags are what
        # absorption decisions turn on (AMFM declined 4872436/4872437 on
        # `ECU:RRM` + `Radio:noSys`; 024's RDS/RBDS split is a Market read).
        # The metadata line is keyed by the same id at the head of the line —
        # a prefix parse, not the body-extraction inference the ReqIF replaces.
        out[cid] = {
            "metadata": (docx_meta or {}).get(cid, {}).get("metadata", ""),
            "text": c["text"],
            "section": sec["section"] if sec else None,
            "section_title": sec["title"] if sec else None,
            "anchor": sec["anchor"] if sec else None,
            "reqif_outline": c["section"],
        }
    return out


def verify_against_bracket(mapping: dict, sections: list[dict],
                           home_doc: str) -> None:
    """The bracket map is now a VERIFIER, and disagreeing is fatal.

    Two independent derivations of the same fact: the ReqIF says which heading
    a clause sits under (tree ancestry); the bracket lookup says which heading
    has the largest anchor not exceeding the clause id (arithmetic over the
    docx). They cannot disagree unless one of the two exports moved — which is
    exactly the event that must stop a build rather than colour a log line, so
    this raises instead of warning.
    """
    problems = []
    for rid, e in sorted(mapping.items()):
        if e.get("doc") != home_doc or e.get("resolution") != "reqif":
            continue
        sec = bracket(sections, e["stla_id"])
        if sec is None:
            problems.append(f"{rid}: clause {e['stla_id']} brackets nowhere")
        elif sec["section"] != e["section"]:
            problems.append(
                f"{rid} (clause {e['stla_id']}): ReqIF places it in "
                f"§{e['section']} {e['section_title']!r}, the bracket lookup in "
                f"§{sec['section']} {sec['title']!r}")
    if problems:
        raise BuildError(
            "THE TWO DERIVATIONS DISAGREE — the ReqIF export and the docx no "
            "longer describe the same document, so neither can be trusted "
...
def build(leaves, sections, external: dict[str, list[str]], home_doc: str,
    ...
    for leaf in leaves:
        ...
        if inside:
            ...
            if para is None:
                raise BuildError(
                    f"{leaf['req_id']}: clause {sid} is inside the {home_doc} "
                    "anchor range but absent from the ReqIF clause index — the "
                    "export and the 037 have diverged")
            # ReqIF is the source of record for section, title and text; the
            # bracket lookup runs anyway, as a verifier (verify_against_bracket).
            entry |= {"doc": home_doc, "section": para["section"],
                      "section_title": para["section_title"],
                      "anchor": para.get("anchor"),
                      "spec_paragraph": para["text"],
                      "spec_paragraph_metadata": para["metadata"],
                      "resolution": "reqif"}
        else:
...
def check_batches(mapping: dict, batches_md: Path) -> list[str]:
    ...
    """
    # The req prefix is derived from the map, never hard-coded: this file was
    # ported from AMFM, where the prefix is SWE-RA-RAD-, and a stale literal
    # would report every SXM leaf as "in no batch" while the table is fine.
    prefix = re.match(r"^(.*?)\d+$", next(iter(mapping))).group(1)
    ...
    for line in batches_md.read_text(encoding="utf-8").splitlines():
        ...
        req_ids = parse_leaf_selector(ids, prefix)
...
def run(args) -> int:
    ...
    # HYBRID (DECISIONS §1): the ReqIF is the source of record for clause ids
    # and text; the docx heading text remains the only source of the printed
    # section number, joined through the heading anchor.
    reqif = resolve_path(cfg, "reqif_doc")
    docx_paragraphs = load_paragraph_anchors(cfts)
    paragraphs = load_clauses_reqif(reqif, sections, docx_paragraphs)
    ...
    # ABORT-level, never a warning: two independent derivations of the same
    # placement must agree, or one of the exports has moved.
    verify_against_bracket(mapping, sections, home_doc)
    for o in overrides:
        mapping[o["req_id"]] |= {
