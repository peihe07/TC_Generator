...
- 日期：2026-08-26（rev 2 —— `<vent off>` 依 R-AM17 回填，B2 收工）
...
**一項須留存之揭露：** 該值**不在 CFTS019 內**。撤回 DR-AM8 所結案者為
**TC 之可執行性**，非**文件之完整性** —— 以規格全文檢索 −16 dB 者仍將查無。
故 287 之 reasoning 明寫出處為 R-AM17 而非文件；若上游日後補入定義且與本值
不符，287 須依文件回改。本節保留此段，不因結案而刪。
