...
## [A-AM18] B7 寫回未完成 —— `diff_cells` 之成本隨模板空列而非資料列決定

**狀態：B7 之 17 條（tc_id 352–368、列 361–377）已生成並通過自檢與 lint，
但未寫入工作簿。** 交付簿停在 `SWQT_AudioMgmt_B1-B6.xlsx`（351 列）。
...
根因：`Test Case Specification` 分頁之 `max_row` 為 **1411**（母本模板帶
約 1,400 列預格式化空列），非資料列數 377。`backend.xlsx_surgical.diff_cells`
逐格比對兩份 workbook，成本為 **1411 × 34 ≈ 48,000 格 × 2 份**，且兩份
完整 workbook 物件同時駐留記憶體。
...
`diff_cells` 之走訪範圍宜以**實際資料列**為界而非 `ws.max_row`，
或改以 `ws.iter_rows(max_row=<last data row>)` 限縮。
`backend/xlsx_surgical.py` 為十五個 feature 共用，改動屬通案，
比照 A-AM06 之處置：登記並建議，不由本 feature 逕改。

**影響面**：本件不僅擋 B7，亦擋 R-AM18 回溯站之寫回（六項，見上繳包 25
§八）—— 該次寫回之資料量更大。建議列為收官前之阻塞項處理。
