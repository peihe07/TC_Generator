...
### 裁定（下放包 30 §一，2026-08-27）

**成立。SWE1_AMM_174 → CFTS019-4866632（改錨）。** 執行層三閘證據全數複核通過。
規格形態為 store→recall 成對：第一對 4866629（`at least one loudspeaker`）／4866632
對應 173（SYS-RA 471）與 **174**（474）；第二對 4866659（`front or rear loudspeakers`）／
4866662 對應 175（489）與 176（492）。葉文雙合、位置窗 4866629 < X < 4866659
將 4866662 排於窗外。**兩葉各有其 recall，本不需共錨。**

連帶處置：
1. 共錨核可請求（包 27 §四.1／包 28 §八.1）**撤回**，R-AM21 待申報項少一筆。
2. 4866632 自「未被認領」轉為 174 之錨，coverage gap 少一項。
3. 174 之 spec_reference 為交付欄 → 真回修 → **併入 R-AM18 回溯站（第 5 項）**，
   待 DR-AM3 回件後單次寫回併辦；交付簿現況仍為 4866662，已於
   `DELIVERY_NOTE.md` §8／§9 揭露。

分析層方法學錯誤已留痕：包 26 §一 援 D-B6-01 為 268 定案，同包 §二 之 174
遇同構卻未施用同一判準。**新立判準須對全批回揃。**

## [A-AM20] 「PENDING 三葉未寫入工作簿」與實測不符 —— 140 有列，8.4.3 門檻未因降轉而滿足

`DELIVERY_NOTE.md` 初版 §1／§2（2026-08-27 撰，依下放包 30 §四.1）記
「有 TC 之葉 314、PENDING 三葉不寫入工作簿」。**實測三葉皆在簿上**：

| 葉 | 列 | tc_id | `spec_reference` |
|---|---|---|---|
| SWE1_AMM_026 | r166 | NR1L-AMM-157 | `PENDING: DR-AM1 …` |
| SWE1_AMM_076（076a） | r167 | NR1L-AMM-158 | `PENDING: DR-AM1 …` |
| SWE1_AMM_140 | r315 | NR1L-AMM-306 | `PENDING: DR-AM10 …` |

369 列／317 唯一 SWE ID —— 每一葉皆有列，314 之數無實測依據。
當初 PENDING 者是**錨**，不是 TC；兩件事在包 30 §四.1 之標題裡被併成一句，
我照抄未實測即寫入交付文件。

**後果**：D-CLOSE-01 指令據此前提而立——「§1 算式改為 316 ＋ 140 一葉無列 = 317」
與「交付簿已無 PENDING 值，IN §8.4.3 之出貨門檻已滿足」兩句，於實測下皆不成立。
026／076a 依 R-AM23 降轉 `NA` 後，簿內仍餘 **1 個 PENDING 值**（140），
lint 檢查 M 仍報其 blocks delivery。

**處置**：
1. 降轉照辦（026／076a 兩列，TC 本體與 tc_id 未動），`generated/B3.json` 同步。
2. `DELIVERY_NOTE.md` §1 更正為 317 並留更正註記；§2 改述三葉待遇之別；
   新增 §2.1 明記門檻**未滿足**，未寫入指令原文之滿足句。
3. **140 已裁（2026-08-27，Pei）**：一併降轉 `NA`，並改寫 DR-AM10 理由。
   140 與前兩葉不同型——前兩葉為查無（錨不存在），140 為上游矛盾（錨或存在但未定）。
   `NA` 一值承載不了此別，故該別移入 reasoning、Remarks 與 DR-AM10 之改寫理由三處
   留痕：「NA 表此處未寫錨，非表錨不存在」。DR-AM10 不因降轉而結案。
4. **連帶發現**：三列之 Remarks 原亦以 `PENDING:` 覆述同一件事，降轉後與相鄰之
   `NA` 自相矛盾，一併改寫。Remarks 欄另有 **133 列** PENDING 註記
   （DR-AM4 126／DR-AM5 5／DR-AM6 1／DR-AM9 1），屬 DBC 訊號名與未定義值之缺件，
   與錨無關；`remarks` 不在 8.4.3 之 REQUIRED 集內，檢查 M 從未涵蓋，不阻出貨。
   **未擅動**——逕行抹除會把未解之 DBC 缺件寫成已解。

**教訓**（承 A-AM08／11／13）：轉抄下放包之框架語句進交付文件，等同以未實測之
命題對外聲明；交付文件之每一個數字都須自簿上讀出。
