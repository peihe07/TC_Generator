...
| 4 | **診斷 session 前提** —— CFTS004 與 SYSAD 皆未載進入診斷所需之 session 狀態（如 extendedDiagnosticSession `10 03`／securityAccess `27`）。pilot01 13 條 TC 之 Pre-Condition 各一行 `PENDING: DR-DIAG-3 diagnostic session prerequisite` | MISSING | 全 388 產出列之 Pre-Condition | **不阻斷** —— 僅 Pre-Condition 一行；步驟與 ER 可寫 | DR-DIAG-3 | **HIGH** |
| 5 | **`$5000` controlOptionRecord 編碼** —— CFTS004-4940283～4940295 列出可選音調（7 kHz／60 Hz／200 Hz／1 kHz／Tones off／externally generated）但未載其 DID 位元組編碼。pilot01 `NR1L-DIAG-006` 之觸發步驟與對應 ER 為 `PENDING: DR-DIAG-4` | MISSING | `$5000` Buzzer Control 40 列 ＋ 其餘 I-O Control 母節 74 列 | **部分阻斷** —— 觸發步驟無法寫，依 R-SEC15(b) 整行 PENDING | DR-DIAG-4 | **HIGH** |
