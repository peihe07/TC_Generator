...
| 8 | **`SX-9830-0095 "Sirius XM Multi-Package Factory Activation OEM Requirements"`** ＋ **`CIP Radio Tables`（`MPFA Select Process`／`MPFA Validate Process` 流程圖）** —— CFTS004-4940451／-4940461／-4940450／-4940462 明引為 `$0307`／`$0309` 兩常式之處理步驟與重試路徑之定義來源，兩件皆不在來源集。名稱逐字取自引用處 | MISSING | **13**（`$0307` 5 ＋ `$0309` 8）| 不阻斷 —— 依 R-DIAG19 同法，ER 只斷言常式回應與 Indication Code；`<Package Indication value>` 之編碼以佔位書寫（14 行） | A-DIAG48 | MED |
