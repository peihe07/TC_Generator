...
| `[A-SEC-15]` | CCVR `Cert Val CS.98` 之 STEPS 欄 **item 2／5 逐字**寫 `adb install -r -t CertProviderServiceManagerTest.apk`，而該 CCVR 目錄之實體檔案為 `CertProviderAndroidInstrumentalTest.apk`（`down/20260916_SEC-01.md` 第 125 行之「不投遞」清單） | 同一來源家族之文字與實體檔名不一致；工作簿 Pre-Condition 之檔名依何者為準無法自 CCVR 內部判定 | 工作簿**依實體檔案**（R-SEC24；SEC-13 review §一 #1：實體檔案為物理證據）；指南 §1.6 第 1 行同列兩名與出處。**請 Steven 確認交付檔名**；若答以 STEPS 為準，19 格再 Revise。A-33 併此改定性為 A-33′ |
| `[A-SEC-16]` | **ECUCert 路徑兩說**：`[NR1L][CCVR]ECU Cert Test Steps.20260826.pdf`（TC 依此）載 `/mnt/vendor/oemkeys/ecu/state/ecucertstatus`（`od -t x1`）、`/data/misc/ecuidentity/ecu.cacert`／`ecu.crt`；CCVR v2.7 工作中版 20260917（`ccvr_v27_wip_20260917`）之 `ECU ID CS.165` 備註（r30~r34，Samuel，註記日 2026/09/15）載 `/mnt/vendor/ecuidentity/state/ecucertstatus`（`od -An -t u1`）、主路徑 `/mnt/vendor/ecuidentity/data/{ecu.cacert,ecu.crt}`（`/data/misc/…` 為 back-up）、L0 `/odm/etc/cert_store/ecu_chain` | 037 未載任何路徑，兩說皆非母體；未裁前不擇一 | **TC 之 Procedure／ER 不改**；ECUCert 本與 Cert Provider 本之相關 TC 於 Remarks 加註兩說並指向本條（SEC-16 §4）；**請 Samuel 確認現行路徑** |
| `[A-SEC-17]` | CCVR `ECU ID CS.165` 之 item **10／11／13／14** 於工作中版標 `out of scope : wait confirm (2026/09/15)`，item **1** 標 `out of scope`（無 wait-confirm 字樣，備註 `discussing with STLA`）；四者之備註皆為 `discussing with STLA, wait for STLA script` | 涉及 CSR 匯出／STLA script（ECUCert CSR 相關 TC、資產 `X-d`／`X-k`）；STLA 確認後可能影響 ECUCert 之 batch 判定與 `X-d` | 登記待 STLA 確認；本包不改 TC。**下放包 §3 記五項同文，實測 item 1 之字樣不同**（見 `up/20260917_SEC-16.md` §5）|
