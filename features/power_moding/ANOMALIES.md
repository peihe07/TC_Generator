...
## A-PMH23 — 告別音之跨螢幕同步無任何 ER 斷言（覆蓋缺口） · 29 包 §六 · PENDING（低）
...
該句經 R-PMH109 裁定**併入 `-009`（啟動音）之 ER4 而不另立 TC**。
**惟其只涵蓋啟動音** —— `-010`（告別音）之 ER 只驗其與關機動畫之同步，
**未斷言告別音於各支援螢幕間之同步**。

**故若該句涵蓋二者，告別音側之跨螢幕同步為一個無 TC 覆蓋之行為。**

**其為缺口而非牴觸** —— 規格未言其不同步，只是本批未驗（R-PMH79 之 `未對照`）。

**處置**：**不另立 TC**（R-PMH109 明令），**併入 `DR-PMH8` Q3 詢問**。
若上游確認該句涵蓋二者，**`-010` 之 ER 須增一條**，其 procedure 亦須增一步
（維持步數 1:1）。
