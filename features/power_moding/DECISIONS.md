...
| 15 | `generated/batch06.json` 之 `-050`／`-051`／`-052`／`-053` 四條 —— **標 `BLOCKED-UNTIL-DR`，已產出而不可執行**（R-PMH142） | `DR-PMH8` Q9（四種互動結果各自之適用條件） | **逐值**：(甲) 答覆**載明各類之條件** → **四條各加其條件為 Pre-Condition，`BLOCKED` 解除**，其 procedure 之步驟 1 隨之具體化；(乙) 答為「**四者皆為可能之結果而無條件之分**」→ **四條併為一條**（其 ER 為「結果為所列四類之一」），並依 R-PMH137 於其餘三 leaf 記 `未涵蓋-重複`；**二路皆須屆時另裁，R-PMH142 明言不預判**；(丙) 未答 → **四條維持封鎖，隨交付附其封鎖依據** | 2026-08-25 |
| 16 | `gen_batch02.py` 之 `-012`／`-013` —— 其 `source_clause` 主語為 `start-up **and** goodbye sounds` 而二條**只驗啟動音側**（A-PMH32） | **無所繫之 DR** —— R-PMH144 令其入本簿，**而本項無 DR 可繫**（同第 9／10 筆之形態） | **逐值**：(甲) 若開問並答為「二者皆須於 startup animation 時播放」→ **二條各增一觸發與一 ER**（關機動畫側），或**各拆為二條**（依 §8.2.2，二者為獨立分支）；(乙) 答為「`SSND 2.1)`／`2.2)` 之主語為贅語，其只管啟動音」→ **二條不動**，A-PMH32 改 `ACCEPTED`；(丙) 未開問 → **告別音於 `Always`／`Once a Day` 二設定下之行為不會有任何一條 TC 驗到**（`Never` 一設定由 `-014` 驗到）。⚠ **本筆無觸發點** | 2026-08-25 |
**合計 16 筆。**

> **39a（2026-08-26）之結案**：**其中 5 筆已結，主表現餘 11 筆**（`DELIVERY_NOTE.md` §8）。
> #11／#12 → `CLOSED-BY-RULING`（R-PMH150）；#9／#10 → `ACCEPTED-RISK`（R-PMH152）；
> #16 → `RESOLVED-BY-R-PMH147`（39 包步驟 2 已執行，39a §二實測確認）。
> **本簿之原文依 R-TM13 全數保留，不刪** —— 結案只加註，其「逐值」欄仍為日後 Revise 之依據。
> **「無所繫之 DR」自此歸零**（#9／#10 已結，#16 已結）。 其分布：`DR-PMH5` 2 筆（#1／#7）、`DR-PMH7` 4 筆（#1／#2／#6／#8）、`DR-PMH8` **8** 筆（#1／#3／#4／#5／#11／#12／#14／#15）、**無所繫之 DR 3 筆（#9／#10／#16）**——一筆得繫於多個問。
...
- **A-PMH30 二例之交付揭露（R-PMH137，36 包步驟 6）**: [OPEN] 依 R-PMH132(b)，下列二例**須列入交付所附之「已知未決清單」之一節**，其節名為「**已被驗證，其未涵蓋僅為追溯之位置**」：(1) `SWE1-HMI-PM-001-01` A1（門關閉 → 3 秒啟動動畫）—— **由 `-028` ER1 涵蓋，該條掛 `SWE1-HMI-PM-006-01`**；(2) `SWE1-HMI-PM-003` A2（Maserati 無逾時）—— **由 `-004` ER2 涵蓋，該條掛 `SWE1-HMI-PM-001-05`**。**其撰寫要求**：載明「該行為已被驗證」，**使讀者不致將 `未涵蓋` 讀成 `未驗證`**。⚠ **若交付方要求「每 leaf 之每一斷言於其本 leaf 上皆有 TC」，R-PMH137 即不適用，二例須補 TC**（36 §七第 2 項待 Pei）
- **反向涵蓋首跑查出 1 處 `無依據`（R-PMH136）**: [BLOCKED — 停止條件 7 觸發] `-004` ER3 `The disclaimer screen is removed and the last mode screen is displayed` —— 其 leaf `SWE1-HMI-PM-001-05` 之 DESC 只載「無逾時、須手動按 Accept」，**未載按下之後之結果**；該結果之依據在 **`-001-04`** 之 DESC。**其形態為 canon §8.4.2 之範圍捏造**（斷言了他 leaf 所有之內容）。**下放包步驟 4 逐字令「發現任一 `無依據` 即停並上呈」，故本包未改。**⚠ **其修正有二路**：刪去 ER3（則 `-004` 只驗「無逾時」而不驗其離開路徑），或維持並承認其跨 leaf（則 R-PMH136 須加一款）。**二者皆須裁。**
- **`desc_coverage.py` 已建（R-PMH138 之解凍）**: [CLOSED] 正向 55 斷言／40 leaf，反向 144 ER 斷言。**其首跑即查出我自己寫錯之一筆**（`SWE1-HMI-PM-010` A1 指向 `-036` ER3 而該條僅 2 條 ER）。**且其 must-hit 錨點 (1) 曾偽陽** —— 原判準「輸出含 `016` 或 `ER4`」在該筆錯誤存在時恆為 True，**改為「須較基準線新增且該新增須指名 `-016` 所掛之 leaf」方為真錨點**。**本次解凍用畢，自本包結束起恢復凍結。**
- **KNOWN-INCOMPLETE（五）反向表之高重疊依據未逐項人讀（R-PMH103 之精化項）**: [37 包步驟 5 登記] `desc_coverage.REVERSE` 之 155 項中，**約 117 項之 DESC 依據為機器以詞重疊挑出**，我只人讀了 25 項低重疊者（36 包）。**風險**：一條 ER 與某斷言用詞相近而其實驗的是另一件事 —— **其被記為「有依據」而實際無依據**，`--must-hit` 之錨點 (2) 只攔「完全無條目」者，**攔不到「條目指錯」者**。**其偽陰未量測，亦無法以現行手段量測**（量之即須逐項人讀，而那正是本項所缺者）。**不再排程**
- **KNOWN-INCOMPLETE（六）`測試執行` 之分類為單一正規式（R-PMH103 之精化項）**: [37 包步驟 5 登記] `desc_coverage` 反向表之 26 項標 `測試執行`（R-PMH97 之二分，**不需 DESC 依據**），其分類由**一條正規式**判定（`^No .*key press|input is given|…|is recorded$` 等）。**風險**：**一條真正的 SUT 斷言若被誤分為 `測試執行`，其永遠不會被要求 DESC 依據** —— **此為本檔最大之單一漏檢面**。已列於 `desc_coverage.LIMITS`。**不再排程**
- **batch 6 已產出，TC 產出階段完畢（37 包步驟 4）**: [CLOSED] `Voice Assistant Key` 5 leaf → 5 條。**六批合計 51 條 TC，涵蓋 45 leaf**；停手 3 leaf（`-002`／`-023`／`-028`）。**48 = 45 + 3。** priority 分布 P0 4／P1 41／P2 6。**其後為 Phase 5–7（`tc_id` 單次指派、寫回、Q10、profile 例外、交付揭露清單）**
- **首次寫回已完成（38 包步驟 5／6）**: [CLOSED] 產出 `output/…_PowerModing_20260825_writeback.xlsx`（SHA256 `070ef73cea7b6ed9`），自 **r10** 起 **51 列**。**母本未被動過**（SHA256 前後同為 `6372fb6b…`，實測）。**四項不變量全數相同**：分頁 9、DV 組數（classic ＋ x14，由 `verify_structure` 逐 member 比對）、`last_capacity_row` 1411、B 欄公式三點抽驗。**zip 48 members 中僅 `xl/worksheets/sheet6.xml` 相異** —— 即目標分頁。**`openpyxl.save()` 未被呼叫**（R-G3）。停手三筆（`-002`／`-023`／`-028`）**未寫入**；封鎖四條**已寫入**且其 `Remarks` 載 `[BLOCKED-UNTIL-DR]`
- **`check_write_back` 之三項首次上場（38 包步驟 5）**: [CLOSED] 其自 04 包實作、經故意失敗驗證後**至今未被任何寫回路徑呼叫**（其為已知未完成）。**本包接線於 `scripts/write_back.py` 之 §1 前置閘，寫回前自動執行，任一失敗即中止**。步驟 5(b) 之故意失敗**三項全被攔下**（起始列取自 `outline_map` 之 44／provisional 批次／列數差）
- **Q10 定案：`Product Document 記錄封面頁` 不填（R-PMH145）**: [CLOSED] Pei 逐字「1 不要」。**其為語料之少數側**（4 空／12 填），**其依據非多數而為三項成本**（五欄字串須自擬、`B4` 取檔名與 R-PMH26(d) 相衝且其值不得自客戶那份複製而與 R-PMH23 相衝、`B7` 帶 DV 而現行三項檢查不涵蓋）。**入交付揭露清單**，載明「依裁定留空，與 12/16 之語料多數不同」，**使交付方一望即知其為選擇而非遺漏**
- **profile §1.2 已落檔（R-PMH146 之一次性授權）**: [CLOSED] `docs/runtime/profiles/FW036_R1L_PowerModing_Profile.md` 增 §1.2（9.1 之 `source_clause` 取 SYS1 之例外），**逐字一次**；SHA256 `8f8a15145bdf3b16` → `80304b6f3baeacb1`，行數 284 → 306（+22）；**§1 原文未改一字（實測 byte 比對）**。⚠ **`PROFILE_INTEGRATION.md` 仍不得動**（R-PMH46 之明文不授權，至今未解）
- **17 §5.4 之結清（39 包步驟 6）**: [部分 CLOSED] **下放包謂「其餘五項」而 17 §5.4 實載 6 項** —— 以 6 項處置。**第 4 項已由實測結清**：R-PMH23 所放行之三頁中，`Reference` 與 `QS Suggestion` 之**內容於母本與客戶那份完全相同**（逐格轉字串後 SHA256 各為 `44d2f19c0832`／`44a1977eadff`，二側同值）→ **未受同一污染**；`Test Case Framework` **只存在於客戶那份，母本無之** → 其無從比對，**而本 feature 從未取用其內容**。**第 1 項大部分已被 R-PMH127 取代** —— 射程之比對已改取 037 之 `Requirement Description`，`section_title` 不再是比對基準；其殘餘（TSV 之 `section_title` 仍取自 SYS1）已無下游用途。**第 2／3／5／6 項延後**，入 KNOWN-INCOMPLETE
- **KNOWN-INCOMPLETE（七）代理量與母體之未回頭套用（17 §5.4 之 2／3／5／6）**: [39 包登記] (2) R-PMH39 之「湊得之門檻不足以支持」未回頭套用於 G3 之收容簇清單與 G5 之區間端點；(3) R-PMH60 之「字元數為代理量」未回頭套用於 A-PMH03 之覆蓋率、雙向 diff 之 6-gram 門檻、granularity 之比例門檻；(5) R-PMH27 未回頭掃其他建於舊母體之結論；(6) R-PMH63 未回頭掃其他下放包之同型措詞。**風險**：四者皆為「同一判準是否應擴及某處」——**其未擴及之處，其結論之支持度未知**。**不再排程**
- **`Cover 封面` 作者欄：留空（Pei 裁定 2026-08-26）**: [CLOSED —— Pei 逐字「**Cover 封面 不處理 空著就好**」] 母體 17 個交付夾之 036 工作簿全數實測：**作者欄 11 空／6 非空**，其非空者為**兩個他人姓名**（`張愷霏 ErinKFChang` ×5、`林政宇 BillyZYLin` ×1），**無一為 Pei**；核准者欄 17/17 皆為 `劉安哲 AllenACLiu`。**「依既有交付件之慣例填」之前提不成立** —— 慣例之多數是**不填**（11/17），而填者填的是別人。**故 Cover 作者欄維持空白（＝母本現況＝多數側）。****2026-08-26 Pei 逐字裁定「Cover 封面 不處理 空著就好」** —— 其留空自此為**裁定之結果**，非停手、非未決；**不入交付揭露之未決清單**（其已非未決），惟仍列於 `DELIVERY_NOTE.md` §6「其他刻意留空之欄位」，**使交付方一望即知其為選擇**。⚠ **工作簿 `AA` 欄（Test Case Author）已填 `PeiPYHsu`** —— 其為 `feature.yaml` 之 `author_value`，**與 Cover 之作者欄為不同欄位，不因本項而動**
- **第二次寫回已完成（39 包步驟 4）**: [CLOSED] 產出 `output/…_20260826_writeback_rev2.xlsx`（SHA256 `01e917b88e050ce1`），**自母本重新產生，未在第一次之產出上疊改**；第一次之產出（`070ef73c…`）**不覆寫、不刪除**。四項不變量全同；zip 48 members 僅 `sheet6.xml` 相異；**母本 SHA256 前後同為 `6372fb6b…`（停止條件 7 未觸發）**；`check_write_back` 之故意失敗三項先跑並全被攔下
- **39a 之三項裁定已執行（Pei 2026-08-26）**: [CLOSED] **R-PMH150**（#11／#12 照預設排除，`DR-PMH8` Q6／Q7 降格為告知性附註、編號不重排）；**R-PMH151**（`-002`／`-028` 依 canon §8.4.2 判 out of scope 並結案，**CFTS009 不再是本 feature 之待取件**；停手清單 3 → **1**，僅餘 `-023`）；**R-PMH152**（#9／#10 不另開 Q10／Q11，其揭露為終態，`ACCEPTED-RISK`）。**三條皆為追加，`RULINGS.md` 既有內容一 byte 未改**（實測：新檔以舊檔為前綴）。**統計數字未變**：48／45／3（3 = 1 停手 ＋ 2 依裁定結案）／51／4
- **`DR-PMH8` 之問數自此定於 9（R-PMH152）**: [CLOSED] Q6／Q7 已就地標註〔告知性附註〕，**編號不重排**（Q1–Q9 之交叉引用已遍布未決清單）；新 SHA256 `0ed8d781cc27e996`。**全表掃描（1054 行、`CFTS009` 命中 9 處）**：唯一以 CFTS009 為標的之 DR 項為 `DR-PMH1`，**而其早已 `CLOSED`**（R-PMH72，從未發出）—— **故未結 DR 中無任何一項因 R-PMH151 而失所繫**
