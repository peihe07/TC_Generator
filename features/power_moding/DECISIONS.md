...
- **`Cover 封面` 作者欄：實測後停手，未自行擇定（39 包步驟 5）**: [BLOCKED — 停止條件 9 之目的] 母體 17 個交付夾之 036 工作簿全數實測：**作者欄 11 空／6 非空**，其非空者為**兩個他人姓名**（`張愷霏 ErinKFChang` ×5、`林政宇 BillyZYLin` ×1），**無一為 Pei**；核准者欄 17/17 皆為 `劉安哲 AllenACLiu`。**「依既有交付件之慣例填」之前提不成立** —— 慣例之多數是**不填**（11/17），而填者填的是別人。**故 Cover 作者欄維持空白（＝母本現況＝多數側），待裁。**⚠ **工作簿 `AA` 欄（Test Case Author）已填 `PeiPYHsu`** —— 其為 `feature.yaml` 之 `author_value`，**與 Cover 之作者欄為不同欄位，不因本項而動**
