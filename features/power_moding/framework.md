...
- 資料來源：`data/layer3_sections.tsv`（48 列，執行層 05 包產出）——
  **有 TC 之 leaf 為 46**（R-PMH72 排除 `SWE1-HMI-PM-028`、**R-PMH117 排除
  `SWE1-HMI-PM-002`**，二列皆保留於台帳並各標 `EXCLUDED-BY-R-PMH72`／
  `EXCLUDED-BY-R-PMH117`）
- Layer 2 提案來源：下放包 06 §5，**經執行層以 TSV 機器複算，48/48 相符（06 包時之
  分母；R-PMH72 之後為 47/47，**R-PMH117 之後為 46/46**）、
...
| 5 | **`Power Transitions`** | **6** | 9.1, 10.5 | Power Management(2)／FOTA Via Wi-fi(2)／WiFi(1)／EV/PHEV Pages(1) |
...
**46 = 3 + 7 + 9 + 6 + 6 + 8 + 5 + 2，R-G10 餘數 0。**

> **由 47 改 46 之依據（R-PMH117，Pei 2026-08-25「核可」）**：
> `SWE1-HMI-PM-002`（outline 7.1.1，`SU1.1) … based on vehicle architecture.
> See CFTS009 for clarification.`）依 canon §8.4.2 之三項判準與 `-028` 完全同型
> （行為定義於外部規格／本 feature 不持有該規格／不取得而撰寫須造值），
> **判 out of scope、不寫入交付工作簿**。其列仍在台帳並標 `EXCLUDED-BY-R-PMH117`。
> `Power Transitions` 因而由 7 leaf 降為 **6**，其 outline 由三個降為二個
> （`7.1.1` 隨 `-002` 移出）。
>
> ⚠ **`SWE1-HMI-PM-023`（10.5）留在本組** —— 其為「停手待 `DR-PMH5`」（R-PMH111），
> **非 out of scope**。**二者不同，不得合併處置**：`-002` 不寫入工作簿，
> `-023` 仍在範圍內而暫不產出 TC。
...
> 門檻表 SHA256：`f574db043d496671f91d681206779e64be07d1e267dfd72a3ccaabdbf5692670`
...
**G1 不可省** —— 存在 G2／G4／G5 全通過而仍過細之組態（46 leaf 分 16 組，
每組 2–3），即隔離錨點 **A6**。
...
| id | 實測 | 門檻 | 結果 |
|---|---|---|---|
| G1 | `8/46 = 0.1739` | `≤ 1/3 = 0.3333` | **PASS**（餘裕近 2 倍） |
| G2 | `min = 2` | `≥ 2` | **PASS** |
| G3 | 零命中 | `= 0` | **PASS** |
| G4 | `9/46 = 0.1957` | `≤ 1/2` | **PASS** |
...
|---|---|---|---|---|
| **A1** | 每 outline 各成一組（**27 組**） | G1 | `0.5870 > 0.3333` ✅ | **結構性連帶** `[G2, G5]` |
| **A2** | 每 leaf 各成一組（**46 組**） | G1、G2 | `1.0 > 0.3333`；`min=1` ✅ | **結構性連帶** `[G5]` |
| **A3** | `Off Road Plus` 拆為**兩個**單 leaf 組（**9 組**） | G2、G5 | `min=1`；逸出 `[1,1]` ✅ | **隔離** |
| **A4** | 新增一組名為 `Misc`（**取 2 leaf**） | G3 | 命中 `['Misc']` ✅ | **隔離** |
| **A5** | 八組併為一組（1 組） | G4、G5 | `1.0 > 0.5`；逸出 `[46]` ✅ | **隔離** |
| **A6** | **46 leaf 分 16 組**（**14×3 ＋ 2×2**） | **G1** | `0.3478 > 0.3333` ✅ | **隔離** —— G2/G3/G4/G5 全 PASS |

> ⚠ **A6 之組態不得沿用** —— 原式 `15×3 + 1×2` 於 46 會得 `15×3 + 1×1`，
> 其 `min=1` 使 G2／G5 一併 FAIL，**隔離即失效**（本錨點須只 FAIL G1）。
> 故改為 `14×3 + 2×2 = 46`，並於程式加一行 `assert min(...) == 2` 攔之。
...
> A1：`k=27 > 23`，`2k=54 > 46`；A2：`k=46 > 23`，`2k=92 > 46`。
...
| A-PMH04 —— 6 則圖片佔位 outline | Phase 4；**46 leaf 無一落在 p3–p7**，不阻斷 |
