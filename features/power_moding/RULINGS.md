...
> **`[PEI-REOPEN]`（2026-08-23，04 包 §三）** —— 上列 R-PMH10 之依據句
> 「語料 5/5 無一填寫」**證據基礎不完整**：分析層取樣之四份已交付件漏了
> **AMFM 20260810**（同在 `ASW-R2/AM:FM/` 樹下），該件之 `D5` **非空**，
> 其值為 `SWE1_AMFM_FM-WI-FSM-037-A03 STLA 報告_SWRA STLA Report_SWRA_20260323`。
> 更根本者：**「已交付件」之母體從未被定義**（`ASW-R2` 全樹之 036 檔含中間版本、
> 同夾多版本、非交付態產物），故「5/5」之分母不成立（R-G8）。
>
> **語料之兩次更新（2026-08-24，05 包 §三）**：
>
> | 母體判準 | 母體 | `D3` 空 | `D4` 空 | **`D5` 空 / 非空** |
> |---|---|---|---|---|
> | 原依據（未定義母體） | 「5」 | 5 | 5 | 5 / **0** |
> | R-PMH19（04 包） | 11 | 11 | 11 | 8 / **3** |
> | **R-PMH24（05 包，現行）** | **16** | **16** | **16** | **9 / 7** |
>
> `D3`／`D4` 於三次量測皆為全空，**其留空無爭議，不需重裁**。
> **變動者只有 `D5`**：非空者由 0 → 3 → **7**。
>
> **處置**：R-PMH10 **現狀維持有效**（D3／D4／D5 一律不寫入），惟標
> `[PEI-REOPEN]` —— Pei 於 2026-08-23 就 Q3 所作之「留空」裁定係基於不完整
> 語料，**分析層與執行層皆不得自行改判**。完整語料已於 05 包 §三備齊
> （母體 16，依 R-PMH24），由 Pei 重裁。
>
> 在重裁前之行為與現行 R-PMH10 相同，故**不阻斷任何步驟**。
> **原條文不改字**（比照 R-PMH6／R-PMH11）。
