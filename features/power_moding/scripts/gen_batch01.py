...
TCS = [
 dict(leaf="SWE1-HMI-PM-001-03", outline="7.1", src="SU1_full", dm=STATE, pri="P1",
   ...
   item=("During the Disclaimer screen (content per CFTS009/legal), if the system is not "
     "ready it displays “Loading…”. When the system is ready it removes “Loading…” and "
     "shows the Accept button.\n\n(System-readiness transition on the disclaimer screen — "
     "distinguishes the not-ready state from the ready state)"),
   pre=["1. The vehicle is a non-Maserati application",
        "2. The head unit is in the start-up sequence with the ignition turned on during "
        "the start-up animation",
        "3. The disclaimer screen is displayed"],
   proc=["1. Observe the disclaimer screen while the system is still loading",
         "2. Wait until the system reports ready and observe the disclaimer screen again"],
   er=["1. “Loading…” is displayed on the disclaimer screen and no Accept button is shown",
       "2. “Loading…” is removed from the disclaimer screen and the Accept button is shown"],
   reason=("P1 主要功能邏輯 —— 免責畫面之載入狀態與就緒狀態之顯示切換。"
     "**設計方法 STATE** —— 其標的為 not-ready → ready 之狀態轉換及其顯示差異。"
     "`source_clause` 取自 PDF p8 之 SU1.)（R-PMH50）。"),
   axis="系統就緒與否（not ready vs ready）"),
 ...
 dict(leaf="SWE1-HMI-PM-001-04", outline="7.1", src="SU1_full", dm=FUNC, pri="P1",
   ...
   item=("The system allows the user to press Accept to go directly to the last mode "
     "screen.\n\n(Accept 按壓路徑 —— 與同 leaf 之逾時路徑區辨)"),
   pre=["1. The vehicle is a non-Maserati application",
        "2. The disclaimer screen is displayed with the Accept button shown "
        "(the system is ready)"],
   proc=["1. Press the Accept button on the disclaimer screen"],
   er=["1. The disclaimer screen is removed and the last mode screen is displayed"],
   reason=("P1 主要功能邏輯。**依 profile §4「不同觸發即拆分」自 leaf `001-04` 拆出** ——"
     "「按 Accept」與「等待逾時」為兩個觸發，結果雖同而路徑不同。"
     "`source_clause` 取自 PDF p8 之 SU1.)（R-PMH50）。"),
   axis="觸發路徑：按壓 Accept"),

 dict(leaf="SWE1-HMI-PM-001-04", outline="7.1", src="SU1_full", dm=STATE, pri="P1",
   title="Disclaimer screen timeout acts as Accept",
   item=("The system allows the user to wait for the screen to timeout, which automatically "
     "equals Accept.\n\n(逾時路徑 —— 與同 leaf 之 Accept 按壓路徑區辨)"),
   pre=["1. The vehicle is a non-Maserati application",
        "2. The disclaimer screen is displayed with the Accept button shown "
        "(the system is ready)"],
   proc=["1. Do not press the Accept button and wait for the disclaimer screen to time out"],
   er=["1. The disclaimer screen is removed without any user input and the last mode screen "
       "is displayed, the same outcome as pressing Accept"],
   reason=("P1 主要功能邏輯。**同 leaf 之第二條**（profile §4 之不同觸發）。"
     "**設計方法 STATE** —— 其標的為逾時所引發之狀態離開。"
     "⚠ **逾時之長度未載於本規格**（PDF 只寫 `wait for the screen to timeout`，"
     "未給秒數）—— 本條不斷言具體秒數，只斷言逾時後之結果。"
     "`source_clause` 取自 PDF p8 之 SU1.)（R-PMH50）。"),
   axis="觸發路徑：等待逾時"),

 dict(leaf="SWE1-HMI-PM-001-05", outline="7.1", src="SU1_full", dm=NEG, pri="P1",
   title="Maserati disclaimer screen provides no timeout",
   item=("Exception: For Maserati applications the system provides no timeout (per "
     "CFTS009); the user must manually press Accept.\n\n"
     "(Maserati 變體之負向側 —— 逾時不得發生，與非 Maserati 之逾時路徑相對)"),
   pre=["1. The vehicle is a **Maserati** application",
        "2. The disclaimer screen is displayed with the Accept button shown "
        "(the system is ready)"],
   proc=["1. Do not press the Accept button and observe the disclaimer screen for a period "
         "longer than the timeout used on a non-Maserati application",
   ...
   reason=("P1 主要功能邏輯。**設計方法 NEG** —— 其標的為「逾時**不**發生」。"
     "變體詞 `Maserati` 逐字取自規格（profile §3.2）。"
     "⚠ 步驟 1 之觀察時長以「長於非 Maserati 之逾時」表述 —— **規格未給任一秒數**，"
     "故不造值（§8.4.1）。`source_clause` 取自 PDF p8 之 SU1.)（R-PMH50）。"),
   axis="變體：Maserati（無逾時）"),
 ...
 dict(leaf="SWE1-HMI-PM-003", outline="7.2", src="SU2", dm=FUNC, pri="P1",
   ...
   item=("SU2.) For Maserati vehicles, while on the disclaimer screen the user will have "
     ...
     "(Maserati 且**未**配備 lower comfort screen —— 與 `-004` 之配備情形相對)"),
   pre=["1. The vehicle is a **Maserati** application",
        "2. The vehicle is **not** equipped with the lower comfort screen",
        "3. The disclaimer screen is displayed"],
   proc=["1. Observe the disclaimer screen for the comfort controls",
         "2. Operate one of the comfort controls shown on the disclaimer screen"],
   er=["1. The comfort controls are displayed on the disclaimer screen",
       "2. The operated comfort control responds to the input"],
   reason=("P1 主要功能邏輯。**依 profile §4「變體即拆分」與 `-004` 成對** ——"
     "本條為未配備 lower comfort screen 之情形。"
     "變體詞 `Maserati` 逐字取自規格。`source_clause` 取自 PDF p8 之 SU2.)（R-PMH50）。"),
   axis="配備：未配備 lower comfort screen"),
 ...
 dict(leaf="SWE1-HMI-PM-004", outline="7.3", src="SU2_1", dm=NEG, pri="P1",
   ...
   item=("SU2.1) Do not display comfort controls on Maserati disclaimer screen when vehicle "
     "is equipped with lower comfort screen.\n\n"
     "(Maserati 且**已**配備 lower comfort screen —— 與 `-003` 之未配備情形相對)"),
   pre=["1. The vehicle is a **Maserati** application",
        "2. The vehicle **is** equipped with the lower comfort screen",
        "3. The disclaimer screen is displayed"],
   proc=["1. Observe the disclaimer screen for the comfort controls"],
   er=["1. The comfort controls are not displayed on the disclaimer screen"],
   reason=("P1 主要功能邏輯。**設計方法 NEG** —— 其標的為「不顯示」。"
     "與 `-003` 為 profile §4 之變體對（配備 vs 未配備 lower comfort screen）。"
     "變體詞 `Maserati`／`lower comfort screen` 逐字取自規格。"
     "`source_clause` 取自 PDF p8 之 SU2.1)（R-PMH50）。"),
   axis="配備：已配備 lower comfort screen"),
 ...
 dict(leaf="SWE1-HMI-PM-005", outline="7.4", src="SU3", dm=FUNC, pri="P1",
   ...
   item=("SU3.) No pop-ups will appear until the disclaimer screen has been removed. If an "
     "item like a traffic announcement is received the user will begin hearing the "
     "announcement in the background but will not see the pop-up until the disclaimer "
     "screen is removed.\n\n"
   ...
   proc=["1. Deliver a traffic announcement while the disclaimer screen is displayed and "
         "observe the screen and the audio output",
         "2. Remove the disclaimer screen and observe the screen"],
   ...
   reason=("P1 主要功能邏輯。**依 profile §4／canon §5.7「同一觸發之多個必然後果不拆」** ——"
     "「不顯示 pop-up」與「音訊照常播放」為同一觸發（免責畫面顯示中收到通告）之"
     "兩個必然後果，寫為同條之多行 ER。`source_clause` 取自 PDF p8 之 SU3.)（R-PMH50）。"),
 ...
 dict(leaf="SWE1-HMI-PM-022-02", outline="10.4", src="PITA6_1", dm=STATE, pri="P1",
   ...
   item=("Upon pressing the power button to On state the disclaimer screen shall be "
     "displayed (see SU6.) unless certain phone call scenarios have occurred.\n\n"
   ...
   proc=["1. Press the power button to change the radio to On state",
         "2. Observe the screen"],
   er=["1. The radio changes to On state",
       "2. The disclaimer screen is displayed"],
   reason=("P1 主要功能邏輯。**設計方法 STATE** —— 其標的為 Power Button Off → On 之"
     "狀態轉換及其畫面。⚠ 規格之 `unless certain phone call scenarios have occurred` "
     "**未列舉該等情境**，故本條**不斷言其例外**，僅驗其正向路徑；"
     "該例外之列舉屬 10.6（`Power Off Behavior` 組），不在本批。"
