...
| 39 | 2026-08-26 | **A-PMH32 修正（擴而不拆）**、第二次寫回、**`DELIVERY_NOTE.md`** | [handoff/39_delivery.md](handoff/39_delivery.md) ＋ [39a](handoff/39a_pei_closures.md) | [upstream/39_delivery.md](upstream/39_delivery.md) | R-PMH147–R-PMH152（逐字抄錄 6/6 相符；**39a 為純追加，既有一 byte 未改**） | A-PMH32 → `RESOLVED`；A-PMH25 → `ACCEPTED-RISK`；A-PMH28／A-PMH29 → `ACCEPTED（裁定排除）` | **三條停止條件全未觸發；母本未被動過；六批 lint 32/32；`Cover` 作者欄實測後停手未自行擇定** |
...
**marker 枚舉（R-PMH54）—— 30／2，「截斷是否系統性」之疑慮解除**
- 無門檻、無取樣、無相似度參數，結果為二值。
- **PDF 30 個 marker，SYS1 缺 2 個**（`SU9.)`／`SU9.1)`），**且只缺這兩個** ——
  其餘五章之末則（`SSND 3)`／`PM1)`／`PITA10:`／`VRLP1:`／`OFF3.)`）全部對得上。
  **13 包 §8 第 4 項之「每章末則是否系統性被截斷」答案為否。**
- **⚠ 首版正規式漏了 `SU1.)` 形態**（數字後尚有句點才接括號），只得 **18／1**，
  **章 12 全數落空**。**若未與分析層之 30／2 對照，18／1 會被當成結果收下** ——
  先算後比之價值於此。
- must-hit：自 SYS1 移除 `SU1.)` → 缺漏 2→3 且該 marker 在清單內，攔下 ✅。
- §3.3 之 PITA 跳號複驗屬實（`4,5,6,6.1,8,9,10`）。**惟子字串探 `PITA1` 會得
  True，因 `PITA10:` 含之** —— 枚舉值為權威，記此以免日後被該假象誤導。

**未涵蓋清單程式化（R-PMH56）—— 抓到我自己的反向錯誤**
- canon 58 節、lint 宣告涵蓋 10 節、**未涵蓋 48 節**。
- 13 包之九節（10 個節號）**10/10 在程式化清單內**；14 §5.4 之七節 **6/7 在**，
  **`§5.2` 不在**。
- **原因是我自己造成的**：我在 `COVERED` **先宣告了 `5.2` 而檢查尚未實作** ——
  清單遂稱「已涵蓋」而實際沒有。
  **這是 R-PMH56 所防之反向錯誤**：漏列使人以為未查者已查；
  **誤列使人以為未查者已查**。前者由 §5.4 抓到，後者由本輪程式化比對抓到。
- 已修：`§5.2` 檢查本輪實作；另將 lint **實際檢查而未宣告**者
  （`§5.5`／`§8.4.3`／`§10.2`／`§10.3`／`§10.7`）補入，5 節 → **10 節**。
- 程式化多出之 38 節逐類具名：meta 5／parent 5／**其餘欄位規則節 28**。
  人讀清單為**策展子集**，程式化清單為**全集**，定義不同非矛盾 ——
  惟 R-PMH56 令以程式為準，lint 輸出改為指向該程式，不再手寫節號。

**batch 1 第三輪 —— lint 29/29 PASS**
- §5.2 步驟字數：**五處**逾限（下放包列四處，機械檢查另抓 `-008` step2 = 23 字），
  五條全拆為三步。
- `-001` ER 之單引號改直雙引號 —— 同一條內兩種引號並存已消除。
- **priority 逐條重判：P0 4／P1 3／P2 1**，四條不落 P0 者各自載明「為何不是 P0」。
  §5.3 之「rubric 未起分辨作用」之疑慮已解。
- **⚠ 重判時之一處自錯**：我先換了 `-008` 之 `reasoning`（改為 P0 依據）
  **而未改其 `pri` 欄位值** —— 正是 R-PMH41 之形態。
  **由我自加之一致性複驗抓到**（比對每條之 `priority` 與 `reasoning` 首句），
  該複驗**非下放包所令**。

**六支檢查全綠**：lint 29/29、marker self-test、canon_coverage、
state_consistency、doc-sync、write_back self-test。

**`DR-PMH3` 之連帶已預先登記**（`DECISIONS.md`）：若上游回覆 SU9／SU9.1
應在 037，`Disclaimer Screen` 自 7 leaf 增為 9，連帶須重驗 Layer 2 計數、
**granularity 之分母由 48 變 50**、`layer3_sections.tsv`／`outline_map.json`、
batch 1 增 2 條。**不預改任何數字。**

**⚠ 本輪自陳之未竟項中，兩項須先看（上繳 §8）**
1. **marker 前綴清單本身是列舉** —— 若規格另有第七種前綴而我未列，
   枚舉法會**靜默漏掉整章**。**而該法已取代句級 diff 之判定責任，
   盲區之代價因而更高。** 建議以「PDF 中所有 `\w+\d+[.):]` 形態」反向掃描驗之。
2. **`-006` 由 P1 改 P2 使該批出現三個級別，恰好「看起來合理」** ——
   **我無法排除自己是因為「八條同值看起來不對」而去找理由拆開。**
   重判之依據我認為站得住，**但其動機不純，據實記載。**

**待 Pei**：三筆 DR 之發出（**不發則 48 leaf 母體與 ch 9 判讀背景之缺口無人在追**）／
marker 前綴完備性之反向掃描／priority 重判之覆核／第三輪 batch 1 之人讀覆核／
14 包 commit 授權。
...
**14 包之 marker 全集 30 是錯的，正確為 31。** 人工列舉之六個前綴漏了 `DS`
（`DS4.1)`，PDF 章 7，夾在 `SU4.)` 與 `SU5.)` 之間，編號與 `SU4.)` 呈父子
而前綴不同 —— 極可能為規格原文筆誤，依 R-PMH26 只登記不開 DR，A-PMH15）。

**該錯為分析層與執行層各自獨立算出之同一值** —— 二者用了同一份人工清單。
**先算後比（R-G7-1）只能抓「算法不同而結果不同」，抓不到
「前提相同而前提本身錯」。**

**must-hit B 之輸出是本輪最該看的一行**：把 `DS` 從前綴清單移除後，
**缺漏數仍為 2**。若當初只比對缺漏數，此錯永遠不會被察覺 ——
唯一之證據是分母由 31 降為 30。**一個錯的分母可以完全不改變
任何看起來重要的數字。**

**priority 之批內矛盾已處置（R-PMH59，採案（甲））**：`-003` 由 P0 降 P1。
**且 `-002` 之依據亦一併改寫 —— 此為本層自查，分析層原判其「依據成立」。**
`-002` 原依據「Accept 為唯一主動路徑」於非 Maserati 不成立（逾時仍在），
與 `-003` 原依據犯的是同一個錯：各自把對方的路徑當作不存在。
`-002` 之 P0 結論維持，依據換為變體差異（Maserati 無逾時，Accept 為其唯一路徑）。
**分布 P0 4/P1 3/P2 1 → P0 3/P1 4/P2 1。**

**`COVERED` 已自 `chk(..., canon=...)` 之呼叫處以 `ast` 自動彙集**（R-PMH58），
並加執行期交叉核對（靜態宣告 vs 本次實際執行到之檢查點）。
自動版與手寫版**完全相同** —— 但那是因為 14 包已修掉該錯；
自動化之價值在於該類錯此後**無從發生**。

**⚠ 本輪自陳之未竟項中，三項須先看（上繳 §10）**
1. **`VERDICT` 表仍是人工判定，只是母體不再是人工的。**
   誤判（把真需求前綴標成 `noise`）一樣會使該章靜默消失，
   **而 must-hit C 只抓「未判定」，不抓「判錯」。**
2. **候選形態本身仍是一個正規式** —— 反向掃描把前提從「前綴清單」
   移到了「候選形態」，**並未消滅前提**。
3. **R-PMH59 只驗了 001–004 這一組。** `-007` 與 `-001`／`-008` 之間
   有無同型矛盾，本包未查。
...
`-012`／`-013` 各由 5:5 增為 **6:6**，其步驟與 ER 擴及告別音側。
**不拆之三項依據**：同軸三值之驗證單位須一致（`-014` 已於一條內涵蓋二者）；
**與 canon §5.7「不同觸發即拆」之張力已具名**；canon §7 只要求「至少一條負向」。

### 二、第二次寫回

新副本 SHA256 `01e917b88e050ce1`；**自母本重新產生，未在第一次之產出上疊改**；
第一次之產出不覆寫不刪除。**母本 SHA256 前後同為 `6372fb6b…`**。四項不變量全同。

### 三、⚠ `Cover 封面` 作者欄 —— 實測後停手

母體 **17 檔全數實測**：**11 空／6 非空**，非空者為**兩個他人姓名**（`張愷霏` ×5、`林政宇` ×1），
**無一為本 feature 之作者**；核准者欄 17/17 皆同一人。

**「依既有交付件之慣例填」之前提不成立** —— 慣例之多數是不填，而填者填的是別人。
**依停止條件 9 未自行擇定，維持空白待裁。**

> ⚠ 我第一次以「夾內最後一個 xlsx」取樣，挑到 037 報告與 SYS1 匯出，17 夾中 14 夾報「無 Cover」；
> 改以檔名含 `FM-WI-FSM-036` 為判準後 **17/17 全數命中**。

### 四、17 §5.4 —— **實載 6 項而非下放包所謂之 5 項**

第 4 項**由實測結清**：`Reference`／`QS Suggestion` 於母本與客戶那份**內容完全相同（未受污染）**；
`Test Case Framework` **只存在於客戶那份**。第 1 項大部分已被 R-PMH127 取代。其餘四項入 KNOWN-INCOMPLETE。

### 五、`DELIVERY_NOTE.md`（十節）＋ 交付揭露清單（**16 筆**）

二者**併為一份**——分立會使同一件事在兩份文件裡各說一次，而其中一份先過期。
⚠ **下放包謂 15 筆，實測 16 筆**（第 16 筆為 38 包所增之 A-PMH32）。

### 六、39a —— Pei 之三項結案裁定（2026-08-26）

| 條 | 效果 |
|---|---|
| **R-PMH150** | 未決 #11／#12 **照預設排除**（037 未載者不納入本輪）；`DR-PMH8` **Q6／Q7 降格為〔告知性附註〕，編號不重排** |
| **R-PMH151** | `-002`／`-028` 依 canon §8.4.2 判 **out of scope 並結案** —— **`CFTS009` 自此不再是本 feature 之待取件**；停手 3 → **1**（僅餘 `-023`） |
| **R-PMH152** | 未決 #9／#10 **不另開 Q10／Q11**，其揭露為**終態**（`ACCEPTED-RISK`）；`DR-PMH8` 之問數自此定於 **9** |

**未決清單 16 → 11；停手 3 → 1；「無所繫之 DR」歸零。統計數字（48／45／3／51／4）一字未動。**

**四條停止條件全未觸發**：`RULINGS.md` 純追加（新檔以舊檔為前綴，實測）；
`DELIVERY_NOTE.md` §1 數字未變；第 16 筆**先實測後結案**（二條之告別音 ER5 明確）；
CFTS009 **經 1054 行全表掃描**後方斷言「未結 DR 中無所繫者」（`DR-PMH1` 早已 `CLOSED` 且從未發出）。

> ⚠ **歷史列不回改**：`docs/INDEX.md` 各包要點中之「停手 3」等語為**當輪之紀錄**，
> 其在當輪為真，依 R-PMH44 之精神保留；**本輪之變更記於此節。**
