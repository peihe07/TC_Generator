...
- 日期：2026-08-25
- 下放包：[handoff/28a_dr_final.md](../handoff/28a_dr_final.md)
- **28 本文以上未改一字**

## 1. 步驟 1 —— 條文逐字抄錄（R-PMH41）

以程式自 28a §二擷取兩個 fenced block，附加至 `RULINGS.md`，
**再自 `RULINGS.md` 讀回**與 handoff 原文逐 byte 比對。

| 條號 | 主旨 | 字數 | handoff SHA256（前 16） | RULINGS **讀回** SHA256（前 16） | 逐字相符 |
|---|---|---|---|---|---|
| R-PMH105 | R-PMH103／R-PMH104 核可生效 | 470 | `abf53c1beca313fa` | `abf53c1beca313fa` | ✅ |
| R-PMH106 | 三筆 DR 之發出授權 | 514 | `11acf30c93066500` | `11acf30c93066500` | ✅ |

**2/2 相符。** 核對表已寫入 `RULINGS.md`（「抄錄逐條核對表（28a 包步驟 1）」）。

## 2. 步驟 7 —— 三份 DR 最終全文之落檔（R-PMH106）

三份全文以程式自 28a §三擷取，寫入 `DATA_REQUESTS.md` **§七**，
**再讀回逐 byte 比對**：

| DR | 主旨 | 字數 | SHA256（前 16） | 讀回相符 | 狀態 | 發出日期 |
|---|---|---|---|---|---|---|
| `DR-PMH5` | p9 之能力矩陣 | 3433 | `5c0c83ba58f0d132` | ✅ | `DRAFT` | **（空）** |
| `DR-PMH6` | PITA6 × 矩陣衝突 ＋ 三項無需求行為 | 2358 | `8dc2fe0c8577b048` | ✅ | `DRAFT` | **（空）** |
| `DR-PMH7` | 三處未定義之記法 | 1933 | `3de29747dfb6861c` | ✅ | `DRAFT` | **（空）** |

### 2.1 R-PMH106 之明令：已撤回主張不得寄出

條文令 `DR-PMH5` 之寄出版本不得含「p9 矩陣與 p8 之 `SU3.)` 相衝」
（A-PMH21 已於 24 包改判 `未對照`）。

**核驗方式為實測而非閱讀**：三份全文各計 `SU3` 之出現次數 —— **各為 0**。

**連帶處置**（**原文一律不刪，R-PMH44**）：

- `DATA_REQUESTS.md` 之「增補（23 包…）—— p9 矩陣自身與 p8 之 `SU3.)` 相衝」
  一節加註**「本增補之主張已撤回（R-PMH106）」**並記其改判依據；
- 舊之 §五（`DR-PMH5` 全文）、§六（`DR-PMH6` 全文）、`DR-PMH7` 之 26 包增補稿
  各加註**「已被取代（28a §三）」**，並指向 §七。

### 2.2 `SENT` 欄之處置（R-PMH43）

三者之 `SENT` 欄**留空**，狀態維持 `DRAFT`。
**執行層未發出、亦不得發出任何一封**（R-PMH83）。

> **「已授權」不等於「已發出」。** 在 Pei 告知實際日期與對象之前，
> `DR-PMH5` **仍凍結 ch 9 之 5 leaf**（`SWE1-HMI-PM-018-01`～`-05`）。
> 此為本包**唯一**之實質阻斷。

## 3. R-PMH105 之三項效力（已落於 `DECISIONS.md`）

| 效力 | 落檔 |
|---|---|
| (a) batch 1 覆核線**結束** | 28 包之 CLOSED 條目加註「28a 經 Pei 核可生效（R-PMH105(a)）」；三項精化＋`-007` 之 `L160` 維持 KNOWN-INCOMPLETE，**不阻斷開批** |
| (b) apparatus **凍結生效** | 新登記一條；**凍結者為增長，非運作** |
| (c) batch 2 **開批** | 同上條目具名 |
...
| # | 條件 | 結果 |
|---|---|---|
| 28 §五 之 9 | **本包新增任何檢查程式或檢查項即停** | **未觸發** —— 新增程式 **0**、新增檢查項 **0**。實測：`scripts/` 之檢查程式 **13 支**（由 `check_table.py` 自身列舉，非人工計數）、lint **32 項**，二者與 28 包相同 |
| 常設 | **ch 9 不得開批** | **遵守** —— `DR-PMH5` 未 `ANSWERED`（且尚未發出），本包未產出任何 ch 9 之 TC |
| 常設 | **batch 1 不得提交寫回** | **遵守** —— 本包零寫回工作簿，`workbook_state = BLANK` 未變 |
| 常設 | **執行層不得代為發出 DR**（R-PMH83） | **遵守** —— 本包只落檔全文，未發出任何一封，`SENT` 三欄全空 |
| 常設 | 不得改動 `scripts/new_feature.py`／他 feature 之檔案／`docs/runtime/` | **遵守** —— 本包所改僅 `RULINGS.md`／`DATA_REQUESTS.md`／`DECISIONS.md`／`docs/INDEX.md`／本上繳檔 |

### 4.1 檢查裝置之複跑（凍結不等於停用）

| 檢查 | 結果 |
|---|---|
| `lint_batch.py generated/batch01.json` | **32/32 PASS**（exit 0） |
| `lint_batch.py generated/batch02.json` | **32/32 PASS**（exit 0） |
| `verdict_form.py` | **0 failure** |
| `check_table.py` | 13 支程式；**未註冊 must-hit 而標「未實測」者 = 4**（不變） |

## 5. 本包是否仍有該驗而未驗者 —— **有**

1. **三筆 DR 仍未發出。** 本包所做者為**落檔**，非發出。**`DR-PMH5` 凍結 ch 9 之
   5 leaf 一事完全未變**；47 leaf 中之 5 leaf 仍無法開始。**授權已到第八輪仍未寄出，
   而寄出是唯一能解凍者。**
2. **`DR-PMH6` 之「三項無需求行為」與 `DR-PMH7` 之三問，其答覆將回頭改變已產出之
   判定。** `DR-PMH7` Q2（`Else: Mute Active`）之答覆會改變矩陣 `r46`／`r47` 之判定，
   而 **batch 2 之六條限定正是依 R-PMH95「涵蓋兩讀」而加**；若答覆為「維持靜音」，
   該二列之限定即為過度限定（不致誤判，但使 TC 較規格所需為窄）。**已具名，未預先調整。**
3. **`animation` 斷言之掃描仍未做**（本包新登為 KNOWN-INCOMPLETE 第四項）。
   **R-PMH105(b) 生效後，其解凍須有一個實測之 TC 缺陷** —— 亦即
   **該掃描已不可能因「謹慎」而被排程，只可能因「已經出錯」而被排程。**
   此為凍結所承擔之風險，於此具名。
4. **batch 2 之七條仍未經任何人讀覆核。** 28 包已具名；本包未改變此事。
   **R-PMH105(c) 之「開批」是流程上的許可，不是產出面的覆核。**
5. **本包所加之「已被取代」註記為我之判斷。** 28a 只令「寫入全文、`SENT` 留空」，
   未令舊稿如何處置。我選擇保留原文並加註而非刪除（R-PMH44 之精神），
   **惟該選擇未經裁定** —— 若原意為舊稿應刪，一句話可反轉。
