...
### 勘誤 1 —— §10 第 3 項之疑慮係**判準之誤用**，非證據不足

§10 第 3 項原文以「分析層之字元數為 15,751，本層 `norm()` 後為 15,167 ——
差 584 字元，極可能來自空白正規化，**但我沒有證明它是**」為由，
將 `sandbox/spec.txt` 與 PDF 之等同性列為未驗項。

**該項之提出方式本身不成立**（16 包 §四，R-PMH60）：

- **R-PMH21 已明文裁定**「規格 PDF 之抽取字元數不得作為規格完整性、
  抽取正確性或版本一致性之判準」。「兩份萃取是否為同一份文件」
  即版本一致性之問題，**以字元數為據提出該疑慮，與以字元數為據消解它，
  同屬判準之誤用**。
- **且該工具差異早於 03 包上繳 §6.2 即已記載**
  （`pymupdf` 15,618 vs `pdftotext -layout` 15,167，明載為「不同工具之差異，
  非衝突」）。15 包將其重提為未解疑點，是**未回查既有記載**。

**正確之判準與其實測**（16 包步驟 4）：以 PyMuPDF 1.28.0 對同一 PDF 另做
一份獨立萃取，兩份之 **marker 集合逐項相等**（全集皆 31、逐章
13／6／1／7／1／3 全同、缺漏皆 `SU9.)`／`SU9.1)`）—— **等同性成立**。

**§10 第 3 項自此結清。** 原文一字未改，其 SHA256 不受本節影響。
