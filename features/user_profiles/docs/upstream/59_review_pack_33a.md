...
- 產出層：執行層｜2026-08-20｜**供分析層逐條覆核**
- 本檔 **17 條**；另半在 `59_review_pack_33b.md`
- 由 `scripts/build_review_pack.py` 產生，不經人手轉錄
- **本檔取代 `57_review_pack_33a.md`**（AA-1，44 包）——該檔無語料指紋，`--verify` 一律判過期
...
| tc_id | digest |
|---|---|
| `NR1L-UserProfiles-157` | `33c04ad4589b` |
| `NR1L-UserProfiles-158` | `8a03423a030b` |
| `NR1L-UserProfiles-159` | `9293cac60f25` |
| `NR1L-UserProfiles-160` | `20c09afe6aaa` |
| `NR1L-UserProfiles-161` | `d483f0085238` |
| `NR1L-UserProfiles-162` | `6201dc19b3cb` |
| `NR1L-UserProfiles-163` | `7f13c43a519e` |
| `NR1L-UserProfiles-164` | `29f9fed053e1` |
| `NR1L-UserProfiles-165` | `4499ced74862` |
| `NR1L-UserProfiles-166` | `db0d280e2129` |
| `NR1L-UserProfiles-167` | `a42f21dd7fc8` |
| `NR1L-UserProfiles-168` | `a650f9afcd87` |
| `NR1L-UserProfiles-169` | `4b6cc3360a5b` |
| `NR1L-UserProfiles-170` | `f4863fae6d48` |
| `NR1L-UserProfiles-171` | `8f57189119a4` |
| `NR1L-UserProfiles-172` | `4dab813796cd` |
| `NR1L-UserProfiles-173` | `4fb1272c5587` |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Welcome popup shown at ignition on and on activation<br>(Verifies that a welcome popup is displayed at ignition on and each time a Profile is activated) |
| tc_title（＝ test_item 首段） | Welcome popup shown at ignition on and on activation |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | PU0841 shown when Tutorials are not complete<br>(Verifies that PU0841 appears for a Profile with incomplete Tutorials and that X on it shows PU0611) |
| tc_title（＝ test_item 首段） | PU0841 shown when Tutorials are not complete |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Small welcome popup shows username, avatar and options<br>(Verifies that the small welcome popup shows the active Profile's username and avatar with switch and close options) |
| tc_title（＝ test_item 首段） | Small welcome popup shows username, avatar and options |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Closing a custom welcome popup acts as Remind me Later<br>(Verifies that closing a custom welcome popup asks nothing and is treated as Remind me Later) |
| tc_title（＝ test_item 首段） | Closing a custom welcome popup acts as Remind me Later |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Welcome popup links to All Profiles or the last known tab<br>(Verifies that Switch Users opens the All Profiles tab while the avatar and Welcome text open the last known tab) |
| tc_title（＝ test_item 首段） | Welcome popup links to All Profiles or the last known tab |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Welcome popup clears as soon as the vehicle moves<br>(Verifies that the welcome popup clears as soon as the vehicle starts moving) |
| tc_title（＝ test_item 首段） | Welcome popup clears as soon as the vehicle moves |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Welcome popup clears immediately on screen interaction<br>(Verifies that the welcome popup clears immediately when the user touches the screen) |
| tc_title（＝ test_item 首段） | Welcome popup clears immediately on screen interaction |
| pre_conditions | 1. The welcome popup of the active Profile is displayed<br>2. The vehicle is stationary |
| input_test_data | NA |
| test_procedure | 1. Press the screen five seconds after the popup, before the 30-second timeout<br>2. Read the screen and check whether the popup is displayed |
...
| remarks | **互動之時點寫在步驟 1**（AE-1，57 包）—— 條文之三個觸發為`whichever comes first`，若互動發生在第 30 秒附近，清除之原因即不可歸屬。該時點為**測試方法所要求之值**（J-4），非條文之值，故不寫入 ER。**原寫於 `input_test_data` 而無步驟引用它** —— 該值為孤兒；且依 §4.5，互動之時點屬 Procedure step，`Input Test Data` 留給獨立資料集，故移入步驟並將該欄改 `NA`。`about` 為 §2 之模糊語，隨移入一併去除。車輛須靜止：否則 motion 分支（`SWE1-HMI-PROF-062-01`）會先於互動成立。 |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Cleared welcome popup returns only on reactivation<br>(Verifies that a cleared welcome popup stays away for the session and returns only on reactivation) |
| tc_title（＝ test_item 首段） | Cleared welcome popup returns only on reactivation |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Remote start time does not count toward the 30 seconds<br>(Verifies that time spent in remote start does not count toward the 30-second dismissal timer) |
| tc_title（＝ test_item 首段） | Remote start time does not count toward the 30 seconds |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Valet Mode welcome popup clears like the other popups<br>(Verifies that a screen press clears the Valet Mode welcome popup as the other welcome popups are cleared) |
| tc_title（＝ test_item 首段） | Valet Mode welcome popup clears like the other popups |
...
| input_test_data | NA |
| test_procedure | 1. Press the screen five seconds after the Valet Mode welcome popup appears<br>2. Read the screen and check whether the popup is displayed |
...
| remarks | **互動之時點寫在步驟 1**（AE-1，57 包）—— 原寫於 `input_test_data` 而無步驟引用它（該值為孤兒）；依 §4.5，互動之時點屬 Procedure step，故移入步驟並將該欄改 `NA`，`about` 一併去除。**條文說的是「與其他 welcome popup 相同之清除／逾時行為」——即三個觸發**（motion／30 秒／互動）。本條取**互動**一側，**為抽樣而非窮舉**（§8.4.2）：另兩側之同型性不由本條保證。取互動一側之理由：三者中唯一**不需等待亦不需移動車輛**者，而 Valet Mode 下移動車輛另有 12.x 之限制介入。同型之三個觸發本身由 `SWE1-HMI-PROF-062-01`／`SWE1-HMI-PROF-062-03`／`SWE1-HMI-PROF-062-02` 各自驗證。 |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | R1 High begins Tutorials after the preferences step<br>(Verifies that an R1 High vehicle follows the preferences step with Tutorials and never launches the account login) |
| tc_title（＝ test_item 首段） | R1 High begins Tutorials after the preferences step |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | New Profile Setup starts from the All Profiles tab<br>(Verifies that the option to add a new Profile on the All Profiles tab displays the first step of the New Profile Setup) |
| tc_title（＝ test_item 首段） | New Profile Setup starts from the All Profiles tab |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Profile Setup is presented as a series of popups<br>(Verifies that every step of the New Profile Setup from the first to the last is presented as a popup) |
| tc_title（＝ test_item 首段） | Profile Setup is presented as a series of popups |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Pressing another button during setup asks to discard<br>(Verifies that pressing another button during setup is treated as cancel and asks whether to discard it) |
| tc_title（＝ test_item 首段） | Pressing another button during setup asks to discard |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Avatars already in use are filtered out of the list<br>(Verifies that avatars already in use are filtered out so that no two Profiles share one) |
| tc_title（＝ test_item 首段） | Avatars already in use are filtered out of the list |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Next available default avatar is highlighted automatically<br>(Verifies that the next available default avatar is highlighted automatically on entering the selection screen) |
| tc_title（＝ test_item 首段） | Next available default avatar is highlighted automatically |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | The same username can be used by two Driver Profiles<br>(Verifies that a username already in use is accepted, the new Profile is saved, and both Profiles are then listed under it) |
| tc_title（＝ test_item 首段） | The same username can be used by two Driver Profiles |
