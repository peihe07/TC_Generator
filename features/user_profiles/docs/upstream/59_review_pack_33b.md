...
- 本檔 **16 條**；另半在 `59_review_pack_33a.md`
- 由 `scripts/build_review_pack.py` 產生，不經人手轉錄
- **本檔取代 `57_review_pack_33b.md`**（AA-1，44 包）——該檔無語料指紋，`--verify` 一律判過期
...
| `NR1L-UserProfiles-175` | `391dd0300e33` |
| `NR1L-UserProfiles-176` | `438d9da314be` |
| `NR1L-UserProfiles-177` | `0deeabcd3e5c` |
| `NR1L-UserProfiles-178` | `3f70a0f50f36` |
| `NR1L-UserProfiles-179` | `2dcf1aaddf42` |
| `NR1L-UserProfiles-180` | `46e36621dd51` |
| `NR1L-UserProfiles-181` | `5055cdc1b912` |
| `NR1L-UserProfiles-182` | `b8ca7fa9eb6e` |
| `NR1L-UserProfiles-183` | `04729475b4a1` |
| `NR1L-UserProfiles-184` | `33d29aeab199` |
| `NR1L-UserProfiles-185` | `fa9bf9cbdd2b` |
| `NR1L-UserProfiles-186` | `98f709e4556b` |
| `NR1L-UserProfiles-187` | `18868070500e` |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Connected vehicles offer create new or download existing<br>(Verifies that the first setup step on a connected vehicle offers to create a new Profile or download an existing one) |
| tc_title（＝ test_item 首段） | Connected vehicles offer create new or download existing |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Next button stays unavailable while the username is empty<br>(Verifies that the Next button stays unavailable while the username field is empty and becomes available at one character) |
| tc_title（＝ test_item 首段） | Next button stays unavailable while the username is empty |
...
| test_procedure | 1. Read the Next button with zero characters in the username field<br>2. Type one character into the username field<br>3. Read the Next button and check whether it is available |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Spaces count toward the username character limit<br>(Verifies that a space is accepted in a username and counts toward the 12-character limit) |
| tc_title（＝ test_item 首段） | Spaces count toward the username character limit |
| pre_conditions | 1. A New Profile Setup is at the username step<br>2. The username field is empty<br>3. The vehicle is stationary |
| input_test_data | Username length: 12 → 13 characters (eleven letters plus one space, then one more character) |
| test_procedure | 1. Type eleven letters and one space, giving twelve characters<br>2. Read the field and record the characters accepted<br>3. Type a thirteenth character and read the username field |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Save, Cancel and Back options on the username step<br>(Verifies that the username step offers Save, Cancel and Back, with Save leading to step 3 and Cancel asking to confirm) |
| tc_title（＝ test_item 首段） | Save, Cancel and Back options on the username step |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Special characters are not accepted in a username<br>(Verifies that letters and digits are accepted in a username while special characters are refused) |
| tc_title（＝ test_item 首段） | Special characters are not accepted in a username |
| pre_conditions | 1. A New Profile Setup is at the username step<br>2. The username field is empty<br>3. The vehicle is stationary |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Highlight moves to the newly chosen avatar<br>(Verifies that the newly chosen avatar is highlighted and the avatar recorded before the choice is not) |
| tc_title（＝ test_item 首段） | Highlight moves to the newly chosen avatar |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Avatar shown above the Save & Continue button on 8.4 inch<br>(Verifies that a screen of 8.4 inches or larger shows the avatar above a button reading Save & Continue) |
| tc_title（＝ test_item 首段） | Avatar shown above the Save & Continue button on 8.4 inch |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Avatar shown next to the Save button on 7 inch screens<br>(Verifies that a 7-inch screen shows the avatar next to a button reading Save) |
| tc_title（＝ test_item 首段） | Avatar shown next to the Save button on 7 inch screens |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | At least ten avatars are offered initially<br>(Verifies that at least ten avatars are offered when none is in use) |
| tc_title（＝ test_item 首段） | At least ten avatars are offered initially |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Avatars are sorted into at least three categories<br>(Verifies that avatars are sorted into at least three categories and that a category button shows its own group) |
| tc_title（＝ test_item 首段） | Avatars are sorted into at least three categories |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Final setup step offers current or default preferences<br>(Verifies that the final setup step offers current or default preferences and that choosing current carries them over) |
| tc_title（＝ test_item 首段） | Final setup step offers current or default preferences |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Setup completion returns to All Profiles with the new Profile<br>(Verifies that completing the setup returns to the All Profiles tab with the new Profile active) |
| tc_title（＝ test_item 首段） | Setup completion returns to All Profiles with the new Profile |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Editing returns to the originating Edit Profile page<br>(Verifies that an edit started from the Edit Profile tab returns to the page it was started from) |
| tc_title（＝ test_item 首段） | Editing returns to the originating Edit Profile page |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Status bar icon updates to the new active Profile<br>(Verifies that the status bar Profile icon updates to the newly created active Profile) |
| tc_title（＝ test_item 首段） | Status bar icon updates to the new active Profile |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Editing only the avatar shows only the avatar popup<br>(Verifies that editing only the avatar shows only the avatar popup and that the back arrow cancels the edit) |
| tc_title（＝ test_item 首段） | Editing only the avatar shows only the avatar popup |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Back arrow returns to the previous step keeping selections<br>(Verifies that the back arrow returns to the previous setup step and keeps the selections already made) |
| tc_title（＝ test_item 首段） | Back arrow returns to the previous step keeping selections |
