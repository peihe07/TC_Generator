...
- 本檔 **11 條**；另半在 `55_review_pack_24b.md`
- 由 `scripts/build_review_pack.py` 產生，不經人手轉錄
- **本檔取代 `44_review_pack_24a.md`**（AA-1，44 包）——該檔無語料指紋，`--verify` 一律判過期
...
| `NR1L-UserProfiles-137` | `6bb36789d9be` |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Default Profiles ordered by their memory seat link<br>(Verifies that default Profiles are ordered by their memory seat link) |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Editing a memory seat link leaves the order unchanged<br>(Verifies that editing a memory seat link leaves the order of the Profiles unchanged) |
