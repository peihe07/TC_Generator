...
- 本檔 **11 條**；另半在 `59_review_pack_24b.md`
- 由 `scripts/build_review_pack.py` 產生，不經人手轉錄
- **本檔取代 `57_review_pack_24a.md`**（AA-1，44 包）——該檔無語料指紋，`--verify` 一律判過期
...
| tc_id | digest |
|---|---|
| `NR1L-UserProfiles-135` | `958632fe36b7` |
| `NR1L-UserProfiles-136` | `b0c50d57723e` |
| `NR1L-UserProfiles-137` | `4b4ca98490ac` |
...
| `NR1L-UserProfiles-144` | `0bc285996d82` |
| `NR1L-UserProfiles-145` | `0ae7affa6fa5` |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Profile order defaults to the order of creation<br>(Verifies that Profiles are ordered by the sequence in which they were added, with the defaults on the left) |
| tc_title（＝ test_item 首段） | Profile order defaults to the order of creation |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Default Profiles ordered by their memory seat link<br>(Verifies that Driver 1, linked to memory seat 1, is the leftmost of the default Profiles on the All Profiles tab) |
| tc_title（＝ test_item 首段） | Default Profiles ordered by their memory seat link |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Editing a memory seat link leaves the order unchanged<br>(Verifies that the recorded order of the Profiles is unchanged after the memory seat link of the second Profile is edited) |
| tc_title（＝ test_item 首段） | Editing a memory seat link leaves the order unchanged |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Restored default Profiles are added to the right<br>(Verifies that default Profiles restored without clearing all Profiles are added to the right) |
| tc_title（＝ test_item 首段） | Restored default Profiles are added to the right |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Clearing all Profiles returns Defaults to default order<br>(Verifies that the default order returns once all Profiles are cleared and the defaults are restored) |
| tc_title（＝ test_item 首段） | Clearing all Profiles returns Defaults to default order |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Active Profile after clearing follows the active memory seat<br>(Verifies that the Profile matching the active memory seat position becomes active after personal data is cleared) |
| tc_title（＝ test_item 首段） | Active Profile after clearing follows the active memory seat |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Active Profile defaults to Driver 1 without memory seats<br>(Verifies that Driver 1 becomes active after clearing on a vehicle without memory seats) |
| tc_title（＝ test_item 首段） | Active Profile defaults to Driver 1 without memory seats |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | PU1089 on confirmation and PU1090 on successful clearing<br>(Verifies that PU1089 marks the progress of clearing and that PU1090 marks its completion) |
| tc_title（＝ test_item 首段） | PU1089 on confirmation and PU1090 on successful clearing |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | PU1091 shown when clearing is not confirmed complete<br>(Verifies that PU1091 appears when the head unit or TBM does not report that clearing completed) |
| tc_title（＝ test_item 首段） | PU1091 shown when clearing is not confirmed complete |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Long press on an avatar drags and reorders Profiles<br>(Verifies that a long press on an avatar allows dragging and reordering without activating that Profile) |
| tc_title（＝ test_item 首段） | Long press on an avatar drags and reorders Profiles |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Usernames are center justified and do not overlap<br>(Verifies that usernames are centred under their avatars and do not overlap one another) |
| tc_title（＝ test_item 首段） | Usernames are center justified and do not overlap |
