...
- 本檔 **11 條**；另半在 `59_review_pack_24a.md`
- 由 `scripts/build_review_pack.py` 產生，不經人手轉錄
- **本檔取代 `57_review_pack_24b.md`**（AA-1，44 包）——該檔無語料指紋，`--verify` 一律判過期
...
| `NR1L-UserProfiles-147` | `6bcb67f8f615` |
| `NR1L-UserProfiles-148` | `12c5f054c083` |
| `NR1L-UserProfiles-149` | `ed41926b40f0` |
| `NR1L-UserProfiles-150` | `c2e85f149bfc` |
| `NR1L-UserProfiles-151` | `99f0a22e2300` |
| `NR1L-UserProfiles-152` | `1afd0d0f610c` |
| `NR1L-UserProfiles-153` | `d74b23e38b29` |
| `NR1L-UserProfiles-154` | `7224c01448a4` |
| `NR1L-UserProfiles-155` | `1629da7d3347` |
| `NR1L-UserProfiles-156` | `9569615dabd3` |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Long usernames are truncated on the All Profiles tab<br>(Verifies that a username too long for its space is truncated following the Core HMI rules) |
| tc_title（＝ test_item 首段） | Long usernames are truncated on the All Profiles tab |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Cloud icon marks Profiles linked to a Connected account<br>(Verifies that a cloud icon appears beside the avatar of a Profile linked to a Connected account) |
| tc_title（＝ test_item 首段） | Cloud icon marks Profiles linked to a Connected account |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | R1 High keeps preferences and begins Tutorials after avatar<br>(Verifies that an R1 High vehicle keeps the current preferences and begins Tutorials without launching the account login) |
| tc_title（＝ test_item 首段） | R1 High keeps preferences and begins Tutorials after avatar |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Welcome popup at vehicle entry prompts to customize<br>(Verifies that a Welcome popup appears at vehicle entry and prompts the user to customize the default Profile) |
| tc_title（＝ test_item 首段） | Welcome popup at vehicle entry prompts to customize |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Welcome popup shows the Profile, Switch Users and Get Started<br>(Verifies that the Welcome popup names the active Profile and offers Switch Users and Get Started) |
| tc_title（＝ test_item 首段） | Welcome popup shows the Profile, Switch Users and Get Started |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Remind me Later closes the popup until next activation<br>(Verifies that Remind me Later closes the Welcome popup until that Profile is activated again) |
| tc_title（＝ test_item 首段） | Remind me Later closes the popup until next activation |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Clicking outside closes the popup without asking<br>(Verifies that clicking outside the Welcome popup closes it without asking Remind me Later or Don't Show me Again) |
| tc_title（＝ test_item 首段） | Clicking outside closes the popup without asking |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Don’t Show me Again turns the popup off for that Profile<br>(Verifies that Don't Show me Again turns the Welcome popup setting off for that Profile only) |
| tc_title（＝ test_item 首段） | Don’t Show me Again turns the popup off for that Profile |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Get Started starts setup and carries over preferences<br>(Verifies that Get Started starts the New Profile Setup and carries the current preferences over without a confirmation popup) |
| tc_title（＝ test_item 首段） | Get Started starts setup and carries over preferences |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Switch Users opens the All Profiles tab<br>(Verifies that Switch Users on the Welcome popup closes it and opens the All Profiles tab of the Profile section) |
| tc_title（＝ test_item 首段） | Switch Users opens the All Profiles tab |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Avatar and Welcome text open the last known tab<br>(Verifies that the avatar and the Welcome text open the last known tab of the Profile section) |
| tc_title（＝ test_item 首段） | Avatar and Welcome text open the last known tab |
