...
- 本檔 **17 條**；另半在 `55_review_pack_33b.md`
- 由 `scripts/build_review_pack.py` 產生，不經人手轉錄
- **本檔取代 `53_review_pack_33a.md`**（AA-1，44 包）——該檔無語料指紋，`--verify` 一律判過期
...
| `NR1L-UserProfiles-163` | `d0e890a43310` |
...
| `NR1L-UserProfiles-168` | `60128cc96815` |
...
| `NR1L-UserProfiles-173` | `4e1ce71fbe14` |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Valet Mode welcome popup clears like the other popups<br>(Verifies that the Valet Mode welcome popup clears in the same way as the other welcome popups) |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | New Profile Setup starts from the All Profiles tab<br>(Verifies that the New Profile Setup starts from the All Profiles tab) |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | Profile Setup is presented as a series of popups<br>(Verifies that every step of the Profile Setup is presented as a popup) |
...
| 欄 | 值 |
|---|---|
| test_item（兩段） | The same username can be used by two Driver Profiles<br>(Verifies that the same username can be used by more than one Driver Profile) |
