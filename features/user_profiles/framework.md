...
**現行之實例**：`TC-001`（4.1）與 `TC-004`（5.9）之引用欄併列 `3.1`–`3.5`
（PLP 表五列），依據為該二條需求之對象**本來就是整張表**
（4.1 之 store **all** … listed in PLP table、5.9 之 **any**）——
併列成立，但兩條 TC 各只實際驗了其中三列與一列。

**故覆蓋稽核之分子一律取「實際被驗之節／列」，不取引用欄。**
PLP 各列之實際覆蓋深度另由 `DECISIONS.md` D-UP16-02（全稱命題以單例驗證）
與 D-UP17-01 承擔。

### 4.2 「180 / 180」之讀法（42 包 §三）

> **全覆蓋說的是 037 之每個 leaf 都有一條 TC，不是「每個 leaf 都驗完了」。**
> **覆蓋率是分母的性質，不是分子的品質。**

41 輪達成 180 / 180。**同一批就有四處於 remarks 具名「本條不保證」之留白**：

| leaf | 留白 |
|---|---|
| `SWE1-HMI-PROF-058`（7.2）| 小版／大版 welcome popup 之**選用條件**條文未載 |
| `SWE1-HMI-PROF-067`（8.3）| popup 之逐步對映在外部之 HMI Pop Up List，只驗形態 |
| `SWE1-HMI-PROF-077`（8.8.1）| 「至少 10 個」未說是否須同一畫面可見 —— 分散在三個分類之實作會通過 |
| `SWE1-HMI-PROF-078`（8.8.2）| 7 吋之獨立分類畫面 037 未另切 leaf，本條取 8.4 吋以上一側 |

**交付說明如需引用覆蓋率，須同時載明此句**，不得只給 180 / 180。
同理適用於 §4.1 之三個數（133／135／180）—— 三者皆為分母之性質。
