...
R-U3 [PARTIALLY SUPERSEDED by R-U25 — 「證據＝內文完整」之解讀；spec 基線檔名與 spec_mode = A 仍生效]  spec 基線 = SYS1_HMI_Personal_Account_HMI_Logic_and_Flow_R1L-R_
...
R-U15 [PARTIALLY SUPERSEDED by R-U27 — 阻斷範圍；其餘三項判讀仍生效] DR #4（PU1087／PU1088）之阻斷範圍
...
```text
R-G4 [PARTIALLY SUPERSEDED by R-G4-1 — 讀者數 2 更正為 3；檔名歸屬與不得無聲覆寫仍生效]  recon.py 之輸出檔名歸屬（全域）
...
R-U22 [PARTIALLY SUPERSEDED by R-U39(2) — 引用範圍之限縮；「先驗可讀性」與「037 沒引用不等於 spec 沒寫」仍生效] PROF-001-01（PLP 表）之處置 —— 先驗可讀性，不逕列阻斷
...
```text
R-G7 [EXTENDED by R-G7-1 — 增補第二用途；第一用途未推翻]  反向驗證之對照組（全域慣例）
...
R-U36 [PREMISE CORRECTED by R-U40 — 前提錯（斷字在 xlsx 側非 PDF 側），產出有效] 字內斷字之全量掃描
...
來源包：`docs/handoff/09a_rulings.md`（Pei 2026-08-17 裁定，08 輪覆核所生）。
**R-G9 為全域條文。** 08 輪核可，Phase 1 收尾成立，framework 定案。
...
## 條文收斂之落地（09 輪，09b 作業 2–5）

**A 類（升格為通則）之十一項已寫入 `docs/fw036/FEATURE_ONBOARDING.md` §9。**
本檔之原條文**全部保留**，各加註其升格去向。

> **升格之範圍為「原則」，非「條文」**（執行層回報，見上繳 09 §2）：
> R-U6／R-U25／R-U16 等條同時含通則與 feature 事實，升上去的只有前者。
> 各條之註記逐條寫明升了哪一部分。

**R-G1～R-G9 已集中於 canon §9.2**，本檔之全域條文段改為**引用**；
原文保留於各輪之落檔位置，不刪。

**C 類六條已加 `[SUPERSEDED by R-xxx]` 前綴，原文一字未改。**

> **執行層對 C 類之範圍有異議，已具名回報（上繳 09 §3）**：
> `R-U3`／`R-U15`／`R-U22` 三條**只有一部分被取代**，其餘部分現仍生效
> 且被其取代者本身所引用。依 09b「只加前綴、原文不改」照辦，
> **其取代範圍逐條記於下**，待裁後再定是否改為 `[PARTIALLY SUPERSEDED]`。

| 條 | 被取代者 | **仍然生效者** |
|---|---|---|
| `R-U3` | 「證據＝內文完整」之解讀（R-U25 拆為結構／內文兩面）| **spec 基線之檔名、`spec_mode = A`** —— R-U1 之 `specification_reference` 格式即依此 |
| `R-U15` | 阻斷範圍（R-U27 收窄為「僅 popup 內文」）| 「非 Phase 1 阻斷」「不得以鄰近 PU id 推定內容」「不是版本不對，是這一版沒有這兩列」|
| `R-U22` | 引用範圍限於 `PROF-001-01`（R-U39(2) 改為可測判準）| 「先驗可讀性不逕列阻斷」「**037 沒引用不等於 spec 沒寫**」「據此重估 A-UP02」—— **R-U28 即建於其上** |
| `R-U36` | 立條前提（R-U40）| 其產出：C 組終值與判準之偽陽性風險 |
| `R-G4` | 讀者數 2（R-G4-1 更正為 3）| 檔名歸屬與「不得無聲覆寫」之要求 |
| `R-G7` | —（R-G7-1 為**增補**非取代）| 全部；其第二用途為新增 |

### A 類升格之對照（09 輪，09b 作業 2）

**原條文全部保留，未改一字。** 各條所升者為其**原則**，非其條文 ——
凡同時含 feature 事實者，該事實留在本檔。

| 條 | 升格去向（`docs/fw036/FEATURE_ONBOARDING.md`）|
|---|---|
| `R-U6` | canon §9.1 第 4 項（style authority 不取自身產出）—— **僅其原則；Home 144 列、BLANK 綁定四項未升** |
| `R-U12` | canon §9.1 第 9 項（保住檔案與保住雜湊是兩件事） |
| `R-U14` | canon §9.1 第 8 項（解除條件須為機器檢查且實跑）—— **僅其原則；A-UP09 之編號未升** |
| `R-U16` | canon §9.1 第 5 項（附件副本不等於 repo 檔）—— **僅其原則；037 之 SHA 與檔名未升** |
| `R-U18` | canon §9.1 第 5 項（同上，與 R-U16 合併） |
| `R-U19` | canon §9.1 第 1 項（引用集合與生成集合為兩個分母） |
| `R-U21` | canon §9.1 第 2 項（Sub Categorization 不作阻斷判準） |
| `R-U23` | canon §9.1 第 6 項（判不可讀前先驗抽取能力） |
| `R-U25` | canon §9.1 第 3 項（spec 基線之兩面）—— **僅其原則；本 feature 之 BASELINE 細節未升** |
| `R-U31` | canon §9.1 第 7 項（增欄不取代） |
| `R-U35` | canon §9.1 第 3 項（其 (a) 併入 spec 基線之兩面）—— **(b)(c)(d) 為 feature-specific，未升** |
| `R-U39` | canon §9.1 第 10 項（條文須以可測判準表述）—— **僅其 (2) 所揭之原則** |

**R-G1～R-G9 已集中於 canon §9.2**，本檔各輪之全域條文段自此為**引用**：
其原文保留於落檔位置（`R-G4`／`R-G7` 另標 `[SUPERSEDED]`），
而**權威版本在 canon** —— 兩處不一致時以 canon 為準。

> **升上去的是修訂後之版本**（`R-G4-1`／`R-G7-1`），不是原版。

---

## 第十輪條文（Phase 2 開工）

來源包：`docs/handoff/10a_rulings.md`（Pei 2026-08-17 裁定，09 輪覆核所生）。
**R-G10 為全域條文。** 09 輪核可，異議兩項均成立。
...
**執行層回報（10 輪，2026-08-17）**

- **R-U42**：六條之標記已改，**原文一字未改**。
  **`[SUPERSEDED by X]` 於本批用例為 0** —— 與該條之判定相符。
- **R-U43**：`R-U8` 歸 **B**，其註記已指明通則部分之權威為 **Comfort `R-C3`**（見下）。
- **R-U45**：`.gitignore` 已加例外，`BASELINE.sha256` 已列入該檔，
  `shasum -c` 全綠。**`git add`／`commit` 未執行** —— 清單見上繳 10 §2。
- **R-U44**：本 feature 之自檢已完成（09 包 §5 第 3 項 → R-U45）。
  **他 feature 未動。**
- **R-G10**：本輪之取樣清單與 PLP 掃描皆以餘數驗證，見上繳 10 §4／§5。

### R-U8 之歸類（R-U43）

```
**`R-U8` 歸 B 類（feature-specific）**，不升格。

| 部分 | 性質 | 權威 |
|---|---|---|
| 三閘之值 180／25／2、182 降對照輸出 | feature-specific | **本條**（R-U8）|
| 「不得以 ID 形態判定 leaf」「閘值須以同一單位計」 | 通則 | **Comfort `R-C3`** —— R-U8 係**引用**，不重複升格 |
```

---

## 第十一輪條文（11a 下放包，Pei 2026-08-17 裁定，逐字）
...
R-U47 [PREMISE CORRECTED by R-U53 — must_carry 實為覆蓋 4／未覆蓋 3，非 3／4；核可與其餘判定有效] pilot 取樣清單 —— 核可
...
**執行層回報（11 輪，2026-08-17）**

- **R-U48**：`docs/upstream/10_pilot.md` 已補落檔，全部內容自實際產物重出，
  未以聊天貼文為據。完整未截斷之 git 指令清單見該檔 §2.4。
- **R-U46**：`PLP_ENABLED = True` 已落地；聯集 4 條具名見上繳 11 §3.1；
  盲區掃描結果逐條記 `DECISIONS.md`（D-UP11-01…），見 §3.2。
- **R-U47**：must_carry 未覆蓋之四條已登記於 `DECISIONS.md`，見上繳 11 §4。
- **R-G11**：本輪之位置指涉判準已聲明其盲區，見上繳 11 §3.3。
- **本輪未生成任何 TC**（11b 作業 5）。

---

## 第十二輪條文（12a 下放包，Pei 2026-08-17 裁定，逐字）
...
**執行層回報（12 輪，2026-08-17）**

- **R-U49**：四步逐一執行並貼出輸出 —— 第 7 項自檢先補、**證明其紅**
  （`無歸宿者 = ['p17']`，exit=1），再改 `PAGE_TO_SECTION` 對照表，
  重跑 **7/7 → 8/8 PASS**。見上繳 12 §2。
- **R-U50**：判準四條件寫入 `DECISIONS.md` D-UP12-01，登記為通則候選。
- **R-U51**：口徑寫入 `DECISIONS.md` D-UP12-02，10.2／10.3.1 定案為否，代價已記。
- **R-U52**：`plp_scan_union()` 收進 `--selfcheck` 第 8 項；
  **重算所得與常數逐條相符**；對照向 `--selfcheck-tamper drop｜add` **兩向皆紅**。
- **R-U53**：R-U47 已標 `[PREMISE CORRECTED by R-U53]`，**原文一字未改**。
- **R-U54**：重述見上繳 12 §5。**其前提有一處須更正** ——
...
來源包：`features/user_profiles/docs/handoff/26_scope.md`
（Pei 2026-08-18 裁定）。**本條為 feature-level**，
其升為全域須 Pei 另行一句（見條文末段）。
...
**執行層回報（26 輪，2026-08-18）**

- **R-U56 已逐字入庫**（上）。`docs/runtime/profiles/FW036_R1L_UserProfiles_Profile.md`
  §0 之範圍段已加註指向本條；profile §4 之上游缺口表據此改列。
- **DR #3／DR #7**：狀態改 **CLOSED — OUT-OF-SCOPE (R-U56)**，
  **全部實測記載保留**（3.1–3.5 可讀之證據、與 Comfort R-C16 之形態比對、
  「8.8 切兩個而 9.1.1 切一個」之佐證）。
  已明記「**3.1–3.5 之使用不受本次關閉影響**」——
  其為 `PROF-001-01` 之 in-scope 依據，`specification_reference` 繼續併列 3.x。
- **RD #5／RD #6**：改為**獨立送出**，已移除「併 DR #3 送出」之字樣；
  英文可寄版落 `docs/upstream/26_rd_queries.md`（Tier 3，**由 Pei 寄出**）。
- **A-UP02**：分類改 **OUT-OF-SCOPE（已裁）**，記載未關閉。
- **連帶掃描（作業 4）**：全批 78 條之 `reasoning`／`remarks` 命中 **3 處**，
  逐條判並改述 **1 處**（`TC-012` 之 PU0609 句）——
  另二處經判**不屬 R-U56 之範圍**，理由見上繳 26 §4。
- **本輪未執行任何 git**（R-G5）。
...
來源包：`features/user_profiles/docs/handoff/39_rd_disposition.md`
（Pei 2026-08-18 裁定）。
...
**執行層回報（39 輪，2026-08-18）**

- **R-U57 已逐字入庫**（上）。`docs/runtime/profiles/FW036_R1L_UserProfiles_Profile.md`
  §0 之範圍段已加註指向本條；`DATA_REQUESTS.md` 之 RD #5／#6 兩節
  各加註本條與其 Urgency 調整（高 → **低**）。
- **餘 55 leaf 之 label 曝險掃描（作業 2）**：命中 **2 節 / 2 leaf** ——
  `5.16`（`SWE1-HMI-PROF-045`）與 `8.2`（`SWE1-HMI-PROF-066`），
  兩者皆為 `Connected account`（**帳號之名詞**，非按鈕 label），
  **且皆不在 ch9**。RD #5 之兩種讀法（列級／及於 ch9 全章）**都不及於它們**。
  詳見上繳 39 §2；**該判定為範圍認定，非「形式差異」之認定**（R-U57 之界線）。
- **未依 R-U57 自行認定任何項為「形式差異」** —— 39 包明令，本輪無此需要：
  RD 尚未寄出，亦無答覆可套用。
