...
add(3, "§4.4/§8.5", "Pre-Condition 僅狀態／環境；無系統預設、待測前提、動作、step 可控狀態",
    not hit, "0 命中；4 則各 1 項（訊號源，§4.5-1 環境資料）" if not hit else str(hit))
...
for tc in TCS:
    ...
    if k in ("010", "012"):
        ...
        pre0 = "transmitted at 0" in tc["pre_conditions"]
        ok9 &= pre0
