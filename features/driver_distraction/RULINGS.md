...
| R-DD20 | **v3** | `-001`／`-002` 之激勵：同一性（定義級）與施加式同 v2；**(c) 改述** —— `-002` 之終止步驟**不附 `$` 指令行**（台架程序層級，非規格缺件），撤 `PENDING: DR-DD9`；**(d) DR-DD9 降緩發，不阻斷出貨** | 19 §二 |
...
| R-DD24 | v1 | 判定之強度（R-DD23 之第四欄）：first-match 須回傳**是否為正面命中**，落 fallback 者一律 **WARN**；WARN 之 detail 須列出**所有未命中之判準**；覆核自此問四欄 | 19 §四 |
| R-DD25 | v1 | 市場範圍：**NAFTA 在案、LATAM 不在案**；`Hong Kong` 在案之依據為**右駕（RHD）非區域**；`-025`~`-028` 判 **OUT OF SCOPE**（不該測，非資訊不足）；A-DD1 改 `CLOSED-BY-SCOPE`；差額須入 [CG-DD2]，不得把 28 寫成 24 而不交代 | 20 §二 |
| R-DD26 | **v2** | 跨包引用之前提須為**已量測之狀態**；**前提之搬運為雙向**（假定已執行而未執行／假定未執行而已執行）；**執行層於執行前先驗前提，不符即驗而不做並寫出驗法**；分析層書前提須標其量測時點 | 22 §二 |
| R-DD27 | v1 | 注入須含**期待為綠**之組（證 carve-out 未關太緊／未引入偽陽性）；一組注入之價值以**新舊之別**陳述，須實跑舊版取其行為，不得由裁定文推定；**臨時驗法之結論入包者須同時寫出其驗法** | 22 §三 |

## `tc_id` 之最終指派與舊 id 對照（Pei 2026-08-29 裁）

**裁定**：出貨閘取**乙案**（即寫回），`tc_id` 依 **leaf 升冪**指派 `001`–`024`
（Pei 2026-08-29；上繳包 19 §七 之材料）。

**何以非任意**：本 feature 已產出之 24 leaf 恰為 `001`–`024` **連續**，
故 leaf 升冪下 **`n` 恆等於 leaf 號** —— 對映由 leaf 導出，非另立一套序。

> **⚠ 舊 id 在改動前後指向不同之 TC**（下表首四列）——
> `newR1L-DD-001` 原指 leaf `009`，現指 leaf `001`。
> **凡包 19 以前之上繳包、handoff 所書之 `newR1L-DD-00n` 皆為舊義**，
> 須以本表回溯。產物與四台帳之引用已於本輪同步改畢（單趟原子替換，9 處）。

| 新 `tc_id` | leaf | 舊 `tc_id` | 批次 |
|---|---|---|---|
| `newR1L-DD-001` | `001` | `newR1L-DD-D001` | Body Off Init |
| `newR1L-DD-002` | `002` | `newR1L-DD-D002` | Body Off Init |
| `newR1L-DD-003` | `003` | `newR1L-DD-B001` | B1 |
| `newR1L-DD-004` | `004` | `newR1L-DD-B002` | B1 |
| `newR1L-DD-005` | `005` | `newR1L-DD-B003` | B1 |
| `newR1L-DD-006` | `006` | `newR1L-DD-B004` | B1 |
| `newR1L-DD-007` | `007` | `newR1L-DD-B005` | B1 |
| `newR1L-DD-008` | `008` | `newR1L-DD-B006` | B1 |
| `newR1L-DD-009` | `009` | `newR1L-DD-001` | pilot |
| `newR1L-DD-010` | `010` | `newR1L-DD-002` | pilot |
| `newR1L-DD-011` | `011` | `newR1L-DD-003` | pilot |
| `newR1L-DD-012` | `012` | `newR1L-DD-004` | pilot |
| `newR1L-DD-013` | `013` | `newR1L-DD-B007` | B1 |
| `newR1L-DD-014` | `014` | `newR1L-DD-B008` | B1 |
| `newR1L-DD-015` | `015` | `newR1L-DD-B009` | B1 |
| `newR1L-DD-016` | `016` | `newR1L-DD-B010` | B1 |
| `newR1L-DD-017` | `017` | `newR1L-DD-C001` | B2 |
| `newR1L-DD-018` | `018` | `newR1L-DD-C002` | B2 |
| `newR1L-DD-019` | `019` | `newR1L-DD-C003` | B2 |
| `newR1L-DD-020` | `020` | `newR1L-DD-C004` | B2 |
| `newR1L-DD-021` | `021` | `newR1L-DD-C005` | B2 |
| `newR1L-DD-022` | `022` | `newR1L-DD-C006` | B2 |
| `newR1L-DD-023` | `023` | `newR1L-DD-C007` | B2 |
| `newR1L-DD-024` | `024` | `newR1L-DD-C008` | B2 |

**舊式之 `B`／`C`／`D` 前綴不合 R-DD2 之 `newR1L-DD-{n:03d}`**（該式只產生數字），
其為各批生成時之工作用識別，**未經任何裁定授權**。本輪寫回時一併歸正。

---

## 下放包裁定（`10-x`）之更正紀錄

`10-x` 為下放包所裁而**不成條**之項，故不在上表。其事後更正若不記於此，
**只存在於某一份上繳包內**，日後讀該裁定者不會看到。

| 裁定 | 出處 | 更正 | 依據 |
|---|---|---|---|
| **10-5** | 下放包 21 §四 | 10-5 之改法**仍成立**，惟其所得據執行層 §4.3 之實測改述 —— 不是「從無防護到有防護」，是「**訊息指向真因**」，且**淨新增價值在注入 U** （一方區間、一方列舉之混用，舊版誤停、新版正確通過）。 | 下放包 22 §1.1；執行層實測見上繳 18 §4.3 |

> **10-5 原文所書之失效態（「一致地取到空集合而通過交叉核對，一致地錯是最壞之失效態」）
> 不存在** —— 舊解析器對該注入 `exit 1`，由閉合總數與第三項覆核各自攔下，
> 且後者直接印出那四個 leaf。**分析層對一份未執行之程式斷言其執行行為**
> （下放包 22 §1.1 自認，同族第五次）。
>
> 本欄之一般性拘束已落為 **R-DD27(b)**：
> 陳述改法所得時須**實跑舊版**取其行為，不得由裁定文之敘述推定。
...
| `R-DD20`（v2） | R-DD20 v3（下放包 19 §二）| **v2 `5ae8694f`** ／ v3 `1f2aef6d` | (c) 只書「不變」，其所指之字串在 v1（中文佔位，違 IN §1）—— **本身未載任何可執行之字串**；且其 `PENDING: DR-DD9` 之缺件認定於 v3 被撤（037 本以通稱書之，上游未擬給指令，IN §8.4.3 之佔位義務於本欄不發生）；**無 (d) 之 DR-DD9 降級** |
| `R-DD26`（v1） | R-DD26 v2（下放包 22 §二）| **v1 `ba96c30f`** ／ v2 `781a0030` | **無 (e)**（前提之搬運為雙向 —— 只治正向，包 21 之反向誤不在其射程）；**無 (f)**（執行層執行前先驗前提之義務）；**無 (g)**（分析層書前提須標量測時點）|
