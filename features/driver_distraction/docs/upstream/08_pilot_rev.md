...
# 上繳包 08（續）—— 下放包 11 之 T17a／T17b

- 對應下放包：`11_rulings_anchor.md` §四（T17a／T17b）、§五（6 項）
- 次序依 §三：**T-抄（上一輪已完成）→ T17a → T17b**
- 範圍依 §二：**只改 `features/driver_distraction/RULINGS.md` 一檔**

> **結論先講**：**T17a 完成**（14 錨點，條文本體 0 差異）。
> **T17b 停於第 5 步，未寫入 tsv** —— 逐行歸因出現**無法歸因於
> 「本線新增 14 列」或「popup 線 R-POP18~20」之差異**：
> popup 有 **4 條條文本體被改寫**、**5 條全新**（R-POP21~25），
> 且其 `RULINGS.md` **尚未 commit**。寫入即把該線之未定稿固化為基線。

---

## 12. T17a —— 加標題錨點

**程式**：`scripts/t17a_add_anchors.py`（新增；預設 dry-run，`--apply` 方寫入）

錨點文字**不由圍籬首行推導**，改以明示對照表（理由見 §12.2）。

### 12.1 §五-1：逐字元 diff 輸出（條文本體 0 差異）

```
擬加錨點 14 個：
  行   41  ## R-DD1        行  127  ## R-DD7
  行   52  ## R-DD2        行  160  ## R-DD8
  行   62  ## R-DD3        行  181  ## R-DD6      ← 現行版（v2）
  行   81  ## R-DD4        行  211  ## R-DD9
  行   93  ## R-DD5        行  233  ## R-DD10
  行  107  ## R-DD6 v1     行  249  ## R-DD11     ← 留存版
  行  269  ## R-DD12       行  287  ## R-DD13

[回證 1] 移除插入行後與原檔逐位元比對：全等 ✓
          原檔 sha256 477300529af21208
          還原 sha256 477300529af21208
[回證 2] diff：刪除 0 行／新增 28 行
          新增行中非「錨點或空行」者：無
          條文本體之差異：0 ✓
[回證 3] 圍籬內文字：原 7269 字元／新 7269 字元；逐字元全等 ✓
```

**三項回證各自獨立**，任一項單獨不足以證明「條文本體未動」：

| 回證 | 所證 | 單獨之不足 |
|---|---|---|
| 1 移除插入行後全檔 sha256 相等 | 除插入外全檔未動 | 依賴「插入位置記錄正確」|
| 2 diff 刪除 0 行、新增行全為錨點或空行 | 無刪除、無改寫 | 不涵蓋「插入到圍籬**內**」之情形 |
| 3 圍籬內文字 7269 字元逐字元全等 | 圍籬內容未被觸及 | 不涵蓋圍籬外之改動 |

**三者合起來才閉合。** R-DD14(b) 要求「差異須為 0」，
本輪以**移除插入行後之 sha256 相等**為其嚴格形式。

### 12.2 ⚠ 錨點命名 —— 圍籬首行之字面與 R-DD14(a) 相反

第一版腳本由圍籬首行推導錨點，得：

```
R-DD6 v1 之圍籬首行 = `R-DD6（訊號名之架構軸）`      → 推導出 `## R-DD6`
R-DD6 v2 之圍籬首行 = `R-DD6 v2（訊號名之架構軸）`   → 推導出 `## R-DD6 v2`
```

**與 R-DD14(a) 恰恰相反** —— 該條明定「現行版 `## R-DDn`；
**留存版**加 `## R-DDn v1`，其標題文字須與索引表之留存列一致」，
而索引留存列為 `` `R-DD6`（v1） ``。
...
```python
ANCHOR_OVERRIDE = {
    "R-DD6":    "R-DD6 v1",   # 留存版（其圍籬首行為 `R-DD6（…）`）
    "R-DD6 v2": "R-DD6",      # 現行版（其圍籬首行為 `R-DD6 v2（…）`）
}
```

> 圍籬首行是**條文自身之標題**（寫於下放時），錨點是**索引之識別**。
> 二者在 R-DD6 這一對上不同步 —— 推導法會靜默地把它們對調，
> 而**兩種結果都跑得出 14 個錨點**，停止條件抓不到。

### 12.3 §五-2：錨點數驗證（應 14）

```
$ python3 scripts/rulings_hash.py --target features/driver_distraction/RULINGS.md --out <scratch>
寫入 …：14 錨點（ruling 14），來源 1 檔

重複 ruling_id 1 組（其中本體不同 1 組）：
  R-DD6: features/driver_distraction/RULINGS.md:117 與 :197（**本體不同**）
```

**14 = 13 現行 ＋ 1 留存 ✓**（停止條件 2 通過）。

逐條：

```
R-DD1  a9b76e4b  L41   R-DD7   f33ed793  L139   R-DD10  6f601dd1  L253
R-DD2  da3cd8ec  L54   R-DD8   5dca74aa  L174   R-DD11  872344f4  L271
R-DD3  cc7aea6c  L66   R-DD6   a5cbaf9c  L197   R-DD12  42ddd946  L293
R-DD4  8d4f36bf  L87   R-DD9   965b6d4c  L229   R-DD13  d46f4c85  L313
R-DD5  75513f31  L101
R-DD6  f28ee265  L117  ← 留存版（`## R-DD6 v1`）
```

**⚠ `R-DD6` 之 v1 與 v2 於 tsv 中同 `ruling_id`。** 成因：
工具之錨點正則將 prime（`′″‴`）與子條 `(a)` 併入 id，**但不併入 ` v1`**
—— ` v1` 落入 `rest`（slug）。故二者皆解析為 `R-DD6`。

**此非本輪所致，亦非缺陷**：現行 tsv 本就有 **17 組**重複 id
（`R-C27`／`R-C4`／`R-TM9` 等），工具亦僅印警告而**二列皆收錄**。
`vehicle_setting` 另用 prime（`R-VS37′`／`R-VS19″`）取得相異 id ——
**該法可使 v1／v2 分離，但那是條號之改寫，非本輪授權範圍。**

**未自行改用 prime，亦未調整條文以消除重複**（停止條件 2 明文）。

### 12.4 一項未列入 T-抄之條文 —— R-DD14 本身

下放包 11 §四 之任務表，T-抄 一列所書為
「R-DD11／R-DD12／R-DD13 入 `RULINGS.md`｜來源 **包 10** §八」。
**R-DD14 未列。**

且停止條件 2 明定錨點數應為 **14（13 現行 ＋ 1 留存）** ——
若本輪一併抄入 R-DD14，現行條文將為 14 條、錨點數為 **15**，
**與停止條件牴觸**。

**故本輪未抄 R-DD14。** `RULINGS.md` 現行 13 條、留存 1 條。
**若係漏列，請於次包補 T-抄**；補後錨點數應為 15，屆時停止條件 2 之數值須同步更新。

---

## 13. T17b —— **停於第 5 步，未寫入**

### 13.1 §五-3：第 4 步之 `--check` 現行差異全貌
...
工作區中之 `RULINGS.md`（重生將一併收錄者）：

```
 M features/bed_lowering/RULINGS.md      ← 0 錨點，工具看不見，不入 tsv
 M features/driver_distraction/RULINGS.md ← 本線 T17a
 M features/popup/RULINGS.md             ← **他線，未 commit**
 M features/vehicle_setting/RULINGS.md   ← 未產生任何 tsv 差異行
```

### 13.2 §五-4：第 5 步之逐行歸因表

現行 **557** 列 ／ 重生 **576** 列；`diff` 差異行 **51**。

```
=== 逐行歸因表（現行 tsv vs 重生）===
ruling_id   來源                                      現行 sha8    重生 sha8    判                     行
----------------------------------------------------------------------------------------------------------------------
R-DD1       driver_distraction                      —          a9b76e4b   新增                    41
R-DD10      driver_distraction                      —          6f601dd1   新增                    253
R-DD11      driver_distraction                      —          872344f4   新增                    271
R-DD12      driver_distraction                      —          42ddd946   新增                    293
R-DD13      driver_distraction                      —          d46f4c85   新增                    313
R-DD2       driver_distraction                      —          da3cd8ec   新增                    54
R-DD3       driver_distraction                      —          cc7aea6c   新增                    66
R-DD4       driver_distraction                      —          8d4f36bf   新增                    87
R-DD5       driver_distraction                      —          75513f31   新增                    101
R-DD6       driver_distraction                      —          f28ee265   新增                    117
R-DD6       driver_distraction                      —          a5cbaf9c   新增                    197
R-DD7       driver_distraction                      —          f33ed793   新增                    139
R-DD8       driver_distraction                      —          5dca74aa   新增                    174
R-DD9       driver_distraction                      —          965b6d4c   新增                    229
R-POP10     popup                                   a2237932   a2237932   行號位移（sha 不變）          85→87
R-POP11     popup                                   a6b5301b   a6b5301b   行號位移（sha 不變）          93→95
R-POP12     popup                                   e323360d   e323360d   行號位移（sha 不變）          101→103
R-POP13     popup                                   bcd97ba8   89306542   **本體改寫**              118→120
R-POP14     popup                                   e025ab99   e025ab99   行號位移（sha 不變）          152→154
R-POP15     popup                                   2c712b56   2c712b56   行號位移（sha 不變）          162→164
R-POP16     popup                                   36e1ed0a   36e1ed0a   行號位移（sha 不變）          186→188
R-POP17     popup                                   7b7868ec   7b7868ec   行號位移（sha 不變）          203→205
R-POP18     popup                                   18ddf460   bfbb3167   **本體改寫**              216→218
R-POP19     popup                                   5d9764bc   5d9764bc   行號位移（sha 不變）          237→251
R-POP20     popup                                   7ac862b3   2894aa0c   **本體改寫**              248→262
R-POP21     popup                                   —          bd4637b8   新增                    295
R-POP22     popup                                   —          1bb746f6   新增                    306
R-POP23     popup                                   —          c5655c80   新增                    324
R-POP24     popup                                   —          e362b2b6   新增                    335
R-POP25     popup                                   —          ec88e973   新增                    348
R-POP5      popup                                   896b4b84   a6816cb1   **本體改寫**              36→36
R-POP6      popup                                   66f64b48   66f64b48   行號位移（sha 不變）          44→46
R-POP7      popup                                   f6d6e22c   f6d6e22c   行號位移（sha 不變）          59→61
R-POP8      popup                                   7425f059   7425f059   行號位移（sha 不變）          66→68
R-POP9      popup                                   a899c245   a899c245   行號位移（sha 不變）          78→80

判別分布： {'新增': 19, '行號位移（sha 不變）': 12, '**本體改寫**': 4}
來源分布： {'driver_distraction': 14, 'popup': 21}
```

### 13.3 判定 —— **停止條件 3 成立，不寫入**

下放包 §三 第 5 步之停止條件逐字：

> **出現無法歸因於「本線新增 14 列」或「popup 線 R-POP18~20」之差異 → 停並回報，不寫入。**

歸因結果：

| 歸因 | 列數 | 是否在允許範圍 |
|---|---|---|
| 本線新增（`R-DD1`~`R-DD13` ＋ `R-DD6` v1／v2 二列）| **14** | ✅ 允許 |
| popup 行號位移（sha 不變，內容未動）| 12 | ✅ 無害 —— sha 為權威，行號僅為指位 |
| **popup 條文本體改寫**（`R-POP5`／`R-POP13`／`R-POP18`／`R-POP20`）| **4** | ❌ **不在允許範圍** |
| **popup 全新條文**（`R-POP21`~`R-POP25`）| **5** | ❌ **不在允許範圍** |

**兩點使其超出「popup 線 R-POP18~20」之預設：**

1. **現行 tsv 已含 `R-POP18`／`R-POP19`／`R-POP20`**（L216／L237／L248）——
   上繳包 07 §1.3 所記之「3 列未收錄」**已由 popup 線自行重生收錄**。
   **該已知項已消化完畢**；本輪所見者為其後之新一輪工作。
2. 現況為 **4 條被改寫 ＋ 5 條新增**，其中 `R-POP18`／`R-POP20` **本體亦已改**
   （`18ddf460`→`bfbb3167`、`7ac862b3`→`2894aa0c`）——
   即使按名稱看似落在「R-POP18~20」之內，**其內容已非當時所指之那三條**。

且 **`features/popup/RULINGS.md` 為未暫存之工作區修改**
（`git diff --stat` = 107 insertions / 4 deletions；`--cached` 為空）：

```
 features/popup/RULINGS.md | 111 ++++++++++++++++++++++++++++++++++++++++++++--
...
**故：未寫入 `docs/fw036/RULINGS.sha.tsv`。第 6 步未執行。**

### 13.4 §五-5：寫入後之 `--check` 結果

**無** —— 第 6 步未執行，故無寫入後之 `--check`。

**現行 tsv 維持 FAIL**（成因為本線 14 列未收錄 ＋ popup 線 9 項未收錄）。
**此 FAIL 為已知且已歸因，非新增之未知態。**

### 13.5 重生之其他觀察（不影響判定，但影響下次）

重生輸出之種類分布為 `group 10／report 12／ruling 550／superseded 4`，
而現行 tsv 之列數為 557、重生為 576。
**`superseded` 一類於現行 tsv 中不存在** —— 其為工具依
`RE_SUPERSEDED`（`作廢｜SUPERSEDED｜已撤回`）所標。
**本輪未追查該 4 條之來源**（非本輪範圍），記之以免下次重生時被誤讀為新增。

### 13.6 解除之條件（供分析層與 Pei）

本線已具備寫入之全部前提（T17a 完成、14 錨點驗訖）。**唯一之阻是他線之中間態。**
任一成立即可續行第 6 步：

- **甲**：popup 線 commit 其 `RULINGS.md` 並**自行重生 tsv**；本線續行時差異僅剩本線 14 列
- **乙**：Pei 明示「popup 之現況得一併固化」，本線據以寫入並於上繳包載明所固化之 9 項
- **丙**：由 popup 線統一重生（其變動較大，且僅其線知悉自身是否定稿）

**分析層意見**：**丙** 最省事故 —— 重生是一次寫全的，
**由變動最大的那一線執行，其他線只需驗自己那幾列**。
本線之 14 列已可被任何一次重生正確收錄，無時序要求。

---

## 14. §五-6：其餘 4 feature 之未轉換狀態（一句提示，不代辦、不催）

| feature | 標題錨點 | 圍籬條文 | 狀態 |
|---|---|---|---|
| `display` | 0 | **59** | **未轉換** |
| `sw_update` | 0 | **38** | **未轉換** |
| `vehicle_category` | 0 | **30** | **未轉換** |
| `bed_lowering` | 0 | **17** | **未轉換** |
| `driver_distraction` | **14** | 14 | **已轉換（本輪）** |

**合計 144 條仍為圍籬體例，工具看不見。**

> ⚠ 一項與上繳包 07 §1.1 之數字差異：`sw_update` 當時實測 **31** 條，
> 本輪為 **38** 條 —— 該線於期間新增了 7 條。
> **148 → 144 之減少，不是「已轉換 4 條」，是本 feature 之 11 條轉出（-11）
> 加上 sw_update 新增 7 條（+7）。** 兩個方向相抵後淨減 4。
> **不逐一核算會把它讀成「轉換了 4 條」。**

依 §二，本輪**未代改、未催**。其轉換由各線或 Pei 統一指示。
...
- **停了，而且是在第 5 步停的。** 前四步全過（T17a 三項回證、14 錨點），
  寫入只差一個動作。**「差一步就完成」是最容易說服自己繼續的時候。**
- **錨點命名沒有照推導走。** 由圍籬首行推導會把 R-DD6 之 v1／v2 對調，
  而**兩種結果都跑得出 14 個錨點** —— 停止條件抓不到這個錯。
  是回頭讀 R-DD14(a) 的字面才發現的。
- **三項回證是互補的，不是同一件事講三遍**（§12.1 表）。
- **§14 那個 148→144 的減法我算了。** 直覺會寫成「已轉換 4 條」。

### 15.2 我做糙的

- **第一版腳本用推導**，那是因為推導寫起來快。若不是回讀條文，會靜默地錯。

### 15.3 我拒絕做的

- **不寫入 tsv。** 前提已備、只差一步、而且寫下去 `--check` 就綠了 ——
  **但綠的代價是把 popup 未 commit 的 107 行插入固化成基線。**
- **不改條文以消除 `R-DD6` 之重複 id**（停止條件 2 明文），
  **也不代 popup 線 commit 或重生**。
- **不代改其餘 4 feature**（§二）。

### 15.4 一件我原本會漏的

我原本打算把「popup 線 R-POP18~20」直接當成允許項打勾 ——
**名稱對得上**。逐行比 sha 才看到 `R-POP18`／`R-POP20` **本體已被改寫**，
即現行 tsv 裡的那三條和重生出來的那三條**不是同一份東西**。

**按名稱歸因會通過，按 sha 歸因才停得下來。** 這正是 sha 台帳存在的理由。
...
| 計數 | 分子 | 分母 |
|---|---|---|
| 錨點 14 | 工具對本檔解析出之 `ruling` 列數 | 本檔圍籬條文 14（13 現行 ＋ 1 留存）|
| 條文本體差異 0 | 移除插入行後與原檔相異之位元組數 | 全檔（原 sha256 `47730052…`）|
| 圍籬內 7269 字元 | `^```\n(.*?)\n```$` 全部命中之串接長度 | 同上，改動前後各算一次 |
| diff 差異行 51 | `diff` 之 `<`／`>` 行數 | 現行 557 列 vs 重生 576 列 |
| 歸因：新增 19／位移 12／改寫 4 | 以 `(ruling_id, source)` 為鍵、比對 `sha8` 與 `line` | 上述 51 行去除表頭與配對後之 35 個判別 |
| 未轉換 144 條 | display 59＋sw_update 38＋vehicle_category 30＋bed_lowering 17 | 15 個 `features/*/RULINGS.md` |

### 16.2 界線

- **歸因以 `(ruling_id, source)` 為鍵**。若同一檔內同 id 之二列**互換順序**，
  本法會將其配對錯而報成二筆「本體改寫」。**本輪 popup 無同 id 重複，不受影響**；
  本檔之 `R-DD6` 二列皆為新增，無配對對象。
- **`superseded` 4 條未追查**（§13.5）。
- **`bed_lowering`／`vehicle_setting` 之工作區修改未產生 tsv 差異行**，
  故未進歸因表 —— 前者 0 錨點（工具看不見），後者其修改未觸及任何錨點之本體或行號。
  **「未產生差異」不等於「未修改」。**
- **T17a 之 `--apply` 只動一檔**，程式內 `TARGET` 為常數，無參數可改指他檔。

### 16.3 檔與開啟方式

| 標的 | 開啟 |
|---|---|
| `features/driver_distraction/RULINGS.md` | **本輪唯一寫入之檔**（T17a） |
| `docs/fw036/RULINGS.sha.tsv` | **唯讀** —— 重生一律導向 scratchpad |
| 其餘 `features/*/RULINGS.md` | **唯讀**（工具掃描） |
| `scripts/rulings_hash.py` | **唯讀**（未改工具）|
...
| # | 事項 | 現況 |
|---|---|---|
| 7 | **tsv 重生之解除** | T17b 停於第 5 步；三案見 §13.6，分析層傾向**丙**（由 popup 線重生）|
| 8 | **R-DD14 未列入 T-抄** | 本輪未抄；若補抄，錨點數將為 15，停止條件 2 之數值須同步更新（§12.4）|
| 9 | **`R-DD6` v1／v2 於 tsv 同 id** | 非本輪所致（現行 tsv 已有 17 組）；`vehicle_setting` 之 prime 法可分離，屬條號改寫，未逕為（§12.3）|
| 10 | 其餘 4 feature 之轉換 | 未代辦；**144 條**（非 148，見 §14 之減法）|

**前 6 項見 §11，其中第 3 項（輸出鍵名二制）仍未獲回覆。**
