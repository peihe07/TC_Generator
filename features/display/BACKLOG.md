...
## 34b 之自陳（R-DM57(b)：入本檔，不佔輪次、不觸發新任務）

下放包 34b 執行時，於母本／輸出之逐部件比對抓到**三項寫回機制之實錯**。
三項已在 34b 之重出中改正，全文見 `docs/upstream/34_closeout.md` §9.2。
此處只記其**通則面**，供日後鷹架複用時查。

| # | 事 | 通則 |
|---|---|---|
| C1 | `_set_row()` 重建 `<c>`／`<row>` 時丟失 `s=` 樣式索引與 `spans`／`s`／`customFormat` | **XML 外科式之「外科」不只指範圍，也指屬性。** 重建一個節點就是把它原有的一切預設為可丟 —— 保留須明寫 |
| C2 | B 欄被賦值，毀去 `ref="B11:B74"` 之共用公式宿主（違反 R-DM15） | **條文在、`feature.yaml` 註記在，腳本仍寫了，且兩份交付本帶著它出門。** 註記寫在「被讀的地方」不等於寫在「被執行的地方」；欄位級之禁令應由腳本之欄表結構承載，而非註解 |
| C3 | 寫入 16 欄而回讀口徑一路寫「15 欄」 | **兩個數不一致而長期無人察覺，因為兩者從未被放在同一句裡比。** C2 改正後名實相符，但這是巧合式的了結，不是機制 |

**未做、亦不在本包範圍**：C1／C2 之類同缺陷未在其他 feature 之
`write_back*.py` 掃描。R-DM57 凍結期內不開新檢查；解凍後值得一掃。

### 待 Pei 之二事（本層不動）

| 項 | 現況 |
|---|---|
| **交付路徑之副本與台帳不符** | 客戶樹之 `…_Display_20260826.xlsx` 實測 `b12bd378…`，非 `DELIVERY.sha256` ENTRY 001 所記之 `4528b937…`。內容仍為 34a 之 23 條，但**已經 Excel 開啟並重存**（inlineStr → sharedStrings、樣式重編、共用公式宿主移為 `B75:B138`）。**交付路徑之複製與處置屬 Pei。** 台帳 ENTRY 002 已具名 |
| **`shasum -c` 之兩行 FAILED 為預期** | 輸出檔名不變（34a §二之要求）＋ 台帳 append-only ⇒ ENTRY 001 之 repo 行必然 FAILED。綠燈判準改以 ENTRY 002 為準 |
