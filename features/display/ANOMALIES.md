...
## A-DM20 — PROXI `Checked by NODE(CHECK)` 欄近乎全空；`Used by NODE` 可用但缺專案 VF 之鍵  [RESOLVED-BY-SCOPE-CHANGE]

> **狀態更正（下放包 30 §四.4，2026-08-26）**：依 **R-DM44**（16 輪）之指示，
> 本項標 `RESOLVED-BY-SCOPE-CHANGE`，**不標 `RESOLVED`** —— 其所求之物未取得，
> 是**用途已由 R-DM33 消滅**（PROXI 改為需求驅動，不再需要對 446 列分類）。
>
> **須具名之事實**：R-DM44 立於 16 輪，其指示之兩處台帳動作
> （DR-DM7 結案、本項改標）**十二輪未執行** —— 直到 28a §2.1(c) 之對帳
> 才被發現（A11）。**與 18 輪之「宣稱已執行而未執行」同型，方向相反：
> 那次是聲明超前事實，這次是事實落後裁決。**
>
> **重開條件（R-DM44 逐字）**：若 Phase 2 之逐 leaf 查詢中，某參數之值域在
> PROXI 中依 VF 而異，則 VF 代碼重新成為必要，DR-DM7 以新編號重開。
> **現況**：`rvc-01` 已觸及 PROXI 之 `r401 Rear_View_Camera`／
> `r494 Rear_View_Camera_Soft_Button`，惟六條 TC **未用到任何 PROXI 值**，
> 故重開條件尚未成立。
...
> **母體加註（下放包 31 §二，2026-08-26；原文依 R-TM13 不刪不改）**：
> 本節之母體為 `1.11.2.2` **一節**，且以舊適用性判準為之（A-DM40／R-G37）。
> 31 輪讀 71 條漏網時發現 **`{4819273}`**（`§1.4.1.2.6 Disassociated Center
> Stack Display (DCSD) - Display Hot`，`[Radio:R1M, R1H, R1L, R1L-R] [EE:All]`，
> **適用本專案**）逐字為 `Execute 'Display is Hot' portion of DCSD Display Hot
> Algorithm - See CFTS013-629.`
> **本節之結論（組 A 與組 B 互斥）不受影響** —— 兩組之適用性重測不變
> （組 A 7／組 B 4），且 `{4819273}` 不在其任一組內。
> **但 `1.11.2.2` 不是本專案 Display Hot 條文之全部。** 登為 A14。
...
> 而非 `{CFTS013-XXX}`。依 R-G69，理由與結論分別查證，故分別更正。
...
## A-DM36 — 26a §二 所述之 `copy` 重複檔，於本層量測時不存在  [CLOSED]
...
> **增列（下放包 32 §三.5，2026-08-26）—— 同一條登記，不另立異常**：
> 31 輪讀 71 條漏網時另見兩條同型者，皆在診斷章：
>
> | 條 | 章節 | Description 逐字 |
> |---|---|---|
> | `{4819233}` | `§1.4.1.1.12 DCSD - Low Voltage` | `Ethernet Cable` |
> | `{4819234}` | `§1.4.1.1.12 DCSD - Low Voltage` | **（空字串）** |
>
> 兩者皆帶正式之 7 位條號而其本體非需求。**與 SYSRA 之三句同型：
> 帶正式編號之空條文。** 其落在診斷章、不入任何 TC，故不阻斷。
> **B26 至此併入本條，不另立。**
...
## A-DM38 — CFTS_013 與 CFTS_020 之 release 家族不同期  [MEDIUM]

28 包 A1 之台帳登記所見（檔名逐字，非推定）：

| 檔 | release 家族 | 文件日期 | sha256（前 16 碼） |
|---|---|---|---|
| CFTS_020（本文） | **`26PI1.5 Mar Release`** | 2026-03-10 | `8696d1f596e33677` |
| CFTS_013（本文） | **`26PI2.5 Jun Release`** | 2026-06-08 | `00d47c9e3bf6e452` |
| SYS2 CFTS_020 匯出 | — | 2026-06-16 | `421c8eef3f5cb01a` |
| SYS2 CFTS013 SYSRA | — | 修訂履歷 A→F（2025-09-03 → 2026-04-27） | `1036b2af9f655441` |

**CFTS_013 比 CFTS_020 晚三個月、且屬下一個 PI 家族。**

### 為何登記

CFTS_020 之 Display Hot 條文轉指 `{CFTS013-629}`／`{-633}`／`{-952}`
（DR-DM4）。若兩份不同期，該轉指所指之條號可能是 **Mar Release 當時**
之 CFTS_013 編號，而手上的是 **Jun Release** —— 見 A-DM39。

**是否構成版本錯配屬 Tier 2**，本層不逕判，量測後回報（28 包 §一之指示）。

### 影響

- **本批（`rvc-01`）不受影響**：其值域來源為 CFTS_020 ＋ DBC，
  未引 CFTS_013 之任一值（停止條件 73 實測 0）
- DR-DM4 之標的須連同本項一併裁定

---

## A-DM39 — CFTS_013 全文之條號為 7 位，DR-DM4 所求之 3 位條號不存在於其中  [HIGH]

28 包 A2 之條號錨定抽取（R-DM52：`Document ID`／條號本體，非全檔子字串）。
...
| 標題式（`{NNN}`） | **44 個相異** |
| 聯集 | **117 個**，**位數分布 100% 為 7 位** |
| 範圍 | `4819633` … `5423093` |

| 目標 | 條號錨定 | `CFTS013-NNN` 子字串 | 裸 `NNN` 子字串 |
|---|---|---|---|
| `629` | **查無** | 0 項 | **0 項** |
| `633` | **查無** | 0 項 | 1 項 |
| `952` | **查無** | 0 項 | **0 項** |

全檔之 `CFTS013-` 形態引用只有兩個：`CFTS013-1009`、`CFTS013-1015`。

### 判定

**手上這一份 CFTS_013 使用 7 位條號體系**（與 CFTS_020 同 —— 後者為
`{4820282}` 等），而 DR-DM4 所求之 `629`／`633`／`952` 為**另一套 3 位編號**。
兩者不是「有沒有這一條」的問題，是**兩套編號**。

一項佐證：本檔之條號集合含 **`4820282`** —— 那正是 CFTS_020 `1.11.2.2`
之 `{4820282}`。**兩份文件共用同一個 7 位編號空間**（Polarion 全域 id）。

### 處置

- **停止條件 71 觸發**：A3（DM10(b) 之對照量測）**停手**，A2 之結果照報
- **DR-DM4 維持 OPEN，惟其標的須重擬** —— 求「3 位條號」可能永遠查無；
  應改求 CFTS_020 所轉指之內容在**現行編號體系下**的條號
- **結構線索（供分析層裁定，本層未抽取其內容）**：本檔有四個 DCSD 專節 ——
  `1.5 DCSD Display Hot Error Behavior {4943077}`、
  `1.5.1 Activating the DCSD Display Hot Algorithm {4943080}`、
  `1.5.2 Standard HU and DCSD DIsplay Hot' … {4943082}`、
  `1.5.3 Multi-stage HU and DCSD Display Hot' … {4943095}`。
  **`1.5.1` 之標題逐字即 DR-DM4 所求之「Display Hot 演算法」。**
  A3 停手故未讀其內容
- **A4 之複核**：本檔內 `{CFTS013-XXX}` **0 項／0 次**、`CFTS013-967` **0 項／0 次**
  （口徑並列）。即 CFTS_020 內那 5 次佔位符**在 CFTS_013 本身找不到對應**
- **A5 之對照**：A-DM37 之兩句樣板殘句（`The TBM shall do this or that`、
  `The HU shall dipslay xxxxxxxx`）於 CFTS_013 全文 **0 項命中**；
  片語探針 `do this or that`／`dipslay`／`TBM shall` 亦皆 0。
  **故該三句為 SYSRA 撰寫殘渣，未升為規格本文殘渣**，A-DM37 之分類不變
  （`-1194` 之 `0` 命中 154 項為單字元比對之必然，不作數）

---

## A-DM40 — 本層之適用性判準漏了 `EE Architecture:All`，一度把 13 條適用條文判成不適用  [CLOSED]

> **本項於 29 輪已登記**（本層自行登記）。下放包 30 §四.1 指定「A-DM40 登記（HIGH）」——
> 其內容已在，**狀態維持 `[CLOSED]`**：缺陷已更正、回溯檢查完畢（21／28 兩輪未受影響）。
> 下放包所稱之 HIGH 為**其重要性**，非未結狀態；其重要性已由
> **R-G37（適用性判準須自該檔之值域導出 —— 全域）** 承載，30 輪立條。
> 若分析層要求改標未結，請明示。
...
- **已更正**，兩次結果並列於上繳 29 §二，**不刪首算**（R-G69：理由與數字分別查證）
