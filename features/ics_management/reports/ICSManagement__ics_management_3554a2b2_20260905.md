# lint036 報告：FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ICSManagement_20260830_Revise1.xlsx

- 來源：`features/ics_management/delivered/FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ICSManagement_20260830_Revise1.xlsx`（唯讀）
- 資料列數：31
...
| M | 空欄三態 | 0 | 0 | 每列每欄 | 已校準 |
| N | 行尾多餘句號 | 0 | 0 | 每行 | 已校準 |
| P | 訊號寫法不合 R-1 v2 | 101 | 28 | 每次命中 | 已校準（SWC 0708：195 —— proc 11／er 184，見上繳 09） |
...
**總計：行計 171**（列計不加總——同一列可觸發多項檢查）

## 明細

### P — 訊號寫法不合 R-1 v2（行計 101／列計 28）
...
| 38 | NR1L-ICS-029 | er | v3 記法殘留（R-G70(h)：`$` 包覆式已撤銷）'$CLIMATIC_PANEL.Radio_btn4$' | 5. CLIMATIC_PANEL.Radio_btn4 = 1 (Pressed) is sent, then $CLIMATIC_PANEL.Radio_b |
