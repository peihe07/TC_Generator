# 交付前體檢 v3 — b01 ~ b06 全 27 條（2026-08-29）
...
- ER 行總數 **126**
- **原始命中 122 行**（未涵蓋實詞 ≥ 1）
- 衍生載具詞 **15** 個（跨 ≥ 5 條 TC，自動導出）：
  `baseline`、`button`、`climatic`、`completed`、`enters`、`felt`、`mute`、`panel`、`radio`、`remains`、`reports`、`rotation`、`screen`、`shown`、`status`
- **殘餘候選 67 行** —— 此為人工複核之實際對象

**殘餘率 53%**（基線 53%，R-ICS34(c)：連續三包 > 60% 須重議門檻）。
...
| b03 | Screen off hardkey starts the three second timer | 1 | `pressed` |
| b03 | Screen off hardkey starts the three second timer | 2 | `pressed` |
...
| b04 | Knob 2 no change sent periodically | 4 | `increment` |
| b04 | Knob 2 signals acted on by the HU | 1 | `currently` |
| b04 | Knob 2 signals acted on by the HU | 2 | `detent` |
| b04 | Knob 2 signals acted on by the HU | 3 | `increment` |
| b04 | Knob 2 signals acted on by the HU | 4 | `differs` |
| b04 | Enter button pressed | 1 | `currently` |
| b04 | Enter button pressed | 4 | `differs` |
| b05 | Knob 2 rotated on a scrollable screen | 1 | `currently` |
| b05 | Knob 2 rotated on a scrollable screen | 2 | `detent` |
| b05 | Knob 2 rotated on a scrollable screen | 3 | `increment` |
| b05 | Knob 2 rotated on a scrollable screen | 4 | `differs` |
| b05 | Knob 2 rotated on a tuner source | 1 | `current` |
| b05 | Knob 2 rotated on a tuner source | 2 | `detent` |
| b05 | Knob 2 rotated on a tuner source | 3 | `increment` |
| b05 | Knob 2 rotated on a tuner source | 4 | `differs` |
