...
| DR-ICS8 | 本 feature 未綁定 DBC。【解法已裁 2026-08-29：R-ICS8 之 LID→CAN 路徑，LID v1_78 實測 9/9 命中（上繳包 01 §六-4）】b02 依 R-ICS8 改寫佔位後提請結案；DBC 查無之備援名保留本 DR 追蹤 | 訊號欄 | **CLOSED（2026-08-30，b16 回填完畢，R-ICS49(a)）**。結案經緯：Pei 裁台架匯流排（BHCAN2）→ Pei 裁寫法權威（R-17：以 DBC 為主）→ b14 判同一物（`TGW_DISP_STATSts` ＠ `BO_ 1500 TELEMATIC_DISPLAY2`）→ b15 以四支 dbc 交叉確認位元佈局與 `VAL_` 17／17 相同 → b16 綁定 `reference.dbc_bh2`（讀者首跑 11／11）並回填 12 處，`pending_census` 之本 DR 殘餘 **0**。**連帶記載二事（不影響結案，不得遺失）**：（a）同一物之判定**僅繫於 R-17(c) 項③**（項①② 對 CFTS020 結構性不可比，A-ICS94），已逐條寫入 `reasoning` 之可重驗項；（b）**A-ICS91 之台架可行性面維持 OPEN** —— TC 2／TC 4 之前提能否於 BHCAN2 上建立，與本 DR 是兩件事，不因本次結案而解決 |
...
| DR-ICS15 | 【新登 2026-08-29，上繳包 03 §四-3】`$TGW_DISP_STAT$`、`$RQ_DISP_INTS$`、`$DCSD_DISP_STAT$`、`$Telematic_Power$` 四訊號不在 LID 點名清單，b03 共 14 處佔位。已令 b04 依 R-ICS8 進 LID→CAN 驗證；驗證後仍查無者以本 DR 向上游要對應 | 006, 007, (011) 之訊號欄 | OPEN（**可結待回收**） |
...
| DR-ICS22 | 【新登 2026-08-30，upstream-17 §3、A-ICS115】**請提供 CFTS022 之 SYS2 匯出**。本 feature 現綁之 SYS2 為 `SYS2_CFTS_020_DISP_TCH_ICS_20260616_…`（CFTS020 範圍），而本線 31 條 TC 之 38 個相異錨中有 **7 個屬 CFTS022**（`4914956`／`57`／`58`／`74`／`75`／`76`／`4914993`，涉 Stuck button、VOLUME knob、Mute），**於現有 SYS2 中全數查無**。請問：（i）CFTS022 是否另有其 SYS2？（ii）若無，該七個物件對本 DUT 是否為在案需求？**本專案不將「不在 CFTS020 之 SYS2」讀為「不在案」**，七條 TC 維持不動，待貴方回覆 | 七條 TC 之在案性；以需求側為主鍵之覆蓋率完整性 | OPEN |
| DR-ICS23 | 【新登 2026-08-30，upstream-17 §4、§2-4、A-ICS122】**SYS2 之三項品質事實，請釋疑**：（i）**37 個列之 `Verification Criteria` 為逐字相同之樣板**（`Check $DCSD_DISP_STAT$ in the external tool. If this is 5 or 6, this should be replaced with last palusible value or ON.`），其內容為 `$DCSD_DISP_STAT$` 不合理值之處置，卻貼於 Power hardkey／Knob／Stuck button／Enter button 等**互不相干**之列 —— **是樣板貼錯，或確為該些列之驗證準則？**（ii）本專案已驗之 38 列中 **14 列貴方判 `Out of Scope`**（含 Power hardkey、Knob 1／2、Two buttons、Tbutton 等核心行為），且理由欄 14／14 全空（全表 123 個 OoS 中 116 個無理由） —— **該 `Out of Scope` 是對 HW 供應商、對 SW 測試、或批次標記之殘留？**此差別直接改變本專案覆蓋率之分子；（iii）`Category` 大小寫不一致（`Out of Scope` 116＋`Out of scope` 7；`Functional Requirement` 79＋`Functional requirement` 1） | 覆蓋率之分子；R-G41(c) 對照之有效性；14 列是否驗了不該驗之行為 | OPEN |
