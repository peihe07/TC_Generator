...
執行層依 R-G64 落 `sandbox/`（.xlsx）與 `generated/b01/`（.json），
...
```
R-ICS19（R-G52 指紋與 R-ICS17(d) 改題之交互）
...
> ⚠ **以下為作廢之重複草案**（A-ICS70：分析層違 R-G62，未取現場號即落條，
...
(d) 本條之立，係分析層於審 upstream-11 時違 R-G62（未取現場號即落條）
...
```
R-ICS49（b16 之採認；DR-ICS8 結案；回凍；三件上呈）

成因：upstream-16。19 項相符、1 項不符（`canon_refs` ＋1，經查不源於本包），
另一項下放所令而未執行者（gate 入簿，理由具名）。
E27／E30／E31／E32 全數未觸發。

(a) **回填採認**。b03 之 12 處佔位全數回填，佔位由 **18 處／14 條**
    降至 **6 處／6 條**（＝18−12，與預期完全相符）。
    訊號名、值名、裸符號三面同批改，無混用；`verify_verbatim` 31／31；
    錨行仍 65，錨變動 0。

(b) **A-ICS97 結案**（A-ICS108）：讀者已建，首跑 **11／11 全 MATCH**。
    十六包所綁之參考件皆未被改動 —— 這是一個**正面之量測結果**，
    非僅「建了一支腳本」。

(c) **gate 入簿之未執行：採認其不執行，並上呈 Pei**（A-ICS109）。
    實測：`docs/runtime/GATES.tsv` 中 `ics_management` 命中 0 列，
    且 `display`／`bed_lowering` 之同名讀者**亦均未登錄** ——
    **「讀者入簿」於本 repo 尚無先例**。
    且該簿之闘開工前即為紅，新增首列之效果無法於本包驗證。
    執行層不逕寫跨線全域簿，**合式且與 R-ICS44(d) 一致**。
    **本線之讀者已實跑並列為第六支自檢，其效力不繫於入簿。**

(d) **`(supporting observation)` 判為仍正確，採認，未改**。
    執行層之理由成立：該標記描述的是該 ER 項**相對於本條主要斷言之證據角色**，
    不是訊號之傳輸方向；八條之主要斷言皆為**畫面狀態**。
    **此判定推翻 b14 §3-4 之推測，而該推測由分析層登為 A-ICS98** ——
    登記推測本身正確，惟**登時應標明其為推測而非發現**；A-ICS98 結案。

(e) **`has_pending` 副作用之拿法入檔**（A-ICS110）：
    回填類作業之收尾清單**須含衍生旗標與 manifest 之同步**。
    本次為分析層之遺漏；執行層自行發現、修正並具名，**取之為先例**。

(f) **`VAL_ 1500` 之三值登為覆蓋缺口**（A-ICS111），**不生成**（凍結中）。
    解凍後須**先查 `4819466`／`4819475` 之適用性**，再定是否生成 ——
    不得以「`VAL_` 有而 TC 無」直接斷為缺口（那是以 DBC 推範圍，與 R-17 之射程不同）。

(g) `RQ_DISP_INTS` 之值書寫（A-ICS112）：**解凍後處理**，本包不改。

(h) **並行寫入之第二個跡象：上呈 Pei**（A-ICS113）。
    與 A-ICS80 合併 —— **二個獨立跡象指向同一結論**：
    本專案有一個本對話以外之寫入者。不再以「可能為並行實例」帶過。
    **分析層不推定其身分**；惟其存在影響圓籬 diff 與 sha 之可信度，須 Pei 確認。

(i) **回凍**。依 R-ICS48(a)，b16 完工自動回到 R-ICS43 之凍結；
    基準 `docs/reports/16_rulings_snapshot.md`（55 錨點）。

(j) **ICS 之現狀句（採執行層 §10-3）**，入凍結記錄：
    **21 條可交付、4 條卡於 DR-ICS9／DR-ICS2、6 條卡於 DR-ICS6／DR-ICS4。**
    三數相加為 31，且每一個都指向一個具名之 DR。
    其中 4 條（V1／V2／V3／B5）**無佔位**，工具一律全綠 ——
    **只有凍結記錄會提醒它們**。
（分析層即裁，2026-08-30；upstream-16 §2、§3、§6、§10、§11）
```
...
```
R-ICS50（第四次窗口式解凍：b17；解凍第一件①之執行）

Pei 於 2026-08-30 下「繼續」。
**分析層之讀法：繼續 ICS（非轉 PM）** —— 該字直接回應
「要繼續得你一個字；不然，PM 那 389 列」之前一分支。
**若誤讀，Pei 一字即可撤回；本包未生任何 TC、未改任何錨，撤回成本為零。**

(a) **第四次窗口式解凍，範圍限於下放包 17**（同 R-ICS44(a)／46(e)／48(a) 之形式）。
    b17 完工自動回凍；繼續須 Pei 另一字。

(b) **b17 之主體為 R-ICS43(b) 之解凍第一件①**：
    以 SYS2 333 列為主鍵之對照表（標各列是否已有 TC 覆蓋）。
    理由：該件被列為「結構性，最重」，且 upstream-13 §11-2 已具名
    「凍結記錄 §1～§5 仍以 CFTS020 為骨架，照 §5 四步開機會再次從 CFTS020 進場」。
    **不先做這件，下一個接手者會重蹈同一個盲區。**

(c) **並同步執行 R-G41(c)**：將 31 條之 ER 與 SYS2 `Verification Criteria`
    （欄 56）逐列對照。理由：二者共用同一張主鍵表，分二包做是重複建表。
    **只對照不改 ER** —— 不符者登記，不逕改。

(d) **A-ICS111（`VAL_ 1500` 三值）依 R-ICS49(f)**：
    **先查 `4819466`／`4819475` 之適用性**，再定是否為覆蓋缺口。
    **本包不生成任何 TC** —— 生成屬範圍擴張，需 Pei 另裁。

(e) **A-ICS112（`RQ_DISP_INTS` 值書寫）只量不改**。
    該訊號為連續量（factor 0.5、單位 `%`）而只有 `255 "SNA"` 一個枚舉值，
    R-17(a)「值之語意標籤逐字取自 `VAL_`」**對非枚舉值之適用未定**。
    本包只出建議，**不改交付欄** —— 其改寫涉 R-17 之射程，屬全案面。

(f) **A-ICS104／105 本包仍不排入**；G5 維持押後；
    A-ICS109（gate 入簿）、A-ICS113（並行寫入）維持待 Pei，本包不碰。

(g) **R-ICS43(b) 之②③（254→31 收斂軌跡、失敗史附錄）不入 b17**，
    理由：二者皆為回讀既有上繳包之編纂工作，與 ① 之量測性質不同；
    合包會使 b17 過大而難審。列為 b18 或分析層自製。
（Pei 裁「繼續」，2026-08-30；R-ICS43(b)；R-G41(c)；R-ICS49(f)(g)；A-ICS85／111／112）
```
...
```
R-ICS51（b17 之採認；E33／E34 之處置；E36 射程過窪之自承；回凍）

成因：upstream-17。17 項相符、3 項不符（#10 E33、#12 E34、#18 gate）。
E33／E34 皆觸發且執行層停下未作範圍判斷，**合式**。

(a) **主鍵表採認**。`docs/reports/17_sys2_master_table.tsv`（333×14）
    及其 `.md` 採為本線之**需求側主鍵**。四桶 260／42／31／0 與 A-ICS86 逐項相符。
    **惟須連帶記載其上下界**（執行層 §12-1-1）：
    人工判只作用於「FR ∧ 軸層適用 ∧ 未錨命中」之 42 列，
    其餘 276 列未逐列查 —— **295 為上限、38 為下限**。
    引用該表之覆蓋率時一律須附此句，不得單引一個百分比。

(b) **E33（七個錨全屬 CFTS022，而 repo 無 CFTS022 之 SYS2）**：
    登 A-ICS115，新登 **DR-ICS22** 向上游要 CFTS022 之 SYS2 匯出。
    **不得將「不在本 SYS2」讀為「不在案」** —— 本 feature 所綁之 SYS2 檔名
    自言為 `SYS2_CFTS_020_…`，其沉默對 CFTS022 不具證據力。
    七條 TC 本包不動、不退、不標異常。

(c) **E34 須二讀分離，不得合併作一個「21 列缺口」**：
    ① **成因一（20 列）** —— 樣板 Criteria 貼錯之嫌疑較大：
      75 個非空 Criteria 中 37 個為逐字相同之樣板（`$DCSD_DISP_STAT$` 不合理值之處置），
      貼於互不相干之列。**登記並併入 DR-ICS23 問上游**，本線不因此改 ER。
    ② **成因二（1 列）** —— 列 39 `4821022`，**實質驗證缺口**：
      Criteria 要求 screen ON 態檢 `$RQ_DISP_INTS$ != [0% Intensity]`，
      而我方 6 行全為 screen OFF 後之 `= 0 (0 %)`。**登 A-ICS116。**
      其補正涉 ER 改寫，**須另開窗口**；本包已回凍，不逕改。

(d) **E36 之射程過窪為分析層之誤**（A-ICS117）。
    下放包 17 將 A-ICS111 之查驗綁死於 `4819466`／`4819475`，
    而二者屬 **§1.5 PowerNet 分支**，而 **R-ICS2 v2(c) 早已載明「§1.5 之需求物件 100% 不適用」**
    —— 即我指定了**最不可能成立的一對例子**。
    成因：接受 upstream-16 §10-1 之點名而**未複驗其所屬章節是否已被排除**。
    **未量即斷第七例**（A-ICS56／64／70／81／83／102 同族）。
    執行層依令不擴張射程、只將 20 列擺上檯面，**合式**。

(e) **20 列之處置：二分**（A-ICS118）。
    ① 載 `Rear_Camera_Display` 之 **19 個**：其 Test Set `Camera Transition`
      已因 DR-ICS2／A-ICS5 轉 out-of-scope-pending，**已有既存成因**，不另處置。
    ② 載 `ON_BLANK`／`SNA` 之 **3 個**（`4819632`／`4819879`／`4821017`）：
      **適用 ∧ 在案 ∧ 未覆蓋 ∧ 無既存成因** —— **登為覆蓋缺口**。
      **其生成屬範圍擴張，上呈 Pei，分析層不逕定。**

(f) **A-ICS94 改述**（A-ICS119）：`4819632` 之規格原文並列書
    `[ON_BLANK / On_blanked_screen]`、`[DISP_REAR_CAMERA / Rear_Camera_Display]`，
    右半即 DBC `VAL_ 1500` 之逐字標籤。
    A-ICS94 曾判「此類語料項①②結構性不可比」—— **該判斷仍成立（項①②確實不可比）**，
    惟其附帶之悲觀語氣須修：**規格自己就寫了對照時，項③ 不是弱判定而是明文證據**。
    R-ICS49 及各條 `reasoning` 中「僅繫於項③」之註記**不改**（事實未變），
    惟日後遇並列寫法者得顯強其可信度。

(g) **`RQ_DISP_INTS` 採案④（分流）**，以 **R-17 補款** 追認現行寫法，**改動 0 行**。
    條文落於全案台帳。**本補款為分析層所擬，Pei 一字即可撤回**。

(h) **gate 六支之誤**（A-ICS120）：下放包 17 令「六支 gate」，
    而 `gate_all.py` 實為**五支**，`verify_reference_binding` 未入集（b16 §2-5 已具名）。
    分析層於下放時將「實跑六支」誤寫為「gate 集為六支」。**不追究執行層**。

(i) **`canon_refs` ＋1 為並行寫入之第三個跡象**（A-ICS121），
    並入 A-ICS80／A-ICS113 之合併上呈。

(j) **凍結記錄之改採執行層 §12-3 之二點**：
    ① §5 第 3 步改為先開主鍵表看 `tc_coverage = 無` 之 295 列；
    ② 新增 §7「覆蓋現況（以需求側為主鍵）」。
    **並須逐字寫入其警告：本表不涵蓋 CFTS022 側**（§3）——
    照本表開機者仍會漏掉那七個錨所在之文件族，
    **這是把一個盲區換成另一個，不是消滅盲區**。分析層自製，不另開包。

(k) **回凍**。依 R-ICS50(a)，b17 完工自動回凍；
    基準 `docs/reports/17_rulings_snapshot.md`（57 行）。
（分析層即裁，2026-08-30；upstream-17 §2、§3、§4、§5、§6、§7、§12）
```
...
```
R-ICS52（第五次窗口式解凍：b18；A-ICS116 之補正；三物件未裁）

Pei 於 2026-08-30 下「要開」。
**分析層之讀法**：該字回應之前一句為「A-ICS116 之補正涉 ER 改寫，
**要另開窗口**才能改」—— 故讀為**開第五次窗口**。

**未讀為「同意生成那三個物件」**：該件之問法為「要不要生成」，
「開」不是其答。**A-ICS118② 維持上呈中，b18 不生成。**
若 Pei 本意包含生成，一字即可追加；反之若分析層逕自生成，則無法回退。

(a) **第五次窗口式解凍，範圍限於下放包 18**（同前四次之形式）。
    b18 完工自動回凍；繼續須 Pei 另一字。

(b) **b18 之主體為 A-ICS116 之補正**：列 39 `4821022` 之 Criteria 要求
    screen ON 態檢 `$RQ_DISP_INTS$ != [0% Intensity]`，而我方 6 行全為
    screen OFF 後之 `= 0 (0 %)`。**這是本線首次依需求側之驗證準則補一個檢查點。**

(c) **補正之形式限於三選一，且須先量再選**：
    ① 於現有某條之 screen ON 段落補一行 ER（若該條已有 screen ON 態之步驟）；
    ② 於現有某條補一步驟＋一行 ER（若無該態之步驟）；
    ③ **新生一條 TC**（若 ①② 皆會破壞現有條之單一驗證目標，IN §2）。
    **③ 屬範圍擴張，若量得須走 ③，停下回報（E37），不得逕生。**
    理由：補一個檢查點與新增一條 TC 之性質不同，後者改變批次數與覆蓋分子。

(d) **補正須同時守二件**：
    ① `$RADIO_B3.RQ_DISP_INTS$` 之值寫法依 **R-17 補款（案④ 分流）**：
      連續量走物理量＋單位，非 `VAL_` 標籤；
    ② 新增之 ER 須可觀察、無 modal（IN §6），且不得使該條出現二個驗證目標。

(e) **並執行 R-ICS43(b) 之②③**（254→31 收斂軌跡、失敗史附錄）。
    理由：b17 已把①做完，②③ 為回讀既有上繳包之編纂，
    與 (b) 之小幅補正合包規模適中；且**二者皆不依上游**。

(f) **A-ICS118② 之三物件（`4819632`／`4819879`／`4821017`）b18 不生成**，
    維持登為覆蓋缺口、維持上呈。

(g) **凍結記錄之修正（R-ICS51(j)）由分析層於本輪自製**，不入 b18 作業。

(h) A-ICS115（CFTS022 無 SYS2）、A-ICS121（並行寫入）、A-ICS122（SYS2 品質）
    皆待上游或 Pei，**b18 不碰**；G5 維持押後；
    A-ICS109（gate 入簿）維持待 Pei。
（Pei 裁「要開」，2026-08-30；A-ICS116；R-ICS43(b)；R-ICS51(c)②(e)；R-17 補款）
```
...
```
R-ICS53（b18 之採認；E39／E40 之處置；A-ICS116 之定性；回凍）

成因：upstream-18。E39／E40 同時觸發，執行層停在 A-1、
**不自行判定 E39 之本意亦不自行定性**，合式。

(a) **E39 放行，且其觸發為分析層之瑕疵**（A-ICS123）。
    下放包 18 之 A-1 步驟 1 令「全部出現」，而預期數字 #3 寫「6 行／3 條」
    —— **二者口徑不同**。實測：全部出現 **14 行／8 條**，
    多出之 8 行全在 `test_item`（規格逐字引，裸符號），
    依 **R-17(b) 不得改**。**可改寫面仍為 6 行／3 條，b17 之數在其口徑下正確。**
    **拿法：凡令「全部出現」者，須同時指明是否含 `test_item`。**

(b) **E40 之定性：補檢查點，非補缺口。非範圍擴張。**（A-ICS124）
    理由三項，缺一不可：
    ① `4821022` 之行為（HU Screen ON 態下發 `$TGW_DISP_STAT$ = [DISP_NORMAL]`
      且 `$RQ_DISP_INTS$ != [0%]`）**已被現有之「Power hardkey pressed while HU screen on」
      部分驗證** —— 其步驟 1 已於 screen ON 態讀 `TGW_DISP_STATSts`。
    ② 所缺者為該行為之**另一半**（亮度非零），
      而 **IN §6 明定「Final ER 須涵蓋完整之 Test Item outcome，部分即不完整」**
      —— 故補之是**修一個不完整之 ER**，非新增驗證對象。
    ③ 本補正**不新生 TC、不使批次數改變**。
    **故與 A-ICS118②（三物件之生成）性質不同，不隨之上呈。**

(c) **惟補正須連帶加錨，否則違 IN §8.2.1**。
    若只補 ER 而不加錨，則該條會驗證一個它不追溯之需求 ——
    正是 IN §8.2.1 所禁之「擴張至 sibling Req」。
    **故 b19 須先量後加**：
    ① `4819637`（§1.8 側，與 `4821022` 幾乎逐字相同，upstream-18 §10-1-1）
      之適用性、SYS2 列、Criteria；
    ② `4821022`（§1.18 側）之適用性；
    ③ 依 **R-ICS40(a)** 定單錨或雙錨 —— **二側皆判適用方得並列**。
    **§1.8 為變體正解（R-ICS37(a)），若 `4819637` 適用則它是主錨。**

(d) **不推廣至 74 列**（採 upstream-18 §10-2）。
    實情：75 個非空 Criteria 中 37 個為樣板（A-ICS122(i)）；
    而已覆蓋列中 Criteria 非空者僅 22，其中 20 為樣板 ——
    **真正可對照者現況只有 2 列**。
    **推廣之前提為 DR-ICS23 先有回覆** ——
    若那 37 個樣板是貼錯，其背後可能藏著 37 個真正之 Criteria 被覆蓋掉，
    屆時分母與性質皆變。**在不知道之前推廣，就是建在未量測之前提上**。

(e) **I-1：`4819578` 與已覆蓋之 `4821693` 相似度 0.97，行為已涵蓋而未加錨**
    （A-ICS125）—— 第 12 輪加錨時未納入。
    **列入 b19 之加錨清單**，依 R-ICS40(b) 之行為等值覆蓋判準重驗後加。

(f) **三個「未覆蓋」數不可相加亦不可互代**（A-ICS126）：
    223（CFTS020 主鍵，只計錨命中）、295（SYS2 主鍵，計錨命中＋行為等值）、
    216（既非錨命中亦非行為等值）。
    **凡引用任一數須同時載其主鍵與判準**；
    並須附 R-ICS51(a) 之上下界句。

(g) **I-2（4）＋I-3（6）＋F（1）＝11 條本就不是本 DUT 之可驗證行為**
    （A-ICS127）。**日後報缺口數時須把它們標出來**，
    混在未覆蓋內會使缺口看起來比實際大。C 類（29）同理 —— 已有既存成因。

(h) **用字依檔案**：A-ICS107／114 之用字為「非唯一**錯**點」（指 edit 比對字串），
    非「錨點」。下放包 18 之用字有誤，**以檔案為準**。

(i) **R-G41(a)③ 之來源須補記 A-ICS57／A-ICS77**（即失敗史第 2 類
    「布林旗標合桶」亦已落為全案條文）。分析層於全案台帳補記。

(j) **並行寫入之第四跡象，且規模跡升**（A-ICS129）：
    掃描檔數 **2524 → 2574（＋50）**，而本包新增二份報告。
    前三個跡象皆為 ＋1 級，**本次為 ＋50** —— 不再是偶發而是持續且大量。
    **合併 A-ICS80／113／121 上呈，列為優先確認件** ——
    圓籬 diff、sha、gate 差值之可信度皆繫於此。

(k) **失敗史 §4-2「共同形狀」提請升 R-G**（A-ICS130）：
    七類中六類同形 —— **把「我沒看見」當成「不存在」，並當作已量測之事實往下傳**。
    其輸出形態一致：**機檢全綠、卻建在未量測前提上之結論，
    且沿 probe → 上繳包 → 下放包傳播**。
    此綜合比任何單一例都有用，且對 PM／SU／DD 一體適用。
    正式提案待 Pei 指示後落於 `docs/fw036/`。

(l) **回凍**。依 R-ICS52(a)，b18 完工自動回凍；
    基準 `docs/reports/18_rulings_snapshot.md`（59 行）。
    **A-ICS116 之補正未執行，待 b19**。
（分析層即裁，2026-08-30；upstream-18 §2、§3、§4、§5、§9、§10；IN §6、§8.2.1）
```
...
```
R-ICS54（Pei 裁：寫回；PENDING 維持；硬性停止條件）

Pei 於 2026-08-30 逐字：**「寫回吧 不要再繼續 pending 就 pending」**。

(a) **寫回授權**。31 條 TC 依現狀寫回 036 工作簿（母本：R-G1 之
    `forms/…_SWQT_20260817_ext.xlsx`）。**本條為寫回授權，非出貨授權**：
    IN §8.4.3「含 PENDING 之工作簿不得出貨」不變，出貨仍繫於 DR 回覆或 Pei 降轉。

(b) **PENDING 維持 PENDING**。6 處佔位（DR-ICS6 × 5、DR-ICS4 × 1）以
    `PENDING: DR-{n} <缺件名>` 原樣寫入，**不降轉 NA、不留白、不臆填**。
    V1／V2／V3／B5 四條（無佔位、ER 判不出對錯）**依現狀寫入**，
    其風險於交付清單與凍結記錄標明，**不在工作簿內加註**（工作簿欄位 English only，備註不入交付欄）。

(c) **硬性停止條件**：寫回完成後，**ICS 線非上游 DR 回覆到達不開窗**。
    分析層**不得**以「發現新事項」「值得量」「一包可完」為由提請開窗；
    此類事由一律登凍結記錄之掛帳，待解凍時併處理。
    **本款約束分析層，非約束 Pei。**
    立法理由：b13 凍結後開了五次窗（b14～b18），每次各有理由，加起來凍結從未生效；
    而 b17／b18 對交付狀態之貢獻為零。分析層三次宣稱「做完這包就可以停」（b13／b16／b18），三次皆不實。

(d) **掛帳入凍結記錄，不再執行**：A-ICS116（列 39 補檢查點）、A-ICS125（`4819578` 加錨）、
    A-ICS118②（三物件）、A-ICS109（gate 入簿）、A-ICS130（R-G 提案）、
    R-ICS43(b)②③ 之編纂已完，其餘全部 OPEN 項。
    **解凍條件唯一：上游 DR 回覆。**

(e) **b19 為寫回包，非量測包**。依 FO §6「dry-run reviewed → commit → --write → tag」之序；
    R-G3（不得以 openpyxl 開啟寫入，用 `xlsx_surgical` splice）；
    xlsx 只得於 `sandbox/<tag>/` 修改，定稿複製入 `delivered/` 附 MANIFEST.tsv。
    **`--write` 與 tag 屬 Pei**（git 操作者）；執行層只做到 dry-run 與 sandbox 產出。

(f) **並行寫入（A-ICS80／113／121／129）不阻斷寫回**，但寫回包之圓籬 diff 與 sha 須具名其可信度受此影響。
（Pei 裁「寫回吧 不要再繼續 pending 就 pending」，2026-08-30；IN §8.4.3；FO §6；R-G1／R-G3）
```
...
```
R-ICS56（R-ICS55(c) 之執行：tc_id 前綴與排序已定案）

本條為 **R-ICS55(c) 所令之普查之結果**，非另一版本。
R-ICS55 非本對話所寫（並行寫入，同 A-ICS70 之形），但其内容與本條不衝突：
它令「先普查再編號」，本條就是那份普查。

成因：upstream-19a §2-1。`generated/` 之 31 條無 `tc_id`，
而 `feature.yaml` 已宣告 `tc_id_format: "NR1L-ICS-{n:03d}"` 及欄位對映 ——
格式在案、值從未生成。執行層依令停下不自行編號，合式。

**本條之判斷基礎為實測，非設計**（全案交付工作簿欄 F 普查，五本 783 列）：
...
(a) **前綴取 `NR1L`。** 依據：`NR1L` 占 **609／783 列、3／5 本**；
    且最新一本（popup，08-28）與最舊一本（sxm，08-13）**皆為 `NR1L`**
    —— 非新舊之別，而是 `newR1L`（bed_lowering）與 `TC-`（display）為離群。
    又其與 `feature.yaml` 既宣告之 `tc_id_format` 一致，**無須改宣告**。
    完整式：**`NR1L-ICS-{n:03d}`**（abbr 取 `ICS`，同宣告）。

(b) **`n` 依工作簿列序，自 001 連續至 031。**
    依據：**五本全數**為 001..N 連續且與列序逐一一致，無例外；
    合 IN §10.3「同一 `{project}-{abbr}` 群內單調遞增」。
    **列序即 19a dry-run 之投影序，即 `generated/` 之現行順序**
    （生成序：b01→b07，批內依 json 陣列序）。
    **不重排**：不依 Test Set 分組、不依 req_id、不依錨號 ——
    重排需要一個實測支持之理由，而普查中無一本重排。

(c) **寫入位置**：`generated/` 之各批 json 逐條增 `tc_id` 欄；
    **不改任何其他欄**。`manifest.json` 同步。
    寫入後須驗：31 個相異、無重號、無缺號、與列序一致。

(d) **`newR1L` 不採，但亦不回改 bed_lowering**。
    `feature.yaml` 中「全案另有 `newR1L` 一式」之註記為**實況陳述**，非二選一之異議；
    其不一致屬全案面（A-ICS132），**不在 ICS 線處理**（R-ICS54(c)）。

(e) **本條不構成開窗**：tc_id 為 b19 自身之未完項（E42），
    非新開之量測或生成，故不觸 R-ICS54(c) 之禁。
    **實測之完成並不使其延伸** —— 普查所見之全案不一致一律只登不行。

(f) **R-ICS55(c)① 之 E45 條件字面上成立，惟已由 Pei 先行排除**。
    E45 令「若二式於已交付件中皆有，停下上呈 Pei」——
    實測二式確實皆有（`NR1L` 三本、`newR1L` 一本、另有 `TC-` 一本），
    但 **Pei 於 2026-08-30 已逐字令「定案並把 ID 寫進 generated/ 的 json」**
    —— 即已將該裁定下放給分析層。**回頭上呈是退回一個已經下放之事。**
    故依實測定案，並於此具名 E45 之條件曾成立。

(g) **R-ICS55(c)① 之先驗經實測推翻**：其載「memory 載 sw_update 用 `newR1L-SU-`，
    故 `newR1L` 較可能為全案式」。實測：`newR1L` 於已交付件中僅 bed_lowering 一本（151 列），
    而 `NR1L` 三本 609 列。**該先驗不成立** —— 而 R-ICS55(c) 自己已寫明
    「這是先驗不是量測，不得據以編號」，其自限正確。

(h) **R-ICS55(c)② 之備位規則不啟用**：其備位為「依 Test Set 分組、組內依生成序」，
    只於「已交付件無一致規則」時適用。**實測五本規則完全一致**（皆依列序、無一本分組重排），
    故適用主規則，備位不啟。
（分析層即裁，2026-08-30；Pei 令「定案並把 ID 寫進 generated/ 的 json」；
  實測：交付工作簿欄 F 普查五本 783 列；IN §10.3；upstream-19a §2-1）
```
...
```
R-ICS55（19a 之採認；TC ID 之裁定路徑；19b 授權）

成因：upstream-19a。11 項相符、E42 觸發（`generated/` 31 條無 `tc_id`）。
執行層依令不自行編號，且正確指出「這是 b19 自身之未完項，非新開量測」。

(a) **dry-run 採認**。母本 sha 與 FORMS.md 相符；R 欄 x14 DV 經 `surgical_save` 保留
    （48 成員一致、DV 計數不變）；投影 34 欄對映、錨行 65、排列違規 0、PENDING 6 處逐字。
    **除 TC ID 外，工作簿之結構與內容皆已就緒。**

(b) **E42 為十八包以來共同之缺，分析層同負其責**（A-ICS131）：
    `feature.yaml` 自第 01 輪即宣告 `tc_id_format: "NR1L-ICS-{n:03d}"`，
    而生成器從未賦值，十八包內無一包量過「tc_id 欄是否有值」。
    **格式在案、值從未生成** —— 屬失敗史「共同形狀」：宣告有而未驗其存。

(c) **TC ID 之二項判定，不由分析層以偏好裁，由量測定**（記憶之格式裁決原則：
    須先普查 Pei 既有已交付工作簿之實際用法，不得自設格式再令語料從之）：
    ① **前綴**（`NR1L` vs `newR1L`）：19b 首作業為**普查全案已交付之工作簿**
      （`features/*/delivered/`、SWC 0708 參考本、及其他 feature 之 `feature.yaml`），
      以 memory 所載之前綴普查法（欄 F、`Test Case ID` 表頭、`^.+-\d{3,4}$`）取實際用法。
      **取多數之實例；若二式在已交付件中皆有，停下上呈 Pei（E45）**。
      分析層之先驗：memory 載 sw_update 用 `newR1L-SU-`，故 `newR1L` 較可能為全案式；
      **但這是先驗不是量測，不得據以編號**。
    ② **`n` 之序**：同次普查已交付工作簿之**列序規則**（依 Test Set 分組？依需求序？依生成序？），
      取實際用法。**若已交付件無一致規則**，則採 IN §4.1.3 之可導航性：
      **列序依 framework.md Layer 2 之 Test Set 順序分組，組內依生成序（b01→b07）；
      ID 依列序遞增**（IN §10.3 單調遞增）。此為備位規則，普查優先。

(d) **`tc_id` 之寫入位置**：寫入 `generated/` 各批 json 與 `manifest.json`（新增鍵），
    **不改其他任何鍵**；寫入後 `verify_verbatim` 須仍 31／31。
    `tc_id_testrail`（欄 E）留空（TestRail 新舊 ID 對照屬交付後，memory 之實務）。

(e) **`sandbox/ics_management_00.xlsx`**（母本之未改動複本，第 01 輪所產）：
    **不動、不用**。作業 C 產於 `sandbox/v1/`（FO 之 `sandbox/<tag>/`），與其並存。
    其清理屬 Pei（git 操作），登掛帳。下放包「無既有交付件」之敘述不精確（A-ICS133）。

(f) **19b 授權**：① 普查 → ② 編號寫入 json → ③ 作業 C（sandbox 產出）。
    **此為 b19 自身之未完項，非新開窗，不違 R-ICS54(c)**。
    ① 若觸發 E45，②③ 不做。

(g) **upstream-19a §5-1 第 3 項之觀測來源待說明**（A-ICS135）：
    「分析層五簿之數輪變更於 `git status` 中已不顯示為未提交」——
    本包自報 git 0 次，則此一觀測由何而來？19b 須具名其來源（他實例之報告？檔案推斷？）。
    **不推定為違規，只問來源**。

(h) **並行寫入之規模已可影響任何全域計數**（A-ICS135）：
    `canon_refs` 本 session 445 → 486，掃描檔數 2523 → 2591，且五簿變更已被他方提交。
    **採執行層建議：`--write` 與 tag 之間若相隔任何時間，tag 前須重跑 `verify_reference_binding`。**
...
```
（Pei 直接裁定，2026-08-30；FO R-G64、R-G1；FORMS.md；
...
`FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ICSManagement_20260830.xlsx` **被 `FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_ICSManagement_20260830_Revise1.xlsx`（sha12 `3554a2b28642`）取代**，取代理由 **R-G72(g)**（Revise-M 機械批）。

- **母本原封**（R-G72(a)／R-TM13）—— 未就地改，新檔以
  `backend.xlsx_surgical.surgical_save` 寫出，`verify_structure` 通過。
- 內容差異＝**18 處（14 列／14 格）** 之機械轉換：R-1 v3 記法 → v4（R-G70(c)(d)）。
