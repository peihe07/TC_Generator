"""W-122(2)（68 包 §5）—— batch19。

**依 R-VS64，本批不寫死條數** —— 池扣除 held_out 後有幾條即取幾條。
...
SEL = ["SWE1-VC-ThreeStagesVentedSeatsManagement-070",
       "SWE1-VC-ThreeStagesVentedSeatsManagement-076",
       "SWE1-VC-ThreeStagesVentedSeatsManagement-077",
       "SWE1-VC-ThreeStagesVentedSeatsManagement-079"]

HELD = [
    ("SWE1-VC-OneStageHeatedSeat-040", "適用性前言（A-VS141 之第六式）。**W-121 之新判準"
     "確判其為前言**，惟該判準因升級條件命中而本輪未套用（上繳 38 §2.1）——"
     "故其分級仍為 W0，**維持 held_out 而未移入 W2**"),
...
def build() -> dict:
    ...
    return {
        "batch": "batch19", "feature": "vehicle_setting", "test_group": "Vehicle Setting",
        "generated_round": 43, "handoff": "docs/handoff/68_review_round42.md",
        ...
        "revision": "W-122(2)（43 輪）：batch19 首版",
...
if __name__ == "__main__":
    d = build()
    (FEAT / "generated/batch19.json").write_text(
        json.dumps(d, ensure_ascii=False, indent=1) + "\n", encoding="utf-8")
    print(f"batch19：{len(d['tcs'])} 條")
    for tc in d["tcs"]:
