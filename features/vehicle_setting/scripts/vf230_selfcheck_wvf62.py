...
# ── 逐字 pattern（R-VF69）────────────────────────────────────────────
# **已知集合，非全集**（R-VF71 三）。pattern 攔已知之表述，
...
def check(tcs: list[dict]) -> list[str]:
    ...
    for t in tcs:
        ...
        # R-VF70 一：tc_title 採純句式，不用冒號
        if ":" in t["tc_title"]:
            e.append(f'{tid}: tc_title 含 ":"（R-VF70 一：純句式，不用冒號）')
        # R-VF70 二：括號內之別名不入 tc_title
        if "(" in t["tc_title"] or ")" in t["tc_title"]:
            e.append(f'{tid}: tc_title 含括號（R-VF70 二：別名不入標題）')
