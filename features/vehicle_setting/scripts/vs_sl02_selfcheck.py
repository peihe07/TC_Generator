...
ROOT = v1.ROOT
REPORTS = {
    "VF230": (v1.VF230, "features/vehicle_setting/reports/vf230_settings_dryrun_v2.tsv", 457),
    "BedLowering": (v1.BL, "features/bed_lowering/reports/bl_settings_dryrun_v2.tsv", 151),
    "VehicleCategory": (v1.VC, "features/vehicle_category/reports/vc_settings_dryrun_v2.tsv", 126),
}
...
def compose(lk: Lookup, g, r: int, alias_by_tc: dict) -> str:
    ...
    res = lk.query(alias.get("hmi_name") or name, item, v1.DR_NO) if status == "exact" else {
        "name": name, "path": None, "control": None, "proxi": [], "flags": []}
...
def main() -> int:
    ...
    alias_by_tc = {a["tc_name"]: a for a in v1.build_alias(lk, names)}
    for r in (150, 151, 14):
        print(f"\n--- row {r}  D={g(r, 4)} ---")
