...
## DR-46（新，**Urgency High** —— VF230 之目標車型未載，致 DBC 取本無據；W-VF81 開立）

> **開號依據**：全庫最大已用 DR 號為 **DR-45**（R-VF10）。**登記，未送出。**

**其起因**：`forms/` 目錄內之二本 DBC 與現行 `inputs/` 之二本**非版次關係，而為車型變體**
（R-VF120 之裁定，其依據為分析層之逐訊號 diff）：

```
版號    現行 R5      →  forms R1        （**相反**）
BH 側   現行 BHCAN   →  forms BHCAN2    （**不同匯流排名**）
訊號    僅 R5 有 122 個（大宗為 BMS_HV_*／BPCM_HV_*／HVBatCellVoltage_* 等高壓電池族）
        僅 R1 有   1 個（EngRPM_Tach，引擎轉速）
```

**即：其差異為 BEV ↔ 含引擎之變體，非時間上之演進。**

**所詢**：**VF230 之目標車型為何？** 其應以哪一本 DBC 為值域來源？

1. 現行 `inputs/`（`R5_FDCAN8` ＋ `R4_BHCAN`，含高壓電池族）？
2. `forms/`（`R1_FDCAN8` ＋ `R1_BHCAN2`，含引擎轉速）？
3. 抑或二者分屬不同車型變體，VF230 須依變體分別出題？

**其現行阻塞為零** —— 本層實測：VF230 所用之 **157 個訊號 token
於二本 DBC 皆有且值域完全一致**；§2 之 3 個值域相異訊號皆不在其內。
**故 440 條不受影響，拆題得逕行**（R-VF120 已裁）。

**其未來阻塞為實質**：若日後之條文觸及該 122 個訊號之任一，
**取本之選擇即決定其值域是否存在**。
...
## DR 草稿（**未取號** —— 依 `down/20260901_VS-SL-01_review.md` §2.2：未送出前不佔號；DR 號依 Pei 送出時之台帳取）

> **本節取代 2026-09-01 落檔之 `DR-49`。** 原 DR-49 以 54 名為標的，
> 經 Tier 2 審閱判為**過寬** —— 54 名中 44 名有可查證之候選，依 R-13 應由 Pei 認別名，
> 不是向上游問。標的縮為下列 **9 名**（Tier 2 之 `tier2_proposal = DR` 者）。
> **`DR-49` 之號本輪釋出，不佔用。**
...
|---|---|
| `E-Save` | 兩表皆無候選 |
| `Charge Power Level` | 兩表皆無候選 |
| `Max Power Level` | 兩表皆無候選 |
| `Enhanced Display Synchronization` | 兩表皆無候選 |
| `Unit Energy` | 兩表皆無候選（HMI Units 內未見 Energy） |
| `Warnings for Low Fuel Inverter Shutdown - Visual Warning` | 兩表皆無候選 |
| `Warnings for Low Fuel Inverter Shutdown - Audible Warning` | 兩表皆無候選 |
| `Rear Guidance Lighting with Approach` | HMI `15. Lights` 無同名；FIP No.272／275 為 Cargo Lights／Light Status，非此 |
| `Time and Date Settings` | HMI `7. Clock` 為分類（r355–r375），無單一同名項；FIP 無 —— 疑為分類級引用，待認 |

### 所求

上列 9 名於 HMI Settings List／FIP 总控表之正式對應名，或確認其不存在。

### 併同送出之二問（VehicleCategory，見 `features/vehicle_category/reports/vc_settings_dryrun_v2.md`）

1. `Vehicle Category feature` 之「具備與否」是否有 PROXI 表述 —— `Vehicle_Category`
   為列舉值（M1／M2／N1／N2），非存在性參數（16 列）。
2. `Camera App` 之指稱 —— 執行層已讀該 2 列需求原文，判為 `Cam App`（見該報告 §B），
   **惟总控表 No.196 `Cam App` 為 `Always false`**，與「Cameras 會出現為一個 tab」相衝，故仍列問。

### 本層之處置

`proxi_proposed` 之 (3) 分支填 `PENDING: DR（未取號…）`，**不猜值**。dry-run 未寫回任何工作簿。

### 其餘 44 名（`manual`）與 1 名（`drop`）
