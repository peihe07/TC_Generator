...
## 3. 畫面層待補之 TC 逐條（**母體 ＝ `screen_pending = yes`**）

**合計 26 條**（43 輪 D-4 起以 `screen_pending` 篩出，不再以 `PENDING` 字面掃描）。

### 兩個標記之分列（68 包 §2）

| 標記 | 條數 | 其義 | 覆後之處置 |
|---|---:|---|---|
| `dr_dependent` | **69** | 該 TC 之**可寫性**繫於某 DR | 逐條複檢**訊號名與值** |
| `screen_pending` | **26** | 該 TC 之**畫面層**待補 | 逐條補**畫面層 ER**，訊號層不動 |

**二者範圍差 43 條 —— 交付時不得以「有 N 條依賴 DR」一句涵蓋。**
