# CROSSLINE — 跨線拘束項（R-VF38）

**兩線共用。`<Feature>, 接手` 時須先讀本檔，位於 `PLAYBOOK.md` §6 與 `INDEX.md` 之前。**

本檔**只收跨線拘束項**，不複述條文全文。**短是其有效性之前提** ——
超過一頁即失去被讀之可能。已解除者立即移入檔末「已解除」區，不刪。

**「未受保護」之意義**（R-VF38 三）：條文仍有拘束力，惟其被無聲違反之風險
未被消除。**非「未生效」。** 須於每次上繳具名，直至補齊機械檢查。

---

## 跨線風險：`writeback_036.py` 不得用於 VF230（R-VF112；A-VF30）

| 標的 | 風險 | 處置 |
|---|---|---|
| `scripts/writeback_036.py` | 其 `BOOK` 為**寫死之絕對路徑且指向 CFTS044**，且**不讀 `feature.yaml`**。用於 VF230 會寫進 **Part 1 之工作簿**，而其 B 欄 1–237 為 `R-VF83` 之凍結欄，且 `R-VF109` 明文排除 Part 1 | **不得用於 VF230。** VF230 之寫回用 `scripts/vf230_wvf77_dryrun.py`（路徑取自 `feature.yaml`，恆不寫檔）。**該腳本屬 Part 1，本線只登記不修**（R-VF112） |

**其危險之性質為潛伏**：VF230 線至今未寫回（R-VF26 凍結），該腳本從未被本線執行；
**凍結使其危險不顯，解凍之首輪 dry-run 為其唯一之暴露時機。**

---

## 保留位登錄（R-VF104；**開號／命名前須查本表**）

**凡被任一錨點逐字引用之編號或字串，不得被實際佔用。** 其佔用會使該錨點
**靜默失效** —— 「必命中」變恆不命中，而不命中不會回報（A-VF4／A-VF27 之同族）。

### 甲、靜默失效型（佔用後無人會知道）

| 保留者 | 引用它的錨點 | 佔用之後果 |
|---|---|---|
| **`R-VF99`** | `grade_overrides.py:check_rvf48()` 必命中 —— 「人為於 `framework.md` 插入『依 R-VF99』→ 須失敗」 | 其成立前提為 `R-VF99` **不在 `defined`**（取自 `^### R-V[SF]\d+ ——`）。若以 `### R-VF99 ——` 落檔任一條，該錨點之必命中變**恆不命中**，而不命中不回報 |

**普查（W-VF74，2026-08-24）**：以「錨點語境 ± 160 字內含『須失敗／不存在／
無定義／不得』」掃全庫腳本，得 15 個候選編號；**逐一驗其定義狀態，
僅 `R-VF99` 未定義** —— 其餘 14 個（`R-VF9`／`R-VF17`／`A-VS129`–`136` 等）
皆為「**須存在**」型之引用（如「人為插入重複 `### R-VF17 ——`」要求 R-VF17 已定義），
**非保留位**。故現行保留位**確為一項**。

### 乙、可見失效型（佔用後會被回報，故不列為保留位而列此備查）

自檢之**破壞式**所用之假值 —— 其若成真，破壞後檢查不報錯，
但腳本之「無法失效之項」清單**會回報**，故非靜默：

`TELEMATIC_VEHICLE_SETUP.PLGAlert_Req = 9 (Bogus)`（項 3／4）／
`SWE1-VC-MutantFamily-001`／`-002`（項 13／14）／`P9`（項 7）／
`W9`（項 10）／`VF230_V1_PDT27_VF_9999`（項 8）／
`99_vf230_test.md`（`grade_overrides` 檢查一）

**二型之別即「失效是否會被回報」** —— 甲型須登錄並永久保留，
乙型由可失效測試自行守住。

### 新增錨點時之義務

**錨點若引用某編號或字串而其成立前提為「該者不存在」，
須同時登錄於甲表**，否則日後必被佔用（R-VF104）。

---

## 受保護（有能失敗之檢查）

| 條文 | 受拘束之對象 | 一句話 | 機械檢查 | 開立 |
|---|---|---|---|---|
| **R-VF17** | `docs/reports/writability.tsv`／`generatable.tsv` 之 4 leaf（見 `data/grade_overrides.tsv`） | 該 4 leaf 之分級為 W0，四欄皆有其應有之值 | `python3 scripts/grade_overrides.py --check` | 2026-08-23 |
| **R-VF20 / R-VF32 / R-VF39** | 同上 | **`writability_driver.py --write` 之尾段已自動呼叫覆寫層**（R-VF39 授權）—— 重跑 driver 會使該 4 leaf **自動回復 W0**，此為裁定之意圖 | `python3 scripts/writability_driver.py --write` 後 `--check` 應 exit 0 | 2026-08-23 |
| **R-VF9 / R-VF44** | `framework.md` 之 VF230 節、`feature.yaml` 之 `profiles.vf230.test_group` | Test Group 兩本同值 `Vehicle Setting`；Layer 2 之 9 個 Test Set 名**已凍結**，變更須經 Pei | `python3 scripts/vf230_wvf38_framework.py`（錨點：合計 627／9 set／二名已消失／`Power Unit` 歸屬） | 2026-08-23 |
| **R-VF16** | `data/vf230_leaves.tsv` | VF230 之 leaf 母體為 627，8 列標 `disagree=1` | `python3 scripts/vf230_layer2.py`（內含 `assert tot_leaf == 627`） | 2026-08-23 |
| **R-VF23** | `docs/handoff/`、`docs/upstream/` 之檔名 | VF230 線之檔須以 `V` 起首；`docs/upstream/vf230/` 不得存在 | `grade_overrides.py --check`（檢查一，R-VF40） | 2026-08-23 |
| **R-VF10** | `RULINGS.md`／`ANOMALIES.md` 之編號 | 同一編號不得有兩個定義 | `grade_overrides.py --check`（檢查二，R-VF40）—— **⚠ 現行即失敗：`R-VS59`–`R-VS66` 各有兩義，見 A-VF10。修法為 W-VF13 之改編，未執行** | 2026-08-23 |

## 未受保護（無機械檢查 —— 每次上繳須具名）

| 條文 | 受拘束之對象 | 一句話 | 缺何檢查 | 開立 |
|---|---|---|---|---|
| **R-VF18** | 全庫之數字陳述 | 母體變更時只改現行有效之陳述，歷史不追改 | 無檢查（`vf230_wvf20_619.py` 只涵蓋 `619` 一數） | 2026-08-23 |
| **R-VF21 / R-VF28** | 任何新寫之判準 | 判準須附必命中／必不命中／鑑別錨點；錨點須先驗存在；以內容定錨不以行號 | 無檢查（僅 `PLAYBOOK.md` 檢查表） | 2026-08-23 |
| **R-VF26** | 全部 036 工作簿 | 現階段不執行任何寫回；解凍須 Pei 明示 | 無檢查 | 2026-08-23 |
| **R-VF13 第 5 項** | 覆寫清單所列 leaf 之 TC `reasoning` | 須具名來源欄名與 reqid | `L-VF1` **已設計未實作**（`docs/reports/wvf26_reasoning_gate.md`） | 2026-08-23 |
| **R-VF33** | 任何既有檔之修改 | 修改前須列其消費者與解析路徑 | 無檢查。已致一次實害：A-VF7 之首行加註會使 `DictReader` 取不到欄 | 2026-08-23 |

## 已解除

（無）
