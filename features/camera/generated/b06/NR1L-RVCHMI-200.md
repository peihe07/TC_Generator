...
**補生成之由**：拆解審計 **CAM-22 §5 #1**（`confidence = H`）—— §27.2.6.1 逐字列舉五款配備，而母列 `NR1L-RVCHMI-056` 以單一 ER 概括五款；依 **CAM-10 審閱 §一-4**「查表對映每項一列」應每款一列。母列改留 `Rear View Camera` 一款之驗證（其 ER 不動，見上繳 §2-2），本列承接其中一款。**既有 197 列不改**（DECISIONS 6-74），本列為新列。本列取 **FFCTL**（Forward Facing）一款，其配備旗標為 `Forward_Facing_Camera`（byte 177 bit 1）—— 該 byte 於 `Toro_ATL_MI` 之 2019 本零命中（**DR-CAM-l**）→ **R-CAM18(b)** `Toro(2261)` 判 `0`。
...
3. Read the Camera app home page and check that the FFCTL camera is offered
4. Select the FFCTL camera soft control
```
...
3. The FFCTL camera is offered on the Camera app home page
4. The FFCTL camera view is displayed
```
