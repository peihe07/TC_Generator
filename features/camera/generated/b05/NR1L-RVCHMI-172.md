...
- **specification_reference**：`SYS1_HMI_HeadUnitCameraSystems_HMI_Logic_and_Flow_R1_SR24_Post_2A_v7_(February_10th,_2023)_34.10.3`（來源列 `NRL-188201`）
...
**來源逐字點名平台** `DT and HDCC programs` → `Vehicle Model` 只勾 `HDCC27`／`DT27`，依 **R-CAM3**（來源直接載平台）。本列與 §34.7.1（`NR1L-RVCHMI-092`）之 Description **差一個 image token**（該列 24 token 含 `(image: …)`，本列 22 token 無），分群實測相異；且父題相異（§34.10 `Accessing AUX Cameras Camera app` vs §34.7 `Accessing AUX Cameras from Surround View`）→ 依 **R-CAM16(c)** 各自出 TC。分工：`-092` 自 **Surround View** 之脈絡驗，本列自 **Camera app** 之脈絡驗替代入口。
...
4. Select the AUX filter and check that the AUX camera soft control is present
```
...
4. The AUX camera soft control is shown in the AUX filter
```
