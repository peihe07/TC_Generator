...
- **specification_reference**：`VF551_V4_PHDCC27_VF_520`（來源列 `SYS-RA-VF551_V4-121`）
...
驗證目標為 `SYS-RA-VF551_V4-121`。與 `NR1L-RVC-105`（`V2-524`，數位側）逐字同句而本與承接列皆不同（R-CAM10）。V4 為 HDCC27 之**類比相機變體**，Pre-Condition 以 `PROXI Rear_View_Camera_Type = 0 (Analogic)` 與 V2 之數位側區別（`HDCC27_initial` row 931 實測 `0=Analogic`）。**只勾 HDCC27** —— V4 本之 anchor 前綴只有 `PHDCC27`（R-CAM11）。
...
2. Read the HU display while the rear view camera image is shown and check for blanking
3. Send CAN: TRANSM_FD_4.ShiftLeverPosition = 1 (P)
4. Read the HU display during the transition back to the non-camera display and check for blanking
```
