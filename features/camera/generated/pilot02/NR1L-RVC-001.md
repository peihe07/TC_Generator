...
> By default the camera shall be off until it is activated by the softkey or the conditions defined in the VF.
...
2. Read the NormalCameraDaemon start-up log
   $ adb logcat -d -s NormalCameraDaemon
3. Read the HU display and check that no camera image is displayed
```
...
3. No camera image is displayed and the HU shows the last known screen
```
