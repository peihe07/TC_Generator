...
驗證目標為 `SYS-RA-VF551_V3-283` 之 `a)` 子句（缺失 → `SNA`）。「訊號缺失」以**停送該 CAN 訊息**表達（下放包 §3）—— 逾時門檻之值於來源未載，**不造**（§8.4.1），ER 只判 SNA 之出現而不判其時限。**來源之 `a)` 子句標的誤植**（RDF-09）—— 寫 `send LVDS STATUS_CCAN4.ReverseGearSts = [SNA]`；主句之 `vehicleUpdate_2.ReverseGearSts` 為正確標的。verbatim 逐字不改，**ER 以主句之標的書寫**（CAM-09 審閱 §一-6）。LVDS 訊號無 DBC（四本零命中），其值以來源 label 逐字書寫、觀察以 bus analyzer 進行，**不造命令**（profile §7.2／§7.3）。
...
```
1. STATUS_CCAN4 is no longer received by the HU
