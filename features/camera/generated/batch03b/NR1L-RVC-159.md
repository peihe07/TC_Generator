...
- **specification_reference**：`VF551_V2_PDT27_VF_1713`（來源列 `SYS-RA-VF551_V2-748`）

## test_item 上半（verbatim，SYS2 逐字）

> a. The Head Unit sends LVDS vehicleUpdate_2.ShiftLeverPosition = SNA when FD-CAN8 TRANSM_FD_4.ShiftLeverPosition is missing and Hybrid_Type != "Range Electric Paradigm Breaker" OR "Battery Electric Vehicle".

## reasoning

驗證目標為 `SYS-RA-VF551_V2-748` 之 DT27 缺失側，與 `-158` 成一對。**hybrid 分支（`V2-749`／`-750`）不生成**（A-CA15）。「訊號缺失」以**停送該 CAN 訊息**表達（下放包 §3）—— 逾時門檻之值於來源未載，**不造**（§8.4.1），ER 只判 SNA 之出現而不判其時限。LVDS 訊號無 DBC（四本零命中），其值以來源 label 逐字書寫、觀察以 bus analyzer 進行，**不造命令**（profile §7.2／§7.3）。**lint `J`（首字小寫）之豁免**：本列 test_item 上半為來源逐字，其首字為來源之子句編號 `a.`；依 **profile §5.1**，§4.3.1 之逐字忠實優先於版面規則。
