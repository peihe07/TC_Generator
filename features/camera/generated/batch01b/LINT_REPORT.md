...
- 來源：`batch01b_lint.xlsx`（唯讀）—— **lint 專用暫存簿**，由本目錄之 12 份 json
...
| U | PENDING 佔位（四欄全掃，含 ER 側） | 15 | 8 | 每次命中 | 計數用（A-PM16：ER 側原不受任何檢查覆蓋） |
...
| X | 導航路徑無固定入口（§5.8／R-G71） | 2 | 2 | 每行 | 未校準（§5.8／R-G71，GC-07 新增）—— **WARN 只報不改** |
...
**總計：行計 29**（列計不加總——同一列可觸發多項檢查）

## 明細

### U — PENDING 佔位（四欄全掃，含 ER 側）（行計 15／列計 8）
...
### X — 導航路徑無固定入口（§5.8／R-G71）（行計 2／列計 2）
