...
> When the HU receives the $SVC_SK_PRSNT$ = [1] the HU shall activate the virtual Surround View Camera button for user select-ability.
...
```
1. Set PROXI Surround_View_Camera = 1 (Present) and cycle the ignition to RUN so that the HU reads the PROXI configuration
2. Read the HU display and check that the virtual Surround View Camera button is present and selectable
```
...
2. The virtual Surround View Camera button is shown in the available state and can be selected
```
