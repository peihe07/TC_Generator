...
### R-CAM19(f)（增補，2026-09-24，Pei 裁：CAM-26 審閱 §一-3）

```text
R-CAM19(f)  (b) 之各自出 TC 只適用於原無任何 TC 掛其 ID 之需求列；已有 TC 之需求，
  其與他列共引之來源不再各自出（§8.2.1）。
```

**原條 R-CAM19 之逐字不動**（R-TM13／GCB-06：增補段以同級錨自立）。
**執行層落實（CAM-27）**：CAM-26 之十列即本款之全部射程（`SWE-CAM-007`／`-013`／`-024`／`-025`、
`SWE1-RVC-069`／`-086`／`-091`／`-102`／`-136`／`-139`），無追加；DECISIONS **6-88**。
