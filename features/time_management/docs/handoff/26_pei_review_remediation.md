...
> **【本判定錯誤，2026-08-25 由 `28` §3 推翻】** `088a4476…` 是真的 ——
> 它是 `ASW-R2/Time Management/…_SWQT_20260822.xlsx`（**repo 外**之已送審
> 交付件）之實測值。本節只搜了 repo 內，把「repo 內查無」誤讀為「不存在」。
> **兩個 0822 是不同的檔案**（F/G 欄各 59 列相異），而這次「更正」把該訊號
> 抹掉了。真正的缺陷是 identifier 欄未經裁定即改名 —— 登記為 A-TM31，
> 已由 R-TM85 裁定回改。本段照留不刪（R-TM13）。
