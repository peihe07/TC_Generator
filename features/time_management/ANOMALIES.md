...
**覆蓋時點修正**：原記 09:13–09:14（兩支），實為 **09:13–09:15**（三支）
—— `build_batch_context.py` 之 mtime 為 09:15:18。

**「第 4 項事實」之 push 狀態亦須更正**：該段記「未 push、歷史仍可重寫」，
現況為**已 push**（分支與 origin 同步）。`554079e` 之歸屬問題自此
不可逆 —— 重寫已發布之歷史屬 Pei 之決定，且成本已不同。
...
（二項之量值於 2026-08-22 更正，見條末「量值更正」；牴觸之成立不變）
...
### 量值更正（2026-08-22，分析層，依 `08` 上繳 §3）

**原記之量值加刪除線保留於上（R-TM13）。牴觸之成立不變，只更正量值。**
...
**執行層回報（2026-08-22）**：已落檔。**「7」之成因分析層已說明**
（單一字串頻次被當成違規總數），與執行層 `08` §3 所述「兩種算法都試不出
7」相符 —— 執行層試的是「相異字串數」與「全部出現數」，而實際條件是
「特定單一字串之出現數」，屬第三種，故試不出。

**本條記入 R-TM31 之延伸意義**：R-TM31 要求「列出而非計數」，本例顯示
即使列出了一個數，**若不同時說明量測條件（哪個欄、相異或全部、有無
...
## A-TM30 — 下放包之基準 SHA 手抄（**前提已部分更正**）

（登記於 `27` §C1，2026-08-25；前提更正見 `28` §3。）

```
A-TM30（OPEN，Tier 2 —— 跨 feature，已由 R-G19-3 承接）

下放包所宣告之基準檔 SHA256 應由腳本對「宣告路徑之檔案」實測產出，
不得手抄。
```

**前提更正（2026-08-25，`28` §3）**：本條原記之成因為「`26` §0 之 SHA
`088a4476…` 抄錯」。**該成因不成立。** `088a4476…` 是真的 —— 它是
`ASW-R2/Time Management/…_SWQT_20260822.xlsx`（**repo 外**之已送審交付件）
之實測值。執行層 `27` §C1 只搜了 repo 內，把「repo 內查無此雜湊」
誤讀為「此雜湊不存在」，據以判為謄錄錯誤並逕改 —— **那次「更正」
抹掉的是一個真訊號**（見 A-TM31）。

本條之**建議仍成立且已升格**：R-G19-3 明訂基準 SHA 須對宣告路徑實測、
基準檔在 repo 外者先取複本入 `inputs/` 再實測。若當初照此辦理，
`26` 與 `27` 的基準分歧會在宣告當下就暴露，而不是繞一圈才被發現。

**教訓（登記之本體）**：查無 ≠ 不存在。以「我搜過的範圍」為「全部範圍」
下結論，錯得比不下結論更貴 —— 因為它會帶著「已核實」的外觀覆蓋掉原值。


## A-TM31 — 交付件與 repo 基準分歧，identifier 欄未經裁定即改名

（登記於 `28` §3，2026-08-25。**已由 R-TM85 處置完畢**。）

```
A-TM31（RESOLVED —— R-TM85，2026-08-25）

兩個「0822」為不同檔案：

| 件 | F 欄 | G 欄 | SHA256 前綴 |
|---|---|---|---|
| ASW-R2 已送審交付件 | NR1L-TimeManagement-NNN | Time Management | 088a4476 |
| repo output 0822    | NR1L-TimeAndDate-NNN    | Time and Date   | 2afd87be |

F、G 各 59 列之改名不在 W-TM-26 之 T1–T6 任何一項，其成因落在 repo 側
某次寫回，且未於任何回繳包申報。G 欄改名有 canon §4.1.1 之依據
（Test Group ＝ spec 標題），但 R-TM2 之 [PROVISIONAL] 值為
"Time Management"，推翻須留痕 —— 未留。
```

**處置（R-TM85，Pei 2026-08-25）**：F/G 回改 `NR1L-TimeManagement-*` /
`Time Management`，與已送審件一致；R-TM2 之 [PROVISIONAL] 就此定案。
W-TM-26-A1 T2 已執行完畢（F/G 各 59 列，非受令欄零變更）。

**形態**：與 A-TM29／A-TM30 同族 —— **產出物外觀完全正常**（欄位齊備、
lint 全綠、格式合規），唯一的判準是它與送審件的 identifier 不一致，
而沒有任何既有閘門以「與 repo 外之送審件一致」為判準。R-G19-4 補上了
這一條（identifier 欄變更一律先裁後動，未經裁定之改名視同缺陷）。
