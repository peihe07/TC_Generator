...
## 0.29 Signed rulings (Pei, 2026-08-12) — R-P100，自檢表清單之範圍, 逐字
...
**R-P99 不就地改寫**，依 §0.3 之通則以本條更正其範圍。

### 併同追認之兩項通則（下放包 22 §0.2 / §0.3）

> **串接驗證以實測邊界定位，不以文件所述行數定位。**
> 下放包 21 述 Charter 為 93 行，實際寫入 repo 者為 95 行
> （canon §5a「不以自身先前輸出為來源」第四次）。執行層以實測 `wc -l`
> 推第二段起點而非硬編碼，故未誤報。**此作法定為常規。**

> **逐字落檔之條文一律不就地改寫。**
> 撤銷標 `SUPERSEDED by R-Pxx`，更正標 `援引經 R-Pxx 更正`，原文永遠保留。
> 審計軌跡之價值在於能看見判斷如何演變。
> 追認執行層對 R-P95 之加註處置（`DECISIONS.md §0.27`）。
