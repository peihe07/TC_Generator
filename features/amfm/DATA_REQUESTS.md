...
`features/amfm/inputs/`; each landing closes or advances the linked anomaly.
...
| 2b | 逐需求附件主檔 `4874050- 4595376- CFTSMV024_CIP_R3_O1965_Excel_Document.xls`（DTC 定義表，Vehicle Configuration Mismatch）—— **實測已在 `inputs/`（37,376 bytes，SHA256 `55666213…`）**。⚠️ 同目錄另有 `4874049- 4595376- CFTSMV024_CIP_R3_O1965_Excel_Document.xls`，**同名同大小但雜湊相異**（`3fd31f94…`），兩份並存之緣由無任何既有記載 → **A-AM17** | ✅ 已入 `inputs/`（R14-C3, 2026-08-13 關列） | Diagnostics 批 context | DTC 細節 | A-AM07 / **A-AM17** | — |
| 2c | 其餘 12 件 O 附件（9 件天線 DTC 表 + 2 件交通圖示表 + 1 件內嵌註記），位於 `…/Reference Docs/CFTS024/`，已全數驗明 | ⚠️ 未入 `inputs/` | audit 舉證用，不阻塞任何批次（Diagnostics 批 097–104 已生成、lint green、零 placeholder） | DTC 細節佐證 | A-AM07 | Low |
...
| 4 | CFTS028 — ✅ 已入 `inputs/`（2026-08-10）：`R1LR_Atl-H_25PI3.5_Speech and Personal Assistant_CFTS_28 Voice Recognition_SR26_20250909_1250.docx`。**檔案不回答範圍問題**：VR 觸發路徑歸屬經 R8 裁定不進本 workbook | ✅ 已入 `inputs/`；範圍裁決 R8 stands（R14-C2, 2026-08-13）—— 關列 | 003, 009, 025, 027 (4) | VR 補充路徑 | A-AM09 | — |
