...
- **G113 之處置 —— 第四批之前置**（23 包）。七項驗證條件實測 **2 / 7**：
  二項**根本不是 OR 結構**（`and if` / `both … and`），三項因
  **右運算元以括號內之 `If` 起首而被分隔符截斷**。
  **後三項可由分隔符集合之調整達成重現，執行層依 23 §I 未為之。**
  **第四批因此未啟動**；範圍已備妥（`SWE-PM-033`–`063`，31 leaf，不受任何 live DR 影響）。
- **`SWE-PM-025` 之八條**（R-P167，裁定於 24 包）—— 三對僅 `ECU` 相異；
  **若裁為應合併，`SWE-PM-015` / `019` 同受影響，三者 16 條中 8 條將消失**。
- **DR-PW9（High，23 包補開）** —— SYS2 `All_Accepted` 與 4 列 `Need rework`。
- **DR-PW11（High，23 包擴大）** —— 由 1 item / 1 leaf 擴為 **4 item / 2 leaf**。
- **DR-PW6 阻斷 `SWE-PM-001`–`009`**、**DR-PW10（Medium）**、**DR-PW8（High）**。
- **G109 不實作**（R-P163）—— ER 依據越界僅能靠取樣與人讀。
- **G108 不保證「同一步內完成」** —— 何時執行仍靠執行者，紀律加工具而非機制。
