...
- **R-P116 —— `SWE-PM-089` 佔不佔列**（待 Pei）。素材已擴查三 feature（Comfort / Home / Privacy），
  結果一致為 **0**，見 `data/b4_089_row_material.md` 與 17 包上繳 §四。
- **Q3 —— Final Step 措詞**（待 Pei）。Arif 144 條末步 `check` 53.5%，§5.2B 完整措詞 12.5%；
  Comfort + Privacy 472 條 0%。
- **DR-PW8（High，17 包新開）** —— `voltage out of range` 之電壓門檻值。
  `015` 為**可撰寫而不可執行**之 TC（A-PW83）。
- **`remarks` 欄無任何格式閘門**（A-PW88）—— G50 之 `LONG_FIELDS` 不含之。
- **`surgical_save` 不保證目標分頁內既有列不變**（R-P120 之回報）——
  append 起始列之正確性仍靠呼叫端，是否補閘門待裁。
- **反向涵蓋之效力取決於人真的讀報告**（R-P124 之保留）—— 第二批行為項數將數倍於首批。
