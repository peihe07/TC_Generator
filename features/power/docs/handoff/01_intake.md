...
## E. framework Layer 2 / Layer 3（**待定版 —— 依 R-P15(b) 逐條裁定後於 04 包定版**）

> 標題由「已定版」改為「待定版」，依 03 下放包 §E。
> 理由：02 包已證明「已定版」缺乏可重現之產生程序。
> leaf 分布數字維持 02 包狀態，未重算、未改寫（03 §I 禁區）。
...
> **03 包異動**：
> - `§1.8.1` 依 **R-P16** 刪除（主章節落點 0）。
>   **惟該前提須複核** —— `SWE-PM-057` 之章節集合含 §1.8.1.1.1，
>   只是在 02 包之主章節規則下未勝出。見 A-PW14。
> - 「已定版」改為「待定版」。11 個跨章節 leaf 待 Pei 依 **R-P15(b)**
>   逐條裁定，素材見 `features/power/data/multi_chapter_leaves.md`。
> - leaf 分布數字（64/24/16/7/3）**未動** —— 03 §I 禁止重算或改寫。
