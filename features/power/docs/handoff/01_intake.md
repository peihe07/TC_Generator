...
## E. framework Layer 2 / Layer 3（**已定版（R-P35）**）
...
|---|---|---|
| Power State | **63** | 009 **§1.6.2.1**、§1.6.2.1.1–.15、§1.7.1、**§1.8.1.1.1**、§1.9.3–.5、§1.9.12 |
| Startup Display | 24 | 009 §1.3.5、§1.6.2.1.16、§1.9.8、§1.9.9、§1.9.10 |
| Branding and Theme | 16 | 009 §1.9.15–.17 |
| Timeout Settings | **8** | 009 **§1.6.2.1.17**、§1.6.3、§1.6.4、§1.6.7 |
| Power Down | 3 | **010** §1.7.1、§1.7.2 |

合計 114 + `SWE-PM-089` 留空（R-P1）= 115。
本表已經 R-P5（合併）、R-P6（Power Down 保留獨立）與
**R-P35（分布定版 63 / 24 / 16 / 8 / 3）** 裁定，**已定版**。
定版後之異動一律需 Pei 重裁（05 §E）。

> **03 包異動**：
> - 「已定版」改為「待定版」。
>
> **04 包異動**：
> - `§1.8.1` 之刪除依 **R-P25 撤回**（R-P16 撤回），改記 **`§1.8.1.1.1`**
>   （`ID 1 Description`）—— `SWE-PM-057` 確實觸及該章節，
>   「零落點」僅在已被 R-P15 廢止之 tie-break 下成立。A-PW14 已處置。
> - 依 **R-P24**，Layer 3 改為記全集（一 leaf 得對應多章節），
>   全集見 `features/power/data/layer3_full.tsv`（140 列、46 個相異章節）。
>   本表僅為 Layer 2 之對照摘要，非 Layer 3 之權威。
> - 依 **R-P26**，待裁者收窄為 `SWE-PM-008` 與 `SWE-PM-057`
>   兩條之 **Test Set**（Layer 2）歸屬；其餘九條 Layer 3 自動記全集，無需裁定。
> - leaf 分布數字（64/24/16/7/3）於 04 包**未動** —— 03／04 §I 皆禁止重算或改寫。
>
> **05 包異動（定版）**：
> - 依 **R-P33** `SWE-PM-008` → **Power State**；依 **R-P34** `SWE-PM-057` →
>   **Timeout Settings**。二條為 Pei 逐條裁定，非演算法指派。
> - 依 **R-P35** 分布定版為 **63 / 24 / 16 / 8 / 3**（原 64 / 24 / 16 / 7 / 3
>   係在「兩條皆歸 Power State」之隱含假設下產生，該假設從未經裁定）。
>   標題改為「已定版（R-P35）」。
> - 驗證：`python features/power/scripts/build_testsets.py` → G21 / G22 皆 PASS，
>   逐 leaf 指派見 `features/power/data/leaf_testset.tsv`（114 列）。
> - **登記一項清單缺漏**：本表 Power State 之 Layer 3 列 `§1.6.2.1.1–.15`，
>   **未涵蓋其父章節 `§1.6.2.1`**（`TLM algorithm requirements`），
>   而 R-P24 之 Layer 3 全集中有 **9 個 leaf** 觸及該父章節；
>   `§1.6.2.1.17` 亦未被本表任一 Test Set 涵蓋（1 leaf）。
>   二者於指派時作為「非 Test Set 候選」登記，**不影響分布數字**
>   （相關 leaf 之其餘章節皆指向單一 Test Set）。待裁。
