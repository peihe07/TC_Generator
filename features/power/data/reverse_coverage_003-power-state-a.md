...
## 現況（batch_003_power_state_a.json，63 條 TC）
...
TC：055, 056, 057, 058, 059, 060, 061, 062, 063

| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Full-Operation" AND STATUS_BH_BCM | 063 | 0.55 | 已覆蓋 | `case`、`equal`、`false`、`pass`、`phone_call.info`、`standby`、`state`、`state.if`、`state.in`、`stay`、`thi`、`tim`、`value` |
...
| 4 | “Phone call management in Timed state” .ELSE IF LTM_OperationalModeSts.Info is | 063 | 0.37 | **無對應** | `behaviour`、`call`、`different`、`else`、`equal`、`false`、`full`、`management`、`menu`、`min`、`operation`、`original`、`phone`、`possible`、`selectable`、`sett`、`signal`、`state`、`stay`、`switchoff_timeout_setting.req`、`thenaccord`、`tim`、`time`、`timeout1`、`two`、`valueand` |
...
| 候選（須人工判 措詞差異 / 真缺口） | **35** | `case`(#1)、`equal`(#1)、`state.if`(#1)、`state.in`(#1)、`thi`(#1)、`equal`(#2)、`maximum`(#2)、`expiration`(#3)、`maxcalltimeout`(#3)、`different`(#4)、`else`(#4)、`equal`(#4) |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Full-Operation"AND signal Front_P | 064 | 0.79 | 已覆蓋 | `set`、`signal`、`value`、`valuethen` |
| 2 | IF TLM_Status.Info and $Telematic_Power$ == "Full-Operation"AND signal CLIMATI | 065 | 0.79 | 已覆蓋 | `set`、`signal`、`value`、`valuethen` |
| 3 | IF TLM_Status.Info and $Telematic_Power$ == "Full-Operation"AND signal Front_P | 066 | 0.74 | 已覆蓋 | `parameter`、`provid`、`proxi`、`set`、`signal`、`value`、`valuethen` |
| 4 | IF TLM_Status.Info and $Telematic_Power$ == "Full-Operation"AND signal CLIMATI | 067 | 0.74 | 已覆蓋 | `parameter`、`provid`、`proxi`、`set`、`signal`、`value`、`valuethen` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Full-Operation"AND PROXI paramete | 068 | 0.72 | 已覆蓋 | `activethen`、`becom`、`manage`、`parameter`、`proxi` |
| 2 | until Rear_Camera_Enable.Info passes to “False” again | 068 | 1.00 | 已覆蓋 | （無） |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Full-Operation"AND PROXI paramete | 069 | 0.50 | 已覆蓋 | `also`、`becom`、`chang`、`could`、`depend`、`manage`、`parameter`、`prioriti`、`proxi`、`rvc`、`show`、`sourc`、`their`、`thentlm`、`value` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Idle"AND signal LTM_OperationalMo | 071 | 0.75 | 已覆蓋 | `set`、`signal`、`value`、`valuethentlm` |
...
|---|---|---|
| 已由他條涵蓋 | **0** | （無） |
| 候選（須人工判 措詞差異 / 真缺口） | **4** | `set`(#1)、`signal`(#1)、`value`(#1)、`valuethentlm`(#1) |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Idle"AND signal Front_Panel_OnOff | 072 | 0.58 | 已覆蓋 | `depend`、`else`、`par`、`parameter`、`proper`、`proxi`、`show`、`signal`、`thi`、`value`、`valuethen` |
| 2 | "Splash Screen logo visualization" logics, for Response_Wait_Time AND TLM has  | 073 | 0.67 | 已覆蓋 | `logic`、`logo`、`set`、`value`、`visualization` |
| 3 | IF TLM_Status.Info and $Telematic_Power$ == "Idle"AND signal CLIMATIC_PANEL.Ra | 074 | 0.58 | 已覆蓋 | `depend`、`else`、`par`、`parameter`、`proper`、`proxi`、`show`、`signal`、`thi`、`value`、`valuethen` |
| 4 | "Splash Screen logo visualization" logics, for Response_Wait_Time AND TLM has  | 073 | 0.67 | 已覆蓋 | `logic`、`logo`、`set`、`value`、`visualization` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Idle"AND signal Phone_Call.Info h | 076 | 0.61 | 已覆蓋 | `apply`、`condition`、`doe`、`r1h`、`set`、`signal`、`sr21`、`state.thi`、`value`、`valuethentlm` |
| 2 | Then, IF Phone_Call.Info turns back to "Not_Active" when TLM_Display.GUI is in | 077 | 0.60 | 已覆蓋 | `action`、`another`、`back`、`due`、`dur`、`set`、`state.if`、`stay`、`turn`、`user` |
| 3 | Refer to TLM HMI document for further details about the screens visualization | 077 | 0.22 | **無對應** | `about`、`detail`、`document`、`further`、`hmi`、`refer`、`visualization` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Idle" AND PROXI parameter Rear_Vi | 079 | 0.57 | 已覆蓋 | `about`、`allow`、`availability`、`but`、`detail`、`parameter`、`pass`、`proxi`、`requirement`、`screen.refer`、`stay`、`thentlm`、`vf551` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Full-Operation” || “Idle”AND sign | 080 | 0.86 | 已覆蓋 | `set`、`signal` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Timed"AND signal LTM_OperationalM | 081 | 0.82 | 已覆蓋 | `set`、`signal`、`valuethentlm` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Timed"AND STATUS_BH_BCM2.RemStAct | 082 | 0.87 | 已覆蓋 | `set`、`signal`、`valuethentlm` |
...
TC：083, 084, 085, 086, 087, 088, 089, 090

| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Timed"AND Front_Panel_OnOff.Req h | 083 | 0.46 | 已覆蓋 | `accept`、`activethen`、`case`、`doe`、`hmi`、`off`、`order`、`pass`、`refer`、`set`、`specification`、`standby`、`state`、`state.if`、`stay`、`thi`、`turn`、`value`、`valuethenif` |
| 2 | IF Phone_Call.Info == Not_ActiveTLM has to stop its active functionality (Medi | 086 | 0.37 | **無對應** | `activation`、`already`、`audio`、`etc`、`media`、`network`、`not_activetlm`、`order`、`quickly`、`request`、`requir`、`respond`、`set`、`stream`、`user`、`value`、`without` |
| 3 | IF TLM_Status.Info and $Telematic_Power$ == "Timed"AND CLIMATIC_PANEL.Radio_Bt | 087 | 0.46 | 已覆蓋 | `accept`、`activethen`、`case`、`doe`、`hmi`、`off`、`order`、`pass`、`refer`、`set`、`specification`、`standby`、`state`、`state.if`、`stay`、`thi`、`turn`、`value`、`valuethenif` |
| 4 | IF Phone_Call.Info == Not_ActiveTLM has to stop its active functionality (Medi | 086 | 0.37 | **無對應** | `activation`、`already`、`audio`、`etc`、`media`、`network`、`not_activetlm`、`order`、`quickly`、`request`、`requir`、`respond`、`set`、`stream`、`user`、`value`、`without` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Timed"AND proxi parameter Brand_C | 091 | 0.65 | 已覆蓋 | `audio`、`equal`、`etc`、`media`、`parameter`、`proxi`、`set`、`standbythen`、`status_bh_bcm1.psngrdoorst`、`stream`、`tuner`、`value` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF Antitheft_Result.Info == "Not_Successfully", THEN TLM has to set Antitheft_ | 094 | 0.50 | 已覆蓋 | `back`、`equal`、`hmi`、`maximum`、`need`、`proper`、`set`、`show`、`value` |
| 2 | IF Antitheft_Result.Info == "Not_Successfully", THEN TLM has to set Antitheft_ | 095 | 0.57 | 已覆蓋 | `back`、`hmi`、`need`、`proper`、`see`、`set`、`show`、`value`、`vf210` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF Antitheft_Result.Info == "Successfully" THEN TLM has to set Antitheft_Activ | 096 | 0.54 | 已覆蓋 | `follow`、`guarante`、`logic`、`min`、`switchoff_timeout_setting.req`、`value` |
| 2 | If Auto_SwitchOn_Setting.Req =="Active ", when Timeout1 == 00 MIN" for LTM Hig | 099 | 0.56 | 已覆蓋 | `case`、`example`、`only`、`parameter`、`restor`、`set`、`specifi`、`thentlm`、`thi`、`value` |
| 3 | AND TLM has to set TLM_Status.Info and $Telematic_Power$ to "Timed" value and  | 098 | 0.75 | 已覆蓋 | `set`、`value` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF Antitheft_Result.Info == "Successfully" THEN TLM has to set Antitheft_Activ | 101 | 0.39 | **無對應** | `antitheft_activation.req`、`back`、`case`、`example`、`false`、`guarante`、`logic`、`only`、`parameter`、`proxi`、`restor`、`set`、`specifi`、`thentlm`、`thi`、`tlm`、`value` |
| 2 | IF SwitchOff_Timeout_Setting.Req == 00 min THEN TLM has to set Timeout1 to the | 102 | 0.47 | 已覆蓋 | `case`、`example`、`only`、`restor`、`set`、`specifi`、`thi`、`tlm`、`value` |
| 3 | AND TLM has to set TLM_Status.Info and $Telematic_Power$ to "Timed" value and  | 103 | 0.75 | 已覆蓋 | `set`、`value` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF Auto_SwitchOn_Setting.Req == Active OR IF Auto_SwitchOn_Setting.Req == Reca | 104 | 0.50 | 已覆蓋 | `depend`、`onthen`、`par`、`recall_last`、`show`、`vplaststatu` |
| 2 | "Splash Screen logo visualization" logics, for Response_Wait_Time | 104 | 0.50 | 已覆蓋 | `logic`、`logo`、`visualization` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF Rear_View_Camera PROXI parameter == "Present", according to the value of Re | 105 | 0.62 | 已覆蓋 | `accord`、`parameter`、`proxi`、`show`、`telematic_power$`、`value` |
...
| # | 行為項 | 最佳對應 | overlap | 狀態 | 透鏡 3 —— 最佳對應所缺之實詞 |
|---|---|---|---|---|---|
| 1 | IF TLM_Status.Info and $Telematic_Power$ == "Standby" OR “Sleep”AND STATUS_BH_ | 106 | 0.77 | 已覆蓋 | `set`、`sleep`、`value`、`valuethentlm` |
