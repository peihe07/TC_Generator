...
|---|---|---|---|
| 是 | 是 | **55** | 規格為多條件結構，且該條件確實改變結果 |
| 是 | 否 | **46** | 規格為多條件，惟未以姊妹枚舉 |
| 否 | 是 | **4** | 規格僅一條件子句，惟姊妹顯示條件改變結果 |
| 否 | 否 | **36** | 二證據皆無 —— 代理判準之疑似偽陽性 |

## 二、逐條

| tc | leaf | 實質前提 | 甲 | 乙 姊妹 | 丙 參數 |
|---|---|---|---|---|---|
| `007` | `SWE-PM-073` | 3. Ecall, ACN and chimes modes are inactive | 11 | `…-008` | 3 |
| `009` | `SWE-PM-073` | 2. The TLM is in BODY ON mode；3. Ecall, ACN and chimes modes are inactive | 11 | `…-013` | 2 |
| `010` | `SWE-PM-073` | 2. The Battery Critical condition is already active | 11 | `…-007` | 2 |
| `011` | `SWE-PM-073` | 3. The Load Shed condition is already active | 11 | `…-007` | 2 |
| `012` | `SWE-PM-073` | 3. A non-Ecall non-ACN call is active and continuing | 11 | `…-007` | 2 |
...
| `015` | `SWE-PM-073` | 2. The Battery Critical condition is already active | 11 | `…-007` | 1 |
| `016` | `SWE-PM-073` | 3. Ecall, ACN and chimes modes are inactive | 11 | `…-008` | 3 |
...
| `025` | `SWE-PM-062` | 2. The TLM is in Full-Operation status | 3 | **無** | 0 |
| `026` | `SWE-PM-062` | 2. The TLM is in Full-Operation status | 3 | **無** | 0 |
| `027` | `SWE-PM-062` | 2. The TLM is in Full-Operation status | 3 | **無** | 0 |
...
| `052` | `SWE-PM-013` | 2. The ignition working condition is Ignition On | 2 | `…-057` | 1 |
| `053` | `SWE-PM-013` | 2. The ignition working condition is Ignition Pre_Start | 2 | `…-057` | 1 |
| `054` | `SWE-PM-013` | 2. The ignition working condition is Ignition Cranking | 2 | `…-057` | 1 |
| `055` | `SWE-PM-013` | 2. The ignition working condition is Ignition On Engine On | 2 | `…-057` | 1 |
...
| `098` | `SWE-PM-027` | 2. Antitheft_Activation.Req reads "True" | 4 | **無** | 1 |
| `099` | `SWE-PM-027` | 2. The TLM is in Partial Operation；3. Antitheft_Activation.Req reads "True" | 4 | **無** | 1 |
| `100` | `SWE-PM-028` | 2. Antitheft_Activation.Req reads "True" | 5 | **無** | 1 |
...
| `104` | `SWE-PM-029` | 2. Antitheft_Activation.Req reads "True" | 5 | **無** | 1 |
...
| `114` | `SWE-PM-034` | 2. TLM_Status.Info and $Telematic_Power$ read "Partial Operation" | 1 | **無** | 1 |
| `121` | `SWE-PM-039` | 2. The TLM is in an Ignition On working condition | 3 | `…-122` | 1 |
| `122` | `SWE-PM-039` | 2. TLM_Status.Info was equal to "Full-Operation" | 3 | `…-121` | 1 |
...
| `136` | `SWE-PM-045` | 2. TLM_Status.Info and $Telematic_Power$ read "Standby" | 2 | `…-137` | 1 |
| `137` | `SWE-PM-045` | 2. TLM_Status.Info and $Telematic_Power$ read "Sleep" | 2 | `…-136` | 1 |
...
| `140` | `SWE-PM-047` | 2. TLM_Status.Info and $Telematic_Power$ read "Standby" | 2 | `…-141` | 1 |
| `141` | `SWE-PM-047` | 2. TLM_Status.Info and $Telematic_Power$ read "Sleep" | 2 | `…-140` | 1 |
| `147` | `SWE-PM-049` | 2. TLM_Status.Info and $Telematic_Power$ read "Idle" | 2 | **無** | 1 |
| `150` | `SWE-PM-052` | 2. TLM_Status.Info and $Telematic_Power$ read "Partial Operation" | 2 | **無** | 1 |
| `152` | `SWE-PM-054` | 2. SDARS_Presence reads "Absent" | 4 | `…-154` | 1 |
| `153` | `SWE-PM-054` | 2. SDARS_Presence reads "Absent" | 4 | `…-154` | 1 |
| `154` | `SWE-PM-054` | 2. SDARS_Presence reads "Present" | 4 | `…-152` | 1 |
| `155` | `SWE-PM-054` | 2. SDARS_Presence reads "Present" | 4 | `…-152` | 1 |
...
| `158` | `SWE-PM-056` | 2. The HU carries a configured vehicle brand | 1 | **無** | 1 |
| `161` | `SWE-PM-059` | 2. TLM_Status.Info and $Telematic_Power$ read "Standby"；3. The boot of the TLM is not ended | 2 | `…-160` | 1 |
| `162` | `SWE-PM-066` | 2. The HU is in a state that reacts to a phone call becoming active | 0 | **無** | 1 |
| `163` | `SWE-PM-066` | 2. The HU is in a state that reacts to a phone call becoming active | 0 | **無** | 1 |
| `175` | `SWE-PM-076` | 2. The HU is not installing a firmware image | 2 | `…-177` | 1 |
| `176` | `SWE-PM-076` | 2. The HU is not installing a firmware image | 2 | `…-177` | 1 |
| `177` | `SWE-PM-076` | 2. The HU is currently installing a firmware image | 2 | `…-175` | 1 |
...
| `181` | `SWE-PM-093` | 2. The HU is in STANDBY MODE | 16 | `…-182` | 1 |
| `186` | `SWE-PM-093` | 2. The HU has just played a start-up animation；3. All other conditions for the animation to play | 16 | **無** | 0 |
| `188` | `SWE-PM-095` | 2. LTM_OperationalModeSts.Info reads "SNA" | 0 | **無** | 1 |
| `189` | `SWE-PM-097` | 2. The HU carries a configured vehicle brand | 1 | **無** | 1 |
...
| `196` | `SWE-PM-101` | 2. SDARS_Presence reads "Absent" | 4 | `…-198` | 1 |
| `197` | `SWE-PM-101` | 2. SDARS_Presence reads "Absent" | 4 | `…-198` | 1 |
| `198` | `SWE-PM-101` | 2. SDARS_Presence reads "Present" | 4 | `…-196` | 1 |
| `199` | `SWE-PM-101` | 2. SDARS_Presence reads "Present" | 4 | `…-196` | 1 |
...
| `220` | `SWE-PM-106` | 2. The HU is configured for a disclaimer screen variation | 1 | **無** | 1 |
| `221` | `SWE-PM-107` | 2. The HU is configured for a disclaimer screen variation | 1 | **無** | 1 |
...
| `231` | `SWE-PM-077` | 2. The HU carries a supported special package configuration | 0 | **無** | 1 |
| `232` | `SWE-PM-078` | 2. The HU carries a configured vehicle brand | 1 | **無** | 1 |
| `233` | `SWE-PM-078` | 2. The HU carries a configured vehicle brand | 1 | **無** | 1 |
| `234` | `SWE-PM-079` | 2. The HU displays a PDO branded element | 1 | **無** | 1 |
...
| `237` | `SWE-PM-081` | 2. The HU is displaying branded text | 0 | **無** | 1 |
| `238` | `SWE-PM-081` | 2. The HU is displaying branded text | 0 | **無** | 1 |
| `239` | `SWE-PM-081` | 2. The HU is displaying branded text | 0 | **無** | 1 |
| `240` | `SWE-PM-082` | 2. The HU is displaying the App icon | 0 | **無** | 1 |
| `241` | `SWE-PM-082` | 2. The HU is displaying the App icon | 0 | **無** | 1 |
| `242` | `SWE-PM-082` | 2. The HU is displaying the App icon | 0 | **無** | 1 |
| `243` | `SWE-PM-083` | 2. The profile screen is reachable on the HU | 0 | **無** | 1 |
| `244` | `SWE-PM-083` | 2. The profile screen is reachable on the HU | 0 | **無** | 1 |
| `245` | `SWE-PM-083` | 2. The profile screen is reachable on the HU | 0 | **無** | 1 |
...
| `252` | `SWE-PM-087` | 2. The seat settings screen is reachable | 2 | **無** | 1 |
| `253` | `SWE-PM-087` | 2. The seat settings screen is reachable；3. The HU carries a configured vehicle brand | 2 | **無** | 1 |
| `254` | `SWE-PM-088` | 2. The performance gauges screen is reachable | 0 | **無** | 1 |
| `255` | `SWE-PM-090` | 2. The "Theme Mode" setting reads "Auto" | 1 | **無** | 1 |
| `256` | `SWE-PM-090` | 2. The "Theme Mode" setting reads "Auto" | 1 | **無** | 1 |
...
- `…-114`（`SWE-PM-034`）：實質前提 2. TLM_Status.Info and $Telematic_Power$ read "Partial Operation"；規格條件子句 1
- `…-130`（`SWE-PM-043`）：實質前提 2. The HU is in Standby mode；3. No HMI screen is required；規格條件子句 1
- `…-158`（`SWE-PM-056`）：實質前提 2. The HU carries a configured vehicle brand；規格條件子句 1
- `…-162`（`SWE-PM-066`）：實質前提 2. The HU is in a state that reacts to a phone call becoming active；規格條件子句 0
- `…-163`（`SWE-PM-066`）：實質前提 2. The HU is in a state that reacts to a phone call becoming active；規格條件子句 0
- `…-188`（`SWE-PM-095`）：實質前提 2. LTM_OperationalModeSts.Info reads "SNA"；規格條件子句 0
- `…-189`（`SWE-PM-097`）：實質前提 2. The HU carries a configured vehicle brand；規格條件子句 1
...
- `…-220`（`SWE-PM-106`）：實質前提 2. The HU is configured for a disclaimer screen variation；規格條件子句 1
- `…-221`（`SWE-PM-107`）：實質前提 2. The HU is configured for a disclaimer screen variation；規格條件子句 1
...
- `…-231`（`SWE-PM-077`）：實質前提 2. The HU carries a supported special package configuration；規格條件子句 0
- `…-232`（`SWE-PM-078`）：實質前提 2. The HU carries a configured vehicle brand；規格條件子句 1
- `…-233`（`SWE-PM-078`）：實質前提 2. The HU carries a configured vehicle brand；規格條件子句 1
- `…-234`（`SWE-PM-079`）：實質前提 2. The HU displays a PDO branded element；規格條件子句 1
- `…-237`（`SWE-PM-081`）：實質前提 2. The HU is displaying branded text；規格條件子句 0
- `…-238`（`SWE-PM-081`）：實質前提 2. The HU is displaying branded text；規格條件子句 0
- `…-239`（`SWE-PM-081`）：實質前提 2. The HU is displaying branded text；規格條件子句 0
- `…-240`（`SWE-PM-082`）：實質前提 2. The HU is displaying the App icon；規格條件子句 0
- `…-241`（`SWE-PM-082`）：實質前提 2. The HU is displaying the App icon；規格條件子句 0
- `…-242`（`SWE-PM-082`）：實質前提 2. The HU is displaying the App icon；規格條件子句 0
- `…-243`（`SWE-PM-083`）：實質前提 2. The profile screen is reachable on the HU；規格條件子句 0
- `…-244`（`SWE-PM-083`）：實質前提 2. The profile screen is reachable on the HU；規格條件子句 0
- `…-245`（`SWE-PM-083`）：實質前提 2. The profile screen is reachable on the HU；規格條件子句 0
- `…-254`（`SWE-PM-088`）：實質前提 2. The performance gauges screen is reachable；規格條件子句 0
- `…-255`（`SWE-PM-090`）：實質前提 2. The "Theme Mode" setting reads "Auto"；規格條件子句 1
- `…-256`（`SWE-PM-090`）：實質前提 2. The "Theme Mode" setting reads "Auto"；規格條件子句 1
