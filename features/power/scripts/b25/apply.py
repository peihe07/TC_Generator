#!/usr/bin/env python3
"""25 包執行層：A／B 型拆分插列套用 ＋ 全本 ID 重排。

25 包 §三／§四。基底 `features/power/sandbox/b19/pm_19.xlsx`（b4dd5ca0…）。
...
Pei 裁定（本包）：規則 2 為準 + 修補規則 1；row 230 存根列略過不給 ID、
B 欄一併重編。
"""
...
SANDBOX = ROOT / "features/power/sandbox/b25"
OUT = SANDBOX / "pm_25.xlsx"
