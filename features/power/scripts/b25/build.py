#!/usr/bin/env python3
"""25 包分析層：A 型逐字拆分內容 ＋ B 型規則化拆分內容 → `plan.json`。

25 包 §一／§二。基底 `features/power/sandbox/b19/pm_19.xlsx`（b4dd5ca0…）。

- **A 型**（rows 11／12／23／179／180 → 14 列）：PROC／ER／括號下半逐字取自
  下放包 §一 之表；PRE 沿原列（179b／180b 增一行）。
- **B 型**（30 列 → 144 列）：依 §二 演算法組裝，內容全取自原列。
  §二 規則 1 之 `PowerSts` 判定加一限制 —— 該步為 PROC 末步時視為觀察步、
  不併入 setup（否則 rows 32／97／102／170 之 setup 吞掉全部步驟、面向數 0，
  規則 3「原列改寫為第一面向」無法成立）。Pei 裁定：規則 2 為準 + 修補規則 1。
- **括號去重**：§四 要求任兩同源面向列括號不得逐字相同，而 lint I-sibling 之
  同源判準為同一 Requirement ID。碰撞者以該列 setup 首步（去編號、首字小寫）
  作前綴消歧 —— 沿用原列既有之 `—` 分隔形態，不引入新文字。
"""
...
def aspect_paren(proc_step: str, er_step: str) -> str:
    """§二 規則 2 之括號下半；逾 20 詞時取 ER 行之核心子句。"""
...
CALL_TRANSITION = [
    ...
    ("b", "An incoming phone call is active on the HU",
...
def pre_with_extra(pre: str, extra: str) -> str:
    """PRE 增一行，置於工具可用行之前（該行依慣例恆為末行）。"""
...
def build_a(src: dict[int, dict]) -> list[dict]:
    ...
    for row in (179, 180):
        ...
        for _tag, extra, proc, er, tail in CALL_TRANSITION:
            out.append({
                "src_row": row, "req": r["req"], "upper": item_upper(r["I"]),
                "J": pre_with_extra(r["J"], extra) if extra else r["J"],
                "L": proc, "M": er,
    ...
    for row in B_ROWS:
        ...
        for j in range(n):
            ...
            out.append({
                ...
                "setup_head": procs[0],
                "J": r["J"],
                "L": renumber(procs[:s] + [procs[k]]),
                "M": renumber(ers[:s] + [ers[k]]),
                "I": compose_item(item_upper(r["I"]),
                                  aspect_paren(procs[k], ers[k])),
            })
    return out, audit
...
def main() -> None:
    ...
    diff = [a for a in audit if a["delta"]]
    print(f"B 型面向數與預核表不一致 {len(diff)}/{len(audit)} 列")
