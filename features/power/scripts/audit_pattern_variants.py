...
def main() -> None:
    ...
    out = ["# G136 —— 字串樣式變體檢查（R-P201(c)）\n",
           ...
           "> **依 R-P230 擇 (b)** —— 本檔為**第三次**判為陳舊"
           "（27 → 30 → 31 包），其陳舊形態非數值過期，"
           "而係**總括語句對新增之腳本已為假**，逐位元組比對抓不到。\n",
           "> **禁止本檔被其他文件引用為證據**；"
           "其人工概括語句全數列於文末「§人工概括語句清單」供逐句複核。\n",
           "> 未擇 (a) 之理由：其判定欄（空白／大小寫／全半形）之依據為"
           "**逐檔讀碼**，非實測可導出 —— 強行自動化將以「有無正則」等表面特徵"
           "取代讀碼判斷，反使該欄失真。\n",
    ...
    # R-P230(b)：列出全部人工概括語句供逐句複核
    manual = [w for _, _, _, w in VERDICT.values()]
    out.append("\n---\n\n## 人工概括語句清單（R-P230(b)）\n\n"
               f"下列 **{len(manual)}** 句為人工撰寫之判定依據，"
               "**非自實測導出**，引用前須逐句複核：\n\n")
    for name in sorted(VERDICT):
        out.append(f"- **`{name}`**：{VERDICT[name][3]}\n")
    out.append("\n另有一句總括語（「未列入判定之腳本…皆為建表 / 產生器類」）"
               "已於 30 包加註「須逐包複查」—— **該句即三次陳舊之源**。\n")
