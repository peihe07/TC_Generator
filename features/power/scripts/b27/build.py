#!/usr/bin/env python3
"""27 包分析層：`test_item` 括號兩案修正 → `plan.json`。

26 包上繳 §八-1／§八-2，Pei 裁定：

- **§八-1 row 109 還原** —— v2 下該列面向數為 1、插 0 列、PROC／ER 與 b19
  原列逐字相同（等於沒拆），故不套面向括號規則，`test_item` 逐字保留原列。
  該列因此變成完全零變動；本案僅此 1 列。
- **§八-2 縮併列括號取形態 B**（`(<驅動步> -> <ER>)`）—— 縮併列之 PROC 為
  「setup + 驅動步 + 觀察步」，原括號只述觀察步、不述 trigger。形態 B 與
  A 型之房規同形（`(send OperationalModeSts = 5 Ignition_Pre_Start ->
  Full-Operation is kept)`）。扣掉 row 109 後影響 16 列。
- **20 詞上限對縮併列不生效** —— §二 之「逾 20 詞取 ER 核心子句」正好會把
  trigger 砍掉，與本案目的相反。上限仍對非縮併之自動生成列生效。

面向切分（規則 1 + 規則 2 v2）、setup 判定、消歧兩段候選一律沿 26 包未動，
故插列數 106、全本 390 列、Test Case ID 001–389 與 26 包完全相同 ——
本包只動 `test_item` 一欄。
"""
...
def build_b(src: dict[int, dict]) -> tuple[list[dict], list[dict]]:
    ...
    for row in B_ROWS:
        ...
        for group in groups:
            ...
            if single:
                item = r["I"]
            elif len(group) > 1:                       # §八-2 縮併列：形態 B
                item = compose_item(
                    item_upper(r["I"]),
                    merged_paren([procs[k] for k in group[:-1]], ers[last]))
            else:
                item = compose_item(item_upper(r["I"]),
