...
**(43) 每一句都帶著它的射程，而射程在傳遞過程中會掉 —— 沒有人寫錯任何一句話，結論仍可誤導。**

出處：上繳包 31 §6（036 母本之「無 DV」）。

台帳三處記「母本無 DV」。逐項覆核，**三處都是對的** ——
`C`／`E`／`S` 確實不在任何 DV 範圍內。上繳包 24 甚至寫明
「`C`／`E` 於母本無 DV、**無 x14 DV**」—— 連擴充命名空間都查了，結論正確。

**而全簿有 5 處 DV，其中 `R` 欄那處是 x14 的** ——
後來被填入一個清單外的值的，正是 `R`。

鏈條是這樣掉的：
**「`C`／`E` 無 DV」→ 記成「母本無 DV」→ 用來推論「`R` 也不受拘束」。**
每一步都只是把限定詞省掉一點，**沒有任何一步是說謊**。

**這與 PLAYBOOK (32)（誤引會沿用）同族而更隱蔽** ——
(32) 是引文變了，**本案是引文沒變而射程掉了**。
引文變了還能回原文比對；射程掉了，回原文比對**也是對的**。

**作法**：
- 陳述帶條件者，**把條件寫進句子本身**，不寫在上下文裡 ——
  「`C`／`E` 欄無標準 DV 亦無 x14 DV」比「母本無 DV」多五個字，射程就不會掉。
- 引用他人之結論時，**回去看它當時的範圍**，不要只看結論句。
- 台帳之陳述尤其要帶射程 —— **台帳是拿來被引用的**，
  而引用者不會回去讀你當時在講哪一欄。

**判準**：問**「這句話被單獨抄走，還是對的嗎？」**

**(44) 一個斷言若刪掉後該 TC 只剩不需任何需求即會發生之事，那句斷言就是驗證點本身，不能刪。**

出處：上繳包 31 §8（`remains operable` 之判定）。

六個中斷處理 TC 的 Final Step 是
「`Version_after equals Version_initial`；**the head unit remains operable**」。
後半查不到來源，於是「刪掉沒來源的那句」看起來是個乾淨的處置。

**拆開來看就知道不能刪**：
- 前半「更新未完成」是**中斷的後果** —— 拔掉電源更新當然不會完成，
  **任何中斷都會產生它，不需要任何需求**；
- 後半「系統未損毀」才是「偵測**並處理**該中斷條件」的**處理**那一面。

刪掉後半，六個 TC 就退化成**驗證物理定律**。

**通則**：一句斷言沒有來源時，有三條路 ——
找來源、開 DR、刪除。**三者的代價完全不對等**，
而「刪除」看起來最省事，所以最容易被選。

**判別**：把那句話遮掉，問**「剩下的部分，不寫這個需求也會發生嗎？」**
會 → 那句就是驗證點，刪它等於放棄這個 TC，**應掛 `PENDING` 走 DR**
（R-SU42 v2(e)），不是刪。

**推論**：「取不到來源」與「不該寫」是兩件事。
取不到來源可能意味著**上游漏寫了一條需求** ——
而那正是 DR 要問的，不是我們該替它決定的。

**(45) 兩個物件各覆蓋一半，拼起來是綜合不是引用 —— 二錨皆真實存在，而合成之命題無人主張過。**

出處：上繳包 32 §T50a-1（`4907673` 與 `4907440`）。

六列 ER 要斷言「中斷後 HU 仍可操作」。查得二個物件：

- `4907673`：`interrupts shall be gracefully handled so that **the OTA client**
  continues operation` —— **時點對**（中斷本身），**主詞不對**（client 非 HU）
- `4907440`：`OTA client … does not impact normal functionality of **the host system**`
  —— **主詞對**，**時點不對**（`when active`，正常路徑）

**各覆蓋一半，交集為空。** 把二者拼起來得出「中斷後 HU 仍可操作」，
**是綜合，不是引用。**

**其偽裝性極高**：二個錨都真實存在、都查得到、都與主題相關，
`reasoning` 寫上去看起來證據充分 ——
**而那個合成之命題，兩份文件都沒有主張過。**

**這比杜撰難抓**：杜撰可由「來源查無」揭穿；
**合成之每一個零件都查得到**，要揭穿它得逐一比對**射程**
（時點、主詞、情態、條件），而射程不會出現在 ObjectID 旁邊。

**作法**：引二個以上之來源支撐一個斷言時，**逐一列其射程並問交集** ——
`(時點, 主詞, 情態)` 三元組相同者方可合併；任一維不同即為二個不同之命題。
交集為空而斷言仍需要者：**那是一個 DR，不是一次引用。**

**判準**：問**「有沒有任何一份文件，單獨說過我要寫的這句話？」**
沒有 → 別寫，去問。

**(46) 涵蓋全部合規結果之斷言，與沒有斷言等價。**

出處：R-SU43(e)（下放包 38 §二）。

六列之判定核心 `Version_after equals Version_initial` 被查出可能誤判 ——
規格對「中斷落於 deployment 階段」建議 `Complete the deployment`，版本會變。

**看起來最省事的修法是把 ER 改成析取**：
「版本未變**或**版本已更新」。**這樣就不會誤判任何合規系統了。**

而它也不會抓到任何不合規系統 —— **因為那兩種情形窮盡了所有可能。**

**一個永遠通過的檢查，其資訊量為零**，但它在報告上與一個真正通過的檢查
**長得一模一樣** —— 這是 PLAYBOOK (30)／(41) 的第三種形態：
不是沒查，不是查不到，是**查了但那個查法不可能失敗**。

**正解是 R-SU43(f) 的降階**：斷不了的東西**降為記錄步驟**
（值仍留為證據），判定核心另尋；找不到就掛 `PENDING` 走 DR ——
**承認這件事現在驗不了，比寫一個永遠通過的 ER 誠實。**

**判準**：對任何 ER 問**「什麼樣的系統會讓這一條 fail？」**
答不出一個具體的系統行為，這條 ER 就沒有在驗東西。

---

## (47) 分析層直接編輯共用檔時，「改了什麼」與「怎麼提交」須在同一包

**來源**：下放包 57 §1.2 採納上繳包 56 §7(丁)；其成因為 `13a465f` 之打包偏差。
...
**其成因值得記住**：canon 之編輯發生在下放包落檔之前，
下放包才說「該單獨提交」—— 當 Pei 令 commit 時，
**工作區已有 canon 變更而下放包尚未被讀，執行層無從得知該變更須單獨提交。**

**併記一個限定（下放包 57 §1.1 採納）**：
> **「無可依之指示」指指示不存在或不可得，
> 不含「指示已在工作區而執行層未讀」** —— 否則它會**獎勵不讀**。

**執行層側之對應動作**：**收到 `git commit` 之令而工作區有自己沒動過的檔時，
先讀那些檔之來源包，再決定打包方式。**

> **已升格為 `FO §8.6-4`**（2026-08-30，下放包 58 §一）——
> 分析層之「三不」增第 4 項「**不先改共用檔再說它該怎麼提交**」。
> **本則於本 feature 保留**（同 FO §9 之升格慣例：來源條文於其 feature 全部保留，
> 各加註其升格去向）；**規則權威在 FO，本則為其來源紀錄。**
> ⚠ **`FO §8.6` 之標題仍寫「三不」而其下已有四項** —— 分析層明記待下輪改標題，
> **免其 `section_sha` 於同一日二次變動**。

---

## (48) 候選表之欄位須與其被用來做的決定相稱

**來源**：下放包 61 §三 #5 採納上繳包 60 §6(丁)；其成因為 F1／F2／F3 之分批
以一個「為說明而寫」的欄位為據，而該欄撐不起那個決定。
...
**判別之例**：**甲／非甲可信**（只問一件事實：首行是否為訊號）；
**「單一插入」不可信**（需要一個未做的計算）。

---

## (49) 「做了會很麻煩」是一個需要被量的命題

**來源**：下放包 67 §2.2 採納上繳包 66 §2 之自陳。

> **B-42 之共同點是「數字寫下時沒有回到產生它的那一步」；
> 本例更前一步 —— 我連那個數都沒去產生，就先估了它的後果。**

**其實例**：`PU` 編號「標題有而描述無」之掃描，我方稱其
「**最該做且最不想做 —— 它會回頭要求重檢已交付之列**」；
**實測該型僅 1 列，且其處置已正確，零列需重檢。**

**規範**：

```
凡以「其後果太大／太麻煩」為由延後或不做一項檢定者，
須先出該後果之量（其規模為何、影響幾列）。
在量它之前，「做了會很麻煩」與「不想做」不可分。
```

**併記分析層之對照**（下放包 67 §2.2）：分析層以「現在做最便宜」為由令其做 ——
**該理由同樣成立，而其成立與否本可先由一次掃描判定。二方皆在未量測時論成本。**
