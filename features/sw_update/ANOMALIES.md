...
## A-SU4 —— 需求物件全文之抽取停界吞併 Description —— **RESOLVED**
...
### 六、執行層之落地（T22a，2026-08-27）

語料 v2 已建，結果見 `docs/upstream/08_corpus_v2.md` §2。
既有之全部分數依處分「三」作廢，已於 T22b 全面重跑。

---

---

## A-SU5 —— T12 之 Description 歸屬受文件前置清單污染 —— **RESOLVED**

**登記時點**：T22a 之雙重閉合檢查失敗（實測 43/94，處分所據之 T12 為 45/92）。

### 一、事實

CFTS_57 於 `1.2 Introduction {4907233}` **之前**有一段前置區，內容為
一連串裸行 `Requirement ID nnnnnnn`（變更清單性質），其中相鄰三行為：

```
Requirement ID 4907907
Requirement ID 4907923
Requirement ID 4907934
```

T12 之歸屬法為「**首見**為準 + 游標於任一處遇需求物件 id 即移動」。
`4907923`／`4907934` 之**首見**即在此前置清單中，而其前一行恰為
`4907907`（一個需求物件），故游標將二者判歸 `4907907`。

**該前置清單不是宣告區** —— 二者之 `[Artifact Type:Description]` 宣告
在文件後段。以**宣告段位置**定歸屬之正確結果為：
...
A-SU4 處分 §二 載「併入之 **45** 個須逐一列出」—— 該數承自 T12，應為 **43**。
處分之**實質**（語料 = 自身全文 + 歸屬於該物件之 Description）不受影響，
**僅其數字須更正**。錨點池 574、Description 總數 137 **皆不受影響**
（分類與計數不用歸屬關係）。

### 三、為何 T22a 抓得到而 T12 抓不到

T12 只做了「正向分類 vs 反向對照」之計數閉合（137 = 137），
**該式對「歸屬給了誰」完全不敏感** —— 錯歸仍然閉合。
T22a 之閉合式把 43/94 拆開單列，才使其可見。

**此為 PLAYBOOK §7(2) 之第三次再現**：閉合檢查閉合的是它量的那個東西。
前二次量錯的是「文字」與「邊界」，本次量錯的是「歸屬」。
...
### 六、執行層之落地（T23b，2026-08-27）

依處分「二」，A-SU4 處分條文之正文已改為 43／94，舊值 45／92 記於
該條文之**更正註**中（不留於正文）。diff 見 `docs/upstream/09_umbrella_scan.md` §3。

依處分「四」，T12「首見為準」歸屬法**全面失效**之落實範圍：
本 feature 以首見定歸屬之產出僅 `ANCHOR_POOL.md` §六 一處（Description 137 筆），
已全部以宣告段位置重驗。`ANCHOR_POOL.md` §一～§五之**分類**（非歸屬）
採「宣告優先於文序」，不受本項影響。
