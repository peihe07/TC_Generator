...
| **DR-SU6** | **能力型／全稱型需求之單次台架判準** —— 該類需求所述為一項**能力**（`prevent … from becoming permanently disabled`／`identical to the reference deployment image`），**而一次執行不能證實一個全稱命題**。若將 ER 寫為其單次表現（「更新後系統仍可開機」），**該 ER 永遠通過** —— 即「查了，但那個查法不可能失敗」（PLAYBOOK (41)／(46)）。**請求**：該類需求於**單次台架執行**中之通過判準為何（如：指定之測試序列、樣本數、注入條件之集合） | **OPEN**｜**Medium** | **6 列**：`375`／`379`／`382`（後半）＋ **batch 14 增 `195`（不得依賴特定匯流排實作）／`198`（`no unintended DTCs`，且 `unintended` 之界未載）／`212`（HMI 可移植性）** | 阻斷 6 列，不阻斷其餘 | — | **Medium** |
...
| **(d) 已確認・第四型** | **看得到但弄不出來** —— 觸發手段不可得。所求為**觸發手段** | **2 列**（`315`／`318`） |
