...
| **DR-SU3** | **統攝型需求之驗證併入確認** —— `SWE1-FOTA-313`（統攝 `315`–`320`）、`SWE1-FOTA-327`（統攝 `328`／`329`）之驗證得否併入其所統攝各列 | **OPEN**｜**Medium**（不阻斷其餘各列） | `313`／`327` 二列 | `newR1L-SU-017` 三欄各掛一 `PENDING`；`327` 之 TC 未起草 | — | **Medium** |
| **DR-SU4** | **中斷處理之判定準據** —— (1) `4907673` 之 `however` 子句要求 `interrupts shall be gracefully handled so that **the OTA client** continues operation`，惟**其於 HU 上之可觀測表徵未載**；(2) `Table 4-6` 依中斷所落之階段給不同要求（含 `Complete the deployment`），而**階段界線於 HU 外部無可觀測之表徵** | **OPEN**｜**High** —— **阻斷 batch 2a 全部六列** | `315`–`320`（6 列） | `newR1L-SU-011`–`016` 之判定核心全部掛 `PENDING`；**本批無一列可交付** | — | **High** |
...
| **第四型**（**R-SU39**） | **觸發手段不可得** —— 外部後果**可觀測**（版本未變、HU 可操作），而測試者**無法使該條件發生**：`315` 需 socket 層之錯誤注入、`318` 需事故偵測訊號而本 feature 未綁 DBC | **觸發手段** —— 注入工具、模擬訊號、測試模式。**不是觀測手段** | `315`／`318`（2 列） |
