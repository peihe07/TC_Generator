...
|---|---|---|---|---|---|---|
| **DR-SU2** | **v2（縮編，下放包 29 §二）** —— **(a)** Error Code 之**顯示途徑**確認；**(b)** Wi-Fi FOTA session 之**正向狀態**觀測；**(c)** **第三型之區辨手段**（`179`／`181`）。「106 列全部之觀測手段」**已不再請求** —— 其負向半由 `Error_Code_List.xlsx` 覆蓋（R-SU35） | **OPEN**｜**第二型 5 / 106**；**第三型 2 列（母群未經盤點）** | 第二型 5 列（`363`–`367`）＋第三型 2 列（`179`／`181`） | 三段式台帳（R-SU32 v2(f)）——**(b) 段之 106 僅對第二型有意義，不得冒充全體上界** | — | **High** |
...
| **(c) 已確認・第三型** | 有外部後果但**不可區辨**，或限定詞**不可量**。所求為**區辨手段** | **2 列**（`179`／`181`） |

**確認進度之記法**：**第二型 5 / 106**；**第三型 2 列，母群未知**。
