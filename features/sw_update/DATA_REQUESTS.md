...
|---|---|---|---|---|---|---|
| **DR-SU2** | **106 列（R-SU30 v2(f)：語形條件聯集人裁）於系統測層級之觀測手段／區辨手段** —— **第二型**（無觀測面）pattern：log tag 清單／診斷 DID 表／服務介面定義／HMI 間接後果之對照；**第三型**（與鄰列不可區辨）pattern：下載請求已發出之可觀測跡象，或上游確認得併入鄰列 | **OPEN**｜**確認進度 6 / 106** | **已確認段 6 列** —— 第二型 5 列（`363`–`367`，`Telematics Client`）＋**第三型 1 列**（`179`） | **未確認母群 106 列**（含該 6 列，佔驗證母體 34%）—— **尚未逐列判定**；滾動增列 | — | **High** |
...
| 段 | 定義（R-SU30 v1(a)(b)、v2(f)） | 現況 |
|---|---|---:|
| **(a) 已確認段** | 該列已由分析層**逐列判定**，其外部可觀測後果**取不到或不可區辨**。**此段之列方為 DR 之實際標的** | **6 列**（`363`–`367`＋`179`） |
| **(b) 未確認之母群** | **R-SU30 v2(f)**：「符合語形條件者」**聯集**「已由人裁判定為無觀測面者」 | **106 列**（含 (a) 之 6 列） |
| **確認進度**（R-SU30(d)） | (a) / (b) | **6 / 106（6%）** |
