...
TCS = [
 ...
 tc("SWE1-FOTA-375", "CFTS057-4907534", FN, "P1", "低",
  ...
  [B, WIFI, PKG,
   "PENDING: DR-SU6 criterion by which the installed image is judged identical to the reference image in one bench run",
   "PENDING: DR-SU6 means of obtaining the reference deployment image for the same target version"],
  ["1. Trigger an update availability check to the OTA Server, accept the update and let it complete",
   "2. PENDING: DR-SU6 step to compare the installed software image with the reference deployment image"],
  ["1. The update completes on the head unit",
   "2. PENDING: DR-SU6 observable evidence that the installed image is identical to the reference deployment image"]),
 ...
 tc("SWE1-FOTA-379", "CFTS057-4907527", FN, "P1", "低",
  ...
  [B, WIFI, PKG,
   "PENDING: DR-SU6 criterion by which the safety mechanism is judged present in one bench run"],
  ["1. Trigger an update availability check to the OTA Server, accept the update and let the installation run",
   "2. PENDING: DR-SU6 step to exercise the safety mechanism that prevents the SOC from being permanently disabled"],
  ["1. The installation runs on the head unit",
   "2. PENDING: DR-SU6 observable evidence that the safety mechanism prevented the SOC from being permanently disabled"]),
 ...
 tc("SWE1-FOTA-382", "CFTS057-4907517", NEG, "P1", "低",
  ...
  [B, WIFI, "A differential update package is staged on the OTA Server for this head unit",
   "PENDING: DR-SU6 criterion by which the resulting image is judged to match the target image in one bench run"],
  ["1. Trigger an update availability check to the OTA Server, accept the update and let the differential update be applied",
   "2. PENDING: DR-SU6 step to read the integrity information validated for the resulting firmware image"],
  ["1. The differential update is applied on the head unit",
   "2. PENDING: DR-SU6 observable evidence that the resulting firmware image was validated against the target image"]),
