...
- 章號取 `sec.split(".")[0]`，章 11 之次級判準取 `k[1] <= 6`。
  ...
  **屆時應歸 #4**。此規則對未來之補節不成立，已記於此。~~
  ...
  > **本項已撤銷 —— 見下放包 06 §二（T39 改記）。**
  > 分析層以權威複本（SYS1 `Description`，SHA256 `1fcc8711…b091d6`）
  > 實測 11.9 群與 12.3 群之內容：**11.9 是觸控之通則，12.3 是同一能力之
  > 具體規格（並增旋鈕路徑）**，二者之驗證標的同為「使用者對一個設定列
  > 做出動作後發生什麼」，非 #4 之標的（設定項本身做什麼）。
  >
  > **裁定：11.9 群歸 Test Set #5，非 #4。分組規則 `k[1] <= 6` 於此
  > 輸出正確，不需修改。** 本輪之語意判讀（「屬行為而非呈現」）**判錯** ——
  > 錯在只讀章節標題之字面（「General logic for setting with options」），
  > 未讀其 `Description` 全文。與 T24 之偽陽性風險同源。
  >
  > 該裁定為**條件性預裁**：11.9 群現為 037 零涵蓋，僅當 DR-VC3 回覆
  > 「應補」而該群進入範圍時生效。連帶之行為重疊已登記 A-VC12。
