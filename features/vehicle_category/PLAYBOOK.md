...
> **⚠ 更正（下放包 33 §三，T171）** —— 上繳包 32 §1 之
> 「本節現為第五條紀律類條文，**五條皆無承載者**」**不正確**。
> **§7.6 有承載者**（`scripts/ledger_xref.py`，經 R-VC29 指定）。
> **正確之數為：六節中 §7.6 有承載者，§7.1／§7.3／§7.4／§7.5 無。**
> （§7.1.1 為 §7.1 之形態要求，其承載者即各掃描器之 self-test 前置。）
>
> **本更正之由來值得記**：我寫「五條皆無承載者」時，
> **沒有去查有沒有現成工具能承載它們** —— 與 §7.4 所禁者同型
> （看到「這是紀律」這個表徵，就下「無承載者」之判斷）。
> **日後若再有同型需求，應先問「有沒有現成的工具能承載它」，而非再寫一條**
> （下放包 33 §三末段）。
...
#### 7.3.1 附註：方案評估時先掃他 feature 之 `scripts/`（下放包 36 §1.2）

```
方案評估時，先掃他 feature 之 `scripts/` 有無同題之既有實作。
```

**由來**：上繳包 27 §3.4(g.3) 記「丙案未實測…本包不主張它可行」，
而 `features/display/scripts/write_back_036.py` **早已實作、跑過、
寫進該 feature 之交付台帳**（zip 成員 48 → 48，逐項相等）。

**「答案已在 repo 裡」之第三次**：
DR-VC6 之實測佐證欄（A-VC20）、R-VC19 之加註（T168）、本次之 display 實作。
**前二次在台帳**（`ledger_xref` 已管）；**本次在跨 feature 之 `scripts/`**
—— 已知盲區之又一段。

**不造工具、不開任務**（下放包 36 §1.2 明文）——
...
### 7.6 條文修訂後之波及檢查，**先查對外文件**（下放包 32 §1.3）
...
**本次之順序恰好相反：先修了不出門的，漏了要出門的。**
R-VC27 修了 `RULINGS.md`（條文）與 `data/tableB_draft.md`（產出草稿），
**未修 `DATA_REQUESTS.md` 之 DR-VC3「發出前須知」段** ——
而該段正在發送中，且明文指示該 DR 之措辭該怎麼寫。

**判準之第一序位是「該處會不會被送出去」**：
DR 是唯一會離開這個 repo 的東西；條文與表 B 錯了是內部問題，
**DR 錯了是對外發出**。

本 feature 之對外文件清單（波及檢查時逐一查）：
...
**⚠ 本節之承載者為 `scripts/ledger_xref.py`，見 R-VC29**（下放包 33 §三，T171）。

上繳包 32 §1 曾記「本節現為第五條紀律類條文，**五條皆無承載者**」——
**那句是錯的**：`ledger_xref` 之本職正是列出一個標的在全部台帳中之提及處，
R-VC27 修訂時若跑一次 `ledger_xref R-VC12`，
**`DATA_REQUESTS.md` 之 DR-VC3 那一段會直接列在輸出裡**。

工具在 T155 造好（下放包 29），而 R-VC27 在 T147（下放包 28）——
**工具晚一包。這一次是時序，不是缺件；此後不是。**

R-VC29 使本節自「紀律」轉為**有承載者之要求**：
未查即修訂者，其修訂**視為未完成**。
§7.1／§7.3／§7.4／§7.5 仍為紀律，無承載者。

### 7.7 編號包之 NN 檢查為**二項**（下放包 35 §一）
...
**由來**：下放包 33 落檔而未被交付執行，34 之 NN 檢查看到 `33_` 存在即通過。
**號碼碰撞抓得到，號碼跳過抓不到** —— 前者是同一號被寫二次，
後者是前一號沒人做。

**根因在傳遞層**：handoff **落檔 ≠ 執行層收到**，那一段機器看不到。
第 (2) 項之所以有效，是因為**上繳包只有執行層會寫** ——
它存在即證明該包被執行過。

**執行層側之對應動作**：收到編號包時，若其前一主號之上繳包不存在，
**先回報「前包疑未執行」再動手** —— 本 feature 於下放包 34 即應如此，
實際是做到一半才發現（`RESUME_PLAN.md` 不存在、R-VC29 未抄錄）。

**本節有承載者之一半**：`list_directory` 可查二項，
