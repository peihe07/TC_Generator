...
# `§n` 之前若有 `IN`／`FO`／`PLAYBOOK`／`§8.2.1` 之類的擁有者，那是 **canon 引用**
# 而非本 feature 之規格節。二者混在一起會使候選表充滿 `§3`／`§7` 之噪音。
# **本檔不剔除它們** —— 只多記一欄 `owner`，讓讀表者自己分。
# 多給資訊不是替人判斷；剔除才是。
...
def main():
    ...
    with OUT_CAND.open("w", encoding="utf-8") as fh:
        ...
        for (kind, key), rows in sorted(
            ...
            # `bare` = 該標的**有無**不帶擁有者之提及。全部帶擁有者者，
            # 幾乎必為 canon 引用（`IN §7`），非本 feature 之規格節。
            bare = "yes" if (kind != "section" or
                             any(not r[3] for r in rows)) else "no"
    ...
    bare_n = sum(1 for (k, _), v in cand.items()
                 if k != "section" or any(not r[3] for r in v))
    print(f"**跨 ≥2 個台帳之候選 {len(cand)} 個** → "
          f"{OUT_CAND.relative_to(ROOT)}")
    print(f"  其中 `bare_section=yes`（至少一處為裸 §，非 canon 引用）"
          f"{bare_n} 個；全為 canon 引用者 {len(cand) - bare_n} 個")
    print("\n**本檔不判斷是否牴觸** —— 只並列。判斷由人做。")
    return 0
