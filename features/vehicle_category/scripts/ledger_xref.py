...
**三段之分工與缺口（下放包 32 §3.3，T167）**：

  跨檔          —— 本工具 ＋ `xref_triage`（DR-VC3 之真牴觸即由此抓到）
  同檔、同條目內 —— PLAYBOOK §7.5（加註時複核該條各款），紀律，無承載者
  同檔、跨條目   —— **無人管，已知第三段盲區**

第三段之例：`RULINGS.md` 內某二個條目互相牴觸，而非同一條之二款。
下放包 31 §4.1 已裁不做第三判準（同檔提及絕大多數為正常，
拿掉跨檔要求即爆量，新增真陽性率無從預估）——**該裁定不變**；
此處補記其為盲區，因前記僅提「同條目內」而漏了「跨條目」。
...
# ⚠ **已知殘餘（T168）**：擁有者表**不含 `framework`／`profile` 等文件名形態**。
# 故 `framework.md §2`、`profile §5.1` 之類仍被當成本 feature 之規格節 ——
# 上繳包 31 §2 之 `§2` 一項即由此而來（判定為噪音）。
# **不另輪修（下放包 33 §二裁定）**：`§2` 已判為噪音、成因已知，
# 而 9 個標的已判定完畢 —— **修它的收益在未來使用，當前無使用場景**。
# 處置同「第一層不擴充」（下放包 25 §2.2）：
# **已知殘餘 —— `framework`／`profile` 形態未納入擁有者表。**
# 若本工具再被使用（DR 回覆後之重審、或他 feature 沿用），屆時一併修。
PATS = {
