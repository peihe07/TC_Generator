...
- **本欄正式列為 A-VC20 之成因補充（下放包 29 §四，T156）**：
  A-VC20 原記其成因為「R-VC12 二(a) 是用它自己所禁之方法立的」；
  本欄使該成因**更精確** —— 不只是「用衍生 PDF 寫的」，
  而是**同期另有一處用權威素材寫對了，二者並存 24 個包而無人比對**。

  | | 寫於 | 內容 | 來源 |
  |---|---|---|---|
  | **正解** | 本欄（T17）| §10.1／10.2 存有二句文字 ＋ 圖佔位 | SYS1 `Description` |
  | **錯解** | R-VC12 二(a)、表 B | 「SYS1 匯出未帶文字」| 衍生 PDF 之判讀 |

  **能攔下它的不是「再測一次」，是「把同一標的的所有記載並列」** ——
  T155 之 `scripts/ledger_xref.py` 即為此而作，本欄為其
  self-test 之已知標的（§10.1 應同時列出本欄與 R-VC12 二(a) 二處）。
