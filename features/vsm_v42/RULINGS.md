...
Feature slug：`vsm_v42`。條號系列 **`R-VL`**（本線單一來源，R-G62 live 取號）。
...
### R-VL28 —— b1 出貨收案：S6 豁免之補記；TC_ID_RE 衝突處置；leaf_ids 緩宣告採認

```
R-VL28（2026-09-02；Pei 以出貨行為實裁，條文分析層補記使台帳自洽）
(a) S6 豁免（甲案）：b1 帶 6 格 PENDING（-058／-059／-060，錨 DR-VL4）出貨 ——
    Remarks 逐列揭露＋DELIVERY_NOTE 六節載明；DR-VL4 回覆後以 Revise2 補寫（PM 線既有模式）。
(b) 交付收案：…_SWQT_VehicleSetupManagementR1Low_20260902.xlsx，sha256 abc7f8ae…
    與候選逐位元相等，三處落檔一致；同名不覆寫斷言採認。128 之 17 部分交付，餘 111 leaf 待續批。
(c) TC_ID_RE 衝突：R-VL3（NR1L-VSM42-{nnn}）維持不改 —— 純字母 VSM 將與 vsm_v43
    （NR1L-VSM43，R-VT3）撞名，分析層實查確撞；正確修法為共用 lint 正規式放寬
    `^NR1L-([A-Za-z][A-Za-z0-9]*)-(\d{3})$`（字母起始、後允數字）。
    入共用腳本一裁清單（第 7 項，Pei）；裁前該 1 紅恆存，歸因已載，各包一句帶過。
(d) delivery.leaf_ids 緩宣告採認（部分交付將誤紅 111 次；全批交付時宣告，理由已註 feature.yaml）。
(e) 執行層自揭 gate 轉紅並補 3／4、兩造皆不自動手之處置，採認為正確行為模式。
```

### R-VL29 —— 上繳 13(b)＋14 合裁：綠色通道重計；-063／K-9／K-11 處置；出貨模式待 Pei

```
R-VL29（分析層裁定 2026-09-02，上繳 13(b)＋14）
(a) 上繳 14（Pei「先 b2」直裁之無下放包作業）採認：列序判準攔下 append、依 037 leaf 序排整本、
    三斷言＋557 格回讀 0＋列序四道＋lint 淨紅 0，皆成立。取號 14 採認（13 為 ship 報告佔用）。
(b) 綠色通道重計：b2-PS 於寫回輪被 lint 拓出 J=2／R=1 之內容層修訂（-013／-020／-021），
    R-VL27(e) 之 1／3 **撤銷**；重計自 b2-2 Camera 起 —— 該批預檢含補強後全部判準且零修訂，
    **計 1／3**；-063 之 BLOCKED 列依規則正確寫成 PENDING，不計修訂（採執行層判）。
(c) J／R 修訂採認：大寫正規化合 R-4；-013 改功能可用性表述合 §8.4.1（037 未指名 PROXI 值）。
    預檢不足第三次實證 → 生成端改呼叫 lint036 判準模組之提議支持，入共用腳本一裁清單第 8 項（Pei）。
    第 7 項（TC_ID_RE）已由 c5f471b 結案，銷項。
(d) K-10（-063）：維持於母體（128 不改；母體定義屬 R-VL4，上游分解權威 §8.2 不單方縮），
BLOCKED 列照寫（privacy R34-3 先例）；併入 DR-VL2(c) 詢問。
(e) K-11：R-VL18(b) 之「疑 DBC 側誤」推定修正為「上游命名不一致（037 內 IPC 側 Guidelines／
    TELEMATIC 側 Gridlines 並存）」（加註不刪）；處置不變（保留原名不加 $）；入 DR-VL2 佐證。
(f) K-9（-013 vs -021）：維持兩條 —— 驗證對象不同層級（單項 Volume 隱藏 vs 總選單隱藏），
    非 §10.6 嚴格等價；恆真合取之發現入 DR-VL2 佐證（疑贅列，同 K-7 型）。
(g) DELIVERY_NOTE 第五節過時段：已交付件附隨文件為歷史快照，不回改；次次交付之 NOTE 更正。
(h) 出貨模式 A／C 待 Pei（B 撞檔名不可行）；分析層建議 **C** —— 往後每批插入（Camera 之 leaf 序
    #24–27／#52–57 即插於 PS 與 EPB 之間）都位移 EPB，A 案將每批重發新 ID；C 案只重編一次，
    全 128 完成後出累積簿＋舊→新 ID 對映表（PM 站 ⑤ 先例）＋NOTE v2。裁前交付凍於 b1 舊件。
```
