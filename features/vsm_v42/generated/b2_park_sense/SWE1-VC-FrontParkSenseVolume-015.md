# SWE1-VC-FrontParkSenseVolume-015

- Test Group: Vehicle Setup Management R1 Low
- Test Set: Park Sense
- TC 數: 1

**reasoning**：驗證 PROXI 之 CAN node 24 (PAM) 為 Present 且 PAM_Configuration 為 1 (Front And Rear) 時，TLM 顯示 Front Park Sense Volume 選單項且使用者可設定。關鍵情境為兩個 PROXI 參數之組合條件（規格段 1230–1231）。本 Req 只述可用之分支，1 條即足；不可用之分支由同族之不支援條覆蓋（§8.2.1 委任）。PAM_Configuration 之段 1 命中 PROXI Format r516 逐字（0 = Rear／1 = Front And Rear），依 R-P375(c) 走 PROXI 路徑不加 $。

## TC 1 — Front Park Sense Volume menu shown for the matching PAM configuration

### test_item
```
If $CAN node 24 (PAM)$ = [Present], the TLM shall display the corresponding customer setting and the user can perform the setting
(Front volume menu available only when the PAM configuration covers front and rear)
```

### pre_conditions
1. PROXI CAN node 24 (PAM) = 1 (Present)
2. PROXI PAM_Configuration = 1 (Front And Rear)
...
### test_procedure
1. Read the named UI element "Vehicle Settings" menu list and check that it contains "Front Park Sense Volume"
2. Select "Front Park Sense Volume" in the "Vehicle Settings" menu list
3. Read the named UI element "Front Park Sense Volume" control and check that it is enabled

### expected_result
1. The named UI element "Front Park Sense Volume" is present in the "Vehicle Settings" menu list
...
- priority: P2
- split_flag: False
- distinguishing_axis: input_data — PAM_Configuration = 1 (Front And Rear) 單值；與 -008 之 037 文字逐字相同（§K K-7），本條為 Front 側且條件較嚴
