# SWE1-VC-PARKSENSEw/oHC.1andPARKSENSEw/oHC.2-002

- Test Group: Vehicle Setup Management R1 Low
- Test Set: Park Sense
- TC 數: 1

**reasoning**：驗證 PROXI 參數 CAN node 24 (PAM) 為 Present 時，TLM 顯示 Park Sense Setting 選單項且使用者可設定。關鍵情境為 PROXI 組態分支，屬功能可用性之閘門（規格段 1203–1204）。本 Req 只述單一 PROXI 值之結果，1 條即足；不支援之對偶由 -021 覆蓋（§8.2.1 委任）。本條與 -006 之 037 Description 逐字相同，依 §8.2.2 各出一條不合併，括號下半區分（§K K-7）。
...
(PROXI Present branch: Park Sense Setting menu item shown and operable)
```
...
- priority: P2
- split_flag: False
- distinguishing_axis: input_data — PROXI CAN node 24 (PAM) = 1 (Present)；與 -006 之 037 文字逐字相同，見 §K K-7，本條為該對之第一列
