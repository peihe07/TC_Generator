...
| 4 | Templates can be created and reused in subsequent runs | ⚠️ Partial | Reuse: `/templates/[id]` `Use in New Run` CTA → builder pre-fills `draft.configure.templateId`; backend persists via `GenerateRequest.templateId`. Usage analytics live (`GET /api/spec-library/{name}/usage`). Deprecate flag shipped (`PATCH /api/spec-library/{name}` + `template_save` event + `Show deprecated` toggle in `/templates`). Clone flow shipped (`POST /api/spec-library/{name}/clone` + Clone button on detail page; reuses source / embedding cache, stamps `clonedFrom`). **Authoring brand-new templates from scratch is deferred** — embedding ingestion pipeline still missing. |
...
- **Experiment promote / resume** — `setExperimentDecision` persists a winner under `tc:experiments:decisions:v1`; `getExperimentAssignment` short-circuits to the winner with `source="concluded"`; Settings panel exposes Promote/Resume controls per variant.
- **Hard isolation (S3)** — `WorkspaceJobStoreRouter` lazily builds per-workspace `SqliteJobStore` at `output/.workspaces/<ws>/jobs.db` (override via `TC_WORKSPACE_ROOT`); HTTP middleware binds `X-Workspace-Id` to a ContextVar for the request; `telemetry_log_path()` lands per-workspace `events.jsonl`; `safe_workspace_segment` blocks `..` / `.` traversal.
- **Template clone** — `POST /api/spec-library/{name}/clone` plus a `CloneButton` on the detail page; new entry stamps `clonedFrom`, version reset to `1.0.0`, opener changelog entry; reuses the origin's source file and content-keyed embedding cache.

## Open follow-ups

- Authoring brand-new templates from scratch — needs an embedding ingestion pipeline. Clone (reusing existing source + cache) is shipped; deprecate is shipped; changelog endpoint patches existing manifest entries.
- Hard isolation S3 partial: per-workspace `jobs.db` and `events.jsonl` shipped; per-workspace authentication still out of scope (this is a localhost dev tool).
