...
Run: python archive/M1/spec_coverage_verify.py
Output: archive/M1/spec_coverage_gaps_final.json + console summary.
"""
...
def main() -> None:
    rows = json.loads((ROOT / "archive/M1/spec_coverage_player.json").read_text(encoding="utf-8"))
    bucket2 = [r for r in rows if not r["cited_by"] and r["req_covered"]]
    bucket3 = [r for r in rows if not r["req_covered"]]  # confident gaps
    reqs = load_swe1_reqs(str(ROOT / "archive/M1/swe1_pla_reqs.json"))
    ...
    (ROOT / "archive/M1/spec_coverage_gaps_final.json").write_text(
