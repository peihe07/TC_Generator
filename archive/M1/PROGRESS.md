...
4. **Stage 3 深拆示範**:PLA-030 Repeat 覆蓋 ~3 → 11 情境、0 幻覺(`archive/M1/stage3_demo_repeat.md`)。
...
- Player domain pack:`archive/M1/domain_pack_player.json`(Gate ① 簽核)
- Baseline / 驗證:`archive/M1/baseline_player*/`、`archive/M1/stage3_demo_repeat.md`、`archive/M1/spec_coverage_*`
- 生成實驗:`output/player_gen_{all,repeat,shuffle}/`(生成 TC + audit/rereview scorecards)
- 規劃文件:`docs/dev/PIPELINE_DESIGN.md`、`archive/M1/EXECUTION_SPEC.md`、`archive/M1/{M0,M2}_NOTES.md`
...
- 主路徑互動式:`review_workbook(..., domain_pack_path=...)` 已可用;CLI `python backend/main.py --review --input X.xlsx --domain-pack archive/M1/domain_pack_player.json`。
