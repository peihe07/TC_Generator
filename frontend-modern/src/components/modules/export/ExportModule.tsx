...
const ExportModule: React.FC = () => {
  ...
  return (
    <div className="modern-workflow-page modern-export-page flex flex-col h-full gap-4">
      {!exportComplete ? (
        <>
          <div className="modern-export-layout flex-1 min-h-0">
            <section className="modern-export-summary">
              <div>
                <span>Rows</span>
              ...
              <div>
                <span>Workbook</span>
                <strong>{jobMetadata?.projectName || 'results'}</strong>
              </div>
            </section>

            <section className="modern-configure-section modern-export-scope">
              <div className="modern-configure-section-head">
                <p className="font-bold text-xs uppercase">Export Scope</p>
                <span>{scope === 'all' ? `${tcRows.length} rows` : `${acceptedCount} accepted`}</span>
              </div>
              <div className="modern-export-scope-grid">
                <label className="modern-export-scope-card" data-active={scope === 'all'} htmlFor="scope-all">
                  <Radio
                    id="scope-all"
                    name="scope"
                    label={null}
                  ...
                  />
                  <strong>All Generated Cases</strong>
                  <span>{tcRows.length} row{tcRows.length === 1 ? '' : 's'}</span>
                </label>
                <label className="modern-export-scope-card" data-active={scope === 'accepted'} htmlFor="scope-accepted">
                  <Radio
                    id="scope-accepted"
                    name="scope"
                    label={null}
                    ...
                    wrapperClassName="modern-options-radio"
                  />
                  <strong>Accepted Only</strong>
                  <span>{acceptedCount} approved row{acceptedCount === 1 ? '' : 's'}</span>
                </label>
              </div>
            </section>

            <section className="modern-configure-section modern-export-settings">
              <div className="modern-configure-section-head">
                <p className="font-bold text-xs uppercase">Output Settings</p>
                <span>{selectedColumns.length} columns</span>
              </div>
              <div className="modern-export-option-grid">
                <Checkbox
                  id="inc-steps"
                  label="Include Steps"
            ...
            <div className="modern-export-file-preview status-bar-field p-3 text-xs leading-relaxed border-sunken">
              <RiSettings4Line className="size-4 inline mr-2 mb-1" />
              <span>Final file</span>
              <strong className="font-mono">{jobMetadata?.projectName || 'results'}_generated.xlsx</strong>
            </div>
          </div>

          <div
            className="modern-workflow-bottom-bar modern-export-action-bar flex justify-between items-center pt-4"
          >
        ...
        <div className="modern-export-success flex-1 flex flex-col items-center justify-center gap-6 py-8 text-center">
          <div className="modern-export-success-icon">
            <RiCheckDoubleLine size={36} style={{ color: 'var(--palette-teal)' }} />
          </div>
          <div>
