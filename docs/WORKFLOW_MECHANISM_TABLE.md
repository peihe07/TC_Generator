...
| Configure: Regroup All | 按 `Regroup All` | `fetchGroupingPreview(forceRegroup=true)` | `POST /api/group` | `POST /api/group` | 全部 row 重新分類；既有 Test Set 當 hint | `classify_test_sets`；model 固定 `gpt-5-mini`；Projection 可走 deterministic fallback | 回 preview；AI 漏 row 時 fallback 依 [TEST_SET_POLICY.md](TEST_SET_POLICY.md)，避免 `Unclassified` 成為正式分類 |
