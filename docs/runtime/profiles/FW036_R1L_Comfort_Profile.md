...
- **generator 凍結（R-C59，CMF-02_review §2，2026-09-21）**：`features/comfort/generated/*.json`
  ...
  那些改動且不留痕跡。十七支入口皆加 guard：無 `--i-know-this-overwrites-cmf` 即
  `SystemExit`。`test_item.py` 之 `HAND_WRITTEN` 為 generator 時代之舊值，已加註
  「已凍結，勿據此重產」；現行下半一律以 `generated/*.json` 為準
...
### 1.1 已認可外部文件（R-C45，下放包 81 §1.1，2026-08-16）

**未列於本表者不得引用。** 認可**不等於納入基線** —— 本表各件皆不進
...
| `HMI Read Me` | **不是** 12.6 所指之 `HMI Notes`（`Auto Comfort` 0 命中）|
| `PDO Graphics Release` | 我方所有者為**釋出封面**（文字層 489 字元），非 icon 對照表 |

## 2. Test Set vocabulary [OVERRIDE — 取代泛用自由標籤]

- ~~Test Group（G 欄）= `Comfort`~~ → **`Climate Control Interface`**（R-C6(amend)）；Test Set（H 欄）取 framework Part N 之
...
> **上半 > 50 token 之摘句，與其首字母（R-C50／R-C50(amend)）** ——
> R-C50（CMF-01_review §3）：上半 > 50 token 之 26 條摘為該句之**連續子字串** ≤ 50 token，
> 取與下半直接相關之段。
>
> R-C50(amend)（CMF-02_review §2，2026-09-21）：摘句自句中段起者必以小寫開頭，
> 而 canon **R-4** 判「句首轉大寫」屬**排版正規化** —— 故**摘句之首字母轉大寫**，
> `test-item-upper-verbatim` gate 比對時對**首字母一字**做大小寫正規化，**其餘逐字**。
> 實測受影響 14 條（CMF-02 候選本之 lint036 `J`＝14 → CMF-03 候選本 `J`＝0）。
> gate 之實作為 `lint_tcs.upper_matches()`，其**負向測試**（首字母以外之差異仍 FAIL）
> 在 `tests/test_comfort_upper_verbatim.py`。
...
> assertion 逐格驗其未外露。
>
> **現行匯出規則（CMF-02 §1-2，2026-09-18 同步；上方刪除線部分即其所取代者）**：
> J 欄**不帶節次括號** —— Pei 2026-08-17 自行編修交付件時移除（DELIVERY ENTRY 032），
> 之後由 ENTRY 034 併入寫入路徑（`write_back.render()` 之 `SECTION_BRACKET`）。
> JSON 內仍保留節次括號（R-C29 之出處），**只是不寫入 J 欄**；條件之出處節次於工作簿內
> 改由 N 欄承載（R-C49：PC 出處節次續載）。0907 本之 J 欄即此形，**非離線偏離**
> （A-CMF1：profile 落後於 pipeline）。
...
> **市場／變體軸之交付範圍（Pei 裁定，2026-08-17）—— 記於此以免日後重問**
>
> **Maserati (`Seat & Wheel`) 不在 R1LR ATL-H 之交付範圍。**
>
> `2.1`（R1C1）之條文寫 `Seats (WS or R1 Low) or Seat & Wheel (Maserati)`，
> 即該 tab 之名稱隨變體而異。**本交付不產該變體之標籤 TC**（§8.7.3 於此
> 不適用），`2.1` 維持 4 條，**tab 名稱一律寫 `Seats`**。
>
> **`test_item` 上半仍會出現 `Seat & Wheel (Maserati)`** —— 該欄上半為條文
> 原文逐字（§3.1.1），而 ER／PC／`tc_title` 只寫本交付所驗者。
> **此非規則衝突，是兩欄各司其職**（98 §2）。
>
> **本註之作用是使「Maserati 要不要做」不必再問一次** ——
> 下放包 97 §2.1 曾以此為停下之理由，而該問題已有答案。
...
sections: 9.1 | 9.2 | 9.3 | 9.4 | 9.4.1
disposition: deferred: DR #41
why: 正向值於 `9.1` 逐字，`9.2`／`9.4` 以「in these variants」回指；
     **`9.1` 自身之列於 97 §2.5 補產後，該節亦成為本候選之使用者**——
     此前七條引該句而該句自身之 leaf 無列，候選之 reach 因而少算一節；
...
negation-users: NR1L-ComfortHMI-019, NR1L-ComfortHMI-035, NR1L-ComfortHMI-053, NR1L-ComfortHMI-054, NR1L-ComfortHMI-085, NR1L-ComfortHMI-103, NR1L-ComfortHMI-104, NR1L-ComfortHMI-105, NR1L-ComfortHMI-119, NR1L-ComfortHMI-122, NR1L-ComfortHMI-149, NR1L-ComfortHMI-150, NR1L-ComfortHMI-151, NR1L-ComfortHMI-223, NR1L-ComfortHMI-224, NR1L-ComfortHMI-230, NR1L-ComfortHMI-241, NR1L-ComfortHMI-283, NR1L-ComfortHMI-293, NR1L-ComfortHMI-294, NR1L-ComfortHMI-295, NR1L-ComfortHMI-403, NR1L-ComfortHMI-430, NR1L-ComfortHMI-431, NR1L-ComfortHMI-432, NR1L-ComfortHMI-433, NR1L-ComfortHMI-471
...
> **工作簿 N 欄之呈現（R-C49，CMF-01_review §3，2026-09-18）** —— 上段為 **JSON 之格式**
> （`spec-ref-stem` gate 所驗，不變）；寫入 N 欄時：
>
> - ~~stem 為上列全名~~ → **stem 去 `SYS1_` 前綴**：`HMI_Comfort_HMI_Logic_and_Flow_R1_SR24_Post_3A_CR24879_(September_25_2023)`。
>   出處為 Pei 2026-08-17 之交付件編修（DELIVERY **ENTRY 032**），由 **ENTRY 034** 併入
>   匯出規則（`write_back._compact_refs()`）；CFTS043 之 `SYS1_` 同去
> - ~~stem 只寫一次、節次以「、」相連~~（ENTRY 034 之寫法）→ **一節一行、每行重述 stem**；
>   ~~**leaf 自身節次置首行**，其餘 Comfort 節次升冪；外部文件各自成行，列於其後~~
>   （canon §10.7，2026-08-21；禁用 `,`／`、`／`;` 串接）
>   → **家族排序改依 canon §10.7（R-C49(amend)，CMF-02_review §2，2026-09-21）**：
>   **外部文件／CFTS 行在前**（SWC 基準 286/286），其後 **leaf 自身節次**，
>   其後其餘 Comfort 節次升冪。R-C49 原文之「leaf 自身節次置首行」與 §10.7 之家族排序
>   相衝，CMF-02 依 R-C49 落地並自陳其衝突（上繳 CMF-02 §4），審閱層裁定依 §10.7。
>   實測受影響 **7 條**（全為帶外部文件行者），**節次集合增減 0**
>   （`features/comfort/docs/reports/cmf03_rc49_refs_before_after.tsv`）
> - **PC 出處節次（2.14／16.2／6.3 等）續載** —— J 欄括號既已移除，N 欄為工作簿內唯一承載處
...
### 3.2.4 `[ext-verbatim]` —— 外部文件之逐字引用 [ADD]（下放包 82 §5.1）

```
[ext-verbatim] <外部文件之逐字片段> — <可讀之狀態句> (<文件簡稱> <識別符>)
```

**定義**：所引者為 **§1.1「已認可外部文件」表中某一件之原話**。

**適用條件（三項全備）**：

一、該文件已列於 §1.1（**未列表者不得引用**，R-C45 一）
二、所引之事實為**版本無關**者：配置條件、對照表、識別符之對應
    （行為性陳述不得跨版本引用，R-C45 二）
三、`specification_reference` 另列該文件之 section，**不併入 Comfort stem**
    （R-C45 三）

**何以另立一類而不借 `spec-verbatim`**：後者之意義是「**本 spec 該節**之原話」，
`source-class-truthful` gate 正是拿該節之 `full_text` 去查證的。
把外部句子標成 `spec-verbatim`，會使那道 gate 去查一句不在那裡的話而 FAIL；
**更糟的是 —— 若它恰巧查得到，讀者會以為那句出自本 spec。**
**一個標籤同時修掉一個誤判與一個誤讀。**

**現行用例**（2026-08-17）：`NR1L-ComfortHMI-430`～`-433` 之後排氣候配置條件，
引 CFTS043 之 `$Rear_HVAC_cfg$ = [Present]`（`NEWR1L-53677`）。

---

### 3.2.3 `[spec-verbatim]` 之兩段式與標點嚴格性 [ADD]（下放包 70 §2／§4）
...
[spec-verbatim] <逐字片段> — <可讀之狀態句> (節次)
  ...
  gate（`source-class-truthful`）檢查之對象為 **` — ` 之前**之逐字片段，
  其須為該節 full_text 之連續子字串；**其後之狀態句不受檢查，
  且不得引入條文未有之事實**（§8.4.1 照舊適用）。
```

**適用範圍**：逐字片段**不能自成條件句**者（以介系詞起首、無主謂結構）
**應**採兩段式；能自成者維持單段。**不強制全面改寫，逐條判。**

**為何逐字之形式得以放寬而其可驗性不減**：R-C42 一要求逐字之目的，
是使該條件**可證明為條文自身之條件**而非作者所造；該目的現由
`source-class-truthful` 之連續子字串檢查承擔（70 §2）。

**現行採用清單（21 行中之 5 行，2026-08-16）**：

| PC | 節 | 逐字片段 | 為何不能自成句 |
|---|---|---|---|
| `PC_CENTRIC` | 14.14 | `For vehicles with dual zone climate versions with dual airflow modes on 8.4", 10.1" Landscape, 10.25" and 12.3" radios` | 介系詞 `For` 起首，無主謂 |
| `PC_LANDSCAPE` | 17.4 | `For 8.4/10.1/12 landscaped screens` | 同上 |
| `PC_DUAL_WIDGET` | 17.5 | `For dual zone climate with dual airflow modes equipped vehicles` | 同上 |

其餘 16 行維持單段（`Some vehicles have …`／`If the hard controls are …`／
`On some vehicles …, there are …`／`On the 50% widget, these features are …`／
`12' Portrait 50% widget also includes fan speed` —— 皆有主謂結構）。

#### 標點嚴格性之處置（70 §4，預先裁定）

比對為**空白正規化後之連續子字串**，故 `Auto, Manual, Open` 與
`Auto/Manual/Open` 不相等而會 FAIL。**該嚴格性刻意保留。**

> **此類 FAIL 之正解為「改 PC 使其逐字」，不得放寬 gate。**
> 理由同 R-C32（裁定使既有 gate 失敗時改判而非放寬）——
> 而此處連改判都不必：**gate 沒有錯，是引用不準。**
...
- ~~**T–Z 欄 Vehicle Model**：一律留白（Privacy R30-4）。
  **A-PV15 同樣適用於 Comfort**：範本七欄止於 27 世代，本專案平台為
  ...
  → **作廢**（R-C48，CMF-01_review §3，2026-09-18）：**W（598）／X（5210）＝`0`**
  （全域 **R-G74**，無 PROXI 之車型）；~~**T／U／V／Y／Z 本輪維持 `1`**（0907 本之值），
  待 PROXI 軸值表（`features/comfort/docs/reports/proxi_axis_values.tsv`）另裁~~。
  CMF-02_A §1：U 欄表頭 `DT27 Atl-Hi` 與 `forms/proxi/DT28_ATL_HI/` 為同一車型（Pei 確認）
  → **基線確定（R-C48(amend)，Pei 2026-09-21，CMF-03_A §1）**：
  「這功能只有 alt-mi 有，所以是 fastback／promaster／toro」。**全列適用**（含 BLOCKED 列
  與新 sibling）：
...
**`SWE1-HVAC-019-03`（VF HVAC document；R-C51，CMF-01_review §3，2026-09-18 增列，
工作簿 row 116、TC ID `-466`）**。
...
| `test_procedure` / `expected_result` | **空** |
| `specification_reference` | 該 leaf 自身之 outline，照常填 |
| Remarks | `[COVERED-BY] <涵蓋之 req_id>` ＋ 一句說明。**R-C27-1（2026-08-16）**：可見範圍實測為約 11 字元（AH 欄寬 11.83／列高 14.0），其可達目標為「可辨識其已被標記」；R-C27 之「首 60 字元」為假定值，不再引用 |
| 其餘 | 依 profile 常規 |
...
**第六項成員（98 §C，2026-08-17）—— `2.12` 之四模式清單與其順序**：

| 成員 | 形態 |
|---|---|
| `16.1`／`18.2`／`18.3`／`18.4` | **整節未被 037 引用** |
| `15.1` 之圖表部分 | 節被引用，而**節內之一部分（一張圖表）未被引用** |
| **`2.12` 之四模式列舉與其順序** | 節被引用、**其行為亦有 leaf**，而該節之**一項列舉**未成為任何 leaf |

`2.12`（C13）之首句為

> There are 4 Airflow Mode displayed in this order (1) Face, (2) Face plus
> Feet, (3) Feet, (4) Feet plus Windshield.

**037 之三個 leaf 無一承載它**：`016-01` 為 ON 態之高亮與放大、`016-02` 為
main category control 之顯示、`016-03` 為互斥。**不得借掛 `016-01` 之 req_id**
—— 那等於以我方之切分冒充 037 之單位（§8.2、R-C33 第一項）。

> **本項之形態與前二者皆不同，須單獨標記**：`16.1` 等為「整節沒被引用」，
> `15.1` 為「節內之一張圖沒被引用」，**本項為「節被引用、其行為亦有 leaf，
> 而該節之一項列舉沒有成為任何 leaf」** —— 更細一層。
> **以節為單位掃描找不到它，以「節內有無未引用之圖」掃描也找不到它**：
> `2.12` 有 leaf、有 TC、在 coverage 分母內，兩種掃描都只會看見一個已涵蓋
> 之節。找到它的是**逐句對 leaf** 的比對。

**其被涵蓋之程度須如實記，不得以一方掩蓋另一方**（98 §C）：

| 條文所述 | 現況 |
|---|---|
| 四個模式**存在** | **間接被驗** —— `NR1L-ComfortHMI-440`～`-443` 逐一選取該四者 |
| 四個模式之**顯示順序** | **未被任何 TC 驗證** |

**鏡射表已獨立記載同一缺口**（`data/ch16_mirror_map.tsv` 之 `16.12 ↔ 2.12`
列，分界欄逐字寫「未涵蓋：C13 之**四模式**清單與其順序（ICE11 為五狀態）」）
...
### 6.1 寫回之義務為同步，非逐輪產檔 [ADD]（下放包 72 §4，2026-08-16）

> **編號說明**：72 §4 指定寫入「§9」，而本 profile 之寫回條款為 §6
> （§9 不存在；Privacy 之 §9 即本節所繼承者）。故置於 §6 之下為 §6.1，
> 內容未改。
...
**首次適用**：上繳 49 §5（本輪未產 ENTRY 020），其實測為
「383 列之 `tc_id` 與 `pre_conditions` 對 ENTRY 019 之產出檔逐列比對，0 處不符」。
