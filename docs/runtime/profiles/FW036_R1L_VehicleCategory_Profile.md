...
### 9.3.1 取材通則：`(image:)` 佔位不入上半（下放包 35 §2.2，逐字）
...
**先例 `066` 與本條之關係**：`066` 之 Description 為
`W0.) Widget title for this feature is Cabrio.\n (image: image23.png)` ——
其佔位在**句號之後**，句子切分自動分離，故取首句即自然不含佔位。
`065-02` 之佔位黏在**引號後而無句號**（`…in motion’\n (image: image18.png)`），
切分帶著它走。**二者是同一件事的兩種黏法**，本條使其同一處置。

**安全鎖**：剝除後之上半仍須為來源之**逐字子串**（第 7b 項照驗，無豁免）。
尾部剝除必然保住子串關係（剩餘為前綴）；
**居中剝除會造出來源沒有的相鄰關係，禁**——遇之停並回報。

**機器承載者**（本條非純紀律）：
- `scripts/verify_batch.py` 第 16 項之 `SENT` 建構
- `scripts/cont_guard.py` 之 `sentence()`
二處**共用同一支 `strip_image_tail()`**，其分歧即為錯（同機讀行之原則）。
...
**工具**：`scripts/ledger_xref.py`（並列）＋ `scripts/xref_triage.py`（粗篩）。
其母體為 9 份台帳，**不含下放包／上繳包**。

**二個判準與制度之衝突，記明**：
1. **對立語彙 vs R-TM13** —— R-TM13（既交付者不改原文，加註保留）保證
   舊斷言與其反駁永遠並存，二者語彙必然對立，故粗篩之第二層**必然**
   收進每一個已解之牴觸。`resolved_marks` 欄為排序用，
   **不得作為結案依據**（帶標記 ≠ 已解、無標記 ≠ 未解）。
2. **跨檔要求 vs 同條目內之牴觸** —— 見上開盲區。其補網為 PLAYBOOK §7.5。
