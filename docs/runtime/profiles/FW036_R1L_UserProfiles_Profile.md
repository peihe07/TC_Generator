...
**判例（49 包 §三）：`why` 不是註解，它是狀態。**
42 輪之產出閘寫「`vehicle_columns is None` 即拒絕產出」——
而它要表達的是「**決定了沒有**」。44 輪定為留空之後，
**值仍然是 `None`，閘不轉綠**。
48 輪改為問「三項是否已決定且 `why` 非空」，並保留 `--force` 之不存在。

> **「決定了留空」與「還沒想好」在 yaml 裡長得一樣，只有 `why` 分得開。**

**其一般形式**：凡以「值的形狀」表達「狀態」之判準，
狀態改變時判準不會跟著動。**狀態要有自己的欄位。**
...
> **本節與 §5.1 之條文已於 53 輪 close-out 升為全域**，
> 見 `docs/fw036/FEATURE_ONBOARDING.md` §9（G-A ～ G-M）。
> **本檔保留原文** —— 其量測條件與成因記在這裡，全域段只記其一句話與來源。
...
### 7.4.1 判準與詞界：`\b` 不適用於中文（G-I，49 包）[ADD]

> **凡判準含 `\b` 而其詞表有非 ASCII 者，一律改以
> `(?<![\u4e00-\u9fff])`／字串包含判定，並附中文命中之方向性案例。**

**成因**：48 輪之交付欄位掃描用 `\b(pending|TBD|待判|未決)\b`，
而 `\b` 是 **ASCII 詞界** —— 中文字兩側不構成詞界，
故 `待判`／`未決` **在該判準下永遠比不到**。
`TC-082` 之「上游未決事項」是在掃**產出檔**時（那次未加 `\b`）才浮出來的。

**與 X-1（`Activate` 未入詞表，40 包）同類**：閘在跑，
**但它看不到那一類輸入** —— 而「看不到」與「看過而判綠」在輸出上完全相同。

**機械化而非宣稱**：`audit_delivery_fields.py` 之 **DF-2** 對詞表中每個中文詞
造一個「夾在中文裡」之探針句並實測其可被命中；
判準若退回 `\b`，DF-2 立刻轉紅。**規則要有一條會叫的閘，否則它只是一句話。**

**查核範圍之修正**：48 輪之 grep 找的是「`\b` 緊鄰 CJK」，
**而出事的形態是 `\b(…|待判|未決)\b` —— `\b` 緊鄰的是 `(`**。
正確判準為「同一 regex 字面值內同時含 `\b` 與 CJK」；
以該判準重查 17 支閘，仍為 **0 命中**（49 輪實測）。

### 7.4.2 素材狀態之判準（G-L，53 包）[ADD]

> **沒有路徑的「到齊」不算到齊。**

`DATA_REQUESTS.md` 之每一項須附其**在檔案系統上之路徑與 SHA**；
**「到齊」定義為 `shasum -c` 對得上**。

**成因（52 輪，兩者剛好相反）**：Pop Up List 之狀態記為「待驗」——
**而它其實在 repo 裡三份**；Tutorials L&F 記為「到齊」——**而它不在**。
兩者都不是誰記錯，是**「素材狀態」當時沒有一個可執行的判準**。

### 7.4.3 「先查他 feature」之範圍含其 `inputs/`（G-M，53 包）[ADD]

§7.4 之常規由「他 feature 之**交付件**」擴為「他 feature 之**全部素材**」。

**成因**：`TC-169`（8.3）自第六批生成起就寫著「popup 之逐步對映不在我方
輸入內，依 §8.4.1 只驗形態」——**而那份輸入一直在
`features/comfort/inputs/` 裡，卡了五十輪。**
...
理由：AB-1 這一類缺陷正是「差異微小而判定翻轉」之形態
（同一句英文在 6.4 是恆真、在 6.1 是歧義）。
產出方應做的是**把差異逐欄列出**（`--changes`），而不是替人判斷它重不重要。

### 7.6 自我測試不得讀當前語料（G-N，58 包）[ADD]

> **自我測試若以當前語料為案例，缺陷修好之後測試即失去證明力 ——
> 而它表現為分數下降，看起來像判準退化，實際上是證明消失。**
>
> **正解**：把原文以**字面釘進測試**，另加一組「**修正後不得再命中**」之回歸。

**成因是 56 輪的一次真實錯誤。** `audit_second_segment` 之 G-K 段
首版寫成「自語料取 `168`／`173`／`166` 三條，斷言命中」——
當時 10/10 全過，證明成立。而 AD-1 之 31 條改寫一落地，
**同一支測試立刻變成 7/10**：三條已經不再命中了，因為它們被修好了。

**那三個 FAIL 不是判準壞了，是判準的證明沒了。**
若當時把它讀成「判準退化」而回去放寬門檻，就會為了讓測試變綠而弄壞判準。
...
ORIG_168 = ("New Profile Setup starts from the All Profiles tab\n"
            ...
            "All Profiles tab)")
...
case("**56 包所點名之 `168` 之原文 → 須命中**", ..., True)   # 判準抓得到該病
case("`168` 之**現況**（AD-1 改寫後）→ 不得命中", ..., False)  # 該病已治好
```
...
合成一個案例，兩個答案就分不開 —— 而**分不開的時候，通常會誤讀成第一個**。

**與 G-D／G-K 並列 —— 三者皆為「證據本身之有效性」**：
...
**適用範圍不限於本 feature 之閘** —— 凡以語料為案例之自我測試皆適用。
`audit_pending` 之 digest 機制是反向的同一件事：那個要求「語料一改就回列重判」，
本條要求「語料一改而**證明不得跟著失效**」。兩者不衝突 ——
digest 管的是**判定**，字面案例管的是**判準之證明**。
