...
**摘句留孤懸標點時取全文**（CAM-18 審閱 §一-2，Pei 准）：來源句以 `,`／`and` 相連之列舉，
若只留其中一項會使標點孤懸（如 `“Rotate 90”,` 之逗號與 `90”` 黏字，刪之即破保序子序列），
而**全文 ≤ 50 token** 者，`test_item` 上半**取全文**；
各列所驗之項改由下半之括號句（`split_reason`）指明。
沿革：CAM-18 之 §28.7.2 三列（`NR1L-RVCHMI-114`～`-116`）全文 15 token，三列同取全文。
...
**豁免之要件為「上半自來源 `Description` 之首 token 起」**（CAM-18 審閱 §一-3，Pei 准）——
摘句若自句中某個小寫 token 起首，該 token 在來源中並非句首，其小寫只是句中形態，
**不在本豁免射程**；此時應**往前保留到一個大寫 token**（仍為保序子序列），而非援引豁免。
沿革：CAM-18 之 `NR1L-RVCHMI-121`／`-122` 摘句自 §28.8.1 之 `however,` 起首，
改為保留首片語 `From the AUX camera video feed,` 後 `J` 歸 0（CAM-18 上繳 §4-4）。
