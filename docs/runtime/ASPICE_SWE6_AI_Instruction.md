...
(h) **v3 記法殘留為 FAIL（GC-12 審閱 §一，Pei 2026-09-05）**：
    `$<MESSAGE>.<Signal>$` 之 `$` 包覆式，**不論其後有無 `= <raw> (<label>)`**，
    一律為 lint 檢查 P 之 **FAIL** 類，非 WARN。
    理由：R-1 v3 已撤銷，殘留即不合規；Revise-C 必須清零，
    WARN 類無法作為出貨閘。
    ⚠ **範圍同 R-6 之 P 範圍** —— 作者側四欄 **＋ `test_item` 之括號下半**
