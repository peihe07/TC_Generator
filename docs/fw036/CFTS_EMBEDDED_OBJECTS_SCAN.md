...
| feature | 母 CFTS | 目錄存在？ | 檔數 | **優先** | 第二段（出表）之時程 |
|---|---|:--:|---:|:--:|---|
| `amfm` | **CFTS024** | ✅ | **16** | **1** | **待辦** —— **已交付（v1 已 tag）**，其執行時機屬 Pei |
| `sxm` | **CFTS024** | ✅ | **16** | **1** | **待辦** —— **已交付**（與 `amfm` 同目錄，出表可共用） |
| `audio_mgmt` | **CFTS019** | ✅ | **31** | **1** | **待辦** —— **已交付** |
| `projection` | CFTS085（母）／CFTS025（DR#8） | ✅（`CFTS025`） | **46** | 2 | **隨其解凍**，不因檔數多而提前 |
...
| 類 | feature 數 |
|---|---:|
| **有嵌入物件、待出表** | **4**（優先 1：`amfm`／`sxm`／`audio_mgmt`，**皆已交付**；優先 2：`projection`，凍結中） |
| 有嵌入物件、已出表 | **1**（`sw_update`） |
| 已查、其母 CFTS 無此目錄 | **8** |
| 無母 CFTS，不適用 | **7** |
| **合計** | **20** |

> ### ⚠ 優先序依「是否已交付」，非依檔數（下放包 57 §二 #2）
>
> **已交付之 feature，若其圖中載有未見於 docx 之規格值，
> 則其已交付之 TC 可能有造值或漏驗** —— **那是交付件之缺陷，不是待辦事項。**
> 未交付者只是「還沒查」；**已交付者是「查了可能要改已經送出去的東西」。**
> **故 `amfm`／`sxm`／`audio_mgmt` 優先於檔數最多之 `projection`。**
> **其執行時機屬 Pei**（已交付件之重驗涉及 DR 與可能之改版）。
