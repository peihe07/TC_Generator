...
| R | Pre-Condition 版面（未編號行／多條件並列） | 11 | 10 | 每行 | 未校準（R-9(a)，21 包新增） |
...
| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 15 | newR1L-SU-244 | test_item | 首字小寫 'the' | the WiFi Update Service shall request extended wake mode to continue the package |

### R — Pre-Condition 版面（未編號行／多條件並列）（行計 11／列計 10）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 12 | newR1L-SU-241 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
| 13 | newR1L-SU-242 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
| 17 | newR1L-SU-246 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
| 18 | newR1L-SU-247 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
| 19 | newR1L-SU-248 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
| 20 | newR1L-SU-249 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
| 21 | newR1L-SU-250 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
| 21 | newR1L-SU-250 | pre | 多條件並列於同一行 | 4. The installation notification popup is displayed and has not been accepted or |
| 22 | newR1L-SU-251 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
| 24 | newR1L-SU-253 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
| 25 | newR1L-SU-254 | pre | 多條件並列於同一行 | 3. A deployment package has been downloaded and is ready for installation on the |
...
| 24 | newR1L-SU-253 | pre | PENDING 佔位（DR-SU2） | 4. PENDING: DR-SU2 means of observing the transition request the WiFi Update Ser |
