...
| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 19 | newR1L-SU-175 | pre | 多條件並列於同一行 | 4. PENDING: DR-SU2 criterion by which the safety mechanism is judged present, gi |
...
| 15 | newR1L-SU-171 | pre | PENDING 佔位（DR-SU2） | 4. PENDING: DR-SU2 means of extracting the installed software image from the hea |
| 15 | newR1L-SU-171 | pre | PENDING 佔位（DR-SU2） | 5. PENDING: DR-SU2 means of obtaining the reference deployment image for the sam |
| 15 | newR1L-SU-171 | proc | PENDING 佔位（DR-SU2） | 2. PENDING: DR-SU2 step to extract the installed software image and compare it w |
| 15 | newR1L-SU-171 | er | PENDING 佔位（DR-SU2） | 2. PENDING: DR-SU2 observable evidence that the installed image is identical to  |
...
| 19 | newR1L-SU-175 | pre | PENDING 佔位（DR-SU2） | 4. PENDING: DR-SU2 criterion by which the safety mechanism is judged present, gi |
| 19 | newR1L-SU-175 | proc | PENDING 佔位（DR-SU2） | 2. PENDING: DR-SU2 step to exercise the safety mechanism that prevents the SOC f |
| 19 | newR1L-SU-175 | er | PENDING 佔位（DR-SU2） | 2. PENDING: DR-SU2 observable evidence that the safety mechanism prevented the S |
...
| 22 | newR1L-SU-178 | pre | PENDING 佔位（DR-SU2） | 4. PENDING: DR-SU2 means of reading the integrity information validated after a  |
| 22 | newR1L-SU-178 | proc | PENDING 佔位（DR-SU2） | 2. PENDING: DR-SU2 step to read the integrity information validated for the resu |
| 22 | newR1L-SU-178 | er | PENDING 佔位（DR-SU2） | 2. PENDING: DR-SU2 observable evidence that the resulting firmware image was val |
