...
| U | PENDING 佔位（四欄全掃，含 ER 側） | 85 | 43 | 每次命中 | 計數用（A-PM16：ER 側原不受任何檢查覆蓋） |
...
### U — PENDING 佔位（四欄全掃，含 ER 側）（行計 85／列計 43）
...
| 54 | NR1L-KI-022 | proc | PENDING 佔位（X-f-2） | 1. PENDING: X-f-2 KeyInstall status test runner |
| 54 | NR1L-KI-022 | er | PENDING 佔位（X-f-2） | 1. PENDING: X-f-2 KeyInstall status test runner |
