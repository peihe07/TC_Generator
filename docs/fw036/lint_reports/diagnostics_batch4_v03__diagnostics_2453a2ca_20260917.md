# lint036 報告：diagnostics_batch4_v03.xlsx

- 來源：`features/diagnostics/sandbox/batch4/diagnostics_batch4_v03.xlsx`（唯讀）
- 資料列數：61
...
| U | PENDING 佔位（四欄全掃，含 ER 側） | 2 | 1 | 每次命中 | 計數用（A-PM16：ER 側原不受任何檢查覆蓋） |
...
### U — PENDING 佔位（四欄全掃，含 ER 側）（行計 2／列計 1）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 61 | NR1L-DIAG-281 | proc | PENDING 佔位（DR-DIAG-1） | 2. PENDING: DR-DIAG-1 failure mode and NRC for the expected audio test tone vali |
| 61 | NR1L-DIAG-281 | er | PENDING 佔位（DR-DIAG-1） | 2. PENDING: DR-DIAG-1 failure mode and NRC for the expected audio test tone vali |
