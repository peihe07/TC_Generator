# lint036 報告：FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_Diagnostics_20260918_v03.xlsx

- 來源：`features/diagnostics/sandbox/delivery/FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_Diagnostics_20260918_v03.xlsx`（唯讀）
- 資料列數：565
...
| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 235 | NR1L-DIAG-226 | proc | PENDING 佔位（DR-DIAG-1） | 2. PENDING: DR-DIAG-1 failure mode and NRC for the expected audio test tone vali |
| 235 | NR1L-DIAG-226 | er | PENDING 佔位（DR-DIAG-1） | 2. PENDING: DR-DIAG-1 failure mode and NRC for the expected audio test tone vali |
| 364 | NR1L-DIAG-355 | er | PENDING 佔位（DR-DIAG-1） | 1. PENDING: DR-DIAG-1 NRC value for an unsuccessful Sirius XM package ID applica |
