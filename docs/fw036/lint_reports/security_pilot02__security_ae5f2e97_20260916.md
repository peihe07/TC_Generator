# lint036 報告：security_pilot02.xlsx

- 來源：`features/security/sandbox/pilot02/security_pilot02.xlsx`（唯讀）
- 資料列數：12
...
| U | PENDING 佔位（四欄全掃，含 ER 側） | 14 | 8 | 每次命中 | 計數用（A-PM16：ER 側原不受任何檢查覆蓋） |
...
| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 17 | NR1L-ECUC-001 | pre | 多條件並列於同一行 | 3. The ECU serial number has been written and the CSR is generated at boot |

### U — PENDING 佔位（四欄全掃，含 ER 側）（行計 14／列計 8）
...
| 14 | NR1L-CP-005 | proc | PENDING 佔位（X-e） | 2. PENDING: X-e revoked certificate + trigger |
| 14 | NR1L-CP-005 | er | PENDING 佔位（X-h） | 2. PENDING: X-h log keyword |
| 15 | NR1L-CP-006 | proc | PENDING 佔位（X-e） | 3. PENDING: X-e OID certificate + trigger |
| 15 | NR1L-CP-006 | er | PENDING 佔位（X-h） | 3. PENDING: X-h log keyword |
| 16 | NR1L-CP-007 | proc | PENDING 佔位（X-e） | 2. PENDING: X-e OID certificate + trigger |
| 16 | NR1L-CP-007 | er | PENDING 佔位（X-h） | 2. PENDING: X-h log keyword |
| 18 | NR1L-KI-001 | proc | PENDING 佔位（X-f） | 2. PENDING: X-f KeyInstall status test runner |
| 18 | NR1L-KI-001 | er | PENDING 佔位（X-f） | 2. PENDING: X-f KeyInstall status test runner |
| 19 | NR1L-KI-002 | proc | PENDING 佔位（X-f） | 2. PENDING: X-f KeyInstall status test runner |
| 19 | NR1L-KI-002 | er | PENDING 佔位（X-f） | 2. PENDING: X-f KeyInstall status test runner |
| 20 | NR1L-SAM-001 | er | PENDING 佔位（X-h） | 3. PENDING: X-h log keyword |
