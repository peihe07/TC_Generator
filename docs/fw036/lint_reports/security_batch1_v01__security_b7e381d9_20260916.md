# lint036 報告：security_batch1_v01.xlsx

- 來源：`features/security/sandbox/batch1/security_batch1_v01.xlsx`（唯讀）
- 資料列數：72
...
| K | CJK 字元 | 10 | 10 | 每列每欄 | 已校準（分級待 R-5） |
...
| U | PENDING 佔位（四欄全掃，含 ER 側） | 65 | 33 | 每次命中 | 計數用（A-PM16：ER 側原不受任何檢查覆蓋） |
...
| SC | 步驟無執行通道／ER 無觀察手段（R-SEC7，Security profile 專屬） | 25 | 21 | 每編號步驟／每 ER 行 | 未校準（R-SEC7，SEC-02 新增）—— **feature 專屬**，僅 `--profile security` 啟用。對既有語料之假陽性率 Procedure 98.4%／ER 74.9%（九本 1,700 列實測，SEC-01 上繳包 8-2），**故絕不可入 PROFILE_CHECKS**；對 Security 自身之假陽性率待 Pilot |
...
**總計：行計 104**（列計不加總——同一列可觸發多項檢查）

## 明細

### K — CJK 字元（行計 10／列計 10）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 20 | NR1L-CP-011 | test_set | 含 CJK 字元 | (非功能) |
| 21 | NR1L-CP-012 | test_set | 含 CJK 字元 | (非功能) |
| 22 | NR1L-CP-013 | test_set | 含 CJK 字元 | (非功能) |
| 23 | NR1L-CP-014 | test_set | 含 CJK 字元 | (非功能) |
| 30 | NR1L-CP-021 | test_set | 含 CJK 字元 | (非功能) |
| 54 | NR1L-KI-022 | test_set | 含 CJK 字元 | (非功能) |
| 55 | NR1L-KI-023 | test_set | 含 CJK 字元 | (非功能) |
| 56 | NR1L-KI-024 | test_set | 含 CJK 字元 | (非功能) |
| 57 | NR1L-KI-025 | test_set | 含 CJK 字元 | (非功能) |
| 58 | NR1L-SAM-001 | test_set | 含 CJK 字元 | (環境) |

### U — PENDING 佔位（四欄全掃，含 ER 側）（行計 65／列計 33）
...
| 48 | NR1L-KI-016 | proc | PENDING 佔位（X-f-2） | 2. PENDING: X-f-2 KeyInstall status test runner |
| 48 | NR1L-KI-016 | er | PENDING 佔位（X-f-2） | 2. PENDING: X-f-2 KeyInstall status test runner |
| 49 | NR1L-KI-017 | proc | PENDING 佔位（X-f-2） | 2. PENDING: X-f-2 KeyInstall status test runner |
| 49 | NR1L-KI-017 | er | PENDING 佔位（X-f-2） | 2. PENDING: X-f-2 KeyInstall status test runner |
...
| 52 | NR1L-KI-020 | proc | PENDING 佔位（X-f-2） | 2. PENDING: X-f-2 KeyInstall status test runner |
| 52 | NR1L-KI-020 | er | PENDING 佔位（X-f-2） | 2. PENDING: X-f-2 KeyInstall status test runner |
| 53 | NR1L-KI-021 | proc | PENDING 佔位（X-f-2） | 2. PENDING: X-f-2 KeyInstall status test runner |
| 53 | NR1L-KI-021 | er | PENDING 佔位（X-f-2） | 2. PENDING: X-f-2 KeyInstall status test runner |
...
| 80 | NR1L-SWDL-005 | proc | PENDING 佔位（X-l） | 1. PENDING: X-l SWDL verification test entry point |
| 80 | NR1L-SWDL-005 | er | PENDING 佔位（X-l） | 1. PENDING: X-l SWDL verification test entry point |
| 81 | NR1L-SWDL-006 | proc | PENDING 佔位（X-l） | 1. PENDING: X-l SWDL verification test entry point |
| 81 | NR1L-SWDL-006 | er | PENDING 佔位（X-l） | 1. PENDING: X-l SWDL verification test entry point |
...
### SC — 步驟無執行通道／ER 無觀察手段（R-SEC7，Security profile 專屬）（行計 25／列計 21）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 25 | NR1L-CP-016 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell ls -l /odm/etc/cert_store |
| 30 | NR1L-CP-021 | er | R-SEC7(c)：ER 每行須含七類觀察手段之一（LOG／FILE／UDS／RC／HOST／CAN／UI） | 2. The adb shell procrank output shows the Cert Provider RAM |
| 34 | NR1L-KI-002 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell od -t x1 /mnt/vendor/oemkeys/<installstate file> |
| 35 | NR1L-KI-003 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell od -t x1 /mnt/vendor/oemkeys/<installstate file> |
| 36 | NR1L-KI-004 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell od -t x1 /mnt/vendor/oemkeys/<installstate file> |
| 37 | NR1L-KI-005 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell od -t x1 /mnt/vendor/oemkeys/<installstate file> |
| 38 | NR1L-KI-006 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell od -t x1 /mnt/vendor/oemkeys/<installstate file> |
| 39 | NR1L-KI-007 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell od -t x1 /mnt/vendor/oemkeys/<installstate file> |
| 44 | NR1L-KI-012 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell ls -l /mnt/vendor/oemkeys |
| 45 | NR1L-KI-013 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell od -t x1 /mnt/vendor/oemkeys/<installstate file> |
| 46 | NR1L-KI-014 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell ls -l /mnt/vendor/oemkeys |
| 57 | NR1L-KI-025 | er | R-SEC7(c)：ER 每行須含七類觀察手段之一（LOG／FILE／UDS／RC／HOST／CAN／UI） | 1. The adb shell procrank output shows the KeyInstall RAM us |
| 57 | NR1L-KI-025 | er | R-SEC7(c)：ER 每行須含七類觀察手段之一（LOG／FILE／UDS／RC／HOST／CAN／UI） | 2. The adb shell df /mnt/vendor/oemkeys output shows the par |
| 58 | NR1L-SAM-001 | er | R-SEC7(c)：ER 每行須含七類觀察手段之一（LOG／FILE／UDS／RC／HOST／CAN／UI） | 2. The adb shell "ps -A \| grep dauth" output lists the Debug |
| 58 | NR1L-SAM-001 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell ls -l /data/vendor/dauth |
| 58 | NR1L-SAM-001 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell ps -A grep dauth |
| 59 | NR1L-SAM-002 | er | R-SEC7(c)：ER 每行須含七類觀察手段之一（LOG／FILE／UDS／RC／HOST／CAN／UI） | 2. The log buffer is cleared |
| 62 | NR1L-SAM-005 | er | R-SEC7(c)：ER 每行須含七類觀察手段之一（LOG／FILE／UDS／RC／HOST／CAN／UI） | 1. The log buffer is cleared |
| 65 | NR1L-ECUC-002 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell ls -l /data/misc/ecuidentity |
| 66 | NR1L-ECUC-003 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell od -t x1 /mnt/vendor/oemkeys/ecu/state/ecucertstat |
| 67 | NR1L-ECUC-004 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell ls -l /data/misc/ecuidentity |
| 67 | NR1L-ECUC-004 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell od -t x1 /mnt/vendor/oemkeys/ecu/state/ecucertstat |
| 73 | NR1L-ECUC-010 | er | R-SEC7(c)：ER 每行須含七類觀察手段之一（LOG／FILE／UDS／RC／HOST／CAN／UI） | 3. The adb pull command reports "1 file pulled" and ecu.crt  |
| 74 | NR1L-ECUC-011 | er | R-SEC7(c)：ER 每行須含七類觀察手段之一（LOG／FILE／UDS／RC／HOST／CAN／UI） | 1. The adb pull command reports "1 file pulled" and ecu.crt  |
| 75 | NR1L-ECUC-012 | er | SEC-04 §3：ER 所引之指令未出現於同編號之 Procedure 步驟 | adb shell ls -l /data/misc/ecuidentity |
