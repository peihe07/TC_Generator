# lint036 報告：diagnostics_batch3_v01.xlsx

- 來源：`features/diagnostics/sandbox/batch3/diagnostics_batch3_v01.xlsx`（唯讀）
- 資料列數：99
...
| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 11 | NR1L-DIAG-134 | proc | 導航標的 'Settings' 而同 TC 無固定入口 | 1. Read the Radio Audio Output Settings |
| 96 | NR1L-DIAG-219 | proc | 導航標的 'menu' 而同 TC 無固定入口 | 1. Send the command to enable in-motion menu options |
| 96 | NR1L-DIAG-219 | proc | 導航標的 'menu' 而同 TC 無固定入口 | $ 2F 50 0B <enable in-motion menu> |
| 97 | NR1L-DIAG-220 | proc | 導航標的 'menu' 而同 TC 無固定入口 | 2. Read the in-motion menu item status |
| 98 | NR1L-DIAG-221 | proc | 導航標的 'menu' 而同 TC 無固定入口 | 2. Read the in-motion menu item status |
| 99 | NR1L-DIAG-222 | proc | 導航標的 'menu' 而同 TC 無固定入口 | 2. Read the in-motion menu item status |
| 100 | NR1L-DIAG-223 | proc | 導航標的 'menu' 而同 TC 無固定入口 | 1. Send the command to disable in-motion menu options |
| 100 | NR1L-DIAG-223 | proc | 導航標的 'menu' 而同 TC 無固定入口 | $ 2F 50 0B <disable in-motion menu> |
| 101 | NR1L-DIAG-224 | proc | 導航標的 'menu' 而同 TC 無固定入口 | 2. Read the in-motion menu item status |
| 102 | NR1L-DIAG-225 | proc | 導航標的 'menu' 而同 TC 無固定入口 | 2. Read the in-motion menu item status |
| 103 | NR1L-DIAG-226 | proc | 導航標的 'menu' 而同 TC 無固定入口 | 1. Send the command to enable in-motion menu options |
| 103 | NR1L-DIAG-226 | proc | 導航標的 'menu' 而同 TC 無固定入口 | $ 2F 50 0B <enable in-motion menu> |
