# lint036 報告：diagnostics_batch5_v01.xlsx

- 來源：`features/diagnostics/sandbox/batch5/diagnostics_batch5_v01.xlsx`（唯讀）
- 資料列數：67
...
| L | test_item 上半過長 (>50 tokens) | 8 | 8 | 每列 | 已校準（閾值待 R-3） |
| M | 空欄三態 | 0 | 0 | 每列每欄 | 已校準 |
| N | 行尾多餘句號 | 0 | 0 | 每行 | 已校準 |
...
| U | PENDING 佔位（四欄全掃，含 ER 側） | 1 | 1 | 每次命中 | 計數用（A-PM16：ER 側原不受任何檢查覆蓋） |
...
### L — test_item 上半過長 (>50 tokens)（行計 8／列計 8）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 64 | NR1L-DIAG-345 | test_item | 上半 52 tokens > 50 | If retries are performed as shown on the flowchart, and the retries are the resu |
| 65 | NR1L-DIAG-346 | test_item | 上半 52 tokens > 50 | If retries are performed as shown on the flowchart, and the retries are the resu |
| 66 | NR1L-DIAG-347 | test_item | 上半 52 tokens > 50 | If retries are performed as shown on the flowchart, and the retries are the resu |
| 67 | NR1L-DIAG-348 | test_item | 上半 52 tokens > 50 | If retries are performed as shown on the flowchart, and the retries are the resu |
| 73 | NR1L-DIAG-354 | test_item | 上半 52 tokens > 50 | If retries are performed as shown on the flowchart, and the retries are the resu |
| 74 | NR1L-DIAG-355 | test_item | 上半 52 tokens > 50 | If retries are performed as shown on the flowchart, and the retries are the resu |
| 75 | NR1L-DIAG-356 | test_item | 上半 52 tokens > 50 | If retries are performed as shown on the flowchart, and the retries are the resu |
| 76 | NR1L-DIAG-357 | test_item | 上半 52 tokens > 50 | If retries are performed as shown on the flowchart, and the retries are the resu |

### U — PENDING 佔位（四欄全掃，含 ER 側）（行計 1／列計 1）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 16 | NR1L-DIAG-297 | er | PENDING 佔位（DR-DIAG-1） | 1. PENDING: DR-DIAG-1 NRC value for an unsuccessful Sirius XM package ID applica |
