# lint036 報告：FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_Security_ECUCert_20260917.xlsx

- 來源：`features/security/sandbox/delivery/FM-WI-FSM-036-A01 STLA 測試用例規範與結果_SWQT STLA Test Case Specification & Result_SWQT_Security_ECUCert_20260917.xlsx`（唯讀）
- 資料列數：25
...
**總計：行計 1**（列計不加總——同一列可觸發多項檢查）

## 明細

### A — 禁用動詞 (proc)（行計 1／列計 1）

| 列 | TC ID | 欄位 | 說明 | 片段 |
| ---: | --- | --- | --- | --- |
| 30 | NR1L-ECUC-021 | proc | 禁用動詞 '2. Verify' | ovided by RD (X-k)> ⏎ 2. Verify the exported CSR signature on the host ⏎ $ openssl r |
