...
2. ~~凍結解除的條件寫不寫進 canon~~ —— **已決（R18-3）**：凍結於
   ...
   三項常設規則。本草案已據此改寫。
3. ~~回溯範圍待定~~ —— **已決（R19-6(b)）**。執行層原建議寫成通則，
   分析層**部分採納但改變形式**：不得寫成結果型通則
   （「發現結構缺損預設不重產」），該形式會使「預設不修」成為交付缺陷的
   默認處置，風險不對稱。改為**判準型通則**，逐字條文如下 ——
   ...
   此段納入 canon 時應置於 §6a 之末，作為「缺損已發生」的處置條文 ——
   前面各條規範的是如何不製造缺損，這一條規範的是缺損既成之後怎麼辦。
