...
## 4. `design_method` 無合法值檢查（Diagnostics CDD-09 `[A-DIAG56]` 揭）

`lint036.py` 現無任何檢查涵蓋 `design_method` 欄。母本工作簿「下拉選單」表已逐字列出十式，
任何拼寫偏差（如漏 `, BVA`）九本全 0 仍會通過。

**建議**：加一項全域檢查，母體為母本之「下拉選單」表逐字值，空值只准 Out of Scope 列（R-DIAG3(amend) 型）。
非 feature 專屬 —— 八個既有 feature 之工作簿亦應複驗。
