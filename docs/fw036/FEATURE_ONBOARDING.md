...
> 而全案有數十處以「canon」加裸節號指向它們之引用（依 R-G18，今應書
...
| 裁決引用 | 依 **R-G13** 之引用制：`R-XX@<sha8>`，執行層自 repo 讀原文並回報所讀 sha8。得另附原文（附了不算錯）| 執行層依編號猜內容 |
...
**[SUPERSEDED by R-G13 — 逐字照錄之要求部分]**（2026-08-24，23 包 §D）：
...
- **一批一上繳**，前批未覆核不得開下批 —— **除 R-G14 綠色通道生效期間**（FO §9.2）
- 升級 chat 覆核之條件由下放包明列
...
以「canon」加裸 §9.1 之引用因而歧義。整併後**每一節號唯一**（R-G18）。
...
| 9.2 | 全域裁決條文 R-G1 ～ R-G27 | 原第一個 §9 之 §9.2 **併** 第二個 §9 之 §9.1 |
...
### 9.2 全域裁決條文 R-G1 ～ R-G27

**本節為 R-G 系列之單一落點。** 整併前 R-G1～R-G12 有兩個表（原第一個 §9 之
...
`docs/fw036/RULINGS.sha.tsv`，供 R-G13 之 `R-Gn@<sha8>` 引用。
...
| R-G13 | 裁決引用制（取代逐字照錄）| 23 包 §D，Pei 2026-08-24 |
| R-G14 | 綠色通道（一批一上繳之例外）| 同上 |
| R-G15 | Pilot 退出準則 | 同上 |
| R-G16 | 預期數字自動生成 | 同上 |
| R-G17 | 閘登錄簿與除役 | 同上 |
| R-G18 | canon 引用唯一可解析 | 同上，措辭依 24 包 §C 裁定 2 |
| R-G19 | prompt 指紋 | 同上 |
| R-G20 | 規則副本同步指紋 | 同上 |
| R-G21 | 自查表機檢對映 | 同上 |
| R-G22 | 條文之任何字元變動皆變更其 sha | 25 包 §C 裁定 C，Pei 2026-08-24 |
| R-G23 | 同一工單同時只得有一份有效下放包 | 26 包 §C [DEFAULT]，待 Pei 追認 |
| R-G23′ | 單一來源擴至每條線；取號一律 live | 27 包 §二，Pei 裁定 2026-08-24（出處 V33 裁定記錄 4）|
| R-G24 | 下放指示之路徑實在性 | bed_lowering 01 包，Pei 裁定 2026-08-26 |
...
| R-G29 | 條文須加框（`body_kind` 不變式） | sw_update 59 包，**分析層 [DEFAULT] 2026-08-30，Pei 得推翻** |
...
> **兩份 canon 之代號**（R-G18 首跑所揭，23a §四-2）：
...
> **R-G23 本體一字不動**（R-G22：動了 sha 就換發）—— 本條為其 ′ 版擴充，
> 另立錨點另計 sha。
>
> **擴充之來源**：R-G23 只管全域線（`docs/fw036`），而**撞號實際發生在
...
> **專門 commit** 之要求同 R-G12（一律帶 pathspec）：清理與內容變更混在
...
> **[DEFAULT] 之理由**：R-G13／R-G22 皆為 Pei 所裁，
...
| 第一個 §9.2 | 全域條文 R-G1～R-G12（集中）| §9.2（**併**，見 §9.8.2）|
| 第一個 §9.3 | 一條裁決只管一件事 | §9.3（**不變**）|
| 第二個 `## 9.` | 全域慣例（53 輪 close-out）| **併入本節之前言** |
| 第二個 §9.1 | R-G1 ～ R-G12（全域裁決條文）| §9.2（**併**，其來源欄升為主表之來源欄）|
...
**歷史 handoff／upstream 檔中之既有引用不追改**（R-G18），其逐處列於
`docs/fw036/CANON_REFS_WAIVER.tsv`；**讀歷史檔遇 §9.1～§9.3 者，以本表判其實指**。

#### 9.8.2 R-G1～R-G12 兩版摘要之出入（逐條比對結果）
...
| R-G12 | 第一版載完整判準與 `git add` 之同規 | 取第一版 |
