# 22 — 流程改善總綱（P1–P7 條文化與工單）

> **[SUPERSEDED by 23_process_improvement.md — 2026-08-24]**
> NN 22 與既有上繳包 `upstream/22_canon_sync_and_lint_profile.md`
> （2026-08-21，Pei 口頭指示、無下放對應）碰撞；整包改編為 23，
> 並併入 Pei 追認記錄與上繳 22 之三項後續（--gate 接入、report_stem
> 覆寫、user_profiles 測試失敗處置）。內容以 23 為準；本檔保留不刪（R-G2）。

日期：2026-08-24
層級：全域（docs/fw036），非單一 feature
狀態：Pei 已裁方向（P1–P6 全數通過 + P7 新增「TC 規則抗漂移」）；
本包為條文化與工單化，條文文字請 Pei 追認一次（見 §D）。
...
## D. 新條文（可直接貼入；Pei 追認後由 W-P1 併入 canon）
...
Project 指令副本所載 sha 比對（現載 fa9833ae64c9092f）。不符 =
先 re-sync Project 副本再進行審查；審查不得在已知不同步之規則副本上進行。
```
...
### W-P2 — expected_numbers.py + GATES.tsv 初版

- expected_numbers.py：以 vehicle_setting 最近一批之下放包手算值為
  已知全集驗證（§5a-12）：工具重算須逐項與該包相符，不符處逐項追因
- GATES.tsv：自 lint_defs 生成閘清單；近輪命中數可自既有上繳包回填者
  回填，不可者標「未知，自本輪起算」— 不得以 0 代替未知（G-D）

### W-P3 — prompt 指紋 + 自查表對映

- make_batch_context.py 輸出 prompt/exemplar sha 入批次 manifest
- §9 自查 17 項逐項對映閘 id 或標人工項，產出對映表入 GATES.tsv
- 對映表之驗證：對已知案例（Home A-H10 之引號規則）確認其對映之閘
  確實會轉紅（G-K）

### W-P4 —（併入 W-P1 第 6 步，不獨立成單）
...
每工單一份上繳包（docs/fw036/upstream/22x_*.md，x = a/b/c 對應 W-P1/2/3）：
預期數字對照（相符者亦列）、不符項目不自行調和、canon diff 摘要、
canon_refs / rulings_hash 實跑輸出、掃描條件揭露、
...
G-A、G-D、G-E、G-K、R-G2、R-G5、R-G12、通則 11、A-PJ51。
