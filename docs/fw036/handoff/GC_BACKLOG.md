...
| GCB-11 | **lint `X`（導航標的無固定入口）之固定入口清單未納 profile §5.3 之常數** —— `X` 之啟發式以「同 TC 內是否有固定入口」判，而各 feature 之 profile §5.3 所定之導航常數（Camera 側為 `ENTER_SETTINGS_APP`／`ENTER_CAMERA_SETTINGS`）不在其識別清單內，致使凡步驟中出現 `screen`／`settings`／`menu` 等標的詞者一律命中。Camera 線實測：B04a 30 列中 **10 列**命中，其導航標的 `AUX Cam settings menu`／`settings pop-up` 為 SYS1 §29–§31 之逐字用語，入口即 `ENTER_CAMERA_SETTINGS`（三 hop 皆為逐字 label）；改寫步驟無法消除（實測改為自該常數進入後計數不變）。**建議**：`X` 之固定入口判定改為「同 TC 之 Procedure 含該 feature profile §5.3 任一常數之完整 hop 串」即認，或提供 per-feature 之入口白名單 | CAM-19 上繳 §2-10／§4-3；CAM-19 審閱 §一-5（Pei 准不消）| 全域 —— 凡以 profile §5.3 常數為導航入口之 feature 皆同受影響 | 中 | OPEN |
