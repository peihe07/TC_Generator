...
| A-GC21 | **`down/` 唯讀目錄第二次被同名覆寫，且發生於 PROTOCOL 增「落檔前證不存在」句之後** | `down/20260905_GC-11.md` 於 21:44:15 由主線落（8,612 B，內容＝主線復歸＋§三 11 項），21:54:54 被 A 線以同名覆寫（3,156 B，內容＝五本 dry-run）。主線 21:44 版之內容**已無磁碟副本**。PROTOCOL 之「分析層 `write_file` 至 `down/` 前須 `get_file_info` 證路徑不存在」句於 21:42:02 已入 —— **條文已在而行為未改** | `down/20260905_GC-11_review.md` §一；`up/20260905_GC-12.md` §6 | 同名寫入無存在性檢查（形態同 A-GC18）—— **加註：防再犯條文之存在不等於其生效** | 已承認。不因此作廢 A 線 GC-11 之任務內容（其五本 dry-run 與 R-G72 方向一致）；主線 21:44 版之未竟三項（§三-5／9／10）移入 GC-12 |
| A-GC22 | **主線分析層斷言「候選表判準非 lint036 式」而未先讀 `lint036.py` 之資料列判準** | GC-11 補遺 §3 對候選表資料列判準表示疑慮，而該判準之定義在 `scripts/lint036.py` 之 `lint_sheet()`（`if not any(fields[k].strip() for k in ("test_item", "proc", "er")): continue`），讀之即知候選表所用者即此式。GC-11 審閱 §三自承「本線之誤在『未先讀 `lint036.py:932` 即斷言』」 | `down/20260905_GC-11_review.md` §三 | 未讀來源即斷言（同 A-GC12 之「人寫之理由代量測」）| 已承認（GC-11 審閱 §三自報）。判準無誤，疑慮不成立。**⚠ 行號漂移**：該審閱所指之 `lint036.py:932`，於 GC-12 現查時該行為空行，判準實體在其後三行（GC-10 之檢查 Y 插入使行號位移）——**以行號指涉程式碼在並行改檔期間不可靠，應指函式名** |
| A-GC23 | **R-VS86 之理由書以來源側記法計入作者側計數（「CFTS044 交付本 78 處」）** | 執行層自量（GC-12 任務 8／GC-13）：`…inputs/…CFTS044_Vehicle Controls_20260819.xlsx`（sha12 `ebe5a65f30a0`）全分頁全欄 `PROXI` 命中 **15**，其中落在作者側四欄（`lint036.P_FIELDS`）者僅 **2 行**，且皆為散文（`* Confirm if Heated_Steering_Wheel == "Present" in PROXI`／`* Correct PROXI must be flashed`，同在列 190），**賦值形態 0**；`…inputs/R1LR_Atl-H_25PI3.5_…CFTS_044_…SR26_20250909-1816.docx`（sha12 `87fe31774eea`）解 zip 取 `*.xml` 去標籤後 `PROXI` 命中 **63**，形態為 `PROXI parameter Rear_Sun_Shade = [1h:Present]`（**規格原文，來源側**）。15＋63＝78 對得上，但其構成不是「交付本 78 處」。正確寫法為「作者側賦值 0／散文 2；來源側 63」 | `up/20260905_GC-12.md` 7 節；`features/vehicle_setting/RULINGS.md` R-VS86 條下之更正註 | **來源側記法計入作者側計數**（A-GC20 同族）—— 即 R-G70(b) 所欲防止之形態，與 R-1 v3 之成因同族 | 已承認（`down/20260905_GC-12_review.md` 二 之 7 定稿）。**R-VS86 之結論不動**（Pei 原話「像 SWC 這份一樣的寫法」，且 v4.1 與之同向），**其理由更正**；理由書數字已改，旗標由「待裁」改「已解」 |
