...
**本包六項作業中，三項全做、一項部分做、兩項保留。** 保留項之理由與
...
| A5 | spec_reference R-2 遷移 | **0/283 —— 全數保留**（§5.1） | — |
...
| spec 首行 `^CFTS0(09\|10)-\d{7}$` | 283−PENDING | **0**（未達成，§5.1） |
...
### 5.1 A5 spec_reference 遷移 —— **前置資料不足，全數保留**

A5 指定以 `features/power/` 既有錨點對照逐列取值。執行層查得
`data/anchor_attribution_53.json`（280 筆 `tc_id → 7 位 anchor`），
惟其品質不足以作為 traceability 錨：

| | 列數 | n_candidates | f1 |
|---|---:|---|---|
| `determined: true` | **125** | 全為 1 | 1.0 |
| `determined: false` | **155** | 2–13（40 筆為 2、41 筆為 4、11 筆為 13） | **0.2–0.8** |
| 無錨點（TC 281–283） | 3 | — | — |

即：283 列中**僅 125 列（44%）之 ObjectID 為確定值**，
155 列之 `anchor` 欄為「候選中之最佳猜測」。

將猜測值寫成 `CFTS009-4941692` 形式，是**把不確定性偽裝成精確錨點** ——
違 §8.4.1（不得造值）與本包共通規則 2（不得自行撰寫缺失內容）。
而若對 155 列標 `PENDING: DR-{n}`，依 §8.4.3「含 PENDING 之工作簿不得出貨」，
將直接阻斷 PM 交付。

僅改 125 列亦不可行 —— 同欄兩制，正是 **03 §四** 判定「執行層選擇全不改
為正確判斷」之情形。

**故全數保留，待裁定。** 三個選項：(a) 先補全 155 列之錨點判定；
(b) 承認 155 列標 PENDING 並接受交付阻斷；(c) A5 降級為僅施於 125 確定列
並明示同欄兩制為已知狀態。執行層不代選。
