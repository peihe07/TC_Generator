...
> （R-G43(c) 加錨 2026-09-05）本錨點為指紋而設：上方 `##` 敘述式標題之條號在括號內，
> 錨點正則要求條號領頭，故本條在 GC-05 §一-7 首次乾跑中**得 0 列**。
> **下放包 §一-1 之 14 條清單未含本條**（該清單只列 `R-G30`～`R-G41`、`R-G33(d)`、台帳側 `R-G42`），
> 而 §一-7 要求「`R-G51`～`R-G69` 逐號在表」—— 二者相衝，執行層依後者補加，具名請覆核。
> 上方 `##` 標題與下框條文本體**一字未動**（下框首行仍書 `R-G19（理由與數字須分別驗證 —— 全域）`）。
...
### R-G30 — 流程檔位與 feature 規模成比例

> （R-G43(c) 加錨 2026-09-05）本錨點為指紋而設：原 `##` 敘述式標題之條號在括號內，
> 錨點正則要求條號領頭，故該條在 GC-04 乾跑中**得 0 列**。上方 `##` 標題與下框條文本體**一字未動**。
...
### R-G33 — test_item 上半之未涵蓋面向須於括號下半明寫
...
### R-G45(b) —— DBC／LID 一類以網段 token 配對

> **原記為「R-G45 補充」，2026-09-05 改號為 `R-G45(b)`（GC-04 審閱 §一-2）**：
> 該標題以 `R-G45 補充` 起首，被錨點正則讀為與主條同號，GC-04 乾跑實測撞號一組
> （`RULINGS_LEDGER.md:1317` vs `:1457`）。改號後與 `R-G48(b)`／`R-G50(b)`／`R-G43(b)` 之既有形式一致。

來源：`docs/fw036/handoff/down/20260905_GC-02_review.md:34`（GC-02 審閱 §二-1）

> **本體補正（2026-09-05，GC-05）**：GC-03 §二-1 之抽取器以「標題之次行」為起點，
> 而本條之標題即其本體首行（`**1　DBC 一類無一對一版本**…`），故首行被切掉，
> 台帳所載自「（`BHCAN`／`FDCAN8`）為配對鍵」起 —— 讀者無從知道配對鍵是誰的。
> 現以來源逐字補回全段。**此為執行層之抽取缺陷，見 `up/20260905_GC-05.md` 自報。**
...
## 下放包 GC-04 審閱之全域條文（Pei 2026-09-05「追認」）

依 `docs/fw036/handoff/down/20260905_GC-05.md` §一-3 落檔（R-G36 機器抽取）。

### R-G43(c) —— 指紋之充分條件為「在掃描面」且「條號領頭」

來源：`docs/fw036/handoff/down/20260905_GC-04_review.md:21`（GC-04 審閱 §一-3，[DEFAULT]，Pei 追認）
...
(g) R-13、R-17 不變。
(h) **v3 記法殘留為 FAIL（GC-12 審閱 §一 增，Pei 2026-09-05）**：
    v3 記法殘留（`$MESSAGE.Signal$` 包覆式，不論其後有無 `= raw (label)`）
    為 P 之 FAIL 類，非 WARN。理由：R-1 v3 已撤銷，殘留即不合規；
...
## 下放包 CDD-01_A 之全域條文（R-G73／R-G74，Pei 2026-09-17「逐項准」）

來源：`docs/fw036/handoff/down/20260917_CDD-01_A.md:§2`；
成因：`up/20260917_CDD-01.md` §9-2（錨點正則）與 §7-2 保留事項 (2)（598／5210 前例衝突）；
審閱：`down/20260917_CDD-01_review.md` §三-1、§五-5。

### R-G73 —— 裁決錨點前綴放寬至四字母（全域，Pei 2026-09-17）
...
> **成因**（CDD-01 §9-2）：`R-DIAG` 為全 repo 首個四字母前綴。放寬前
> `### R-DIAG1` 實測 `NO MATCH`，`rulings_hash.py --target features/diagnostics/RULINGS.md`
> 回報 **0 錨點**，九條無法進 `RULINGS.sha.tsv` —— **R-G52 之 `R-XX@<sha8>` 引用制對該 feature 全失效**。
> 放寬前之 18 種前綴為 `R-PMH`／`R-VF`／`R-VS`／`R-TM`／`R-G`／`R-ICS`／`R-C`／`R-DD`／
> `R-VL`／`R-SEC`／`R-VT`／`R-POP`／`R-CAM`／`R-AM`／`R-PH`／`R-U`／`R-P`／`R-SU`，**皆 ≤3 字母**。
>
> **回歸實測（CDD-01_A T1，2026-09-17）**：改前全 repo 821 錨點、改後 830；
> 共同 788 個 ruling_id 之 `sha8` 與 `body_sha8` **變動 0**；消失 0；新增恰 `R-DIAG1`～`R-DIAG9` 九個。
> 上繳包 `up/20260917_CDD-01_A.md` §1 附全表。
>
> **本條之教訓不在正則**：選定條號前綴時未先查解析器之接受範圍
> （審閱 §四 A4）。前綴是**全域命名空間**，其可行性由全域工具定義，不由 feature 自選。

### R-G74 —— Commander (598)／Regengade (5210) 全域一律 0（全域，Pei 2026-09-17）

```text
R-G74  Commander (598)／Regengade (5210) 全域一律 0
  R-CAM2(b) 由 Camera 兩本升為全域：FW036 Vehicle Model 之 598／5210 欄一律 0。
  已交付之 SWC 0708（202 列填 1）依 R-G72 不回修；其後任何 Revise 本一併改 0。
```

> **成因**（CDD-01 §7-2 保留事項 (2)）：`R-DIAG7(b)` 引「R-CAM 前例：598／5210 一律 0」，
> 而已交付之 SWC 0708（`FM-WI-FSM-036-A01 …_SWQT_SWC_20260708.xlsx`，2026-07-04）
> 之 `Test Case Specification` 分頁實測 285 列中 **202 列填 1**
> （`1 1 1 1 1 1 1` ×126、`0 0 1 1 1 1 1` ×76），即將 598／5210 視為 Atl-Mi 群成員。
> R-CAM2(b) 立於 2026-09-16、明文限「Camera 兩本」，為 Pei 較晚之明示裁定。
>
> **升為全域之理由**（審閱 §五-5）：該二車型已不支援為**專案事實**，非 Camera 之特性；
> 逐 feature 再問即逐 feature 再撞同一衝突。
> **與 R-G72 之銜接**：SWC 0708 為已交付本，依 R-G72(a) 不就地改；
> 其若因其他判準入選回修候選，598／5210 併入該本之轉換範圍（同 R-G72(f) 對 `Y` 之處置）。

## 下放包 CDD-01_A 審閱追補 B 之全域條文（R-G75，Pei 2026-09-17）

來源：`docs/fw036/handoff/down/20260917_CDD-01_A_review_B.md:§三`；
成因：`[A-DIAG15]` 之併發覆寫事件（`up/20260917_CDD-01_A.md` §3.1，實測三次）。

### R-G75 —— `RULINGS.sha.tsv` 之併發寫入過渡規則（全域，Pei 2026-09-17）

```text
R-G75  RULINGS.sha.tsv 之併發寫入過渡規則（GC-16 前）
...
> **成因之事實**（CDD 線實測）：該檔由 CDD 與 Security 兩線併行寫入，
> 兩邊都只寫自己那幾列而整檔覆寫，後寫者必覆蓋先寫者，**且覆寫在 diff 上不顯眼**。
> CDD 線於 2026-09-17 一日內實測 **三次** 覆寫：
>
> | 次 | 時點 | 結果 |
> |---:|---|---|
> | 1 | CDD-01_A T3 落檔後、commit `b432893` 前 | DIAG 16 列消失 |
> | 2 | `b432893` **之後** | 工作區再度 0 列 DIAG（Security 加 `R-SEC22(a)`）|
> | 3 | CDD-02 T0 之 (a) 檢查時 | 工作區再度 0 列 DIAG（Security 再加 `R-SEC24`）|
>
> **第二次之意義**：commit **不終止爭用** —— 它只把損失擋在 HEAD 之外。
> HEAD 完好而工作區已處於損失狀態時，下一個以該工作區提交此檔之 session
> 會把損失寫進 HEAD，且其 diff 看起來只是「加了自己那列」。
> **第三次之意義**：本條 (a) 於其落檔前即以人工形式攔下該次覆寫 ——
> 本條不是預防性規範，是對已發生三次之事故之最小止血。
>
> **(b) 之必要**：僅比對列數不足。三次事件中列數皆「看似合理」
> （33→34→35），因對方每次都確實加了一列；**丟失是在別人的列上，不在總數上**。
