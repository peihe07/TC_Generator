...
| **requirement_coverage** | findings **不足** | None | findings 只看「已存在的 TC」分組,看不到「產 0 TC 的需求」。需 source 需求清單(parser/job rows)才算得出。→ 退化 None,報告標「缺需求母體」 |
| **traceability_completeness** | 需 `traceability` 輸入 | 視輸入 | findings 不帶 per-TC spec 對應狀態(`evidence_req_spec` 只在部分 Tier2 finding)。需 spec_matcher 的 per-TC 對應結果另外餵入(介面已有 `traceability` 參數)。無 → None |
| **field_completeness** | 需 `validation` 輸入 | 視輸入 | 真正的 Stage 5 完整性要 `validator.validate_row` 輸出(介面已有 `validation` 參數)。findings 只有零星結構訊號,不等於完整性。無 → None |
